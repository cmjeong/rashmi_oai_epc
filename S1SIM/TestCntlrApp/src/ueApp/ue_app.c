...
PRIVATE S16 ueAppSndAuthResponse(UeCb *ueCb, UeSQN sqnRcvd,UeSQN maxSqnRcvd);
...
)
{
   ...
   switch (idType)
   {
      case CM_EMM_MID_TYPE_IMSI:
      {
         ...
         identRsp->msId.len = CM_EMM_MAX_IMSI_DIGS;
...
PRIVATE S16 ueProcUeAuthResp(UetMessage *tfwMsg, Pst *pst)
{
   ...
   ret = ueAppSndAuthResponse(ueCb, sqnRcvd,maxSqnRcvd);
   if (ret != ROK) 
   {
...
)
{
   ...
   msg->u.conReq.apn.len       =  ueUetPdnConReq->nasPdnApn.len;

   cmMemcpy((U8 *)&msg->u.conReq.apn.apn, 
         (U8 *)&ueUetPdnConReq->nasPdnApn.apn, ueUetPdnConReq->nasPdnApn.len);
