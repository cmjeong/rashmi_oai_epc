...
PUBLIC Void handlUeCntxtRelReq(ueCntxtRelReq_t* data)
{
   ...
   msgReq->t.ueCntxtRelReq.cause = data->cause;
...
PUBLIC S16 handleEndToEndAttachReq(ueAttachRequest_t *data)
{
   ...
   ret = fwStartTmr(fwCb, ueIdCb, fwHndlAttachTmrExp, 200);
   if (ROK != ret)
   {
...
PUBLIC S16 handleUeConfig(ueConfig_t *data)
{
   ...
   cmMemcpy((U8*)ueCfg->imsi, (U8*)data->imsi, 15);
...
PUBLIC S16 handleResetRequest(ResetReq *data)
{
   ...
   {
      msgReq->t.resetReq.u.completeRst.s1IntfId = data->u.completeRst.s1IntfId;
   }
   else if(msgReq->t.resetReq.rstType == NB_PARTIAL_RESET)
   {
      msgReq->t.resetReq.u.partialRst.numOfConn = data->u.partialRst.numOfConn;
