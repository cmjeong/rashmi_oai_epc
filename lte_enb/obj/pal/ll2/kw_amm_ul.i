...
extern S16 KwDlUdxStaProhTmrStart (Pst *pst, SpId spId, CmLteRlcId *rlcId )
                                                            ...
                                                            ;
static Void kwAmmUlAssembleCntrlInfo (KwCb *gCb, KwUlRbCb *rbCb);

static S16 kwAmmExtractHdr (KwCb *gCb, Buffer *pdu, KwAmHdr *amHdr, U8 *fByte)


                                           ;

static Bool kwAmmUlPlacePduInRecBuf (KwCb *gCb, Buffer *pdu, KwUlRbCb *rbCb, KwAmHdr *amHdr)


                                                            ;

static Void kwAmmTriggerStatus (KwCb *gCb, KwUlRbCb *rbCb, KwSn sn, Bool discFlg)


                                                  ;

static S16 kwAmmUlReassembleSdus (KwCb *gCb, KwUlRbCb *rbCb, KwAmRecBuf *recBuf)

                                                         ;

static Void kwAmmProcPduOrSeg (KwCb *gCb, KwUlRbCb *rbCb, KwAmHdr *amHdr, Buffer *pdu)


                                                   ;

static Void kwAmmUpdExpByteSeg (KwCb *gCb,KwAmUl *amUl, KwSeg* newSeg);

static Void kwAmmExtractElmnt (KwCb *gCb, Buffer *pdu, KwExtHdr *hdrInfo);

static Void kwAmmUlHndlStatusPdu (KwCb *gCb, KwUlRbCb *rbCb, Buffer *cntrlPdu, U8 *fByte)


                                                    ;
static S16 kwAmmUlSetNackInfo
(
KwSn sn,
Bool isSegment,
U16 soStart,
...
static Void kwAmmUlAssembleCntrlInfo
(
KwCb *gCb,
KwUlRbCb *rbCb
)
...

{
   ...
   sapCb = (gCb->u.ulCb->udxUlSap);

   { if (SGetStaticBuffer(sapCb->pst.region, sapCb->pst.pool, (Data **)&pStatusPdu, (Size) sizeof(KwUdxDlStaPdu), 0) == 0) { cmMemset((U8 *)(pStatusPdu), 0, sizeof(KwUdxDlStaPdu)); } else { (pStatusPdu) = 0L; } }
   ...
   sn = rbCb->m.amUl.vrR;
   { mSn = (sn - rbCb->m.amUl.vrR) & 0x3ff; };
   { vrMs = (rbCb->m.amUl.vrMs - rbCb->m.amUl.vrR) & 0x3ff; };

   while (mSn < vrMs )
   {

      if (rbCb->m.amUl.recBuf[sn] == 0L && nackCnt < 100 )
      {
         if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe0001,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, sn, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0,                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"         ,                                    275         , "Missing PDU's SN = %d UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
      ...
      else if ((rbCb->m.amUl.recBuf[sn]->pdu == 0L) &&
             (rbCb->m.amUl.recBuf[sn]->segLst.count > 0))
      {



         seqSo = 0;
         { CmLList *tmpNode; if((tmpNode=((&rbCb->m.amUl.recBuf[sn]->segLst)->crnt = (&rbCb->m.amUl.recBuf[sn]->segLst)->first))) seg = (KwSeg *)tmpNode->node; else seg = 0L; };
         while (seg != 0L && nackCnt < 100)
         {
            ...
            if (seg->amHdr.so != seqSo)
            {
               ...
               if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe0002,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, sn, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0,                                               "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"               ,                                               308               , "Missing byte segment's" " SN:%d UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }




                                                ;
               if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe0003,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, seqSo, seg->amHdr.so - 1, rbCb->rlcId.ueId, rbCb->rlcId.cellId,                                               "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"               ,                                               314               , "soStart and soEnd = %d, %d UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
            ...
            seqSo = seg->soEnd + 1;
            { CmLList *tmpNode; (rbCb->m.amUl.recBuf[sn]->segLst).crnt = &((seg)->lstEnt); if((tmpNode = ((&rbCb->m.amUl.recBuf[sn]->segLst)->crnt = ((&rbCb->m.amUl.recBuf[sn]->segLst)->crnt ? (&rbCb->m.amUl.recBuf[sn]->segLst)->crnt->next : (&rbCb->m.amUl.recBuf[sn]->segLst)->first)))) seg = (KwSeg *)tmpNode->node; else seg = 0L; };
         }


         { CmLList *tempNode = 0L; ((&rbCb->m.amUl.recBuf[sn]->segLst)->crnt = (&rbCb->m.amUl.recBuf[sn]->segLst)->last); tempNode = ((&rbCb->m.amUl.recBuf[sn]->segLst)->crnt); if (tempNode != 0L) seg = (KwSeg *)tempNode->node; else seg = 0L; };
         if ((seg != 0L) &&
             (seg->amHdr.lsf != 1 && nackCnt < 100))
         {
            staPduEncSize += kwAmmUlSetNackInfo(sn,
                                                1,
                                                seqSo,
            ...
            if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe0004,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, sn, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0,                                            "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"            ,                                            338            , "kwAmmUlAssembleCntrlInfo: Missing (last) byte " "segment's SN:%d UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }




                                             ;
            if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe0005,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, seqSo, 0x7fff, rbCb->rlcId.ueId, rbCb->rlcId.cellId,                                            "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"            ,                                            344            , "soStart and soEnd = %d, %d UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
      ...
      if((nackCnt == 100) &&
          ((rbCb->m.amUl.recBuf[sn] == 0L) ||
            ((rbCb->m.amUl.recBuf[sn]->pdu == 0L) &&
             (rbCb->m.amUl.recBuf[sn]->segLst.count > 0))))
      {
         break;
      }
   ...
   if ( mSn == vrMs)
   {
      pStatusPdu->ackSn = rbCb->m.amUl.vrMs;
   }
   else
   {
      pStatusPdu->ackSn = sn;
   ...
   if( L_UNUSED < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe0006,L_UNUSED,DBG_RBID, rbCb->rlcId.rbId, pStatusPdu->ackSn, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0,                              "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"   ,                              381   , "kwAmmUlAssembleCntrlInfo: ACK PDU's SN = %d" "UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   pStatusPdu->controlBo = (staPduEncSize + 7) >>3;

   rbCb->m.amUl.staTrg = 0;
   rbCb->m.amUl.gatherStaPduInfo = 0;
   ...
   if (KwUlUdxStaPduReq(&sapCb->pst,
                        ...
                        pStatusPdu) != 0)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe0007,L_ERROR,DBG_RBID, rbCb->rlcId.rbId, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0, 0,                                 "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"      ,                                 397      , "Failed to Send Sta Pdu UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }


                                  ;
      { (Void) SPutStaticBuffer(sapCb->pst.region, sapCb->pst.pool, (Data *) pStatusPdu, (Size) sizeof(KwUdxDlStaPdu), 0); pStatusPdu = 0L; }
...

{
   ...
   numPduToProcess = (pduInfo->numPdu) < (4) ? (pduInfo->numPdu) : (4);
   if( L_UNUSED < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe0008,L_UNUSED,DBG_RBID, rbCb->rlcId.rbId, numPdu, numPduToProcess, rbCb->rlcId.ueId, rbCb->rlcId.cellId,                              "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"   ,                              483   , "numPdu[%ld],numPduToProcess[%ld] UEID:%ld CELLID:%ld", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   while (numPdu < numPduToProcess)
   {
      ...
      if (! pdu)
      {
         ...
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe0009,L_ERROR,DBG_RBID, rbCb->rlcId.rbId, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0, 0,                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"         ,                                    495         , "Null Pdu UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
      ...
      if (kwAmmExtractHdr(gCb, pdu, &amHdr, &fByte) != 0)
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe000a,L_ERROR,DBG_RBID, rbCb->rlcId.rbId, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0, 0,                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"         ,                                    516         , "Header Extraction Failed UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
      ...
      sn = amHdr.sn;
      if (kwAmmUlPlacePduInRecBuf(gCb,pdu, rbCb, &amHdr) == 1)
      {
         Bool tmrRunning;
         KwSn tVrMr;
         KwSn mVrH;
         { mSn = (sn - amUl->vrR) & 0x3ff; };
         { mVrH = (amUl->vrH - amUl->vrR) & 0x3ff; };
         if (mSn >= mVrH)
         {
            amUl->vrH = (sn + 1) & 0x3FF;

            if( L_UNUSED < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe000b,L_UNUSED,DBG_RBID, rbCb->rlcId.rbId, amUl->vrH, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0,                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"            ,                                    597            , "kwAmmProcessPdus: Updated vrH = %d UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
         ...
         { tVrMr = (amUl->vrMr - amUl->vrR) & 0x3ff; };
         ...
         if (sn == amUl->vrMs)
         {
            ...
            if (amUl->recBuf[sn]->allRcvd)
            {
               ...
               { mSn = (tSn - amUl->vrR) & 0x3ff; };
               while (mSn <= tVrMr)
               {
                 if ((amUl->recBuf[tSn] == 0L) ||
                     (!amUl->recBuf[tSn]->allRcvd))
                 {
                    if( L_UNUSED < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe000c,L_UNUSED,DBG_RBID, rbCb->rlcId.rbId, tSn, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0,                                                "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"                    ,                                                622                    , "kwAmmProcessPdus: Updated vrMs:%d " "UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
            ...
            if (amUl->recBuf[sn]->allRcvd)
            {
               ...
               while (mSn <= tVrMr)
               {
                      ...
                      (amUl->recBuf[tSn]->allRcvd))
                  {
                     ...
                     { if (rbCb->m.amUl.recBuf[tSn] != 0L) { (Void) SPutSBuf(gCb->init.region, gCb->init.pool, (Data *) rbCb->m.amUl.recBuf[tSn], (Size) sizeof(KwAmRecBuf)); rbCb->m.amUl.recBuf[tSn] = 0L; } };
         ...
         tmrRunning = kwChkTmr(gCb,(U32)rbCb, 2);
         if (tmrRunning)
         {
            Bool snInWin = ((((amUl->vrX) - (amUl->vrR)) & 0x3ff) < (((amUl->vrMr) - (amUl->vrR)) & 0x3ff));
         ...
         if (!tmrRunning)
         {
            ...
            {
               kwStartTmr(gCb,(U32)rbCb, 2);
               amUl->vrX = amUl->vrH;

               if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe000d,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, amUl->vrX, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0,                                          "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"               ,                                          699               , "kwAmmProcessPdus: Updated vrX = %d UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
...

{
   ...
   cmMemset((U8 *)(&hdrInfo), 0, sizeof(KwExtHdr));;


   SFndLenMsg(pdu,&pduSz);
   SRemPreMsg(fByte, pdu);
   amHdr->dc = (*fByte & 0x80) >> 7;
   if (0 == amHdr->dc)
   {
      return(0);
   }
   ...
   amHdr->rf = (*fByte & 0x40) >> 6;
   amHdr->p = (*fByte & 0x20) >> 5;
   amHdr->fi = (*fByte & 0x18) >> 3;
   e = amHdr->e = (*fByte & 0x04)>> 2;

   SRemPreMsg(&snByte, pdu);
   sn = (U16)(((*fByte & 0x03) << 8 ) | snByte);
   amHdr->sn = sn;
   if (amHdr->rf == 1)
   {
   ...
   while (e && (amHdr->numLi < (2 * 100)))
   {
      ...
      if (! hdrInfo.val)
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xbe000e, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c",823, "Received LI as 0", RLOG_FILE_ID, RLOG_MODULE_NAME); };
         return(1);
      }
   ...
   if(e && (amHdr->numLi >= (2 * 100)))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev2(0xbe000f, L_ERROR, amHdr->numLi, (2 * 100), "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"      , 836      , "LI Count [%u] exceeds Max LI Count[%u]", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                       ;
      return(1);
   }
   ...
   if ( totalSz >= pduSz )
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev3(0xbe0010, L_ERROR, amHdr->sn, totalSz, pduSz, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"      , 846      , "SN [%d]:Corrupted PDU as TotSz[%lu] PduSz[%lu] ", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                         ;
      return(1);
   }
...
static Void kwAmmUlHndlStatusPdu
(
KwCb *gCb,
KwUlRbCb *rbCb,
Buffer *cntrlPdu,
...

{
   ...
   if (hdrInfo.hdr & 0xE0)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe0011,L_ERROR,DBG_RBID, rbCb->rlcId.rbId, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0, 0,                                 "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"      ,                                 908      , "Reserved value for CPT received UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   sapCb = (gCb->u.ulCb->udxUlSap);
   { if (SGetStaticBuffer(sapCb->pst.region, sapCb->pst.pool, (Data **)&pStaPdu, (Size) sizeof(KwUdxStaPdu), 0) == 0) { cmMemset((U8 *)(pStaPdu), 0, sizeof(KwUdxStaPdu)); } else { (pStaPdu) = 0L; } }


                                           ;
   pStaPdu->nackCnt = 0;

   hdrInfo.hdr = hdrInfo.hdr << 3;
   ...
   kwAmmExtractElmnt(gCb, cntrlPdu, &hdrInfo);
   e1 = (U8)hdrInfo.val;
   if( L_UNUSED < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe0012,L_UNUSED,DBG_RBID, rbCb->rlcId.rbId, pStaPdu->ackSn, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0,                              "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"   ,                              943   , "kwAmmUlHndlStatusPdu: ACK SN = %d UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   while (e1 && (pStaPdu->nackCnt < 100))
   {
      ...
      if (pStaPdu->nackInfo[pStaPdu->nackCnt].isSegment)
      {
         ...
         if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe0013,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, pStaPdu->nackInfo[pStaPdu->nackCnt].soStart, pStaPdu->nackInfo[pStaPdu->nackCnt].soEnd, rbCb->rlcId.ueId, rbCb->rlcId.cellId,                                         "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"         ,                                         982         , "kwAmmUlHndlStatusPdu: soStart and soEnd = %d %d" "UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   if(pStaPdu->nackCnt == 100)
   {
      pStaPdu->ackSn = (pStaPdu->nackInfo[100 -1].sn + 1) & 0x3ff;
   }
...

{
   ...
   { CmLList *tmpNode; if((tmpNode=((&recBuf->segLst)->crnt = (&recBuf->segLst)->first))) seg = (KwSeg *)tmpNode->node; else seg = 0L; };
   while (seg != 0L)
   {
      SPutMsg((seg->seg));;
      cmLListDelFrm(&(recBuf->segLst),&(seg->lstEnt));
      { (Void) SPutSBuf(gCb->init.region, gCb->init.pool, (Data *) seg, (Size) sizeof(KwSeg)); seg = 0L; };
      { CmLList *tmpNode; if((tmpNode=((&recBuf->segLst)->crnt = (&recBuf->segLst)->first))) seg = (KwSeg *)tmpNode->node; else seg = 0L; };
   }

   return;
}
static Bool kwAmmAddRcvdSeg
(
KwCb *gCb,
KwUlRbCb *rbCb,
KwAmHdr *amHdr,
...
)
{
   ...
   if (rbCb->m.amUl.recBuf[amHdr->sn] == 0L)
   {
      { if (SGetSBuf(gCb->init.region, gCb->init.pool, (Data **)&recBuf, (Size) sizeof(KwAmRecBuf)) == 0) { cmMemset((U8 *)(recBuf), 0, sizeof(KwAmRecBuf)); } else { (recBuf) = 0L; } };
      rbCb->m.amUl.recBuf[amHdr->sn] = recBuf;
   }
   else
   {
      recBuf = rbCb->m.amUl.recBuf[amHdr->sn];
   ...
   { CmLList *tmpNode; if((tmpNode=((&recBuf->segLst)->crnt = (&recBuf->segLst)->first))) seg = (KwSeg *)tmpNode->node; else seg = 0L; };
   while ((seg != 0L) && (seg->amHdr.so < amHdr->so))
   {
      expSo = seg->amHdr.so + seg->segSz;
      { CmLList *tmpNode; (recBuf->segLst).crnt = &((seg)->lstEnt); if((tmpNode = ((&recBuf->segLst)->crnt = ((&recBuf->segLst)->crnt ? (&recBuf->segLst)->crnt->next : (&recBuf->segLst)->first)))) seg = (KwSeg *)tmpNode->node; else seg = 0L; };
   }
   ...
   SGetSBuf(gCb->init.region, gCb->init.pool, (Data **)&tseg, (Size) sizeof(KwSeg));
   tseg->seg = pdu;
   tseg->segSz = pduSz;
   cmMemcpy((U8*)&tseg->amHdr, (U8 *)amHdr, sizeof(KwAmHdr));;
   recBuf->amHdr.rf = 1;
   ...
   tseg->lstEnt.node = (U32)tseg;
   kwAmmUpdExpByteSeg(gCb,&rbCb->m.amUl,tseg);

   return(1);
}
static Bool kwAmmUlPlacePduInRecBuf
(
KwCb *gCb,
Buffer *pdu,
KwUlRbCb *rbCb,
...

{
   ...
   if (!((((sn) - (amUl->vrR)) & 0x3ff) < (((amUl->vrMr) - (amUl->vrR)) & 0x3ff)))
   {
      if( L_UNUSED < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe0014,L_UNUSED,DBG_RBID, rbCb->rlcId.rbId, sn, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0,                                      "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"      ,                                      1243      , "kwAmmUlPlacePduInRecBuf: SN  %d outside the window" "UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   if (amHdr->rf == 0)
   {
      ...
      if (amUl->recBuf[amHdr->sn] == 0L)
      {
         { if (SGetSBuf(gCb->init.region, gCb->init.pool, (Data **)&recBuf, (Size) sizeof(KwAmRecBuf)) == 0) { cmMemset((U8 *)(recBuf), 0, sizeof(KwAmRecBuf)); } else { (recBuf) = 0L; } };
         amUl->recBuf[sn] = recBuf;
      }
      else if (amUl->recBuf[amHdr->sn]->allRcvd != 1)
      {
      ...
      recBuf->pduSz = pduSz;
      recBuf->allRcvd = 1;
      cmMemcpy((U8*)&recBuf->amHdr, (U8 *)amHdr, sizeof(KwAmHdr));;
      return(1);
   }
   else
   {
...
static Void kwAmmTriggerStatus
(
KwCb *gCb,
KwUlRbCb *rbCb,
KwSn sn,
...

{
   ...
   { tVrMr = (amUl->vrMr - amUl->vrR) & 0x3ff; };
   { tVrMs = (amUl->vrMs - amUl->vrR) & 0x3ff; };
   { tSn = (sn - amUl->vrR) & 0x3ff; };



   if ((discFlg) || (tSn < tVrMs) || (tSn >= tVrMr))
   {
      if( L_UNUSED < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xbe0015,L_UNUSED,DBG_RBID, rbCb->rlcId.rbId, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0, 0,                                       "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_amm_ul.c"      ,                                       1356      , "kwAmmTriggerStatus: Set Status Trigger UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }


                                        ;

      amUl->staTrg = 1;
      amUl->gatherStaPduInfo = 0;


      tmrRunning = kwChkTmr(gCb,(U32) rbCb, 3);

      if (!tmrRunning)
      {
         amUl->gatherStaPduInfo = 1;
      }
   }

   return;
}
static Void kwAmmUlGetSdu
(
KwCb *gCb,
U16 *liCnt,
KwAmHdr *amHdr,
...

{
   ...
   if ((rbCb->m.amUl.expSn != amHdr->sn) || (rbCb->m.amUl.expSo != amHdr->so))
   {


      rbCb->m.amUl.isOutOfSeq = 1;
      { if (rbCb->m.amUl.partialSdu != 0L) { SPutMsg(rbCb->m.amUl.partialSdu); } rbCb->m.amUl.partialSdu = 0L; };
   }
   ...
   if (amHdr->fi & 0x02)
   {
      ...
      if (rbCb->m.amUl.partialSdu == 0L)
      {
         kwAmmUlGetSdu(gCb,&liCnt, amHdr, &pdu, &sdu);
         { if (sdu != 0L) { SPutMsg(sdu); } sdu = 0L; };
      }
      else if (amHdr->numLi == 0)
      {
         SCatMsg(pdu, rbCb->m.amUl.partialSdu, 1);
         SPutMsg((rbCb->m.amUl.partialSdu));;
      }
      else
      {
         kwAmmUlGetSdu(gCb,&liCnt, amHdr, &pdu, &sdu);
         SCatMsg(sdu, rbCb->m.amUl.partialSdu, 1);
         SPutMsg((rbCb->m.amUl.partialSdu));;
   ...
   if (amHdr->fi & 0x01)
   {
      ...
      rbCb->m.amUl.partialSdu = pdu;
   }
   else if (pdu != 0L)
   {
      rbCb->m.amUl.partialSdu = 0L;
      kwUtlSndDatInd(gCb,rbCb, pdu);
   }

   return;
}
static S16 kwAmmUlReassembleSdus
(
KwCb *gCb,
KwUlRbCb *rbCb,
KwAmRecBuf *recBuf
...

{
   ...
   if (recBuf->amHdr.rf == 0)
   {
      ...
      rbCb->m.amUl.expSn = (recBuf->amHdr.sn + 1) & 0x3FF;
      rbCb->m.amUl.expSo = 0;
   }
   else
   {
      ...
      { CmLList *tmpNode; if((tmpNode=((&recBuf->segLst)->crnt = (&recBuf->segLst)->first))) seg = (KwSeg *)tmpNode->node; else seg = 0L; };
      rbCb->m.amUl.expSn = recBuf->amHdr.sn;
      rbCb->m.amUl.expSo = 0;
      while(seg)
      {
         kwAmmProcPduOrSeg(gCb,rbCb, &seg->amHdr, seg->seg);
         rbCb->m.amUl.expSo = seg->soEnd + 1;
...

{
   ...
   { mVrMr = (rbCb->m.amUl.vrMr - rbCb->m.amUl.vrR) & 0x3ff; };
   { mSn = (sn - rbCb->m.amUl.vrR) & 0x3ff; };


   while (mSn < mVrMr)
   {
      if (rbCb->m.amUl.recBuf[sn] != 0L)
      {
         if (rbCb->m.amUl.recBuf[sn]->allRcvd == 1)
         {
            kwAmmUlReassembleSdus(gCb,rbCb, rbCb->m.amUl.recBuf[sn]);
         }
         else
         {

            if(rbCb->m.amUl.recBuf[sn]->pdu)
            {
               SPutMsg((rbCb->m.amUl.recBuf[sn]->pdu));;
            }

            kwAmmUlRlsAllSegs(gCb,rbCb->m.amUl.recBuf[sn]);
         }

         { (Void) SPutSBuf(gCb->init.region, gCb->init.pool, (Data *) rbCb->m.amUl.recBuf[sn], (Size) sizeof(KwAmRecBuf)); rbCb->m.amUl.recBuf[sn] = 0L; };
         rbCb->m.amUl.recBuf[sn]=0L;
      }
      sn = (sn + 1) & 0x3FF;
      { mSn = (sn - rbCb->m.amUl.vrR) & 0x3ff; };
   ...
   if(1 == kwChkTmr(gCb,(U32)rbCb,2))
   {
       kwStopTmr(gCb,(U32)rbCb, 2);
   }
   if(1 == kwChkTmr(gCb,(U32)rbCb,3))
   {
       kwStopTmr(gCb,(U32)rbCb, 3);
   }

   rbCb->m.amUl.vrR = 0;
   rbCb->m.amUl.vrH = 0;
   rbCb->m.amUl.vrX = 0;
   rbCb->m.amUl.vrMr = (rbCb->m.amUl.vrR + 512) & 0x3FF;
   rbCb->m.amUl.vrMs = 0;
   rbCb->m.amUl.staTrg = 0;
   rbCb->m.amUl.gatherStaPduInfo = 0;
   rbCb->m.amUl.expSn = 0;
   ...
   kwKwSap = gCb->u.ulCb->kwuUlSap + 1;

   if(sendReEst)
   {
      KwUiKwuReEstCmpInd(&kwKwSap->pst, kwKwSap->suId, rlcId);
      rbCb->m.amUl.isOutOfSeq = 0;
   }

   return;
}
 Void kwAmmReOrdTmrExp
(
KwCb *gCb,
KwUlRbCb *rbCb
)
...

{
   ...
   { mSn = (sn - amUl->vrR) & 0x3ff; };
   { mVrMr = (amUl->vrMr - amUl->vrR) & 0x3ff; };

   while (mSn < mVrMr)
   {
      if ( (amUl->recBuf[sn] == 0L) ||
            ((amUl->recBuf[sn] != 0L) &&
   ...
   { mVrH = (amUl->vrH - amUl->vrR) & 0x3ff; };
   { mVrMs = (amUl->vrMs - amUl->vrR) & 0x3ff; };
   if (mVrH > mVrMs)
   {
      kwStartTmr(gCb,(U32)rbCb, 2);
      amUl->vrX = amUl->vrH;
...
static Void kwAmmUpdExpByteSeg
(
KwCb *gCb,
KwAmUl *amUl,
KwSeg *seg
...

{
   ...
   { CmLList *tmpNode; (amUl->recBuf[sn]->segLst).crnt = &((seg)->lstEnt); if((tmpNode = ((&amUl->recBuf[sn]->segLst)->crnt = ((&amUl->recBuf[sn]->segLst)->crnt ? (&amUl->recBuf[sn]->segLst)->crnt->next : (&amUl->recBuf[sn]->segLst)->first)))) seg = (KwSeg *)tmpNode->node; else seg = 0L; };
   while(seg)
   {
   ...
   if (lstRcvd == 1)
   {
      amUl->recBuf[sn]->allRcvd = 1;
   }

   return;
}
 Void kwAmmFreeUlRbCb
(
KwCb *gCb,
KwUlRbCb *rbCb
)
...

{
   ...
   do
   {
      if ( amRecBuf[curSn] != 0L )
      {
         if (amRecBuf[curSn]->pdu != 0L)
         {
            SPutMsg((amRecBuf[curSn]->pdu));;
         }

         kwAmmUlRlsAllSegs(gCb,amRecBuf[curSn]);
         { (Void) SPutSBuf(gCb->init.region, gCb->init.pool, (Data *) amRecBuf[curSn], (Size) sizeof(KwAmRecBuf)); amRecBuf[curSn] = 0L; };
      }
      curSn++;
   }while ( curSn < windSz );


   { if (rbCb->m.amUl.recBuf != 0L) { (Void) SPutSBuf(gCb->init.region, gCb->init.pool, (Data *) rbCb->m.amUl.recBuf, (Size) (windSz)* sizeof(KwAmRecBuf*)); rbCb->m.amUl.recBuf = 0L; } };
