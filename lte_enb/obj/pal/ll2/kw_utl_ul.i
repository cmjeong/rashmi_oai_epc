...

{
   ...
   if( 0 != kwDbmFetchUlUeCb(gCb,datIndInfo->rnti,datIndInfo->cellId,&(ueCb)))
   {

      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xd20001,L_ERROR,DBG_CELLID, datIndInfo->cellId, datIndInfo->rnti, 0, 0, 0,                               "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_utl_ul.c"      ,                               144      , "UEID:%d UeCb not found", RLOG_FILE_ID, RLOG_MODULE_NAME); }
      ...
      for(i = 0; i< datIndInfo->numLch; i++)
      {
         for(j = 0; j < datIndInfo->lchData[i].pdu.numPdu; j++)
         {
            ...
            {
               SPutMsg((datIndInfo->lchData[i].pdu.mBuf[j]));;
            }
         }
      }
...
static Void dumpRLCUlRbInformation(KwUlRbCb* ulRbCb)
{
   if(ulRbCb->mode == 2)
   {
      U32 i;
      ...
      for(i = 0; i< windSz; i++)
      {
         if(ulRbCb->m.umUl.recBuf[i] != 0L)
         {
            pdusInReceptionBuffer++;
         }
      }

      if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xd20002,L_DEBUG,DBG_RBID, ulRbCb->rlcId.rbId, (int)ulRbCb->rlcId.ueId, (int)ulRbCb->rlcId.cellId, (int)pdusInReceptionBuffer, 0,                                                 "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_utl_ul.c"      ,                                                 319      , "UM UL UEID:%d CELLID:%d Reception Buffer size = %d", RLOG_FILE_ID, RLOG_MODULE_NAME); }



                                                  ;
   }
   else if(ulRbCb->mode == 3)
   {
      U32 i;
      U32 pdusInReceptionBuffer = 0;
      U32 totalSegs = 0;
      ...
      if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xd20003,L_DEBUG,DBG_RBID, ulRbCb->rlcId.rbId, (int)ulRbCb->rlcId.ueId, (int)ulRbCb->rlcId.cellId, (int)pdusInReceptionBuffer, (int)totalSegs,                                     "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_utl_ul.c"      ,                                     343      , "AM UL UEID:%d CELLID:%d Reception Buf size = %d" "total segs = %d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
...
Void DumpRLCUlDebugInformation(Void)
{
   ...
   while (0 == cmHashListGetNext(&ulCb->ueLstCp,
                                   (U32) ueCb,
                                   (U32 *)&ueCb))
   {
      U32 i;
