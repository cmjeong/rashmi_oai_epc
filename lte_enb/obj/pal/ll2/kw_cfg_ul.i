...
extern Void DumpRLCUlDebugInformation (Void);
 ...
 S16 kwValidateRbCfgParams (KwCb *gCb, CmLteRnti ueId, CmLteCellId cellId,
                                  CkwEntCfgInfo *cfgToValidate, CmStatus *status);
static S16 kwCfgFillUlRbCb
(
KwCb *gCb,
KwUlRbCb *rbCb,
KwUlUeCb *ueCb,
...

{
   ...
   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc00001,L_DEBUG,DBG_UEID, rbCb->rlcId.ueId, rbCb->rlcId.cellId, entCfg->rbId, entCfg->rbType, 0,                              "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"   ,                              348   , "kwCfgFillRbCb(cellId(%d), rbId(%d), rbType(%d))", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   switch (entCfg->entMode)
   {
      ...
      case 3:
      {
         ...
         rbCb->m.amUl.vrMr = rbCb->m.amUl.vrR + 512;
      ...
      default:
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc00002,L_ERROR,DBG_UEID, rbCb->rlcId.ueId, rbCb->rlcId.cellId, entCfg->rbId, 0, 0,                            "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"         ,                            402         , "INVALID RB Mode cellId(%d), rbId(%d)", RLOG_FILE_ID, RLOG_MODULE_NAME); }
...

{
   ...
   switch (rbCb->mode)
   {
      ...
      case 2:
      {
         ...
         ueCb->lCh[rbCb->lch.lChId - 1].ulRbCb = 0L;
...
)
{
   ...
   {
      do { if (!((gCb->init.dbgMask & ((0x00000002 << 2) | ((0x00000002 << 4) << 5))) ^ ((0x00000002 << 2) | ((0x00000002 << 4) << 5)))) { { snprintf((gCb)->init.prntBuf, 255, "[%s]::", __func__); SPrint((gCb)->init.prntBuf); snprintf(gCb->init.prntBuf, 255,"kwValidateRbCfgParams ueId (%u) cellId (%u) Invalid rguSapId (%d))\n", ueId, cellId, cfgToValidate->rguSapId); SPrint(gCb->init.prntBuf); }; } }while(0)
                                                  ;
      status->reason = 23;
      return(1);
   }

   if((1 == cfgToValidate->cfgType) ||
      (2 == cfgToValidate->cfgType))
   {

      if ((cfgToValidate->lCh[0].lChId <= 0) ||
           ((cfgToValidate->entMode == 3) &&
             (cfgToValidate->lCh[1].lChId <= 0)))
      {
         status->reason = 18;
         return(1);
      }
      if((cfgToValidate->entMode == 2) &&
         (cfgToValidate->m.umInfo.ul.snLen != 1) &&
         (cfgToValidate->m.umInfo.ul.snLen != 2))
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc00003,L_ERROR,DBG_UEID, ueId, cfgToValidate->rbId, cfgToValidate->m.umInfo.ul.snLen, 0, 0,                                               "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"         ,                                               543         , "CellId[%u]:UM Mode RB[%d],Invalid SN Len[%d]", RLOG_FILE_ID, RLOG_MODULE_NAME); }
      ...
      if (ueId == 0)
      {
         if(cfgToValidate->rbId >= 10)
         {
            status->reason = 6;
            return(1);
         }

         if ((cfgToValidate->lCh[0].type != 3) &&
               (cfgToValidate->entMode != 1))
         {
            status->reason= (cfgToValidate->entMode != 1)? 12:
               5;
            return(1);
         }
      }
      ...
      {
         if (!(((cfgToValidate->rbType == 0 && cfgToValidate->rbId < 3) || (cfgToValidate->rbType == 1 && cfgToValidate->rbId < 32))))
         {
            status->reason = 6;
            return(1);
         }

         if(cfgToValidate->entMode == 1)
         {
            status->reason = 5;
            return(1);
         }
         if (!(((cfgToValidate->lCh[0].type == 5) &&
               (cfgToValidate->entMode != 2))||
               (cfgToValidate->lCh[0].type == 4)) )
         {
            status->reason = 12;
            return(1);
         }
...

{
   ...
   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc00004,L_DEBUG,DBG_UEID, cfgTmpData->ueId, cfgTmpData->cellId, cfgToValidate->cfgType, 0, 0,                                      "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"   ,                                      645   , "cellId(%d), cfgType(%d)", RLOG_FILE_ID, RLOG_MODULE_NAME); }


                                       ;

   if(0 != kwValidateRbCfgParams(gCb,
                                   cfgTmpData->ueId,
                                   cfgTmpData->cellId,
                                   cfgToValidate,
   ...
   cfgEntData->entUlCfgCfm.status.reason = 0;
   switch(cfgToValidate->cfgType)
   {
      case 1:
      {
         if (cfgTmpData->ueId == 0)
         {
            ...
            if(!cfgTmpData->cellCb)
            {
               ...
               { if (SGetSBuf(gCb->init.region, gCb->init.pool, (Data **)&cfgTmpData->cellCb, (Size) sizeof(KwUlCellCb)) == 0) { cmMemset((U8 *)(cfgTmpData->cellCb), 0, sizeof(KwUlCellCb)); } else { (cfgTmpData->cellCb) = 0L; } };
               if(!cfgTmpData->cellCb)
               {
                  if( L_FATAL < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc00005,L_FATAL,DBG_UEID, cfgTmpData->ueId, cfgTmpData->cellId, 0, 0, 0,                                          "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"                  ,                                          674                  , "Memory allocation failure CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }

                                           ;
                  cfgEntData->entUlCfgCfm.status.reason = 3;
                  return(1);
               }
            ...
            {
               if (( cfgTmpData->cellCb->rbCb[cfgToValidate->rbId] != 0L))
               {
                  cfgEntData->entUlCfgCfm.status.reason = 4;;
                  return(1);
               }
            }

            { if (SGetSBuf(gCb->init.region, gCb->init.pool, (Data **)&cfgEntData->rbCb, (Size) sizeof (KwUlRbCb)) == 0) { cmMemset((U8 *)(cfgEntData->rbCb), 0, sizeof (KwUlRbCb)); } else { (cfgEntData->rbCb) = 0L; } };
            if (!cfgEntData->rbCb)
            {
               cfgEntData->entUlCfgCfm.status.reason = 1;
               return(1);
            }
         }
         else
         {
            ...
            if(!cfgTmpData->ueCb)
            {
               { if (SGetSBuf(gCb->init.region, gCb->init.pool, (Data **)&cfgTmpData->ueCb, (Size) sizeof(KwUlUeCb)) == 0) { cmMemset((U8 *)(cfgTmpData->ueCb), 0, sizeof(KwUlUeCb)); } else { (cfgTmpData->ueCb) = 0L; } };
               if(!cfgTmpData->ueCb)
               {
                  if( L_FATAL < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc00006,L_FATAL,DBG_UEID, cfgTmpData->ueId, cfgTmpData->cellId, 0, 0, 0,                                          "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"                  ,                                          708                  , "Memory allocation failure CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }

                                           ;
                  cfgEntData->entUlCfgCfm.status.reason = 2;
                  return(1);
               }
            ...
            {
               (cfgEntData->rbCb) = ((cfgToValidate->rbType) == 0) ? (cfgTmpData->ueCb)->srbCb[(cfgToValidate->rbId)] : (cfgTmpData->ueCb)->drbCb[(cfgToValidate->rbId)];


                                      ;
               if(cfgEntData->rbCb != 0L)
               {
                  cfgEntData->entUlCfgCfm.status.reason = 4;;
                  return(1);
               }
         ...
         if(2 == cfgToValidate->entMode)
         {
            U16 winLen;


            cfgEntData->rbCb->m.umUl.umWinSz = 2 << (((cfgToValidate->m.umInfo.ul.snLen *5)-1)-1);
                                                            ;
            winLen = cfgEntData->rbCb->m.umUl.umWinSz << 1;
            { if (SGetSBuf(gCb->init.region, gCb->init.pool, (Data **)&cfgEntData->rbCb->m.umUl.recBuf, (Size) (winLen * sizeof(KwUmRecBuf*))) == 0) { cmMemset((U8 *)(cfgEntData->rbCb->m.umUl.recBuf), 0, (winLen * sizeof(KwUmRecBuf*))); } else { (cfgEntData->rbCb->m.umUl.recBuf) = 0L; } }

                                                    ;
         }
         else if(3 == cfgToValidate->entMode)
         {

            { if (SGetSBuf(gCb->init.region, gCb->init.pool, (Data **)&cfgEntData->rbCb->m.amUl.recBuf, (Size) ((512 << 1) * sizeof(KwAmRecBuf *))) == 0) { cmMemset((U8 *)(cfgEntData->rbCb->m.amUl.recBuf), 0, ((512 << 1) * sizeof(KwAmRecBuf *))); } else { (cfgEntData->rbCb->m.amUl.recBuf) = 0L; } }
      ...
      case 3:
      {
         if (cfgTmpData->ueId == 0)
         {
            ...
            if(!cfgTmpData->cellCb)
            {
                if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc00007,L_ERROR,DBG_CELLID, cfgTmpData->cellId, cfgTmpData->ueId, 0, 0, 0,                                      "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"                ,                                      767                , "CellCb not found UEID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }

                                       ;

               cfgEntData->entUlCfgCfm.status.reason = 8;
               return(1);
            }
            ...
            if (!cfgEntData->rbCb)
            {
               ...
               cfgEntData->entUlCfgCfm.status.reason = 6;
               return(1);
            }
            return(0);
         }
         else
         {
            ...
            if(!cfgTmpData->ueCb)
            {
               if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc00008,L_ERROR,DBG_CELLID, cfgTmpData->cellId, cfgTmpData->ueId, 0, 0, 0,                                     "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"               ,                                     789               , "UeId [%d]: UeCb not found", RLOG_FILE_ID, RLOG_MODULE_NAME); }

                                      ;
               cfgEntData->entUlCfgCfm.status.reason = 7;
               return(1);
            }
   ...
   if(cfgToValidate->cfgType == 2)
   {
      if(cfgToValidate->entMode != cfgEntData->rbCb->mode)
      {
         cfgEntData->entUlCfgCfm.status.reason = 12;
         return(1);
      }

      if(cfgToValidate->m.umInfo.ul.snLen != cfgEntData->rbCb->m.umUl.snLen)
      {
         cfgEntData->entUlCfgCfm.status.reason = 22;
         return(1);
      }
   }
   return(0);
}
 S16 kwCfgRollBackUlRb
(
KwCb *gCb,
CmLteRnti ueId,
CkwEntCfgInfo *cfg,
...
)
{
   ...
   if(1 == cfg->cfgType)
   {
      if(2 == cfg->entMode)
      {
         { if (cfgEntData->rbCb->m.umUl.recBuf != 0L) { (Void) SPutSBuf(gCb->init.region, gCb->init.pool, (Data *) cfgEntData->rbCb->m.umUl.recBuf, (Size) (cfgEntData->rbCb->m.umUl.umWinSz << 1) * sizeof(KwUmRecBuf*)); cfgEntData->rbCb->m.umUl.recBuf = 0L; } }

                                                                             ;
      }
      else if(3 == cfg->entMode)
      {

         { if (cfgEntData->rbCb->m.amUl.recBuf != 0L) { (Void) SPutSBuf(gCb->init.region, gCb->init.pool, (Data *) cfgEntData->rbCb->m.amUl.recBuf, (Size) ((512 << 1) * sizeof(KwAmRecBuf *))); cfgEntData->rbCb->m.amUl.recBuf = 0L; } }

                                                            ;

      }
      { if (cfgEntData->rbCb != 0L) { (Void) SPutSBuf(gCb->init.region, gCb->init.pool, (Data *) cfgEntData->rbCb, (Size) sizeof(KwUlRbCb)); cfgEntData->rbCb = 0L; } };
   }
...
)
{
   ...
   switch(cfgToAply->cfgType)
   {
      ...
      case 3:
      {
         if (cfgTmpData->ueId == 0)
         {
            cfgTmpData->cellCb->rbCb[cfgToAply->rbId] = 0L;
         ...
         else
         {
            ...
            if( 2 == cfgEntData->rbCb->mode )
            {
               kwUmmFreeUlRbCb(gCb, cfgEntData->rbCb);
            }
            else if(3 == cfgEntData->rbCb->mode)
            {
            ...
            if ( cfgToAply->rbType == 0 )
            {
               cfgTmpData->ueCb->srbCb[cfgToAply->rbId] = 0L;
            }
            else
            {
               cfgTmpData->ueCb->drbCb[cfgToAply->rbId] = 0L;
            }
         }
...

{
   ...
   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc00009,L_DEBUG,DBG_UEID, cfgTmpData->ueId, cfgTmpData->cellId, cfgToValidate->cfgType, 0, 0,                                                       "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"   ,                                                       1061   , "cellId(%d), cfgType(%d)", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   {
      cfgEntData->entUlCfgCfm.status.reason = 7;;
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc0000a,L_ERROR,DBG_UEID, cfgTmpData->ueId, cfgTmpData->cellId, 0, 0, 0,                              "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"      ,                              1069      , "UeId is 0 for CELLID;%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   if(0 != kwDbmFetchUlUeCb(gCb,cfgTmpData->ueId, cfgTmpData->cellId, &cfgTmpData->ueCb))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc0000b,L_ERROR,DBG_CELLID, cfgTmpData->cellId, cfgTmpData->ueId, 0, 0, 0,                            "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"      ,                            1078      , "UeId [%d]: UeCb not found", RLOG_FILE_ID, RLOG_MODULE_NAME); }
...

{
   ...
   kwDbmDelUlUeCb(gCb,cfgTmpData->ueCb, 0);
   return;
}
 S16 kwCfgValidateDelUlCell
(
KwCb *gCb,
CmLteCellId cellId,
CkwEntCfgInfo *cfgToValidate,
...
)
{
   ...
   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc0000c,L_DEBUG,DBG_CELLID, cellId, cfgToValidate->cfgType, 0, 0, 0,                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"   ,                                    1156   , "cfgType(%d)", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                     ;

   cfgTmpData->cellCb = 0L;


   if ( cellId == 0 )
   {
      cfgEntData->entUlCfgCfm.status.reason = 8;;
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc0000d,L_ERROR,DBG_CELLID, cellId, 0, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c",1164, "CellId is 0", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
   ...
   {
      cfgEntData->entUlCfgCfm.status.reason = 8;;
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc0000e,L_ERROR,DBG_CELLID, cellId, 0, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c",1173, "CellCb not found", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
   return(0);
}
 Void kwCfgApplyDelUlCell
(
KwCb *gCb,
...
)
{
   ...
   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc0000f,L_DEBUG,DBG_UEID, ueId, cellId, cfgToValidate->rbId, 0, 0,                            "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"   ,                            1248   , "cellId(%d) RBID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   if (cfgEntData->rbCb == 0L)
   {
      if( L_WARNING < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc00010,L_WARNING,DBG_UEID, ueId, cellId, cfgToValidate->rbId, 0, 0,                                "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"      ,                                1262      , "CellId [%u]: rbId [%d] not found", RLOG_FILE_ID, RLOG_MODULE_NAME); }
...

{
   ...
   {
      cfgTmpData->cfgEntData[0].entUlCfgCfm.status.reason = 16;
      cfgTmpData->cfgEntData[0].entUlCfgCfm.status.status = 2;
      return(1);
   }

   if(0 == kwDbmFetchUlUeCb(gCb,newUeInfo->ueId, newUeInfo->cellId, &ueCb))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc00011,L_ERROR,DBG_CELLID, newUeInfo->cellId, newUeInfo->ueId, 0, 0, 0,                            "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"      ,                            1381      , "NewUeId[%d]:ueCb already exists", RLOG_FILE_ID, RLOG_MODULE_NAME); }

                             ;
      cfgTmpData->cfgEntData[0].entUlCfgCfm.status.reason = 20;
      cfgTmpData->cfgEntData[0].entUlCfgCfm.status.status = 2;
      return(1);
   }

   if(0 != kwDbmFetchUlUeCb(gCb,ueInfo->ueId, ueInfo->cellId,
                              &cfgTmpData->ueCb))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc00012,L_ERROR,DBG_CELLID, ueInfo->cellId, ueInfo->ueId, 0, 0, 0,                        "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"      ,                        1392      , "UeId [%d]: UeCb not found", RLOG_FILE_ID, RLOG_MODULE_NAME); }

                         ;
      cfgTmpData->cfgEntData[0].entUlCfgCfm.status.reason = 7;
      cfgTmpData->cfgEntData[0].entUlCfgCfm.status.status = 2;
      return(1);
   }
...
)
{
   ...
   cmHashListDelete(&(gCb->u.ulCb->ueLstCp), (U32) cfgTmpData->ueCb);

   cfgTmpData->ueCb->ueId = newUeInfo->ueId;
   cfgTmpData->ueCb->cellId =newUeInfo->cellId;
   if(0 != cmHashListInsert(&(gCb->u.ulCb->ueLstCp),
                              (U32)cfgTmpData->ueCb,
                              (U8 *)&(cfgTmpData->ueCb->ueId),
                              (U16) sizeof(CmLteRnti)))

   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc00013,L_ERROR,DBG_CELLID, newUeInfo->cellId, newUeInfo->ueId, 0, 0, 0,                           "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_cfg_ul.c"      ,                           1450      , "UeId[%u] HashList Insertion Failed", RLOG_FILE_ID, RLOG_MODULE_NAME); }
