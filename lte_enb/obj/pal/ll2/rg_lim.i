...

{
   ...
   if((ret = RgLiTfuBndReq (&pst, suId, spId)) != 0)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xb60001, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_lim.c",124, "Call to RgLiTfuBndReq() failed", RLOG_FILE_ID, RLOG_MODULE_NAME); };
   }
   return(ret);
}
 S16 rgLIMTfuUbndReq
...

{
   ...
   if((ret = RgLiTfuUbndReq (&pst, tfuSap->sapCfg.spId, reason)) != 0)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xb60002, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_lim.c",173, "Call to RgLiTfuUbndReq() failed", RLOG_FILE_ID, RLOG_MODULE_NAME); };
   }
   return(ret);
...

{
   ...
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev2(0xb60003, L_ERROR, tfuSap->sapCfg.suId, suId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_lim.c"      , 227      , "Incorrect SuId. Configured (%d) Recieved (%d)", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                      ;
      return(1);
   }
...

{
   ...
   if( suId != tfuSap->sapCfg.suId)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev2(0xb60004, L_ERROR, tfuSap->sapCfg.suId, suId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_lim.c"      , 266      , "Incorrect SuId. Configured (%d) Recieved (%d)", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                      ;
      return(1);
   }
   if (tfuSap->sapSta.sapState != 2)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xb60005, L_ERROR, tfuSap->sapCfg.suId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_lim.c"      , 272      , "Lower SAP not enabled SuId (%d)", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                ;
      return(1);
   }
   return(0);
...

{
   ...
   if ((error == 1) && (datInd->datIndLst.count > 0))
   {
      node = datInd->datIndLst.first;
      while (node)
      {
         datInfo = (TfuDatInfo*)node->node;
         { if (0L != (datInfo->mBuf)) { SPutMsg((datInfo->mBuf)); datInfo->mBuf = 0L; } };
         node = node->next;
      }
   }
   { if (0L != (datInd)) { cmFreeMem((datInd)); datInd = 0L; } };
   return;
}
 S16 RgLiTfuDatInd
(
...

{
   ...
   if (ret == 0)
   {
      rgLIMUtlFreeDatIndEvnt(datInd, 0);
   }
   else
   {
      rgLIMUtlFreeDatIndEvnt(datInd, 1);
   }


   SStopTask(startTime, 30228);
...

{
   ...
   if((ret = RgLiTfuDatReq (&tfuSap->sapCfg.sapPst, tfuSap->sapCfg.spId,
                            datReq)) != 0)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xb60006,L_ERROR,DBG_CELLID, datReq->cellId, 0, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_lim.c",517, "Call to RgLiTfuDatReq() failed", RLOG_FILE_ID, RLOG_MODULE_NAME); };
   }
   return(ret);
}
 S16 RgLiTfuTtiInd
...

{
   ...
   SStartTask(&startTime, 30211);


   if ((ret = rgLIMValidateSap (pst->dstInst - 0,suId)) != 0)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xb60007,L_ERROR,DBG_CELLID, ttiInd->cells[0].cellId, 0, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_lim.c",566, "SAP Validation failed", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(ret);
   }
