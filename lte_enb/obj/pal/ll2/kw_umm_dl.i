...
static Void kwUmmEstHdrSz (KwUmDl *umUl);

static Void kwUmmCreatePdu (KwCb *gCb, KwDlRbCb *rbCb, Buffer *pdu, U8 fi, KwPduInfo *datReqPduInfo)
...

{
   ...
   SGetSBuf(gCb->init.region, gCb->init.pool, (Data **)&sdu, (Size) (Size)sizeof(KwSdu));
   ...
   {
      kwUtlSndDStaRsp(gCb,rbCb,rbCb->m.umDl.bo,rbCb->m.umDl.estHdrSz,0,0);
   }
   return;
}
 Void kwUmmProcessSdus
(
KwCb *gCb,
KwDlRbCb *rbCb,
KwDatReq *datReq
...

{
   ...
   while ((pduSz > 0) && (rbCb->m.umDl.sduQ.count > 0) &&
           (rbCb->m.umDl.numLi < 28) && (pduInfo->numPdu <= 2))
   {
      ((((firstNode) = ((&rbCb->m.umDl.sduQ)->crnt = (&rbCb->m.umDl.sduQ)->first)) != 0L) ? (firstNode)->node : 0L);
      sdu = (KwSdu *)(firstNode->node);

      if ((sdu->mode.um.isSegmented == 0) && (rbCb->discTmrInt > 0) &&
            (rbCb->rlcId.rbType == 1))
      {
         ...
         if (timeDiff >= rbCb->discTmrInt)
         {
            ...
            { if(sdu->mBuf) { SPutMsg(sdu->mBuf); } cmLListDelFrm(&rbCb->m.umDl.sduQ,&sdu->lstEnt); { if (sdu != 0L) { (Void) SPutSBuf(gCb->init.region, gCb->init.pool, (Data *) sdu, (Size) sizeof(KwSdu)); sdu = 0L; } }; };
            continue;
         }
      }
      if (!pdu)
      ...
      else if (sdu->sduSz < pduSz)
      {
         ...
         if (sdu->sduSz < 2048 && rbCb->m.umDl.numLi < 28)
         {
         ...
         else
         {
            ...
            if ( pduInfo->numPdu == 2)
            {
               ...
               if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xef0001,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0, 0,                                          "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_umm_dl.c"               ,                                          451               , "Maximum Pdu limit has been reached UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
...

{
   ...
   ((((tmpNode) = ((&rbCb->m.umDl.sduQ)->crnt = (&rbCb->m.umDl.sduQ)->first)) != 0L) ? (tmpNode)->node : 0L);

   if (tmpNode)
   {
      KwSdu *sdu = (KwSdu *)tmpNode->node;

      if (sdu->mode.um.sduId == sduId && sdu->mode.um.isSegmented == 0)
      {
