...
extern Void rgLaaInitTbInfoLst ( RgLaaTbReq **tbInfoLst )
  ...
  ;
static Void rgDHMBldTfuDatReq (RgCellCb *cellCb, RgDlSf *dlSf, RgDlHqProcCb *hqP, RgTfuDatReqPduInfo *datReq)
                                                       ;
 S16 rgDHMHqEntInit
(
Inst inst,
...

{
   ...
   {
      if (rgAllocSBuf(inst,(Data **)&(hqE->procs[idx1]),sizeof(RgDlHqProcCb)) != 0)
      {
         while(idx1--)
         {
            rgFreeSBuf(inst,(Data **)&(hqE->procs[idx1]), sizeof(RgDlHqProcCb));
         }
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0x1170001, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_dhm.c",170, "Memory Alloc Failure for RgDlHqProcCb", RLOG_FILE_ID, RLOG_MODULE_NAME); };
         return(1);
      }
      ...
      for(idx2 = 0; idx2 < 2; idx2++)
      {
         ...
         hqE->procs[idx1]->tbInfo[idx2].tb = 0L;
      }
...

{
   ...
   if(hqE->procs[0])
   {
      ...
      for (i=0; i < hqE->numHqProcs; i++)
      {
         ...
         rgDHMFreeSavedHqP((cell->macInst - 0), hqE, i);
      }
   }
   return;
}
...

{
   ...
   {
      rgDHMHdlBufFree(cell->macInst - 0, &hqP->tbInfo[tbIndex-1].tb);
   }
   hqP->tbInfo[tbIndex-1].schdTa.pres = 0;
   ...
   if (hqP->tbInfo[tbIndex-1].sfLnk.node != 0L)
   {
      cmLListDelFrm(&hqP->tbInfo[tbIndex-1].sf->tbs,
                    &hqP->tbInfo[tbIndex-1].sfLnk);
      hqP->tbInfo[tbIndex-1].sfLnk.node = (U32)0L;
   }
   hqP->tbInfo[tbIndex-1].sf = 0L;


   hqP->tbInfo[tbIndex-1].tbSz = 0;
   hqP->tbInfo[tbIndex-1].contResCe = 0;
   hqP->tbInfo[tbIndex-1].contResId = 0L;
...
)
{
   ...
   for(i=0;i< 2;i++)
   {
      if (hqP->tbInfo[i].sf == dlSf)
      {
         ...
         if (hqP->tbInfo[i].tb == 0L)



         {

            if (hqP->tbInfo[i].schdTa.pres == 1 ||
                  hqP->tbInfo[i].contResCe == 1)
...

            {
               ...
               if (0 != rgMUXBldPdu(inst,&bldPdu, &(hqP->tbInfo[i].tb), err))
               {
                  return(1);
               }
            }
...

{
   ...
   for(i=0;i<datReq->nmbOfTbs;i++)
   {
      ...
      if (hqProc->tbInfo[i].sfLnk.node == 0L)
      {
      ...
      { if (0L != (hqProc->tbInfo[i].tb)) { SPutMsg((hqProc->tbInfo[i].tb)); hqProc->tbInfo[i].tb = 0L; } };
      ...
      if(rgMUXBldPdu(inst,&bldPdu, &(hqProc->tbInfo[i].tb), err) != 0)
      {
...

{
   ...
   ;
...

      {
         ...
         rgFillDgnParams(inst,&dgn, 2);
         rgLMMStaInd(inst,1, 1,
               (256 + 0), &dgn);
         return(1);
      }
   ...
   if(rgMUXBldPdu(inst,&bldPdu, &(hqProc->tbInfo[0].tb), err) != 0)
   {
      { if (0L != (datReq->pdu)) { SPutMsg((datReq->pdu)); datReq->pdu = 0L; } };
      return(1);
   }
...
)
{
   ...
   {
      if((ue=rgDBMGetUeCb (cell, ueInfo->allocInfo[ueIdx].rnti)) == 0L)
      {

         if((ue=rgDBMGetUeCbFromRachLst (cell,
                     ueInfo->allocInfo[ueIdx].rnti)) == 0L)
         {
            if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x1170002,L_ERROR,DBG_CELLID, cell->cellId, ueInfo->allocInfo[ueIdx].rnti, 0, 0, 0,                                                   "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_dhm.c"            ,                                                   916            , "CRNTI:%d No ueCb found", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                    ;
      ...
      if((allocInfo->tbInfo[0].isReTx == 1) &&
            (hqP->tbInfo[0].tbSz == 0))
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x1170003,L_ERROR,DBG_CELLID, cell->cellId, allocInfo->rnti, hqP->procId, 0, 0,                                              "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_dhm.c"         ,                                              933         , "CRNTI:%d RETX hqP(%d) tb(0) for a failed New Tx", RLOG_FILE_ID, RLOG_MODULE_NAME); }

                                               ;
         continue;
      }
      if((allocInfo->tbInfo[1].isReTx == 1) &&
            (hqP->tbInfo[1].tbSz == 0))
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x1170004,L_ERROR,DBG_CELLID, cell->cellId, allocInfo->rnti, hqP->procId, 0, 0,                                              "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_dhm.c"         ,                                              941         , "CRNTI:%d RETX hqP(%d) tb(1) for a failed New Tx", RLOG_FILE_ID, RLOG_MODULE_NAME); }
               ...
               allocInfo->nmbOfTBs); idx++)
      {
         ...
         hqP->tbInfo[idx].sfLnk.node = (U32)hqP;
         ...
         if(allocInfo->tbInfo[idx].isReTx == 1)
         {
         ...
         { if (0L != (hqP->tbInfo[idx].tb)) { SPutMsg((hqP->tbInfo[idx].tb)); hqP->tbInfo[idx].tb = 0L; } };
         ...
         if (allocInfo->tbInfo[idx].schdDat[0].lcId == 0x00)
         {
            ...
            {
               if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x1170005,L_DEBUG,DBG_CELLID, cell->cellId, allocInfo->rnti, hqP->procId, 0, 0,                                 "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_dhm.c"               ,                                 1041               , "UEID:%d MSG4 with only contResId hqP(%d)", RLOG_FILE_ID, RLOG_MODULE_NAME); }
            ...
            {
               if ((rgAllocShrablSBuf(inst,(Data**)&cStaInd, sizeof(RguCStaIndInfo))) != 0)
               {
                  err->errType = (((0 + 11) + 1) + 2);
                  err->errCause = 0 + 3 + 1;
                  return(1);
               }
            }
            ...
            if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x1170006,L_DEBUG,DBG_CELLID, cell->cellId, allocInfo->rnti, hqP->procId, cStaInd->lcId, 0,                              "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_dhm.c"            ,                              1082            , "RNTI:%d UE:MSG4 grant for CCCH hqP(%d) LCID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
         ...
         else
         {
            ...
            { if (0L != (hqP->tbInfo[idx].tb)) { SPutMsg((hqP->tbInfo[idx].tb)); hqP->tbInfo[idx].tb = 0L; } };
            if((0L == dStaInd[rguDlSuId]) && (allocInfo->tbInfo[idx].numSchLch))
            {
               if ((rgAllocShrablSBuf (inst,(Data**)&dStaInd[rguDlSuId], sizeof(RguDStaIndInfo))) != 0)
               {
                  err->errType = (((0 + 11) + 1) + 2);
                  err->errCause = 0 + 3 + 1;
                  ...
                  lcIdx < allocInfo->tbInfo[idx].numSchLch; lcIdx++)
            {
               ...
               {
                  if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x1170007,L_ERROR,DBG_CELLID, cell->cellId, hqP->tbInfo[idx].schdData[lcIdx].lcId, allocInfo->rnti, 0, 0,                                                                             "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_dhm.c"                  ,                                                                             1126                  , "CCCH grant in DStaInd for LCID:%d CRNTI:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
...

{
   ...
   for(i=0;i<2;i++)
   {
      ...
      if ((hqP->tbInfo[i].sf == dlSf) && (hqP->tbInfo[i].tb != 0L))
...

      {
         ...
         SIncMsgRef(hqP->tbInfo[i].tb, (rgCb[inst].rgInit.region),
               (rgCb[inst].rgInit.pool), &datReq->mBuf[i]);

         {
            ...
            if(SFndLenMsg(datReq->mBuf[i], &dbgBufLen))
            {
               if(dbgBufLen == 0)
               {
                  if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x1170008,L_ERROR,DBG_CELLID, cellCb->cellId, datReq->rnti, hqP->tbInfo[i].timingInfo.sfn, hqP->tbInfo[i].timingInfo.subframe, i,                                                         "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_dhm.c"                  ,                                                         1319                  , "RNTI:%d SFN:%d subframe:%d tbIdx:%d Sdu Length 0 ", RLOG_FILE_ID, RLOG_MODULE_NAME); }



                                                          ;
                  if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x1170009,L_ERROR,DBG_CELLID, cellCb->cellId, datReq->isTApres, hqP->numOfTBs, datReq->dciInfo.format, 0,                                                 "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_dhm.c"                  ,                                                 1324                  , "taPres [%d] numOfTbs [%d] format[%d]", RLOG_FILE_ID, RLOG_MODULE_NAME); }
...

{
      ...
      ||( cell->cellId != rlshqUeInfo->cellId))
   {
      ...
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x117000a,L_ERROR,DBG_CELLID, rlshqUeInfo->cellId, 0, 0, 0, 0,                                             "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_dhm.c"      ,                                             1672      , "No cellCb found with cellId", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                              ;
      return(1);
   }

   if(0L == rlshqUeInfo->ueHqInfo)
   {
      return(1);
   }
   ...
   {
      if((ue=rgDBMGetUeCb (cell, rlshqUeInfo->ueHqInfo[idx1].rnti)) == 0L)
      {

         if((ue=rgDBMGetUeCbFromRachLst (cell,
                     rlshqUeInfo->ueHqInfo[idx1].rnti)) == 0L)
         {
            if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x117000b,L_ERROR,DBG_CELLID, rlshqUeInfo->cellId, rlshqUeInfo->ueHqInfo[idx1].rnti, 0, 0, 0,                                                     "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_dhm.c"            ,                                                     1690            , "CRNTI:%d No ueCb found", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                      ;
            continue;
         }
      }
      ...
      if (0x02 == rlshqUeInfo->ueHqInfo[idx1].rlsOperationType)
      {
      ...
      if(rlshqUeInfo->ueHqInfo[idx1].status[0] != 1)
      {
      ...
      for(idx2=0; idx2 < rlshqUeInfo->ueHqInfo[idx1].numOfTBs; idx2++)
      {
         if(rgDHMRlsHqProcTB(cell, hqP,
               rlshqUeInfo->ueHqInfo[idx1].tbId[idx2]) != 0)
         {
            if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x117000c,L_ERROR,DBG_CELLID, rlshqUeInfo->cellId, rlshqUeInfo->ueHqInfo[idx1].rnti, 0, 0, 0,                                                  "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_dhm.c"            ,                                                  1736            , "CRNTI:%d Failure in releasing hq TB", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   SStopTask(startTime,30010);

   return(0);
}
 S16 RgSchMacHndlRelReq
(
 RgCellCb *cell,
 RgUeCb *ue,
 RgInfUeHqInfo *ueHqInfo
...

{
   ...
   U8 tbId;
   CmLList *node;
   RgDlSavedHqProcInfo *hqPNode = 0L;
   U8 procId;
   U16 saveId;
   Inst inst = cell->macInst - 0;
   ...
   switch (ueHqInfo->rlsOperationType)
   {
      case 0x01:
      {
         ...
         hqPNode->savedHqPLnk.node = (U32)hqPNode;
      ...
      case 0x02:
      {
         ...
         while(node)
         {
            ...
            for (idx = 0; idx < ueHqInfo->numOfTBs; idx++)
            {
               ...
               rgDHMHdlBufFree(cell->macInst - 0,
                     &hqPNode->tb[tbId]);
               hqPNode->tb[tbId] = 0L;
