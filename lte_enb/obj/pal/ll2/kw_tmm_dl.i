...
static Void kwTmmSndStaRsp (KwCb *gCb, KwDlRbCb *rbCb, MsgLen bo, KwuDatReqInfo *datReqInfo)
                                                                       ;
extern U32 rgMacGT ;
 Void kwTmmQSdu
(
...

{
   ...
   { if (SGetSBuf(gCb->init.region, gCb->init.pool, (Data **)&sdu, (Size) sizeof(KwSdu)) == 0) { cmMemset((U8 *)(sdu), 0, sizeof(KwSdu)); } else { (sdu) = 0L; } };
   if ( rbCb->lch.lChType == 1 ||
        rbCb->lch.lChType == 2 )
   {
...

{
   ...
   ((((node) = ((&(rbCb->m.tm.sduQ))->crnt = (&(rbCb->m.tm.sduQ))->first)) != 0L) ? (node)->node : 0L)
                            ;
   ...
   while (node != 0L)
   {
            ...
            rbCb->lch.lChType == 2 )
      {
         ...
         if ((sdu->mode.tm.sfn != sfn) ||
               (sdu->mode.tm.subframe != ((subframe+2)%10)))
         {
            node = node->next;
            if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc80001,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, sdu->mode.tm.rnti, staInd->rnti, rbCb->rlcId.ueId, rbCb->rlcId.cellId,                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_tmm_dl.c"            ,                                    249            , "Releasing SDU of RNTI = %d for RNTI = %d UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }




                                     ;
            if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc80002,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, sfn, subframe, rbCb->rlcId.ueId, rbCb->rlcId.cellId,                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_tmm_dl.c"            ,                                    255            , "sfn %d subframe %d  UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
            ...
            { if (sdu->mBuf != 0L) { SPutMsg(sdu->mBuf); } sdu->mBuf = 0L; };
            { if (sdu != 0L) { (Void) SPutSBuf(gCb->init.region, gCb->init.pool, (Data *) sdu, (Size) sizeof(KwSdu)); sdu = 0L; } };
         }
         else
         {
            break;
      ...
      else
      {
         ...
         if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc80003,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, staInd->rnti, sdu->mode.tm.rnti, rbCb->rlcId.ueId, rbCb->rlcId.cellId,                                 "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_tmm_dl.c"         ,                                 282         , "TMM: TmSdu Sta Indication received for Rnti %d Sdu Rnti %d " " UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                  ...
                                  ;
         if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc80004,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, timeDiff, rbCb->m.tm.sduQ.count, rbCb->rlcId.ueId, rbCb->rlcId.cellId,                                 "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_tmm_dl.c"         ,                                 289         , "TMM: TmSdu Sta Indication received : timeDiff %d SduQCnt %lu" " UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
         ...
         if (timeDiff > 40)
         {
            ...
            node = node->next;
            if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc80005,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, sdu->mode.tm.rnti, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0,                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_tmm_dl.c"            ,                                    299            , " timeDiff greater than 40, so deleting the Sdu %u " " UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
         ...
         if (sdu->mode.tm.rnti != staInd->rnti)
         {
            ...
            node = node->next;
            if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc80006,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, staInd->rnti, sdu->mode.tm.rnti, rbCb->rlcId.ueId, rbCb->rlcId.cellId,                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_tmm_dl.c"            ,                                    316            , "TMM: Searching for Rnti %d Skipping Sdu for Rnti %d" " UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                     ...
                                     ;
            if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc80007,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, timeDiff, sdu->arrTime, rbCb->rlcId.ueId, rbCb->rlcId.cellId,                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_tmm_dl.c"            ,                                    323            , " timeDiff %d sdu->arrTime %d" " UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                     ...
                                     ;
            if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc80008,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, curTime, rbCb->m.tm.sduQ.count, rbCb->rlcId.ueId, rbCb->rlcId.cellId,                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_tmm_dl.c"            ,                                    330            , "curTime %d SduQCnt %lu and continuing" " UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
         ...
         {
            if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc80009,L_DEBUG,DBG_RBID, rbCb->rlcId.rbId, sdu->mode.tm.rnti, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0,                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_tmm_dl.c"            ,                                    339            , "TMM: TmSdu found %u UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   if ( node == 0L )
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xc8000a,L_ERROR,DBG_RBID, rbCb->rlcId.rbId, rbCb->rlcId.ueId, rbCb->rlcId.cellId, 0, 0,                                 "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_tmm_dl.c"      ,                                 350      , "SDU not found TM Queue is empty UEID:%d CELLID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   sdu = (KwSdu *)node->node;
    ...
    { if (SGetStaticBuffer(gCb->u.dlCb->rguDlSap[suId].pst.region, gCb->u.dlCb->rguDlSap[suId].pst.pool, (Data **)&cDatReqInfo, (Size) (Size)sizeof(RguCDatReqInfo), 0) == 0) { cmMemset((U8 *)(cDatReqInfo), 0, (Size)sizeof(RguCDatReqInfo)); } else { (cDatReqInfo) = 0L; } }
...

{
   ...
   { if (SGetStaticBuffer(gCb->u.dlCb->rguDlSap[rbCb->rguSapId].pst.region, gCb->u.dlCb->rguDlSap[rbCb->rguSapId].pst.pool, (Data **)&staRspInfo, (Size) sizeof(RguCStaRspInfo), 0) == 0) { cmMemset((U8 *)(staRspInfo), 0, sizeof(RguCStaRspInfo)); } else { (staRspInfo) = 0L; } }
