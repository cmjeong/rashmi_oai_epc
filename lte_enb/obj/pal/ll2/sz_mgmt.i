...
static Bool szMgmtChkConnLst ( SztResetTyp *tknIE, S1apPdu *pdu, U8 *cause )



          ;
PFSZMGMTUIMSGHANDLER szMgmtUiEnb[73] =
{
...

{
   ...
   if(peerCb->skipUeSm)
   {
      ...
      if ((ret = szLiSndToLower(peerCb, 0L, pdu)) != 0)
      {
         if((ret == 7) ||
               (ret == 8))
         {
...

{
   ...
   if ((ret = szLiSndAssocReq(peerCb->assocCb)) != 0)
   {

      if ((ret = szLiSndToLower(peerCb, 0L, peerCb->setupReq)) != 0)
      {
         if((ret == 7) ||
               (ret == 8))
         {
...

{
   ...
   if(peerCb->skipUeSm == 0)
   {
      ...
      if (peerCb->state == 1)
      {
         ...
         if ((ret = szMgmtChkConnLst(((SztResetTyp *)tknIE), peerCb->resetPdu, cause)) == 1)
         {
            uDatEvnt.transId.pres = 0;
            uDatEvnt.peerId.pres = 1;
            uDatEvnt.peerId.val = peerCb->peerId;
   ...
   if(peerCb->skipUeSm == 0)
   {
      ...
      if (((SztResetTyp *)tknIE)->choice.val == 0)
      {
         (Void) szNdbRelConns(peerCb);
      }
      else
      {
         ...
         for (idx = 0; idx < nmbConns; idx++)
         {
            ...
            if (((SztResetTyp *)tknIE)->val.partOfS1_Intf.member[idx].value.u.
                  sztUE_assocLogS1_ConItem.pres.pres != 0)
            {

               {

                  if (((SztResetTyp *)tknIE)->val.partOfS1_Intf.member[idx].
                        value.u.sztUE_assocLogS1_ConItem.eNB_UE_S1AP_ID.pres != 0)
                  {
                     ...
                     conRefType = 1;
                        ...
                        value.u.sztUE_assocLogS1_ConItem.mME_UE_S1AP_ID.pres != 0)
                  {
                     conRefNo = ((SztResetTyp *)tknIE)->val.partOfS1_Intf.member[idx].
      ...
      if ((ret = szDbFillResetFrmPdu(pdu, &peerCb->resetPdu, mem)) != 0)
      {
         *cause = 9;
         { if (pdu != 0L) { cmFreeMem ((Ptr)(pdu)); pdu = 0L; } };
         return(ret);
   ...
   if ((ret = szLiSndToLower(peerCb, 0L, pdu)) != 0)
   {
      ...
      { if (peerCb->resetPdu != 0L) { cmFreeMem ((Ptr)(peerCb->resetPdu)); peerCb->resetPdu = 0L; } };
      return(ret);
   }
...

{
   ...
   if(peerCb->skipUeSm == 0)
   {
      ...
      if (peerCb->state == 1)
      {
         ...
         if ((ret = szMgmtChkConnLst(((SztResetTyp *)tknIE), peerCb->resetPdu, &cause)) == 1)
         {
            ...
            szLiSndToLower(peerCb, 0L, rstAckPdu);
...

               {
                        ...
                        value.u.sztUE_assocLogS1_ConItem.eNB_UE_S1AP_ID.pres != 0)
                  {
                     ...
                     conRefType = 1;
                        ...
                        u.sztUE_assocLogS1_ConItem.mME_UE_S1AP_ID.pres != 0)
                  {
                     conRefNo = ((SztResetTyp *)tknIE)->val.partOfS1_Intf.member[idx].
                                value.u.sztUE_assocLogS1_ConItem.mME_UE_S1AP_ID.val;
   ...
   if ((ret = szUiSndUDatInd(peerCb, &uDatEvnt)) != 0)
   {
...

{
   ...
   U8 j;
   U16 nmbConns = 0;
   SzConRefNo conRefNo = 0;
   TknU8 *tmpReset = 0L;
   U16 tmpConns = 0;
   SzConRefNo tmpCon;
   Bool isEqual = 0;
   SztUE_assocLogS1_ConItem *conItem, *tmpConItem;
   ...
   *cause = 0;
   ...
   szGetIE(&pdu->pdu, 92, &tmpReset);

   if((tknIE == 0L) || (tmpReset == 0L))
   {
      *cause = 18;
      return(0);
   }
   ...
   if (((SztResetTyp *)tmpReset)->choice.val == 0)
   {
      if ((tknIE->choice.val == 1) ||
          (tknIE->choice.val == 0))
         return(1);
   }
   ...
   for (i = 0; i < nmbConns; i++)
   {
      ...
      if (conItem->pres.pres != 0)
      {
         if (conItem->mME_UE_S1AP_ID.pres != 0)
         {
            conRefNo = conItem->mME_UE_S1AP_ID.val;
         }
         else if (conItem->eNB_UE_S1AP_ID.pres != 0)
         {
      ...
      else
      {
         ...
         { if ((&szCb.init)->dbgMask & (0x00000100)) { snprintf((&szCb.init)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", szCb.nodeDbg, (&szCb.init)->procId, (&szCb.init)->ent, (&szCb.init)->inst,
                                                              "/home/mrehwald/Desktop/Radisys/lte_enb/src/s1ap/sz_mgmt.c"
         ,
                                                              1322
         ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "szMgmtChkConnLst: S1 Connection is empty \n"); SPrint((&szCb.init)->prntBuf); } }
                                                               ;
      }
      ...
      for (j = 0; j < tmpConns; j++)
      {
        ...
        if (tmpConItem->pres.pres != 0)
        {
            if (tmpConItem->mME_UE_S1AP_ID.pres != 0)
            {
