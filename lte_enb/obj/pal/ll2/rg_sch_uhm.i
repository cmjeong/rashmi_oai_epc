...
extern S16 rgSCHCmnDlAllocErrIndTxRb ( RgSchCellCb *cell, RgSchDlSf *subFrm, RgSchUeCb *ue, U32 bo, U32 *effBo, RgSchDlHqProcCb *proc, RgSchCmnDlRbAllocInfo *cellWdAllocInfo )
  ...
  ;
 U8 rgRvIdxTable[] = {0, 3, 1, 2};
 U8 rgRvTable[] = {0, 2, 3 ,1};
...
)
{
   ...
   if((hqProc->isSpsActvnHqP != 1) &&
      (hqProc->isSpsOccnHqP != 1))

   {
      ueUl->ulLaCb.deltaiTbs += 3;
   ...
   ue->tenbStats->stats.nonPersistent.sch[(U8)((cell->cellId >= rgSchCb[cell->instIdx].genCfg.startCellId) ? (cell->cellId - rgSchCb[cell->instIdx].genCfg.startCellId)&(8 -1): 0)].ulAckNackCnt++;
   if(hqProc->alloc)
   {
      ue->tenbStats->stats.nonPersistent.sch[(U8)((cell->cellId >= rgSchCb[cell->instIdx].genCfg.startCellId) ? (cell->cellId - rgSchCb[cell->instIdx].genCfg.startCellId)&(8 -1): 0)].ulTpt += (hqProc->alloc->grnt.datSz << 3);
      cell->tenbStats->sch.ulTtlTpt +=(hqProc->alloc->grnt.datSz << 3);
   }
...
)
{
   ...
   ue->tenbStats->stats.nonPersistent.sch[(U8)((cell->cellId >= rgSchCb[cell->instIdx].genCfg.startCellId) ? (cell->cellId - rgSchCb[cell->instIdx].genCfg.startCellId)&(8 -1): 0)].ulNackCnt++;
...

{
   ...
   else
   {
      ...
      {
         ue = hqProc->alloc->ue;
         if (ue && (1 == hqProc->isSpsActvnHqP))
         {
            ulSpsUe = &(ue->ul.ulSpsInfo);
            if (ulSpsUe->state == 0x04)
            {
               ulSpsUe->pdcchToRetx = 1;
               ulSpsUe->actPdcchCrcFailCount++;
            }
         }
      }
   }
   hqProc->isSpsActvnHqP = 0;
   hqProc->isSpsOccnHqP = 0;
   hqProc->alloc = 0L;
   hqProc->ulSfIdx = 0xff;
...
{
   U8 i;
   RgSchCmnUlUe *ueUl = (&(((RgSchCmnUe *)((ueCb->cellInfo[ueCb->cellIdToCellIdxMap [(U8)((cellCb->cellId >= rgSchCb[cellCb->instIdx].genCfg.startCellId) ? (cellCb->cellId - rgSchCb[cellCb->instIdx].genCfg.startCellId)&(8 -1): 0)]])->sch))->ul));
   ...
   for(i=0; i < ueUl->hqEnt.numHqPrcs; i++)
   {
      ...
      ueUl->hqEnt.hqProcCb[i].ulSfIdx = 0xff;
      ueUl->hqEnt.hqProcCb[i].alloc = 0L;


      ueUl->hqEnt.hqProcCb[i].isSpsActvnHqP = 0;
      ueUl->hqEnt.hqProcCb[i].isSpsOccnHqP = 0;
...

{
   ...
   RgSchCmnUlCell *cellUl = &(((RgSchCmnCell *)((cellCb)->sc.sch))->ul);
   ...
   if(cellUl->hqFdbkIdx[idx] != 0xff)
   {
      ...
      while (ulAlloc)
      {
         ...
         if((ulAlloc->ue != 0L) && ((1 != ulAlloc->forMsg3) &&
                  ((ulAlloc->ue->measGapCb.isMeasuring == 1) ||
                   (ulAlloc->ue->ackNakRepCb.isAckNakRep == 1))))
         {
            ...
            if (rgSCHUtlGetPhichInfo (ulAlloc->hqProc, &rbStart, &nDmrs) != 0)
         ...
         if(nDmrs != 10)
         {
            ...
            if(ulAlloc->hqProc->rcvdCrcInd)
            {
               ...
               rgSCHUtlAddPhich (cellCb, frm, 1, nDmrs, rbStart, ulAlloc->forMsg3);

            }

            else
            {
               rgSCHUtlAddPhich (cellCb, frm, 0, nDmrs, rbStart, ulAlloc->forMsg3);
