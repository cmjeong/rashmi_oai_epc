...
static S16 rgDBMInitUeCbLst ( RgCellCb *cellCb, U16 numBins);
...
static S16 rgDBMInitSpsUeCbLst ( RgCellCb *cellCb, U16 numBins )
...

{
   ...
   if ((ret = rgDBMInitUeCbLst(cellCb, 128)) != 0)
      return(ret);



   if ((ret = rgDBMInitSpsUeCbLst(cellCb, 128)) != 0)
      return(ret);
...

{
   ...
   return(cmHashListInit(&cellCb->ueLst, numBins, (U16)((U32)&(ue.ueLstEnt) - (U32)&ue), 0, 5, rgCb[inst].rgInit.region, rgCb[inst].rgInit.pool))
                                                                                        ;
...

{
   ...
   return(cmHashListInit(&cellCb->spsUeLst, numBins, (U16) ((U32) &(ue.spsUeLstEnt) - (U32) &ue), 0, 5, rgCb[inst].rgInit.region, rgCb[inst].rgInit.pool))
                                                                                        ;
...

{
   ...
   return(cmHashListInsert(&cellCb->ueLst, (U32)ueCb, (U8 *)&ueCb->ueId, (U16)sizeof(ueCb->ueId)))
                                                  ;
...

{
   ...
   cmHashListGetNext(&cellCb->ueLst, (U32) ueCb, (U32 *)&nextUeCb);
   return(nextUeCb);
}
...

{
   ...
   return(cmHashListDelete(&cellCb->ueLst, (U32)ueCb));
}
 S16 rgDBMDelSpsUeCb
(
RgCellCb *cellCb,
...

{
   ...
   for (idx = 0; idx < 10; ++idx)
   {
      ...
      ueCb->ul.lcCb[idx].lcId = 255;
      ueCb->ul.lcCb[idx].lcgId = 255;
      ueCb->dl.lcCb[idx].lcId = 255;
   }

   for (idx = 0; idx < 4; ++idx)
   {

      ueCb->ul.lcgArr[idx].lcgId = 255;
...

{
   ...
   cellCb->bcchBchInfo.lcId = 255;
   cellCb->pcchInfo.lcId = 255;

   cellCb->numBcchDlschInfo = 0;
   for (idx = 0; idx < 2; idx++)
   {
      cellCb->bcchDlschInfo[idx].lcId = 255;
      cellCb->bcchDlschInfo[idx].tb = 0L;
   }
...

{
   ...
   {
      cellCb->bcchDlschInfo[idx].lcId = 255;
      { if (0L != (cellCb->bcchDlschInfo[idx].tb)) { SPutMsg((cellCb->bcchDlschInfo[idx].tb)); cellCb->bcchDlschInfo[idx].tb = 0L; } };
   }
   cellCb->numBcchDlschInfo = 0;
...

{
   ...
   ueCb->dl.lcCb[dlLcCb->lcId - 1].lcId = 255;
...

{
   ...
   if(ueCb->dl.lcCb[idx-1].lcId == 255)
   {
      return(0L);
   }
...

{
   ...
   if(ueCb->ul.lcgArr[gId].lcgId == 255)
   {
...

{
   ...
   {
      ueCb->ul.lcgArr[ulLcCb->lcgId].lcgId = 255;
   }
...

{
   ...
   ueCb->ul.lcCb[ulLcCb->lcId - 1].lcgId = 255;
   ueCb->ul.lcCb[ulLcCb->lcId - 1].lcId = 255;
...

{
   ...
   if (idx > 0x0A || idx < 0x01)
   {
      return(0L);
   }
   if(ueCb->ul.lcCb[idx-1].lcId == 255)
   {
      return(0L);
   }
...

{
   ...
   if(cellCb->bcchBchInfo.lcId != 255)
   {
...

{
   ...
   ((((tmpNode) = ((&cellCb->raInfo.ueRachLst)->crnt = (&cellCb->raInfo.ueRachLst)->first)) != 0L) ? (tmpNode)->node : 0L);
   while(tmpNode)
   {
      ...
      ((((tmpNode) = ((&cellCb->raInfo.ueRachLst)->crnt = ((&cellCb->raInfo.ueRachLst)->crnt ? (&cellCb->raInfo.ueRachLst)->crnt->next : (&cellCb->raInfo.ueRachLst)->first)))!= 0L) ? (tmpNode)->node : 0L);
   }
   return(0L);
}
...

{
   ...
   if(!ueCb)
   {
      return(cellCb->raInfo.ueRachLst.first ? (RgUeCb *)(cellCb->raInfo.ueRachLst.first->node) : 0L)
                                                                         ;
   }
   return(ueCb->rachLstEnt.next ? (RgUeCb *)(ueCb->rachLstEnt.next->node) : 0L)
                                                                ;
}
