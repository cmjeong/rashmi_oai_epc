...
S16 RgMacSchBrdcmDedBoUpdtReq (Inst inst, CmLteCellId cellId, CmLteRnti rnti, CmLteLcId lcId, S32 bo );
...

{
       ...
       || (cell->cellId != datReq->cellId))
   {
      ...
      err.errCause = 0 + 3 + 1 + 2 + 1;
      if(cell != 0L)
      {

         rgUpdtRguDedSts(inst,cell->rguDlSap,1, datReq);
      }
      return(1);
   }
   ...
   for(idx = 0; idx < datReq->nmbOfUeGrantPerTti; idx++)
   {
      ...
      if( (sf->txDone == 1) ||
            (!((sf->schdTime.sfn == timingInfo.sfn) && (sf->schdTime.subframe == timingInfo.subframe))))
      {
         ...
         err.errType = ((((0 + 11) + 1) + 1) + 1);
         err.errCause = 0 + 3 + 1 + 2 + 1 + 1;

         rgUpdtRguDedSts(inst,cell->rguDlSap,1, datReq);
         printf("\n Dropping due to time\n");

         { U32 idx5,idx6,idx7; for (idx5=0; idx5 < datReq->datReq[idx].nmbOfTbs; idx5++) { for (idx6=0; idx6 < datReq->datReq[idx].datReqTb[idx5].nmbLch; idx6++) { for (idx7=0; idx7 < datReq->datReq[idx].datReqTb[idx5].lchData[idx6].pdu.numPdu; idx7++) { { if (0L != (datReq->datReq[idx].datReqTb[idx5]. lchData[idx6].pdu.mBuf[idx7])) { SPutMsg((datReq->datReq[idx].datReqTb[idx5]. lchData[idx6].pdu.mBuf[idx7])); datReq->datReq[idx].datReqTb[idx5]. lchData[idx6].pdu.mBuf[idx7] = 0L; } }; printf("\n Dropping MAC PDU \n"); } } }};

         continue;

      }

      if ((ue = rgDBMGetUeCb(cell, datReq->datReq[idx].rnti)) == 0L)
      {
      ...
      if (rgDHMHndlDedDatReq(inst,hqProc, &datReq->datReq[idx], &err) == 1)
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xb80001,L_ERROR,DBG_CELLID, datReq->cellId, datReq->datReq[idx].rnti, 0, 0, 0,                                            "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_rom.c"         ,                                            239         , "Handling of Data request in DHM failedi RNTI:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }

                                             ;
         err.errType = ((((0 + 11) + 1) + 1) + 1);


         rgUpdtRguDedSts(inst,ue->rguDlSap,1, datReq);
...

{
      ...
      ||(cell->cellId != datReq->cellId))
   {
      ...
      if(cell != 0L)
      {
         rgUpdtRguCmnSts(inst,cell->rguDlSap,1);
      }
      return(1);
   }
   ...
   else
   {
      ...
      rgUpdtRguCmnSts(inst,cell->rguDlSap,2);
   }
...

{
   ...
   if ((ue = rgDBMGetUeCb(cell, datReq->u.rnti)) == 0L)
   {
      if ((ue = rgDBMGetUeCbFromRachLst(cell, datReq->u.rnti)) == 0L)
      {
   ...
   if (rgDHMHndlCmnDatReq(inst,hqProc, datReq, err) == 1)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xb80002,L_ERROR,DBG_CELLID, cell->cellId, datReq->u.rnti, datReq->lcId, 0, 0,                                       "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_rom.c"      ,                                       515      , "Handling of Data request in DHM failed RNTI:%d LCID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
...

{
   ...
   SLogError(rgCb[inst].rgInit.ent, rgCb[inst].rgInit.inst, rgCb[inst].rgInit.procId, (Txt *)
                                                            "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_rom.c"
   , (S32)
                                                            623
   , (ErrCls) (0x2), (ErrCode) (((0 + 0) + 5)), (ErrVal) ((ErrVal)datReq->lcId), (Txt *) "rgROMHndlBcchPcchDatReq(): Invalid Lc Id")
                                                             ;
   err->errCause = 0 + 3 + 1 + 2 + 1 + 3;
...

{
   ...
   if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xb80003,L_ERROR,DBG_CELLID, staRsp->cellId, staRsp->rnti, staRsp->lcId, 0, 0,                                      "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_rom.c"   ,                                      687   , "Invalid cell for CRNTI:%d LCID:%d ", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                       ;
...

{
      ...
      || (cell->cellId != staRsp->cellId))
   {
      ...
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xb80004,L_ERROR,DBG_CELLID, staRsp->cellId,staRsp->u.rnti, staRsp->lcId, 0, 0,                                                                               "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_rom.c"      ,                                                                               759      , "Invalid cell for CRNTI:%d LCID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                                                ;
      err.errType = (((((0 + 11) + 1) + 1) + 1) + 3);
      err.errCause = 0 + 3 + 1 + 2 + 1;
      return(1);
   }
...

{
   ...
   if (rgDBMChkCmnLcCb(cell, staRsp->lcId) != 0)
   {

      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xb80005,L_ERROR,DBG_CELLID, cell->cellId,staRsp->lcId, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_rom.c",939, "Invalid LCID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      err->errCause = 0 + 3 + 1 + 2 + 1 + 3;
      return(1);
   }
   ...
   if (cell->bcchBchInfo.lcId == staRsp->lcId)
   {
      ...
      nextBchSfn = (cell->crntTime.sfn + 4 - (cell->crntTime.sfn%4)) % 1024;
...

{
   ...
   if(!dlSf->remDatReqCnt)
   {
      ...
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xb80006,L_ERROR,DBG_CELLID, cellCb->cellId, 0, 0, 0, 0,                                                          "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_rom.c"      ,                                                          1009      , "RX new data while remDatReqCnt is 0 for cell", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                           ;
      return(1);
   }
   ...
   if((0 == --dlSf->remDatReqCnt) && !(dlSf->txDone) &&
      (((cellCb->crntTime.sfn == dlSf->schdTime.sfn) && (cellCb->crntTime.subframe == dlSf->schdTime.subframe))) && (dlSf->statIndDone))
   {
      ...
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xb80007,L_ERROR,DBG_CELLID, cellCb->cellId, 0, 0, 0, 0,                                                             "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_rom.c"         ,                                                             1028         , "Unable to process downlink subframe for cell", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                              ;
         err.errType = ((((0 + 11) + 1) + 1) + 1);
      }
