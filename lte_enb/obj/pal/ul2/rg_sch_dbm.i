...
static S16 rgSCHDbmInitUeCbLst ( RgSchCellCb *cellCb, U16 numBins);
...
static S16 rgSCHDbmInitSpsUeCbLst (RgSchCellCb *cellCb, U16 numBins)
                                   ;
...

{
   ...
   if ((ret = rgSCHDbmInitUeCbLst(cellCb, 128)) != 0)
      return(ret);
   if ((ret = rgSCHDbmInitSpsUeCbLst(cellCb, 128)) != 0)
      return(ret);
...

{
   ...
   return(cmHashListInit(&cellCb->ueLst, numBins, (U16)((U32)&(ueCellInfo.ueLstEnt) - (U32)&ueCellInfo), 0, 5, rgSchCb[cellCb->instIdx].rgSchInit.region, rgSchCb[cellCb->instIdx].rgSchInit.pool))
...

{
   ...
   return(cmHashListInit(&cellCb->spsUeLst, numBins, (U16) ((U32) &(ue.spsUeLstEnt) - (U32) &ue), 0, 5, rgSchCb[cellCb->instIdx].rgSchInit.region, rgSchCb[cellCb->instIdx].rgSchInit.pool))
...
{
   RgSchUeCellInfo *ueCellInfo = 0L;
   ;

   ueCellInfo = ueCb->cellInfo[ueCb->cellIdToCellIdxMap[(U8)((cellCb->cellId >= rgSchCb[cellCb->instIdx].genCfg.startCellId) ? (cellCb->cellId - rgSchCb[cellCb->instIdx].genCfg.startCellId)&(8 -1): 0)]];

   return(cmHashListInsert(&cellCb->ueLst, (U32)ueCellInfo, (U8 *)&ueCb->ueId, (U16)sizeof(ueCb->ueId)))
                                                  ;
...

{
   ...
   return(cmHashListInsert(&cellCb->spsUeLst, (U32)ueCb, (U8 *)&ueCb->spsRnti, (U16)sizeof(ueCb->spsRnti)))
                                                        ;
...

{
   ...
   cmHashListFind(&cellCb->ueLst, (U8 *)&ueId,
      sizeof(ueId), 0, (U32 *)&ueCellInfo);

   return(!ueCellInfo?0L:ueCellInfo->ue);
}
...
{
   RgSchUeCellInfo *ueCellInfo = 0L;
   RgSchUeCellInfo *nextUeCellInfo = 0L;
   ...
   if (ueCb)
   {
      ueCellInfo = ueCb->cellInfo[
         ueCb->cellIdToCellIdxMap[(U8)((cellCb->cellId >= rgSchCb[cellCb->instIdx].genCfg.startCellId) ? (cellCb->cellId - rgSchCb[cellCb->instIdx].genCfg.startCellId)&(8 -1): 0)]];
   }

   cmHashListGetNext(&cellCb->ueLst, (U32) ueCellInfo, (U32 *)&nextUeCellInfo);
   return(!nextUeCellInfo?0L:nextUeCellInfo->ue);
}
...
{
   RgSchUeCb *nextUeCb = 0L;

   ;

   cmHashListGetNext(&cellCb->spsUeLst, (U32) ueCb, (U32 *)&nextUeCb);
   return(nextUeCb);
}
 S16 rgSCHDbmDelUeCb
(
...

{
   ...
   return(cmHashListDelete(&cellCb->ueLst, (U32)ueCellInfo));
}
...

{
   ...
   return(cmHashListDelete(&cellCb->spsUeLst, (U32)ueCb));
}
 S16 rgSCHDbmInitUe
(
RgSchUeCb *ueCb
...

{
   ...
   for(idx = lcCb->lcId; idx < 0x0A; idx++)
   {
      if(ueCb->dl.lcCb[idx])
      {
         return(ueCb->dl.lcCb[idx]);
...

{
   ...
   if(cellCb->cmnLcCb[0].lcId != 255)
   {
      return(&(cellCb->cmnLcCb[0]));
   }
   return(0L);
}
...

{
   ...
   if(cellCb->cmnLcCb[1].lcId != 255)
   {
      return(&(cellCb->cmnLcCb[1]));
   }
   return(0L);
}
...

{
   ...
   if(cellCb->cmnLcCb[2].lcId != 255)
   {
      return(&(cellCb->cmnLcCb[2]));
   }
   return(0L);
}
...

{
   ...
   if(cellCb->cmnLcCb[3].lcId != 255)
   {
      return(&(cellCb->cmnLcCb[3]));
   }
   return(0L);
}
...

{
   ...
   cellCb->cmnLcCb[0].lcId = cmnDlLcCb->lcId;
   cellCb->cmnLcCb[0].boLst = cmnDlLcCb->boLst;
...

{
   ...
   if(cellCb->cmnLcCb[1].lcId == 255)
   {
      cellCb->cmnLcCb[1].lcId = cmnDlLcCb->lcId;
      cellCb->cmnLcCb[1].boLst = cmnDlLcCb->boLst;
      cellCb->cmnLcCb[1].si = 0;
   }
   else if(cellCb->cmnLcCb[2].lcId == 255)
   {
      cellCb->cmnLcCb[2].lcId = cmnDlLcCb->lcId;
      cellCb->cmnLcCb[2].boLst = cmnDlLcCb->boLst;
      cellCb->cmnLcCb[2].si = 1;
   }
...

{
   ...
   cellCb->cmnLcCb[3].lcId = cmnDlLcCb->lcId;
   cellCb->cmnLcCb[3].boLst = cmnDlLcCb->boLst;
...

{
   ...
   cmnBoRpt->boLstEnt.next = 0L;
   cmnBoRpt->boLstEnt.prev = 0L;
   cmnBoRpt->boLstEnt.node = (U32)cmnBoRpt;
...

{
   ...
   ((((tmpNode) = ((&cellCb->raInfo.raCbLst)->crnt = (&cellCb->raInfo.raCbLst)->first)) != 0L) ? (tmpNode)->node : 0L);
   while(tmpNode)
   {
      ...
      ((((tmpNode) = ((&cellCb->raInfo.raCbLst)->crnt = ((&cellCb->raInfo.raCbLst)->crnt ? (&cellCb->raInfo.raCbLst)->crnt->next : (&cellCb->raInfo.raCbLst)->first)))!= 0L) ? (tmpNode)->node : 0L);
   }
   return(0L);
}
...

{
   ...
   cfgElem->cfgReqLstEnt.next = 0L;
   cfgElem->cfgReqLstEnt.prev = 0L;
...

{
   ...
   cfgElem->cfgReqLstEnt.next = 0L;
   cfgElem->cfgReqLstEnt.prev = 0L;
   cfgElem->cfgReqLstEnt.node = (U32)cfgElem;
...

{
   ...
   if(!cfgElem)
   {
      return(cellCb->rgCfgInfo.crntRgrCfgLst.first ? (RgSchCfgElem *)(cellCb->rgCfgInfo.crntRgrCfgLst.first->node) : 0L)
                                                                                      ;
   }
   return(cfgElem->cfgReqLstEnt.next ? (RgSchCfgElem *)(cfgElem->cfgReqLstEnt.next->node) : 0L)
                                                                           ;
}
...

{
   ...
   if(!cfgElem)
   {
      return(cellCb->rgCfgInfo.pndngRgrCfgLst.first ? (RgSchCfgElem *)(cellCb->rgCfgInfo.pndngRgrCfgLst.first->node) : 0L)
                                                                                       ;
   }
   return(cfgElem->cfgReqLstEnt.next ? (RgSchCfgElem *)(cfgElem->cfgReqLstEnt.next->node) : 0L)
                                                                           ;
...

{
   ...
   ((((tmpNode) = ((&cellCb->rgCfgInfo.pndngRgrCfgLst)->crnt = (&cellCb->rgCfgInfo.pndngRgrCfgLst)->first)) != 0L) ? (tmpNode)->node : 0L);
   while(tmpNode)
   {
      ...
      ((((tmpNode) = ((&cellCb->rgCfgInfo.pndngRgrCfgLst)->crnt = ((&cellCb->rgCfgInfo.pndngRgrCfgLst)->crnt ? (&cellCb->rgCfgInfo.pndngRgrCfgLst)->crnt->next : (&cellCb->rgCfgInfo.pndngRgrCfgLst)->first)))!= 0L) ? (tmpNode)->node : 0L);
   }
   return(0L);
}
...

{
   ...
   if(rgSCHUtlAllocSBuf(cellCb->instIdx,
       (Data **)&cellCb->rntiDb.rntiPool,maxRntis*sizeof(RgSchRntiLnk)) != 0)
   {
      return(1);
   }
...

{
   ...
   cellCb->rntiDb.freeRnti = 0L;
   cellCb->rntiDb.lastRnti = 0L;
...

{
   ...
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xe60001,L_ERROR,DBG_CELLID, cellCb->cellId, cellCb->rntiDb.count, 0, 0, 0,                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_dbm.c"      ,                                    1825      , "RNTI exhausted count:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                     ;
      return(0L);
   }
   ...
   if (cellCb->rntiDb.freeRnti != 0L)
   {
      cellCb->rntiDb.freeRnti->prv = 0L;
   }
   else
   {
      cellCb->rntiDb.lastRnti = 0L;
