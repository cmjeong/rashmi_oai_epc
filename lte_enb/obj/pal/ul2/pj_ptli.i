...
extern S16 SGetRegInfo (Region region, SsMemDbgInfo *dbgInfo);
...
typedef struct ssos
{
   ...
   SsSTskEntry sTskTbl[8];





   Region threadMemoryRegionMap[16];

   SsCntr numSTsks;
   SsIdx nxtSTskEntry;

   pthread_mutex_t sTskTblLock;


   U8 dmndQLookupTbl[256];





   SsDrvrTskEntry drvrTskTbl[70];
   ...
   ActvTsk rtrTskTbl[255];
   pthread_mutex_t rtrTskLocks[255];





   SsTmrEntry tmrTbl[45];
   SsCntr numTmrs;
   SsIdx nxtTmrEntry;

   pthread_mutex_t tmrTblLock;


   struct cmMmGlobRegCb *globRegCb;

   SsCntr numRegions;
   SsRegionEntry regionTbl[8];
   SsCntr numDynRegions;
   SsRegionEntry dynRegionTbl[8];

   sem_t regionTblSem;
   SMultiCoreInfo mCInfo;

   pthread_mutex_t mCILock;
 S8 *configFilePath;
} SsOs;
...
extern S16 SInit (void);

extern S16 SFini (void);

extern S16 SDeInit (void);


extern S16 ssdInitGen (void);
extern S16 ssdInitMem (void);
extern S16 ssdInitTsk (void);
extern S16 ssdInitDrvr (void);
extern S16 ssdInitTmr (void);




extern S16 ssdInitLog (void);

extern Void ssdDeinitGen (void);
extern Void ssdDeinitMem (void);
extern Void ssdDeinitTsk (void);
extern Void ssdDeinitDrvr (void);
extern Void ssdDeinitTmr (void);

extern Void ssdDeinitLog (void);

extern Void ssdStart (void);

extern S16 ssdAttachTTsk (SsTTskEntry *);
extern S16 ssdDetachTTsk (SsTTskEntry *);
extern S16 ssdCreateSTsk (SsSTskEntry *);
extern S16 ssdDestroySTsk (SsSTskEntry *);
extern S16 ssdPstTsk (Pst *, Buffer *, SsTTskEntry *);
extern S16 ssdRegTmr (SsTmrEntry *);
extern S16 ssdDeregTmr (SsTmrEntry *);
extern S16 ssdError (Seq, Reason);
extern Void ssdLogError (Ent, Inst, ProcId, Txt *, S32, ErrCls, ErrCode, ErrVal, Txt *)
                                                              ;

extern Void mtTmrHdlrPublic (void);
extern S16 ssdProcTTskTerm (SsTTskEntry *tTsk, SsIdx idx);



extern S16 ssdRegDrvrTsk (SsDrvrTskEntry *);

extern S16 ssdDeregDrvrTsk (SsDrvrTskEntry *);
extern S16 ssdSetAffinity (SSTskId *tskId, U32 coreId);
extern S16 ssdGetAffinity (SSTskId *tskId, U32 *coreId);
extern void DumpSSIDemandQDebugInformation (void);
extern void mtSigSegvHndlr (void);
extern void mtSigUsr2Hndlr (void);
...
extern S16 cmUnpkKwuFlowCntrlIndInfo ( KwuFlowCntrlIndInfo *param, Buffer *mBuf )
 ...
 U8 SUpdMbufMemInfo (Pst *pst,Buffer *mBuf);




extern S16 tipUL2LiKwuBndReq(Pst *ppst,SuId suId,SpId spId);
extern S16 tipSendRlcDataMsgReq(Pst *pst,SuId suId,KwuDatReqInfo *datReq,Buffer *mBuf);

extern S16 PtLiKwuBndReq ( Pst *post, SuId suId, SpId spId )



  ;

extern S16 PtLiKwuUbndReq ( Pst *post, SpId spId, Reason reason )



  ;

extern S16 PtLiKwuDiscSduReq ( Pst *post, SpId spId, KwuDiscSduInfo *discSdu )



  ;

extern S16 PtLiKwuDatReq ( Pst *post, SpId spId, KwuDatReqInfo *datReq, Buffer *mBuf )
 ...
 KwuDatReq pjLiKwuDatReqMt [] =
{
 ...
 KwuDiscSduReq pjLiKwuDiscSduReqMt [] =
{
...

{
   ;
    ...
    tipUL2LiKwuBndReq(pst, suId, spId);
...

{
   ...
   ;
   {
      ...
      ret = tipSendRlcDataMsgReq(pst, spId, datReq, mBuf);
