...
extern S16 pjLibPktAdmCtrl ( PjDlRbCb *pjRbCb, Buffer *pkt )
  ...
  ;
extern S16 pjTipSendToCipher ( PjLibTrans *libTrans, Buffer *mBuf, Buffer **opSdu )



  ;

extern S16 pjTipSendToDeCipher ( PjLibTrans *libTrans, Buffer *mBuf, Buffer **opSdu )



  ;

extern S16 pjSendToTipKeyDeriveFunc ( PjLibTrans libTrans, U8 *key )


  ;

extern S16 pjTipKDF ( PjLibTrans libTrans, U8 *key )


  ;

extern S16 pjTipIntegProtPkt ( PjLibTrans *libTrans, Buffer **mBuf )


  ;

extern S16 pjTipIntegProtVerifyPkt ( PjLibTrans *libTrans, Buffer **mBuf, U32 macI )



  ;
static const char* RLOG_MODULE_NAME="PDCP";
static int RLOG_FILE_ID=285;
static int RLOG_MODULE_ID=1024;
 S16 PjLibObdCipherCloseReq
(
Void *cpCxtId
)
...

{
   ...
   if(libTrans->intAlgoType == 0x01)
   {
      ret = pjTipIntegProtVerifyPkt(libTrans, &mBuf, macI);
   }
   else if (libTrans->intAlgoType == 0x00)
   {

   }
   else
   {
     if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x11d0001,L_ERROR,DBG_UEID, libTrans->ueId, libTrans->intAlgoType, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ptlib.c",1817, "Integrity Verification Algorithm Type %d not supported !!", RLOG_FILE_ID, RLOG_MODULE_NAME); };
   }

   return(ret);
}
 S16 PjLibObdDecipherReq
(
PjLibTrans *libTrans,
Buffer *mBuf,
Buffer **opSdu
...

{
   ...
   ;
      ...
      if(libTrans->ciphAlgoType == 0x01)
      {
         ret = pjTipSendToDeCipher(libTrans, mBuf, opSdu);
      }
      else if (libTrans->ciphAlgoType == 0x00)
      {

      }
      else
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x11d0002,L_ERROR,DBG_UEID, libTrans->ueId, libTrans->ciphAlgoType, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ptlib.c",1873, "Deciphering Algorithm Type %d not supported !!", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      }

   return(ret);

}
 S16 PjLibObdDlCloseReq(Void)
...

{
   ...
   libTrans.key = kenbInf->secKey;




   libTrans.strS[0] = 0x15;
   libTrans.strS[2] = 0x00;
   libTrans.strS[3] = 0x01;
   libTrans.strS[5] = 0x00;
   libTrans.strS[6] = 0x01;
   ...
   libTrans.strS[1] = 0x3;
   libTrans.strS[4] = kenbInf->ciphAlgoType;

   ret = pjSendToTipKeyDeriveFunc(libTrans, kdfCfm->cpCiphKey);
   if(ret!=0)
   {
       if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0x11d0003, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ptlib.c",2013, "Control Plane Cipher Key derivation failed", RLOG_FILE_ID, RLOG_MODULE_NAME); };
       return(1);
   }
   ...
   ret = pjSendToTipKeyDeriveFunc(libTrans, kdfCfm->upCiphKey);
   if(ret!=0)
   {
       if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0x11d0004, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ptlib.c",2027, "User Plane Cipher Key derivation failed", RLOG_FILE_ID, RLOG_MODULE_NAME); };
       return(1);
   }

   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0x11d0005, L_DEBUG, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ptlib.c",2031, "Ciphering Key (UP) Derived", RLOG_FILE_ID, RLOG_MODULE_NAME); };




   libTrans.strS[1] = 0x4;
   libTrans.strS[4] = kenbInf->intgAlgoType;

   ret = pjSendToTipKeyDeriveFunc(libTrans, kdfCfm->intKey);
   if(ret!=0)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0x11d0006, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ptlib.c",2042, "Integrity Key derivation failed", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
    }

   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0x11d0007, L_DEBUG, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ptlib.c",2046, "Integrity Key Derived", RLOG_FILE_ID, RLOG_MODULE_NAME); };
...

{
   ...
   libTrans.strS[0] = 0x13;
   libTrans.strS[3] = 0x00;
   libTrans.strS[4] = 0x02;
   libTrans.strS[7] = 0x00;
   libTrans.strS[8] = 0x02;


   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev2(0x11d0008, L_DEBUG, kenbInf->physCellId, kenbInf->dlCarrFreq, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ptlib.c"   , 2084   , "Value of Cellid, dl Freq is %d %d ", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                  ;
   ...
   libTrans.strS[1] = kenbInf->physCellId >> 8;
   libTrans.strS[2] = (U8)kenbInf->physCellId;


   libTrans.strS[5] = kenbInf->dlCarrFreq >> 8;
   libTrans.strS[6] = (U8)kenbInf->dlCarrFreq;

   libTrans.strSlen = 9;

   if((pjSendToTipKeyDeriveFunc(libTrans, kdfCfm->secKey)) != 0)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0x11d0009, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ptlib.c",2101, "In pjLibObdPrcKenbStarCfg: pjTipKDF failed", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      ret = 1;
   }
...

{
   ;;
      ...
      if (libTrans->intAlgoType == 0x01 )
      {
          S16 ret = pjTipIntegProtPkt(libTrans, mBuf);
          return(ret);
      }
      else if ( libTrans->intAlgoType == 0x00)
      {
      ...
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x11d000a,L_ERROR,DBG_UEID, libTrans->ueId, libTrans->intAlgoType, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ptlib.c",2193, "Integrity Algorithm Type %d is not supported !!", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      }
...

{
   ...
   if (libTrans->ciphAlgoType == 0x01 )
   {
      S16 ret = pjTipSendToCipher(libTrans, mBuf, opSdu);
      return(ret);
   }
   else if ( libTrans->ciphAlgoType == 0x00)
   {
   ...
   {
     if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x11d000b,L_ERROR,DBG_UEID, libTrans->ueId, libTrans->ciphAlgoType, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ptlib.c",2233, " Ciphering Algorithm Type %d is not supported !!", RLOG_FILE_ID, RLOG_MODULE_NAME); };
   }
 ...
 U32 dbgSpaccThreshHoldDropCnt = 0;
 U32 dbgPdcpQSizeThreshHoldDropCnt = 0;
 U32 dbgPdcpMemCount = 0;
 U32 pdcpDropMemTh = 0;
 U32 dbgICCMemThreshHoldDropCnt = 0;
 U32 dbgSSIMemThreshHoldDropCnt = 0;
 U32 dbgICCMemAlarm;
 U32 dbgPdcpQMsSpaccQFull = 0;
 U32 dbgPdcpRbOverload = 0;
