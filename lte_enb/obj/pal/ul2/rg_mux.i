...
extern S16 ssInitDmndQ (SsDmndQ *queue);
extern S16 ssDestroyDmndQ (SsDmndQ *queue);
extern S16 ssDmndQPut (SsDmndQ *queue, Buffer *mBuf, Prior prior, Order order)
                                                                        ;
extern S16 ssDmndQGet (SsDmndQ *queue, Buffer **mBuf, Order order)
                                                                        ;
extern S16 ssDmndQWait (SsDmndQ *queue);

extern S16 ssFndLenDmndQ (SsDmndQ *queue, Prior prior, QLen *len);
typedef U8 SsCntr;
typedef U8 SsIdx;
...
static Void rgMUXGet20bitRarGrnt (U8 ulBw, RgInfRarUlGrnt *msg3Grnt, U8 *grnt)

                                                  ;
static U16 rgMUXCalcRiv (U8 bw, U8 rbStart, U8 numRb)
...
)
{
   ...
   if (0L != pdu->contResId)
   {
      if(pdu->schdTbSz >= (1 +6))
      {
         if(pdu->schdTbSz > (1 +6))
         {
            { subHdr.shLen = 1; subHdr.shData[0] = (0x3F & ((1 << 5) | 0x1C)); ; ret = SAddPstMsgMult(&subHdr.shData[0], subHdr.shLen, ceShdrBuf);};
         }
         else
         {
            { subHdr.shLen = 1; subHdr.shData[0] = (0x1F & 0x1C); ; ret = SAddPstMsgMult(&subHdr.shData[0], subHdr.shLen, ceShdrBuf);};
         }

         if(ret != 0)
         {
            err->errCause = (0 + 2);
            { if ((&rgCb[inst].rgInit)->dbgMask & ((0x00000100 << 1))) { snprintf((&rgCb[inst].rgInit)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", "MAC", (&rgCb[inst].rgInit)->procId, (&rgCb[inst].rgInit)->ent, (&rgCb[inst].rgInit)->inst, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c", 227); SPrint((&rgCb[inst].rgInit)->prntBuf); sprintf (rgCb[inst].rgInit.prntBuf, "RGERR_MUX_BLD_CEHDR_FAIL \n"); SPrint((&rgCb[inst].rgInit)->prntBuf); } };
            return(1);
         }

         { ; ret = SAddPstMsgMult((U8 *)(&(pdu->contResId->resId[0])), 6, ceBuf);};
   ...
   if (1 == pdu->ta.pres)
   {
      if(pdu->schdTbSz >= (1 +1))
      {
         ...
         if(pdu->schdTbSz > (1 +1))
         {
            { subHdr.shLen = 1; subHdr.shData[0] = (0x3F & ((1 << 5) | 0x1D)); ; ret = SAddPstMsgMult(&subHdr.shData[0], subHdr.shLen, ceShdrBuf);};
         }
         else
         {
            { subHdr.shLen = 1; subHdr.shData[0] = (0x1F & 0x1D); ; ret = SAddPstMsgMult(&subHdr.shData[0], subHdr.shLen, ceShdrBuf);};
         }

         if(ret != 0)
         {
            err->errCause = (0 + 2);
            { if ((&rgCb[inst].rgInit)->dbgMask & ((0x00000100 << 1))) { snprintf((&rgCb[inst].rgInit)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", "MAC", (&rgCb[inst].rgInit)->procId, (&rgCb[inst].rgInit)->ent, (&rgCb[inst].rgInit)->inst, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c", 258); SPrint((&rgCb[inst].rgInit)->prntBuf); sprintf (rgCb[inst].rgInit.prntBuf, "RGERR_MUX_BLD_CEHDR_FAIL \n"); SPrint((&rgCb[inst].rgInit)->prntBuf); } };
            return(1);
         }

         taVal = pdu->ta.val;
         { ; ret = SAddPstMsgMult((U8 *)(&(taVal)), 1, ceBuf);};

         if(ret != 0)
         {
            err->errCause = (0 + 3);
            { if ((&rgCb[inst].rgInit)->dbgMask & ((0x00000100 << 1))) { snprintf((&rgCb[inst].rgInit)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", "MAC", (&rgCb[inst].rgInit)->procId, (&rgCb[inst].rgInit)->ent, (&rgCb[inst].rgInit)->inst, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c", 268); SPrint((&rgCb[inst].rgInit)->prntBuf); sprintf (rgCb[inst].rgInit.prntBuf, "RGERR_MUX_BLD_CE_FAIL \n"); SPrint((&rgCb[inst].rgInit)->prntBuf); } };
            return(1);
         }
         pdu->schdTbSz -= (1 +1);
         if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xe50001, L_DEBUG, pdu->ta.val, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",272, "*********** TA muxed by MAC: %u", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      }
   }
   return(0);
}
...
)
{
   ...
   { U8 _hdrLen; lenBytes = (msgLen <= 127) ? 1 : 2; _hdrLen = lenBytes + 1; elmTotLen = _hdrLen + msgLen;};
   ...
   {
      RgMUXSubHdr subHdr;
      { ret = 0; subHdr.shData[0] = (0x3F & ((1 << 5) | lcId)); if(msgLen <= 127) { subHdr.shLen = 2; subHdr.shData[1] = (0x7F & msgLen); subHdr.shData[2] = 0; } else { subHdr.shLen = 3; subHdr.shData[1] = (0x80 | (0x7F & (msgLen >> 8))); subHdr.shData[2] = (0xFF & msgLen); } ; ret = SAddPstMsgMult(&subHdr.shData[0], subHdr.shLen, sHdrBuf);};
      if(ret != 0)
      {
         err->errCause = (0 + 4);
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xe50002, L_ERROR,lcId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",373, "RGERR_MUX_BLD_SDUHDR_FAIL for LCID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); };
         return(1);
      }


      { ret = SCatMsg(sduBuf, sdu, 0);};
      ...
      {
         err->errCause = (0 + 5);
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xe50003, L_ERROR,lcId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",388, "RGERR_MUX_BLD_SDU_FAIL for LCID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); };
         return(1);
      }
                     ...
                     (*schdTbSz >= (elmTotLen - lenBytes)))
   {
      ...
      if (*schdTbSz != (elmTotLen - lenBytes))
      {
         ...
         if(numPdHdrs)
         {
            ...
            U8 sHdrArr[2] = {0x3F, 0x3F};
            if(numPdHdrs > 2)
            {
               err->errCause = (0 + 6);
               if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xe50004, L_ERROR,lcId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",409, "RGERR_MUX_BLD_PADHDR_FAIL for LCID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); };
               return(1);
            }
            ...
            {
               err->errCause = (0 + 6);
               if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xe50005, L_ERROR,lcId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",416, "RGERR_MUX_BLD_PADHDR_FAIL for LCID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); };
               return(1);
            }
            *schdTbSz-=numPdHdrs;
         }
      ...
      { ret = 0; subHdr.shLen = 1; subHdr.shData[0] = (0x1F & lcId); ; ret = SAddPstMsgMult(&subHdr.shData[0], subHdr.shLen, sHdrBuf);};
      if(ret != 0)
      {
         err->errCause = (0 + 4);
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xe50006, L_ERROR,lcId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",427, "RGERR_MUX_BLD_SDUHDR_FAIL for LCID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); };
         return(1);
      }
      ...
      {
         err->errCause = (0 + 5);
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xe50007, L_ERROR,lcId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",441, "RGERR_MUX_BLD_SDU_FAIL for LCID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); };
         return(1);
      }
   ...
   else
   {
      ...
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev4(0xe50008, L_ERROR, lcId, ((S16)elmTotLen), lenBytes, lcId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c"      , 453      , "Failed lcId %u, elmTotLen %d lenBytes %d LCID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                     ;
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev3(0xe50009, L_ERROR, ((S16)msgLen), ((S16)*schdTbSz), lcId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c"      , 455      , "msglen %d schdTbSz %d LCID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                    ;
      return(1);
   }
   return(0);
}
static S16 rgMUXAddPadd
(
Inst inst,
MsgLen *schdTbSz,
Buffer *sHdrBuf,
...
)
{
   ...
   if(*schdTbSz)
   {
      ...
      {
         { if(0) { ret = SAddPreMsg(0x3F, sHdrBuf); } else { ret = SAddPstMsg(0x1F, sHdrBuf); }};

         if(ret != 0)
         {
            err->errCause = (0 + 6);
            if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xe5000a, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",520, "RGERR_MUX_BLD_PADHDR_FAIL", RLOG_FILE_ID, RLOG_MODULE_NAME); };
            return(1);
         }
      ...
      if (*schdTbSz)
      {
         if (*schdTbSz <= 4096)
         {
            ...
            { Buffer *sduEnd = 0L; SsMsgInfo *mInfo = 0L; mInfo = (SsMsgInfo *)sduBuf->b_rptr; ssGetDBufOfSize(mInfo->region,*schdTbSz, &padBuf); if (padBuf == 0L) { if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xe5000b, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",532, "RGERR_MUX_BLD_CEHDR_FAIL", RLOG_FILE_ID, RLOG_MODULE_NAME); }; return(1); } if (mInfo->endptr == 0L) { sduEnd = sduBuf; } else { sduEnd = mInfo->endptr; } sduEnd->b_cont = padBuf; padBuf->b_wptr += *schdTbSz; mInfo = (SsMsgInfo *)sduBuf->b_rptr; mInfo->endptr = padBuf; mInfo->len += *schdTbSz; };
            if(ret != 0)
            {
               err->errCause = (0 + 7);
               if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xe5000c, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",544, "RGERR_MUX_BLD_PAD_FAIL", RLOG_FILE_ID, RLOG_MODULE_NAME); };
               return(1);
            }
            *schdTbSz = 0;
         }
         else
         {
            while (*schdTbSz)
            {
               if (*schdTbSz > 4096)
               {
                  ...
                  { Buffer *sduEnd = 0L; SsMsgInfo *mInfo = 0L; mInfo = (SsMsgInfo *)sduBuf->b_rptr; ssGetDBufOfSize(mInfo->region,4096, &padBuf); if (padBuf == 0L) { if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xe5000d, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",556, "RGERR_MUX_BLD_CEHDR_FAIL", RLOG_FILE_ID, RLOG_MODULE_NAME); }; return(1); } if (mInfo->endptr == 0L) { sduEnd = sduBuf; } else { sduEnd = mInfo->endptr; } sduEnd->b_cont = padBuf; padBuf->b_wptr += 4096; mInfo = (SsMsgInfo *)sduBuf->b_rptr; mInfo->endptr = padBuf; mInfo->len += 4096; };
                  if(ret != 0)
                  {
                     err->errCause = (0 + 7);
                     if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xe5000e, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",569, "RGERR_MUX_BLD_PAD_FAIL", RLOG_FILE_ID, RLOG_MODULE_NAME); };
                     return(1);
                  }
               ...
               else
               {
                  ...
                  { Buffer *sduEnd = 0L; SsMsgInfo *mInfo = 0L; mInfo = (SsMsgInfo *)sduBuf->b_rptr; ssGetDBufOfSize(mInfo->region,*schdTbSz, &padBuf); if (padBuf == 0L) { if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xe5000f, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",578, "RGERR_MUX_BLD_CEHDR_FAIL", RLOG_FILE_ID, RLOG_MODULE_NAME); }; return(1); } if (mInfo->endptr == 0L) { sduEnd = sduBuf; } else { sduEnd = mInfo->endptr; } sduEnd->b_cont = padBuf; padBuf->b_wptr += *schdTbSz; mInfo = (SsMsgInfo *)sduBuf->b_rptr; mInfo->endptr = padBuf; mInfo->len += *schdTbSz; };
                  if(ret != 0)
                  {
                     err->errCause = (0 + 7);
                     if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xe50010, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",591, "RGERR_MUX_BLD_PAD_FAIL", RLOG_FILE_ID, RLOG_MODULE_NAME); };
                     return(1);
...
)
{
      ...
      case 4:
         ...
         if (0L != cDatReq)
         {
            if(rgMUXInsSdu(inst,&pdu->schdTbSz,
                     0x00, cDatReq->pdu,
                     sHdrBuf, sduBuf, err) != 0)
            {
               return(1);
            }
            { if (0L != (cDatReq->pdu)) { SPutMsg((cDatReq->pdu)); cDatReq->pdu = 0L; } };
         }
         break;
                     ...
                     idx2++)
                  {
                     if(pdu->schdTbSz)
                     {
                        if(rgMUXInsSdu(inst,&pdu->schdTbSz,
                           ...
                           sHdrBuf, sduBuf, err) != 0)
                        {
                           if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xe50011, L_ERROR,datReqTb->lchData[idx1].lcId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",685, "FAILED for LCID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); };
                           return(1);
                        }
                     }
                     { if (0L != (datReqTb->lchData[idx1].pdu.mBuf[idx2])) { SPutMsg((datReqTb->lchData[idx1].pdu.mBuf[idx2])); datReqTb->lchData[idx1].pdu.mBuf[idx2] = 0L; } };
                  }
               }
            }
                     ...
                     idx2++)
                  {
                     if(pdu->schdTbSz)
                     {
                        if(rgMUXInsSdu(inst,&pdu->schdTbSz,
                           ...
                           sHdrBuf, sduBuf, err) != 0)
                        {
                           if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev2(0xe50012, L_ERROR, ((S16)pdu->schdTbSz), datReqTb->lchData[idx1].lcId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c"                           , 711                           , "FAILED TB Size %d LCID:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                                                      ;
                           return(1);
                        }
                     }
   ...
   if(rgMUXAddPadd(inst,&pdu->schdTbSz, sHdrBuf, sduBuf, 0, err) != 0)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xe50013, L_ERROR,(S16)pdu->schdTbSz, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",729, "FAILED for TB Size:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
   return(0);
}
...

{
   ...
   if (rgGetMsg(inst, &mBuf1) != 0)
   {

      err->errCause = 0 + 1;
      err->errType = (0 + 1);
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xe50014, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",789, "FAILED to get Msg", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }

   if (rgGetMsg(inst, &mBuf2) != 0)
   {

      err->errCause = 0 + 1;
      err->errType = (0 + 1);
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xe50015, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",798, "FAILED to get Msg", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      { if (0L != (mBuf1)) { SPutMsg((mBuf1)); mBuf1 = 0L; } };
      return(1);
   }

   if(rgMUXAddCes(inst, pdu, mBuf1, mBuf2, err) != 0)
   {
      { if (0L != (mBuf1)) { SPutMsg((mBuf1)); mBuf1 = 0L; } };
      { if (0L != (mBuf2)) { SPutMsg((mBuf2)); mBuf2 = 0L; } };
      err->errType = (0 + 1);
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xe50016, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",808, "FAILED to get Msg", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }

   if(rgMUXAddSdus(inst, pdu, mBuf1, mBuf2, err) != 0)
   {
      { if (0L != (mBuf1)) { SPutMsg((mBuf1)); mBuf1 = 0L; } };
      { if (0L != (mBuf2)) { SPutMsg((mBuf2)); mBuf2 = 0L; } };
      err->errType = (0 + 1);
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xe50017, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",817, "FAILED to mux add sdu", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }

   if(SCatMsg(mBuf1, mBuf2, 0) != 0)
   {
      { if (0L != (mBuf1)) { SPutMsg((mBuf1)); mBuf1 = 0L; } };
      { if (0L != (mBuf2)) { SPutMsg((mBuf2)); mBuf2 = 0L; } };
      err->errType = (0 + 1);
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xe50018, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",826, "FAILED to catMsg", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
   { if (0L != (mBuf2)) { SPutMsg((mBuf2)); mBuf2 = 0L; } };
...

{
   ...
   if ((ret = rgGetMsg(inst,&hdrBuf)) != 0)
   {

      err->errCause = 0 + 1;
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xe50019,L_ERROR,DBG_CELLID, cell->cellId, 0, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",1236, "FAILED to getMsg", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
   if ((ret = rgGetMsg(inst,&datBuf)) != 0)
   {

      err->errCause = 0 + 1;
      { if (0L != (hdrBuf)) { SPutMsg((hdrBuf)); hdrBuf = 0L; } };
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xe5001a,L_ERROR,DBG_CELLID, cell->cellId, 0, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",1244, "FAILED to getMsg", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }

   if (1 == alloc->backOffInd.pres)
   {
      ...
      { ret = SAddPstMsg(hdrByte, hdrBuf);};
      if(ret != 0)
      {
         err->errCause = (0 + 8);
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xe5001b,L_ERROR,DBG_CELLID, cell->cellId, 0, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",1268, "RGERR_MUX_BLD_BI_FAIL", RLOG_FILE_ID, RLOG_MODULE_NAME); };
         { if (0L != (hdrBuf)) { SPutMsg((hdrBuf)); hdrBuf = 0L; } };
         { if (0L != (datBuf)) { SPutMsg((datBuf)); datBuf = 0L; } };
         return(1);
      }
      schdTbSz--;
   }
         ...
         (schdTbSz >= 6 +1); idx++)
   {
      ...
      if(phdrByte.pres == 1)
      {
         ...
         { ret = SAddPstMsg(phdrByte.val, hdrBuf);};
         if(ret != 0)
         {
            err->errCause = (0 + 9);
            if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xe5001c,L_ERROR,DBG_CELLID, cell->cellId, 0, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",1296, "RGERR_MUX_BLD_RAPIDHDR_FAIL", RLOG_FILE_ID, RLOG_MODULE_NAME); };
            { if (0L != (hdrBuf)) { SPutMsg((hdrBuf)); hdrBuf = 0L; } };
            { if (0L != (datBuf)) { SPutMsg((datBuf)); datBuf = 0L; } };
            return(1);
         }
      ...
      if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xe5001d,L_DEBUG,DBG_CELLID, cell->cellId, alloc->crntiInfo[idx].rapId, alloc->crntiInfo[idx].tmpCrnti, 0, 0,                                              "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c"      ,                                              1315      , "Rar,Rapid=%d, Temp CRNTI:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }


                                               ;

     ;
      if(SAddPstMsgMult(&data[0], 6, datBuf) != 0)
      {
         err->errCause = (0 + 10);
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xe5001e,L_ERROR,DBG_CELLID, cell->cellId, 0, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",1321, "RGERR_MUX_BLD_RAPID_FAIL", RLOG_FILE_ID, RLOG_MODULE_NAME); };
         { if (0L != (hdrBuf)) { SPutMsg((hdrBuf)); hdrBuf = 0L; } };
         { if (0L != (datBuf)) { SPutMsg((datBuf)); datBuf = 0L; } };
         return(1);
      }
   ...
   if(phdrByte.pres == 1)
   {
      ...
      {
         err->errCause = (0 + 9);
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xe5001f,L_ERROR,DBG_CELLID, cell->cellId, 0, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",1336, "RGERR_MUX_BLD_RAPIDHDR_FAIL", RLOG_FILE_ID, RLOG_MODULE_NAME); };
         { if (0L != (hdrBuf)) { SPutMsg((hdrBuf)); hdrBuf = 0L; } };
         { if (0L != (datBuf)) { SPutMsg((datBuf)); datBuf = 0L; } };
         return(1);
      }
   ...
   if(rgMUXAddPadd(inst,&schdTbSz, hdrBuf, datBuf, 1, err) != 0)
   {
      { if (0L != (hdrBuf)) { SPutMsg((hdrBuf)); hdrBuf = 0L; } };
      { if (0L != (datBuf)) { SPutMsg((datBuf)); datBuf = 0L; } };
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xe50020,L_ERROR,DBG_CELLID, cell->cellId, 0, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",1347, "FAILED to mux add padding", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }


   if(SCatMsg(hdrBuf, datBuf, 0) != 0)
   {
      { if (0L != (hdrBuf)) { SPutMsg((hdrBuf)); hdrBuf = 0L; } };
      { if (0L != (datBuf)) { SPutMsg((datBuf)); datBuf = 0L; } };
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xe50021,L_ERROR,DBG_CELLID, cell->cellId, 0, 0, 0, 0, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_mux.c",1356, "FAILED to catMsg", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
   { if (0L != (datBuf)) { SPutMsg((datBuf)); datBuf = 0L; } };
