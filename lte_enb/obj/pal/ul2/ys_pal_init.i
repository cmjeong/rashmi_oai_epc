...
typedef unsigned int OsSize;
typedef OsVoid * OsFile;
typedef int OsInt;
typedef long OsLong;
...
extern S32 osMatchKeyword (Keys *keys, S8 *tkn);
extern S8 *osGetWord (S8* line, S8* word);
extern OsFile *osFopen (const S8 *fName, const S8* flags);
extern OsInt osFclose (OsFile *file);
extern OsLong osStrtol (const S8 *str, S8 **ptr, OsInt base);
extern S8 *osFgets (S8* buf, OsInt len, OsFile *file);

extern S8 *osGets (S8* buf,S32 maxBufLen);
extern OsInt osStrcmp (const S8 *s1, const S8 *s2);
extern OsInt osStrncmp (const S8 *s1, const S8 *s2, OsSize size);
extern OsVoid *osCalloc (OsSize nelem, OsSize size);
extern S8 *osStrcpy (S8 *s1, const S8 *s2);
extern OsInt osReadAccess (const S8 *file);
extern OsVoid *osMemcpy (OsVoid *s1, const OsVoid *s2, OsSize len);
extern OsSize osFread (OsVoid *ptr, OsSize, OsSize, OsFile *);
extern OsSize osFwrite (const OsVoid *ptr, OsSize, OsSize, OsFile *);
extern OsSize osStrlen (const S8 *str);
extern S8 *osStrcat (S8 *dst, const S8 *src);
extern OsInt osFseek (OsFile *fp, OsLong offset, OsInt whence);
extern OsInt osGetopt (OsInt argc, S8 * const *argv, const S8 *optStr);
extern Void osSprintf (S8 *s, const S8 *fmt, ...);
extern Void osPrintf (const S8 *fmt, ...);
extern Void osFree (OsVoid *ptr);
extern OsInt osIsascii (OsInt c);
extern OsInt osIsprint (OsInt c);
extern OsInt osIsspace (OsInt c);
extern Txt *osTmpnam (void);
extern Void osUnlink (const S8 *fName);
extern Void osSleep (U32 seconds);
extern Void osMsSleep (U32 mSeconds);
extern S8 *osMmap (S8 *addr, OsSize len, OsInt prot, OsInt flags, OsInt fildes, OsLong off)
                             ;
extern OsInt osFileno (OsFile *file);
extern OsInt osMunmap (S8 *addr, OsSize len);
extern Void osBzero (OsVoid *ptr, OsSize size);
extern OsVoid *osMemset (OsVoid *s, OsInt c, OsSize len);
extern Void osExit (OsInt code);
extern Void osPerror (Txt *msg);
extern S16 osAdd2LList ( DLList **dlList, Txt *node);
extern Txt *osDelFrmLList ( DLList **dlList, DLList *nodePtr);
extern DLList *osNextDLList ( DLList *dlList);
extern DLList *osPrevDLList ( DLList *dlList);
extern Txt *osNodeDLList ( DLList *dlList);
extern OsInt osRand (void);
extern OsInt osGetchar (void);
extern S16 osInitCon (void);
extern S16 osRdCon (Data *dataPtr);
extern S16 osGetTimeOfDay (U32 *tmSec, U32 *tmUsec);
extern S16 osOpen (S8 *path, S32 oflag, U32 mode, S32 *fd);
extern S16 osFtruncate (S32 fd, U32 length);
extern S16 osMlockall (S32 flags);
extern S16 osMunlockall (void);
extern S16 osGeteuid (S32 *euid);
extern S16 osClose (S32 fd);
 OsSize osRead (S32 fd, Data *data, OsSize nitems);
 OsSize osWrite (S32 fd, Data *data, OsSize nitems);
 S32 osSeekStart (S32 fd);



extern Void osErrQuit (const S8 *fmt, ...);
extern Void osErrSys (const S8 *fmt, ...);
extern Void osErrRet (const S8 *fmt, ...);
extern Void osErrDump (const S8 *fmt, ...);
...
typedef struct mesg
{
   ...
   Pst pst;
   S32 sIdx;
   Txt mesg_data[(8192-(2*sizeof(S32))-sizeof(Pst))];
}Mesg;

extern S32 osMsgSend (S32 id, Mesg *mPtr);
extern S32 osMsgRecv (S32 id, Mesg *mPtr, Bool wFlag);
extern S32 osMsgCreate (S32 key, S32 *id);
extern S32 osMsgGet (S32 key, S32 *id);

extern S32 osStripBlanks (S8 *rawStr);


extern S16 osOpenLockFile (Txt *file, S32 *fd);
extern S16 osTryLockFile (S32 fd);
extern S16 osLockFile (S32 fd);
extern S16 osUnlockFile (S32 fd);

extern U8 osSystem (Txt *command);


extern Void osCmsInit (Void);
extern Void osCmsLog (Txt *logMsg);



extern S16 osThrCreate (void *stackBase, U32 stackSize, TFN startRoutine, void *arg, U32 thrFlags, U32 *newThread)

                                                           ;
extern S16 osThrSuspend (U32 targetThread);
extern S16 osThrSelf (U32 *threadId);
...
static S16 ysPalInit ( Region region);





U32 YS_TTI_TMR_VAL_CFG;
U16 YS_PORT_ENB;
U16 YS_PORT_UE;
U8 YS_IP_ADDR_ENB[64];
U8 YS_IP_ADDR_UE[64];
U8 WIRESHARK_SRVR_IP_ADDR[64];
U16 YS_CELL_ID;
U16 ysPalNumCells = 1;

static S8 ysCfgParams[8][256] =
{
   "YS_IP_ADDR_ENB",
   "YS_IP_ADDR_UE",
   "YS_PORT_ENB",
...

{
   ...
   if((fp = (OsFile*)osFopen("ys_cfg.txt","r")) == (S32)0L)
   {


      SLogError(ysCb.ysInit.ent, ysCb.ysInit.inst, ysCb.ysInit.procId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteclpal/ys_pal_init.c", 159, 0x4, 0, (ErrVal)0, "FAILED to open the file ys_cfg.txt\n");;
      exit(0);
   }

   numParams = (U8)(sizeof(ysCfgParams)/256);

   lineNo = 1;

   for (;;)
   {
      cmMemset((U8*)line, 0, 256);

      ret1 = osFgets(line, 256, fp);

      if (ret1 == (S8*)-1 || ret1 == 0L)
      {
         { if (ysCb.ysInit.dbgMask >= (2)) { { if ((&ysCb.ysInit)->dbgMask & (2)) { snprintf((&ysCb.ysInit)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", "Ys", (&ysCb.ysInit)->procId, (&ysCb.ysInit)->ent, (&ysCb.ysInit)->inst, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteclpal/ys_pal_init.c", 175); SPrint((&ysCb.ysInit)->prntBuf); sprintf (ysCb.apb, "%s", "Failed: "); SPrint((&ysCb.ysInit)->prntBuf); } }; { if ((&ysCb.ysInit)->dbgMask & (2)) { snprintf((&ysCb.ysInit)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", "Ys", (&ysCb.ysInit)->procId, (&ysCb.ysInit)->ent, (&ysCb.ysInit)->inst, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteclpal/ys_pal_init.c", 175); SPrint((&ysCb.ysInit)->prntBuf); sprintf (ysCb.apb, "End of File reached, ys_cfg.txt\n"); SPrint((&ysCb.ysInit)->prntBuf); } }; } };
         break;
      }
      ...
      while ((lPtr = osGetWord(lPtr, word)) != 0L)
      {
         if (word[0] == '#')
         {
            break;
         ...
         if (inToken)
         {
            ...
            for (idx = 0; idx < numParams; idx++)
            {
               ...
               if (idx == numParams)
               {
                  { if (ysCb.ysInit.dbgMask >= (2)) { { if ((&ysCb.ysInit)->dbgMask & (2)) { snprintf((&ysCb.ysInit)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", "Ys", (&ysCb.ysInit)->procId, (&ysCb.ysInit)->ent, (&ysCb.ysInit)->inst, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteclpal/ys_pal_init.c", 213); SPrint((&ysCb.ysInit)->prntBuf); sprintf (ysCb.apb, "%s", "Failed: "); SPrint((&ysCb.ysInit)->prntBuf); } }; { if ((&ysCb.ysInit)->dbgMask & (2)) { snprintf((&ysCb.ysInit)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", "Ys", (&ysCb.ysInit)->procId, (&ysCb.ysInit)->ent, (&ysCb.ysInit)->inst, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteclpal/ys_pal_init.c", 213); SPrint((&ysCb.ysInit)->prntBuf); sprintf (ysCb.apb, " invalid configuration parameter \n"); SPrint((&ysCb.ysInit)->prntBuf); } }; } };
                  if(fp) osFclose(fp);
                  return(1);
               }
            }
         ...
         else
         {
            ...
            switch(idx)
            {
               ...
               case 4:
               {
                  YS_TTI_TMR_VAL_CFG = (U32)osStrtol(word, 
                                                          ((void *)0)
                                                              , 10);
                  break;
               }
               ...
               case 7:
               {
                  ysPalNumCells = (U16)osStrtol(word, 
                                                     ((void *)0)
                                                         , 10);
                  break;
               }
...

{
   ...
   for (idx=0; idx < 2; idx++)
   {
      ysCb.tmrTq[idx].first = 0L;
      ysCb.tmrTq[idx].tail = 0L;
   }
   ysCb.tmrTqCp.nxtEnt = 0;
   ysCb.tmrTqCp.tmrLen = 2;
...

{
   ...
   ysCb.ctfSap.sapPst.selector = 0;
   ysCb.ctfSap.sapPst.dstProcId = SFndProcId();
   ysCb.ctfSap.sapPst.srcProcId = SFndProcId();
   ysCb.ctfSap.sapPst.dstEnt = 0xdf;
   ysCb.ctfSap.sapPst.srcEnt = 0xd5;
   ...
   for(indx = 0; indx < ysPalNumCells; indx++)
   {
      ...
      cellCb->peerPst.dstProcId = 2;
      cellCb->peerPst.srcProcId = SFndProcId();
      cellCb->peerPst.dstEnt = 0xd5;
      cellCb->peerPst.srcEnt = 0xd5;
      cellCb->peerPst.region = region;
      cellCb->peerPst.pool = ysCb.ysInit.pool;
      cellCb->peerPst.selector = 0;
