...

{
   ...
   if( L_INFO < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x840001,L_INFO,DBG_CELLID, cell->cellId, tmpCrnti, 0, 0, 0,                     "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_ram.c"   ,                     157   , "CREATE UECB FOR CRNTI:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                      ;

   if (rgAllocSBuf(inst,(Data **)&ueCb, sizeof(*ueCb)) != 0)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x840002,L_ERROR,DBG_CELLID, cell->cellId,tmpCrnti, 0, 0, 0,                                                                "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_ram.c"      ,                                                                162      , "Memory allocation FAILED for CRNTI:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                                 ;
      err->errCause = 0 + 3 + 1;
      return(0L);
   }
   ...
   if(insert == 1)
   {
      ...
      ueCb->rachLstEnt.next = 0L;
      ueCb->rachLstEnt.prev = 0L;
      ueCb->rachLstEnt.node = (U32)(ueCb);
...

{
   ...
   while ((ueCb = rgDBMGetNextUeCbFromRachLst(cell, 0L)) != 0L)
   {
...

{
   ...
   for(idx1 = 0; idx1 < rarInfo->numRaRntis; idx1++)
   {
      ...
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x840003,L_ERROR,DBG_CELLID, cell->cellId, rarInfo->raRntiInfo[idx1].raRnti, 0, 0, 0,                                                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_ram.c"         ,                                                                    324         , "RARNTI:%d Creation of RAR" "PDU for failed", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                                     ;
         continue;
      }
   }
