...
extern S16 rgSCHCmnDlAllocErrIndTxRb ( RgSchCellCb *cell, RgSchDlSf *subFrm, RgSchUeCb *ue, U32 bo, U32 *effBo, RgSchDlHqProcCb *proc, RgSchCmnDlRbAllocInfo *cellWdAllocInfo )
  ...
  ;
static Void rgSCHDrxTtiHdlOnDurUl ( RgSchCellCb *cell, U16 ulIndex )


  ;
static Void rgSCHDrxTtiHdlOnDurDl ( RgSchCellCb *cell, U16 dlIndex )


  ;
static Void rgSCHDrxTtiHdlDlHarqRTT ( RgSchCellCb *cell, U16 dlIndex )


  ;
static Void rgSCHDrxTtiHdlUlHarqRTT ( RgSchCellCb *cell, U16 ulIndex )


  ;
 static S16 rgSCHDrxTtiHdlOnDur (RgSchCellCb *cellCb, U16 dlIndex, U16 ulIndex)
                                                     ;
 static S16 rgSCHDrxTtiHdlInActv (RgSchCellCb *cellCb, U16 dlIndex, U16 ulIndex)
                                                     ;
 static S16 rgSCHDrxTtiHdlShortCycle (RgSchCellCb *cell, U16 dlIndex, U16 ulIndex)
                                                    ;
 static S16 rgSCHDrxTtiHdlDlHarq (RgSchCellCb *cellCb, U16 dlIndex, U16 ulIndex)
                                                    ;
 static S16 rgSCHDrxCpyUeCfg (RgSchDrxUeCb *drxCb, RgrUeDrxCfg* ueDrxCfg)
                                                            ;

 static S16 rgSCHDrxGetNxtOnDur (RgSchCellCb* cell,RgSchDrxUeCb* drxCb, CmLteTimingInfo* nxtOnDur, U8 delta)

                                                 ;

static Void rgSCHDrxMvToNxtOnDurOcc (RgSchCellCb* cell, RgSchUeCb* ue, U16 idx, Bool calcFrmOffst)
...

{
   ...
   while (node)
   {
      ...
      if ( drxUe->drxInactDistance != -1 )
      {
         continue;
      }


      drxUe->drxDlInactvMask |= 0x00000008;
      ...
      if (drxUe->isShortCycleCfgd)
      {
         ...
         shrtCycleExpIndx = (dlIndex + (drxUe->shortCycleTmrLen *
                  drxUe->shortDrxCycle)) % 256;

         drxUe->drxShortCycleDistance = (drxUe->shortCycleTmrLen *
               drxUe->shortDrxCycle) / 256;


         if (drxUe->shortCycleIndx != 0xFFFF)
         {
         ...
         drxUe->shortCycleEnt.node = (U32)ue;
         drxUe->shortCycleIndx = shrtCycleExpIndx;
         rgSCHDrxMvToNxtOnDurOcc(cell,ue,((2 + 0)),1);
   ...
   while (node)
   {
      ...
      drxUe->drxUlInactvMask |= 0x00000008;
...

{
   ...
   while (node)
   {
      ...
      if ( --drxUe->drxShortCycleDistance != -1)
      {
         continue;
      }
...

{
   ...
   while(node)
   {
      ...
      if ( ueDrxCb->drxInactvIndx != 0xFFFF )
      {
      ...
      ueDrxCb->inActvTmrEnt.node = (U32)ueCb;

      ueDrxCb->drxInactvIndx = inActvTmrExpIndx;


      ueDrxCb->drxUlInactvMask &= ~0x00000008;
      ueDrxCb->drxDlInactvMask &= ~0x00000008;
      ...
      if ( delInUlScan == 1)
      {
         ...
         {
            ueDrxCb->drxInactDistance = -1;
            ueDrxCb->drxDlInactvMask |= 0x00000008;




            if ( !(ueDrxCb->drxDlInactvMask ^ 0xFFFFFFFF) )
            {
               ueCb->dl.dlInactvMask |= 0x08;
...

{
   ...
   harqRTTExpIndx = ((hqP->tbInfo[tbCnt].timingInfo.sfn * 10) +
         hqP->tbInfo[tbCnt].timingInfo.subframe + 8)
      % 256;

   fdbkDelta = (cell->crntTime.sfn*10+cell->crntTime.subframe) < (hqP->tbInfo[tbCnt].timingInfo.sfn*10+hqP->tbInfo[tbCnt].timingInfo.subframe)? ((cell->crntTime.sfn+1024)*10+cell->crntTime.subframe) - (hqP->tbInfo[tbCnt].timingInfo.sfn*10+hqP->tbInfo[tbCnt].timingInfo.subframe) : (cell->crntTime.sfn*10+cell->crntTime.subframe) - (hqP->tbInfo[tbCnt].timingInfo.sfn*10+hqP->tbInfo[tbCnt].timingInfo.subframe);
   if(fdbkDelta + ((2 + 0)) >= 8)
   {
...

{
   ...
   if(ret != 0)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xa30001,L_ERROR,DBG_CELLID, cell->cellId,ue->ueId, 0, 0, 0,                                                                        "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_drx.c"      ,                                                                        1172      , "Memory allocation FAILED for DRX UECB CRBTI:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                                         ;
      return(ret);
   }
   ...
   ueDrxCb->drxDlInactvMask = 0xFFFFFFFF;
   ueDrxCb->drxUlInactvMask = 0xFFFFFFFF;
   ue->dl.dlInactvMask |= 0x08;
   ue->ul.ulInactvMask |= 0x08;

   for(cellIdx = 0; cellIdx < 8; cellIdx++)
   {
      ue->drxCb->drxDlInactvMaskPerCell[cellIdx] = 0xFFFFFFFF;
      ue->drxCb->drxUlInactvMaskPerCell[cellIdx] = 0xFFFFFFFF;
   }
   ...
   ueDrxCb->onDurIndx = 0xFFFF;
   ueDrxCb->onDurExpIndx = 0xFFFF;
   ueDrxCb->drxInactvIndx = 0xFFFF;
   ueDrxCb->shortCycleIndx = 0xFFFF;


   ueDrxCb->onDurExpDistance = -1;
   ueDrxCb->drxInactDistance = -1;
   ueDrxCb->drxShortCycleDistance = -1;
   ueDrxCb->distance = -1;


   if(0 == ueDrxCb->isShortCycleCfgd)
   {
      ueDrxCb->isLongCycle = 1;
   }
   else
   {
      ueDrxCb->isLongCycle = 0;
   }


   rgSCHDrxGetNxtOnDur (cell, ueDrxCb, &nxtOnDur,0);


   nxtOnDurTime = ((nxtOnDur.sfn * 10) + nxtOnDur.subframe);
   curTime = ((cell->crntTime.sfn * 10) +
                     cell->crntTime.subframe);

   onDurIndx = nxtOnDurTime % 256;

   ueDrxCb->distance = (nxtOnDurTime - curTime) / 256;
   if (ueDrxCb->distance < 0)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xa30002,L_ERROR,DBG_CELLID, cell->cellId, ueDrxCb->distance, ue->ueId, 0, 0,                                                                      "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_drx.c"      ,                                                                      1229      , "DRXUE. Invalid " "value for distance, %d CRNTI:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                                       ;
   }
   ...
   ueDrxCb->onDurationEnt.node = (U32)ue;
   ueDrxCb->onDurIndx = onDurIndx;


   if(1 == ueDrxCb->isShortCycleCfgd)
   {
      ueDrxCb->drxShortCycleDistance = (ueDrxCb->shortCycleTmrLen *
               ueDrxCb->shortDrxCycle) / 256;
      ...
      ueDrxCb->shortCycleEnt.node = (U32)ue;
   }
...

{
   ...
   if ( curTime <= drxCb->drxStartOffset )
   {
      ...
      nxtOnDur->sfn = drxCb->drxStartOffset / 10;
      nxtOnDur->subframe = (U8)(drxCb->drxStartOffset % 10);
   ...
   else
   {
      ...
      if (0 == (curDist % cycleLen))
      {
         ...
         nxtOnDur->sfn = (U16)curTime / 10;
         nxtOnDur->subframe = (U16)curTime % 10;
         nxtDist = ((nxtOnDur->sfn * 10) + nxtOnDur->subframe);
      ...
      else
      {
         nxtDist = drxCb->drxStartOffset + (numOfCycles + 1) *
                              cycleLen;
         nxtOnDur->sfn = (U16)nxtDist / 10;
         nxtOnDur->subframe = (U16)nxtDist % 10;
   ...
   if((nxtDist - (curTime - delta)) <= ((((2 + 0)) < ((4 - 2)))?((4 - 2)):((2 + 0))))
   {
...

{
     ...
     )
  {
     ...
     if ( ret != 0 )
     {
        if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xa30003,L_ERROR,DBG_CELLID, cell->cellId, ue->ueId, 0, 0, 0,                          "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_drx.c"        ,                          1506        , "rgSCHdrxUeReCfg():""Memory allocation FAILED for DRX UE Cb CRNTI:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
     ...
     ue->drxCb->drxDlInactvMask = 0xFFFFFFFF;
     ue->drxCb->drxUlInactvMask = 0xFFFFFFFF;
     ue->dl.dlInactvMask |= 0x08;
     ue->ul.ulInactvMask |= 0x08;
     ...
     ue->drxCb->onDurIndx = 0xFFFF;
     ue->drxCb->onDurExpIndx = 0xFFFF;
     ue->drxCb->drxInactvIndx = 0xFFFF;
     ue->drxCb->shortCycleIndx = 0xFFFF;


     ue->drxCb->onDurExpDistance = -1;
     ue->drxCb->drxInactDistance = -1;
     ue->drxCb->drxShortCycleDistance = -1;
     ue->drxCb->distance = -1;
  ...
  if( ue->drxCb == 0L )
  {
   ...
   if ( (ue->isDrxEnabled == 1 )
            &&
        (ueReCfg->ueDrxRecfg.isDrxEnabled == 0)
      )
   {
   ...
   else
   {
      ...
      if(ueDrxCb->shortCycleIndx != 0xFFFF)
      {
         cmLListDelFrm(&(cell->drxCb->drxQ[ueDrxCb->shortCycleIndx].shortCycleQ),
               &(ueDrxCb->shortCycleEnt));

         ueDrxCb->shortCycleEnt.node = (U32) 0L;
         ueDrxCb->shortCycleIndx = 0xFFFF;
      }


      if(0 == ue->drxCb->isShortCycleCfgd)
      {
         ue->drxCb->isLongCycle = 1;
      }
      else
      {
         ue->drxCb->isLongCycle = 0;
      }
      ...
      if((ueDrxCb->onDurExpIndx != 0xFFFF) && (ueDrxCb->onDurExpDistance != -1))
      {
         curIndx = (curTime + ((2 + 0))) % 256;
         if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xa30004,L_DEBUG,DBG_CELLID, cell->cellId, ueDrxCb->onDurExpIndx, curIndx, nxtOnDurTime, 0,                           "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_drx.c"         ,                           1615         , "OLD ONDUR RUNNING-EXPIRES at %d curIdx-%d nxtOnDurTime-%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
         ...
         {
            onDurExpTime = curTime + ((ueDrxCb->onDurExpDistance+1) * 256)+ (ueDrxCb->onDurExpIndx - curIndx + ((2 + 0)));
         }
         else
         {
            onDurExpTime = curTime + (ueDrxCb->onDurExpDistance * 256)+ (ueDrxCb->onDurExpIndx - curIndx + ((2 + 0)));
         }
      ...
      if (ueDrxCb->distance < 0)
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xa30005,L_ERROR,DBG_CELLID, cell->cellId, ueDrxCb->distance, ue->ueId, 0, 0,                                                                         "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_drx.c"         ,                                                                         1649         , "DRXUE. Invalid " "value for distance, %d CRNTI:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                                          ;
      }
...

{
   ...
   if ( ueDrxCb->onDurExpIndx != 0xFFFF )
   {
...

{
   ...
   if(ret != 0)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xa30006,L_ERROR,DBG_CELLID, cell->cellId, 0, 0, 0, 0,                                                         "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_drx.c"      ,                                                         1900      , "rgSCHDrxCellCfg():" "Memory allocation FAILED for DRX cell Cb", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                          ;
      return(ret);
   }
   if ( ((4 - 2)) > ((2 + 0)) )
   {
      cell->drxCb->delInUlScan = 0;
   }
   else
   {
      cell->drxCb->delInUlScan = 1;
   }
...

{
   ...
   drxCb->drxUlInactvMask &= ~0x00000001;
   drxCb->srRcvd = 1;

   if((ue->ul.ulInactvMask == 0)) { ue->ul.ulInactvMask &= ~0x08; } else { ue->ul.ulInactvMask &= ~0x08; if((ue->ul.ulInactvMask == 0)) { rgSCHUtlUlActvtUe(cell, ue); } };
...

{
   ...
   drxCb->drxUlInactvMask &= ~0x00000002;

   if((ueCb->ul.ulInactvMask == 0)) { ueCb->ul.ulInactvMask &= ~0x08; } else { ueCb->ul.ulInactvMask &= ~0x08; if((ueCb->ul.ulInactvMask == 0)) { rgSCHUtlUlActvtUe(cellCb, ueCb); } };

   drxCb->drxDlInactvMask &= ~0x00000002;

   if((ueCb->dl.dlInactvMask == 0)) { ueCb->dl.dlInactvMask &= ~0x08; } else { ueCb->dl.dlInactvMask &= ~0x08; if((ueCb->dl.dlInactvMask == 0)) { rgSCHUtlDlActvtUe(cellCb, ueCb); } };
...

{
   ...
   if(calcFrmOffst == 0)
   {
      if (drxUe->isLongCycle)
      {
         nxtOnDurIndex = ((idx + drxUe->longDrxCycle)
               % 256 );
         drxUe->distance = drxUe->longDrxCycle/256;
      }
      else
      {
         nxtOnDurIndex = ((idx + drxUe->shortDrxCycle)% 256);

         drxUe->distance = drxUe->shortDrxCycle / 256;
      }
   }
   else
   {
      ...
      nxtOnDurInSf = ((nxtOnDur.sfn * 10) +
                         nxtOnDur.subframe);

      curTime = ((cell->crntTime.sfn * 10) +
            cell->crntTime.subframe);

      nxtOnDurIndex = nxtOnDurInSf % 256;
      drxUe->distance = (nxtOnDurInSf-curTime) / 256;
      if (drxUe->distance < 0)
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xa30007,L_ERROR,DBG_CELLID, cell->cellId, drxUe->distance, ueCb->ueId, 0, 0,                                                                         "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_drx.c"         ,                                                                         2171         , "DRXUE. Invalid " "value for distance, %d CRNTI:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                                          ;
      }
   }


   if ( drxUe->onDurIndx != 0xFFFF )
   {
   ...
   drxUe->onDurationEnt.node = (U32)ueCb;
...

{
   ...
   while (node)
   {
      ...
      drxUe->drxUlInactvMask &= ~0x00000004;
      ...
      if ( delInUlScan == 1 )
      {
         ...
         rgSCHDrxMvToNxtOnDurOcc(cell,ue,ulIndex,0);
      }
   }
...

{
   ...
   while (node)
   {
      ...
      drxUe->drxDlInactvMask &= ~0x00000004;




      if((ue->dl.dlInactvMask == 0)) { ue->dl.dlInactvMask &= ~0x08; } else { ue->dl.dlInactvMask &= ~0x08; if((ue->dl.dlInactvMask == 0)) { rgSCHUtlDlActvtUe(cell, ue); } };


      if (drxUe->onDurExpIndx != 0xFFFF)
      {
         if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xa30008,L_DEBUG,DBG_CELLID, cell->cellId, ue->ueId, drxUe->onDurExpIndx, drxUe->onDurExpDistance, dlIndex,                      "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_drx.c"         ,                      2747         , "UEID:%d PreExisted[%d:%d]in onDurExpQ new[%d]", RLOG_FILE_ID, RLOG_MODULE_NAME); }
      ...
      drxUe->onDurationExpEnt.node = (U32)ue;
      ...
      if ( delInUlScan == 0 )
      {
         ...
         rgSCHDrxMvToNxtOnDurOcc(cell,ue,dlIndex,0);
      }
...

{
   ...
   while (node)
   {
      ...
      drxHq = &((dlHq)->drxCb);
      drxUe = ((ue)->drxCb);
      cellIdx = ue->cellIdToCellIdxMap[(U8)((dlHq->hqE->cell->cellId >= rgSchCb[dlHq->hqE->cell->instIdx].genCfg.startCellId) ? (dlHq->hqE->cell->cellId - rgSchCb[dlHq->hqE->cell->instIdx].genCfg.startCellId)&(8 -1): 0)];
      ...
      if (drxUe->drxRetransTmrLen > drxHq->retxTmrReduction)
      {
         ...
         if(drxHq->reTxIndx == 0xFFFF)
         {
            ...
            drxHq->harqRetxEnt.node = (U32)dlHq;
            drxHq->reTxIndx = reTxExpIndx;
         }
         else
         {
            if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xa30009,L_ERROR,DBG_CELLID, cell->cellId, ue->ueId, reTxExpIndx, dlIndex, drxHq->reTxIndx,                              "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_drx.c"            ,                              2954            , "CRNTI:%d " "Adding Retx Node to expire at RetxIndx: %d at dlIndex %d " "drxHq->reTxIndx %d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
         ...
         drxUe->drxDlInactvMask &= ~(0x00000010 << dlHq->procId);
...

{
   ...
   while (node)
   {
      ...
      drxUe->drxUlInactvMask &= ~(0x00000010 << dlHq->procId);
