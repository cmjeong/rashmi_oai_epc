...
static S16 szLiHdlDecFail (CmPAsnErr asnErr,S1apPdu *pdu,SzPeerCb *peerCb);


PFSZLIMSGHANDLER szLiRcvEnb[73] =
{
   szLiCOMsgHandler,
   szLiCOMsgHandler,
   szLiCOMsgHandler,
...

{
   ...
   if (sctSap == (SzSctSapCb *)0L)
   {
      ...
      { if (pdu != 0L) { cmFreeMem ((Ptr)(pdu)); pdu = 0L; } };
   ...
   if ((retVal = szEncode(&mem, &pdu, &mBuf)) != 0)
   {
   ...
   else
   {
      SzLiSctDatReq(&sctSap->pst, sctSap->spId,
                    peerCb->assocCb->spAssocId,
                    &peerCb->assocCb->assocCfg->dstAddrLst.nAddr[0],
                    conCb->cd.streamNo, 0, 0, 0, 18, mBuf);
   }



   { U8 _msgIdx; SzPduSts *_p; Cntr *_ptr; szUtilsGetMsgIdx(&_msgIdx, pdu); if(_msgIdx < 73) { _p = &peerCb->peerSts.cfm.pdu; _ptr = (Cntr *)(((SzCntr *)_p) + _msgIdx); *_ptr = (*_ptr) + 1; _p = &peerCb->sctSapCb->sts.cfm.pdu; _ptr = (Cntr *)(((SzCntr *)_p) + _msgIdx); *_ptr = (*_ptr) + 1; } if (conCb != 0L) { peerCb->peerSts.cfm.szDedProcSts.tx++; peerCb->sctSapCb->sts.cfm.szDedProcSts.tx++; } else { peerCb->peerSts.cfm.szComProcSts.tx++; peerCb->sctSapCb->sts.cfm.szComProcSts.tx++; } };
...

{
   ...
   if ((retVal = szDecode(&mem, &asnErr, &pdu, &mBuf)) != 0)
   {
   ...
   if ((peerCb->state == 1) && (evnt != 23))
   {
      { if ((&szCb.init)->dbgMask & (0x00000008)) { snprintf((&szCb.init)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", szCb.nodeDbg, (&szCb.init)->procId, (&szCb.init)->ent, (&szCb.init)->inst,

                                 "/home/mrehwald/Desktop/Radisys/lte_enb/src/s1ap/sz_li.c"
      ,

                                 483
      ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "szLiRcvFromLower : Peer is under reset. Ignoring the               Dat indication \n"); SPrint((&szCb.init)->prntBuf); } }
   ...
   { SzPduSts *_p; Cntr *_ptr; if(evnt < 73) { _p = &peerCb->peerSts.cfm.pdu; _ptr = (((Cntr *)(((SzCntr *)_p) + evnt)) + 1); *_ptr = (*_ptr) + 1; _p = &peerCb->sctSapCb->sts.cfm.pdu; _ptr = (((Cntr *)(((SzCntr *)_p) + evnt)) + 1); *_ptr = (*_ptr) + 1; } };





   if (peerCb->nodeType == 1)
   {
...

{
   ...
   ,
                                                         553
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "szLiCOMsgHandler : Received dedicated procedure\n"); SPrint((&szCb.init)->prntBuf); } }
                                                          ;
   ...
   if(peerCb->skipUeSm)
   {
      ...
      uDatEvnt.transId.pres = 0;
      uDatEvnt.peerId.pres = 1;
   ...
   else
   {
      ...
      if (peerCb->nodeType == 1)
      {
         ...
         szGetIE(&(pdu->pdu), 8, (TknU8 **)&eNbIdTkn);



         if (eNbIdTkn == 0L)
         {
            szGetIE(&(pdu->pdu), 99, (TknU8 **)&ueS1apId);
      ...
      szSmExcMt(conCb, peerCb, pdu, evnt, 2);
...

{
   ...
   szIncErrSts(peerCb, 3, SztCauseProtsemantic_errorEnum);



   ret = szLiSndToLower(peerCb, 0L, errPdu);
   if (ret != 0)
   {
...

{
   ...
   ,
                                                    ...
                                                    776
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "--------S1AP:SzLiSctBndCfm-----(proc(%d),ent(%d),inst(%d))-------\n", pst->dstProcId, pst->dstEnt, pst->dstInst); SPrint((&szCb.init)->prntBuf); } }
   ...
   if (!szCb.sctSapLst || ( (sctSapCb = szCb.sctSapLst[suId]) == 0L))
   {
   ...
   if(result == 2)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, 8, 0, &dgn); };


      szStopTmr(&sctSapCb->timers[0]);
      return(1);
   }
   else
   {
      ...
      ,
                                                                              838
      ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "S1AP:SzLiSctBndCfm - Binding with lsap (%d) successful\n", suId); SPrint((&szCb.init)->prntBuf); } }
                                                                               ;
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, 11, 0, &dgn); };
...
)
{
   ...
   ,
                                                    ...
                                                    943
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "--------S1AP:SzLiSctEndpOpenCfm-----(proc(%d),ent(%d),inst(%d))-------\n", pst->dstProcId, pst->dstEnt, pst->dstInst); SPrint((&szCb.init)->prntBuf); } }
   ...
   if(suId < 0 || suId > szCb.genCfg.nmbSctSaps)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 2), 3, &dgn); };
      return(1);
   }
       ...
       (sctSapCb->state != 2))
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 2), 2, &dgn); };
      return(1);
   }
   ...
   if (result != 0)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 2), cause, &dgn); };
      return(1);
   }
   else
   {

      sctSapCb->endpCb.epState = 3;

      sctSapCb->endpCb.locSpEndpId = spEndpId;
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 2), (256 + 8), &dgn); };

   }
   return(0);

}
 S16 SzLiSctEndpCloseCfm
(
Pst *pst,
SuId suId,
UConnId suEndpId,
...
)
{
   ...
   ,
                                                    ...
                                                    1090
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "--------S1AP:SzLiSctEndpCloseCfm-----(proc(%d),ent(%d),inst(%d))-------\n", pst->dstProcId, pst->dstEnt, pst->dstInst); SPrint((&szCb.init)->prntBuf); } }
   ...
   if(suId < 0 || suId > szCb.genCfg.nmbSctSaps)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 3), 3, &dgn); };
      return(1);
   }
   ...
   if (!szCb.sctSapLst || ( (sctSapCb = szCb.sctSapLst[suId]) == 0L))
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 3), 2, &dgn); };
      return(1);
   }
   ...
   if (result != 0)
   {
      ...
      ,
                                                                1126
      ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "S1AP:SzLiSctEndpCloseCfm - EndpCloseReq fails \n"); SPrint((&szCb.init)->prntBuf); } }
                                                                 ;

      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 3), (256 + 9), &dgn); };
      return(1);
   }
   else
   {
      ...
      sctSapCb->endpCb.epState = 1;
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 3), (256 + 10), &dgn); };
   }
   return(0);
...
)
{
   ...
   ,
                                                    ...
                                                    1209
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "--------S1AP:SzLiSctAssocInd-----(proc(%d),ent(%d),inst(%d))-------\n", pst->dstProcId, pst->dstEnt, pst->dstInst); SPrint((&szCb.init)->prntBuf); } }
   ...
   if(!(assocParams))
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 4), 13, &dgn); };
      return(1);
   }
   ...
   if(suId < 0 || suId > szCb.genCfg.nmbSctSaps)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 4), 3, &dgn); };
      return(1);
   }
       ...
       || sctSapCb->state != 2)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 4), 2, &dgn); };
      return(1);
   }
...
)
{
   ...
   ,
                                                    ...
                                                    1432
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "--------S1AP:SzLiSctAssocCfm-----(proc(%d),ent(%d),inst(%d))-------\n", pst->dstProcId, pst->dstEnt, pst->dstInst); SPrint((&szCb.init)->prntBuf); } }
   ...
   if(suId < 0 || suId > szCb.genCfg.nmbSctSaps)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 5), 3, &dgn); };
      return(1);
   }
       ...
       || sctSapCb->state != 2)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 5), 2, &dgn); };
      return(1);
   }
...
)
{
   ...
   ,
                                                    ...
                                                    1571
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "--------S1AP:SzLiSctTermInd-----(proc(%d),ent(%d),inst(%d))-------\n", pst->dstProcId, pst->dstEnt, pst->dstInst); SPrint((&szCb.init)->prntBuf); } }
   ...
   if(suId < 0 || suId > szCb.genCfg.nmbSctSaps)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 6), 3, &dgn); };
      return(1);
   }
       ...
       || sctSapCb->state != 2)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 6), 2, &dgn); };
      return(1);
   }
   ...
   if (((sztSapCb = peerCb->sztSapCb) != 0L) &&
       (sztSapCb->state == 2))
   {

      SzUiSztStaInd (&sztSapCb->pst, sztSapCb->spId, &sta);

      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 5; dgn.u.peerId.pres = 1; dgn.u.peerId.val = peerCb->peerId; szSendLmAlarm(1, (256 + 6), (256 + 3), &dgn); };
   }
   return(0);
}
 S16 SzLiSctTermCfm
...
)
{
   ...
   ,
                                                    ...
                                                    1735
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "--------S1AP:SzLiSctTermCfm-----(proc(%d),ent(%d),inst(%d))-------\n", pst->dstProcId, pst->dstEnt, pst->dstInst); SPrint((&szCb.init)->prntBuf); } }
   ...
   if(suId < 0 || suId > szCb.genCfg.nmbSctSaps)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 7), 3, &dgn); };
      return(1);
   }
       ...
       || sctSapCb->state != 2)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 7), 2, &dgn); };
      return(1);
   }
   ...
   ,
                                                               1794
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "S1AP:SzLiSctTermCfm : Association is terminated \n"); SPrint((&szCb.init)->prntBuf); } }
                                                                ;

   if (peerCb->delPcb)
   {

      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 5; dgn.u.peerId.pres = 1; dgn.u.peerId.val = peerCb->peerId; szSendLmAlarm(1, (256 + 7), (256 + 4), &dgn); };
...
)
{
   ...
   ,
                                                    ...
                                                    1885
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "--------S1AP:SzLiSctDatInd-----(proc(%d),ent(%d),inst(%d))-------\n", pst->dstProcId, pst->dstEnt, pst->dstInst); SPrint((&szCb.init)->prntBuf); } }
   ...
   if (suId < 0 || suId > szCb.genCfg.nmbSctSaps)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 9), 3, &dgn); };
      { if (mBuf != 0L) { SPutMsg (mBuf); mBuf = 0L; } };
      return(1);
   }
   ...
   if (!szCb.sctSapLst )
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 9), 2, &dgn); };
      { if (mBuf != 0L) { SPutMsg (mBuf); mBuf = 0L; } };
      return(1);
   }
   ...
   if ((sctSapCb == 0L) || sctSapCb->state != 2)
   {
...

{
   ...
   ,
                                                    ...
                                                    2018
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "--------S1AP:SzLiSctFlcInd-----(proc(%d),ent(%d),inst(%d))-------\n", pst->dstProcId, pst->dstEnt, pst->dstInst); SPrint((&szCb.init)->prntBuf); } }
   ...
   if(suId < 0 || suId > szCb.genCfg.nmbSctSaps)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 8), 3, &dgn); };
      return(1);
   }
       ...
       || sctSapCb->state != 2)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 8), 2, &dgn); };
      return(1);
   }
   ...
   { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 5; dgn.u.peerId.pres = 1; dgn.u.peerId.val = peerCb->peerId; szSendLmAlarm(1, (256 + 8), flcInd.status, &dgn); };
...
)
{
   ...
   ,
                                                    ...
                                                    2330
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "--------S1AP:SzLiSctStaInd-----(proc(%d),ent(%d),inst(%d))-------\n", pst->dstProcId, pst->dstEnt, pst->dstInst); SPrint((&szCb.init)->prntBuf); } }
   ...
   if(suId < 0 || suId > szCb.genCfg.nmbSctSaps)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 10), 3, &dgn); };
      return(1);
   }
       ...
       || sctSapCb->state != 2)
   {
      ...
      { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 3; dgn.u.sapId = suId; szSendLmAlarm(2, (256 + 10), 2, &dgn); };
      return(1);
   }
    ...
    switch(status)
    {
        case 0x01:
        {
              ...
              (sztSapCb->state == 2))
          {
              ...
              { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 5; dgn.u.peerId.pres = 1; dgn.u.peerId.val = peerCb->peerId; szSendLmAlarm(1, (256 + 5), (256 + 2), &dgn); };
          }
           ...
           if((peerCb->nodeType == 1) && (peerCb->setupReq != 0L))
           {
              szLiSndToLower (peerCb, 0L, peerCb->setupReq);
              peerCb->setupReq = 0L;
           }
        ...
        case 0x04:
           ...
           { SzUstaDgn dgn; cmMemset((U8*)&dgn, 0, sizeof(SzUstaDgn)); dgn.type = 5; dgn.u.peerId.pres = 1; dgn.u.peerId.val = peerCb->peerId; szSendLmAlarm(1, (256 + 11), cause, &dgn); };
           break;
...

{
   ...
   if (assocCb->assocState == 1)
   {
      ...
      ,
                                                                         2551
      ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "szLiSndAssocReq: Association is already up and running \n"); SPrint((&szCb.init)->prntBuf); } }
                                                                          ;
      return(1);
   }


   assocCb->assocState = 12;
...

{
   ...
   if (assocCb->assocState == 0)
   {
     { if ((&szCb.init)->dbgMask & (0x00000008)) { snprintf((&szCb.init)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", szCb.nodeDbg, (&szCb.init)->procId, (&szCb.init)->ent, (&szCb.init)->inst,
                                                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/s1ap/sz_li.c"
     ,
                                                                    2618
     ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "szLiTermAssoc: Association is already set to down \n"); SPrint((&szCb.init)->prntBuf); } }
                                                                     ;
   ...
   if(assocState == 12)
   {
      ...
      ret = SzLiSctTermReq(&(assocCb->peer->sctSapCb->pst),
            ...
            assocCb->suAssocId, 1, assocAbort);
   }
   else
   {
      ...
      ret = SzLiSctTermReq(&(assocCb->peer->sctSapCb->pst),
                  assocCb->peer->sctSapCb->spId,
                  assocCb->spAssocId, 2, assocAbort);
   }
...

{
      ...
      (asnErr.errCode != 0x03))
   {
      ...
      szDbFillErrInd(0L, 3, cause, &errPdu, mem, &asnErr);



      szIncErrSts(peerCb, 3, cause);



      retVal = szLiSndToLower(peerCb, 0L, errPdu);
      if (retVal != 0)
      {
   ...
   else
   {
      ...
      if(msgType == 0)
      {
         ...
         szUtilsBldUnsucOutComeMsg(pdu, 3, cause, &errPdu, mem,
               &asnErr);
      ...
      else
      {
         ...
         if(conCb)
         {
            ...
            { SztLclErrInd _errInd; cmMemset ((U8 *)&_errInd, 0, sizeof (SztLclErrInd)); _errInd.causeType = (U8) 16; _errInd.causeValue = (U8)procCode; _errInd.transId.pres = 1; _errInd.transId.val = conCb->cd.suConnId; SzUiSztErrInd (&peerCb->sztSapCb->pst, peerCb->sztSapCb->suId, &_errInd); }
                                                                         ;
         }
      }
