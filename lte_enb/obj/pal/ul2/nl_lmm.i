...
extern S16 nlSendSonMeasRptIntervalInd ( U32 ueMeasInterval, NlCellCb *cellCb )
  ...
  ;
static U16 nlLnlGenCfg ( NlCfg *cfg )

  ;

static U16 nlNluSapCfg ( NlCfg *cfg )


  ;

static S16 nlCellCfg ( LnlCellCfg *cellCfg )

  ;

static S16 nlNeighborCellCfg ( LnlNghCellCfg *nghCellCfg )

  ;

static S16 nlCellReCfg ( LnlCellReCfg *cellReCfg )

  ;

static Void nlLnlFillCfmPst ( Pst *cfmPst, Pst *reqPst, NlMngmt *cfg )



  ;

static Void nlShutdown ( Void )

  ;

static S16 nlProcessCellUpInd ( LnlEnbStaInd *statInd )

  ;

static S16 nlCopyLnlNghCellInfo ( LnlNghCellCfg *srcCellCfg, NlTmpNghCellCfg *tgtCellCfg )


  ;

static S16 nlCopyLnlNghCellAddInfo ( LnlNghCellAdd *srcCellAdd, NlTmpNghCellAdd *tgtCellAdd )


  ;

static S16 nlCopyLnlNghCellDelInfo ( LnlNghCellDel *srcCellDel, NlTmpNghCellDel *tgtCellDel )


  ;

static S16 nlCopyLnlNghCellModInfo ( LnlNghCellMod *srcCellMod, NlTmpNghCellMod *tgtCellMod )
...

{
   ...
   Pst cfmPst;
   ;;
   { if (0 == SGetSBuf(nlCb.init.region, nlCb.init.pool, (Data **)(&cfm), (sizeof(NlMngmt))) ) { cmMemset((U8 *)(*(&cfm)), 0, (sizeof(NlMngmt))); } else { (*(&cfm)) = 0L; } };
   if(0L == cfm)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf90001, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",160, "Failed to allocate memory for confirm", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      { (Void)SPutSBuf((pst->region), (pst->pool), (Data *)(cfg), (Size)(sizeof(NlMngmt))); (cfg) = 0L; };
      return(1);
   }
...
{
   U16 ret = 0;
   NlUiSapCb *upSapCfg = 0L;

   ;


   if(nlCb.init.cfgDone != 1)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf90002, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",316, "NLU SAP configuration received before Gen config", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(8);
   }
   if((cfg->u.nluSap.selector != 2) &&
      (cfg->u.nluSap.selector != 1) &&
      (cfg->u.nluSap.selector != 0))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf90003, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",323, "Invalid NLU SAP seletor value received", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(13);
   }
   if(0 == nlCb.nluSap.sapState)
   {
      nlCb.nluSap.sapState = 1;
   }
...

{
   ...
   if (1 == nlCb.init.cfgDone)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf90004, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",386, "Gen config received second time", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(3);
   }
   if ((lmPst->selector != 1) &&
       (lmPst->selector != 0) &&
       (lmPst->selector != 2))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf90005, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",393, "Invalid LNL SAP selector value received", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(13);
   }

   { cmMemcpy((U8*) ((U8 *)&nlCb.init.lmPst), (U8*) ((U8 *)lmPst), (sizeof(Pst))); }
                         ;

   nlCb.init.lmPst.srcProcId = nlCb.init.procId;
   nlCb.init.lmPst.srcEnt = nlCb.init.ent;
   nlCb.init.lmPst.srcInst = nlCb.init.inst;
   nlCb.init.lmPst.event = 0x00;
   ...
   if( L_INFO < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev3(0xf90006, L_INFO, nlCb.genCfg.pciSelType, nlCb.genCfg.earfcnSelType, nlCb.genCfg.prachSelType, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c"   , 411   , "Configured PCI Sel Type: %d Earfcn Sel Type: %d Prach Sel Type: %d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   nlCb.nluSap.sapState = 0;


   nlCb.init.cfgDone = 1;


   if(0 != nlVendorDSonConnEstReq())
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf90007, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",424, "Established Connection with Vendor DSON Failed", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(13);
   }
   else
   {
      if( L_INFO < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf90008, L_INFO, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",429, "Connection with Vendor DSON successfully established", RLOG_FILE_ID, RLOG_MODULE_NAME); };
   }
   nlCb.timeRes = cfg->u.genCfg.tmrRes;
   if (0 != nlRegInitTmr())
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf90009, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",434, "nlGenCfg: nlRegInitTmr Failed", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
   return(ret);
}
...

{
   ...
   if(0L == cfm)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf9000a, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",484, "Failed to allocate memory for control confirm", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      { (Void)SPutSBuf((pst->region), (pst->pool), (Data *)(cntrl), (Size)(sizeof(NlMngmt))); (cntrl) = 0L; };
      return(1);
   }
   ...
   { (Void)SPutSBuf((pst->region), (pst->pool), (Data *)(nlMngmt), (Size)(sizeof(NlMngmt))); (nlMngmt) = 0L; };

   return(0);
}
static S16 nlProcessCellUpInd
(
...

{
      ...
      switch(usta->staType)
      {
         case LNL_STAT_ENB :
         {
            switch(usta->u.stEnbStaInfo.enStaType)
            {
               ...
               default:
               {
                  if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf9000b, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",590, "Invalid eNB status Indication received", RLOG_FILE_ID, RLOG_MODULE_NAME); };
                  ret = 1;
                  break;
               }
            }
         ...
         default:
         {
            if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf9000c, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",604, "Invalid status type received from SM", RLOG_FILE_ID, RLOG_MODULE_NAME); };
            ret = 1;
            break;
         }
      }
   ...
   { (Void)SPutSBuf((pst->region), (pst->pool), (Data *)(statInd), (Size)(sizeof(NlMngmt))); (statInd) = 0L; };
   return(ret);
}
static Void nlShutdown
(
Void
)
...
{
   NlCellCb *cellCb = 0L;
   NlNeighCellCb *nghEnbCb = 0L;
   CmLListCp *nghCbLst = 0L;
   CmLListCp *nghNghCbLst = 0L;
   CmLListCp *pendNghLst = 0L;
   CmLListCp *pendReCfgLst = 0L;
   CmLList *nghNode = 0L;
   CmLList *nghNghNode = 0L;
   NlNghServCellCb *nghServCb = 0L;
   CmLList *pendNghNode = 0L;
   CmLList *pendReCfgNode = 0L;
   NlNghNghCellCb *nghNghCb = 0L;
   NlNghCfgPendMsg *pendNghMsg = 0L;
   NlCellReCfgPendMsg *pendReCfgMsg = 0L;

   ;;

   nlStopTimers();

   SDeregCfgTmrMt((Ent)0xea,(Inst)0,nlCb.timeRes,100,nlActvTmr);
   while(cmHashListGetNext(&nlCb.cellCbList, 0L, (U32 *)&cellCb) == 0)
   {

      pendNghLst = &(cellCb->pciModPendMsg.nghCfgMsgList);
      ((((pendNghNode) = ((pendNghLst)->crnt = (pendNghLst)->first)) != 0L) ? (pendNghNode)->node : 0L);
      while(0L != pendNghNode)
      {
         pendNghMsg = (NlNghCfgPendMsg *)pendNghNode->node;
         cmLListDelFrm(pendNghLst, pendNghNode);
         { (Void)SPutSBuf((nlCb.init.region), (nlCb.init.pool), (Data *)(pendNghMsg), (Size)(sizeof(NlNghCfgPendMsg))); (pendNghMsg) = 0L; };
         pendNghNode = 0L;
         ((((pendNghNode) = ((pendNghLst)->crnt = (pendNghLst)->first)) != 0L) ? (pendNghNode)->node : 0L);
      }
      pendReCfgLst = &(cellCb->pciModPendMsg.cellReCfgMsgList);
      ((((pendReCfgNode) = ((pendReCfgLst)->crnt = (pendReCfgLst)->first)) != 0L) ? (pendReCfgNode)->node : 0L);
      while(0L != pendReCfgNode)
      {
         pendReCfgMsg = (NlCellReCfgPendMsg *)pendReCfgNode->node;
         cmLListDelFrm(pendReCfgLst, pendReCfgNode);
         { (Void)SPutSBuf((nlCb.init.region), (nlCb.init.pool), (Data *)(pendReCfgMsg), (Size)(sizeof(NlCellReCfgPendMsg))); (pendReCfgMsg) = 0L; };
         pendReCfgNode = 0L;
         ((((pendReCfgNode) = ((pendReCfgLst)->crnt = (pendReCfgLst)->first)) != 0L) ? (pendReCfgNode)->node : 0L);
      }
      while(cmHashListGetNext(&cellCb->neighEnbCb.nghEnbListCp, 0L, (U32 *)&nghEnbCb) == 0)
      {
         nghCbLst = &(nghEnbCb->servCellLst);
         ((((nghNode) = ((nghCbLst)->crnt = (nghCbLst)->first)) != 0L) ? (nghNode)->node : 0L);
         while(0L != nghNode)
         {
            nghServCb = (NlNghServCellCb *)nghNode->node;
            nghNghCbLst = &nghServCb->nghCellLst;
            ((((nghNghNode) = ((nghNghCbLst)->crnt = (nghNghCbLst)->first)) != 0L) ? (nghNghNode)->node : 0L);
            while(0L != nghNghNode)
            {
               nghNghCb = (NlNghNghCellCb *)nghNghNode->node;
               cmLListDelFrm(nghNghCbLst, nghNghNode);
               { (Void)SPutSBuf((nlCb.init.region), (nlCb.init.pool), (Data *)(nghNghCb), (Size)(sizeof(NlNghNghCellCb))); (nghNghCb) = 0L; };
               nghNghNode = 0L;
               ((((nghNghNode) = ((nghNghCbLst)->crnt = (nghNghCbLst)->first)) != 0L) ? (nghNghNode)->node : 0L);
            }
            cmLListDelFrm(nghCbLst, nghNode);
            { (Void)SPutSBuf((nlCb.init.region), (nlCb.init.pool), (Data *)(nghServCb), (Size)(sizeof(NlNghServCellCb))); (nghServCb) = 0L; };
            nghNode = 0L;
            ((((nghNode) = ((nghCbLst)->crnt = (nghCbLst)->first)) != 0L) ? (nghNode)->node : 0L);
         }
         cmHashListDelete(&cellCb->neighEnbCb.nghEnbListCp, (U32 )nghEnbCb);
         { (Void)SPutSBuf((nlCb.init.region), (nlCb.init.pool), (Data *)(nghEnbCb), (Size)(sizeof(NlNeighCellCb))); (nghEnbCb) = 0L; };
      }
      cmHashListDeinit(&cellCb->neighEnbCb.nghEnbListCp);
      cmHashListDelete(&nlCb.cellCbList, (U32 )cellCb);
      { (Void)SPutSBuf((nlCb.init.region), (nlCb.init.pool), (Data *)(cellCb), (Size)(sizeof(NlCellCb))); (cellCb) = 0L; };
   }
...

{
   ...
   if(0 != nlValidateCellEarfcnCfg(cellCfg))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf9000d, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",754, "Cell config received with improper EARFCN configuration", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
   if(0 != nlValidateCellPciCfg(cellCfg))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf9000e, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",759, "Cell config received with improper PCI configuration", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
   if(0 != nlValidateCellPrachCfg(&cellCfg->prachCfgInfo))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf9000f, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",764, "Cell config received with improper PRACH configuration", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
   hashKey = cellCfg->cellId;
   if(0 == cmHashListFind(&(nlCb.cellCbList),
                (U8 *)&hashKey, sizeof(hashKey), 0, (U32 *)&cellCb))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xf90010, L_ERROR, cellCfg->cellId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",771, "CellCb already created for cell Id %d", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
   { if (0 == SGetSBuf(nlCb.init.region, nlCb.init.pool, (Data **)(&cellCb), (sizeof(NlCellCb))) ) { cmMemset((U8 *)(*(&cellCb)), 0, (sizeof(NlCellCb))); } else { (*(&cellCb)) = 0L; } };
   if(0L == cellCb)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf90011, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",777, "Failed to allocate memory for cell Cb", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
   cellCb->cellId = hashKey;
   if(0 != cmHashListInsert(&(nlCb.cellCbList), (U32)cellCb,
             (U8 *)&cellCb->cellId, sizeof(cellCb->cellId)))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xf90012, L_ERROR, hashKey, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",784, "failed to insert cell Id into hash table with cellId %d", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      { (Void)SPutSBuf((nlCb.init.region), (nlCb.init.pool), (Data *)(cellCb), (Size)(sizeof(NlCellCb))); (cellCb) = 0L; };
      return(1);
   }
   ...
   { cmMemcpy((U8*) (&cellCb->cellInfo.ecgi), (U8*) (&cellCfg->ecgi), (sizeof(LnlEcgi))); };
   cellCb->cellInfo.ecgi.cellId = cellCfg->ecgi.cellId;

   cmHashListInit(&(cellCb->neighEnbCb.nghEnbListCp), 32, 0,
       0, 4, nlCb.init.region, nlCb.init.pool);

   { cellCb->state = NL_CELL_STATE_INIT; };


   if(0 != nlCellSelectEarfcn(cellCb, cellCfg))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf90013, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",802, "Earfcn selection failed", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }

   if(0 != nlCellStorePciCfg(cellCb, cellCfg))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf90014, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",808, "storing PCI configuration failed", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }

   if(0 != nlCellStorePrachCfg(cellCb, cellCfg))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf90015, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",814, "storing PRACH configuration failed", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }

   if(0 != nlCellStoreTxPowerCfg(cellCb, cellCfg))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf90016, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c",820, "storing Tx Power configuration failed", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
   return(0);
}
static S16 nlCopyLnlNghCellModInfo
(
LnlNghCellMod *srcCellMod,
...

{
   U32 nghCount;
   U32 nghNghCount;
   NlTmpNghCellModInfo *nlCellMod = 0L;
   LnlNghCellModInfo *lnlCellMod = 0L;
   ;;
...
static S16 nlCopyLnlNghCellDelInfo
(
LnlNghCellDel *srcCellDel,
...
static S16 nlCopyNghPrachInfo
(
NlPrachCfg *tgtPrachCfg,
LnlNghPrachCfg *srcNghPrachCfg
)
...
static S16 nlCopyLnlNghCellAddInfo
(
LnlNghCellAdd *srcCellAdd,
...

{
   U32 nghCount;
   U32 nghNghCount;
   NlTmpNghCellAddInfo *nlCellAddInfo = 0L;
   LnlNghCellAddInfo *lnlCellAddInfo = 0L;
   ;;
   ...
   for(nghCount = 0; nghCount < tgtCellAdd->numNgh; nghCount ++)
   {
      ...
      {
         nlCellAddInfo->nghCellInfo.isPrachCfg = 1;
      ...
      {
         nlCellAddInfo->nghCellInfo.isPrachCfg = 0;
      }
...
static S16 nlCopyLnlNghCellInfo
(
LnlNghCellCfg *srcCellCfg,
...

{
   ...
   if(0 != cmHashListFind(&(nlCb.cellCbList), (U8 *)&hashKey,
     sizeof(hashKey), 0, (U32 *)&cellCb))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xf90017, L_ERROR, nghCellCfg->servCellId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c"      , 1185      , "Invalid cellId %d received for neighbor configuration", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                              ;
      return(1);
   }
...

{
   ...
   if(0 != cmHashListFind(&(nlCb.cellCbList), (U8 *)&hashKey,
     sizeof(hashKey), 0, (U32 *)&cellCb))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xf90018, L_ERROR, cellReCfg->cellId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_lmm.c"      , 1234      , "Invalid cellId %d received for cell reconfiguration", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                         ;
      return(1);
   }
