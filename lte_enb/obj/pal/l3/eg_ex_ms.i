...

{
   ...
   if(mBuf == 0L)
   {
      SLogError(egCb.init.ent, egCb.init.inst, egCb.init.procId, (Txt *)
                                               "/home/mrehwald/Desktop/Radisys/lte_enb/src/egtpu/eg_ex_ms.c"
      ,
                                               175
      , 0x4, ((0 + 0) + 95), (ErrVal)pst->event, (Txt *)"egActvTsk: received mBuf is NULLP");
                                                ;
      { if (egCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (egCb.init.prntBuf, " [%s 0x%x:%x]", "Eg", egCb.init.ent,egCb.init.inst); printf("%s\n", egCb.init.prntBuf); }; } { { sprintf (egCb.init.prntBuf, " [%s:%d]","/home/mrehwald/Desktop/Radisys/lte_enb/src/egtpu/eg_ex_ms.c",176); printf("%s\n", egCb.init.prntBuf); }; } { sprintf (egCb.init.prntBuf, " Exiting\n"); printf("%s\n", egCb.init.prntBuf); }; } return(1); };
   }
      ...
      case 0x0E:
         {
            switch(pst->event)
            {
               ...
               default:
                  ...
                  {
                     { if (mBuf != (Buffer*) 0L) { { if (egCb.init.dbgMask & ((0x00000100 << 8))) { { { sprintf (egCb.init.prntBuf, " [%s 0x%x:%x]", "Eg", egCb.init.ent,egCb.init.inst); printf("%s\n", egCb.init.prntBuf); }; } { { sprintf (egCb.init.prntBuf, " [%s:%d]","/home/mrehwald/Desktop/Radisys/lte_enb/src/egtpu/eg_ex_ms.c",512); printf("%s\n", egCb.init.prntBuf); }; } { sprintf (egCb.init.prntBuf, " [ MEM  : %d %d %p %ld]\n", 2, 2,(Void*)mBuf, (U32)0 ); printf("%s\n", egCb.init.prntBuf); }; } }; (Void)SPutMsg(mBuf); mBuf = 0L; } };
                     SLogError(egCb.init.ent, egCb.init.inst, egCb.init.procId, (Txt *)"/home/mrehwald/Desktop/Radisys/lte_enb/src/egtpu/eg_ex_ms.c", 513, 0x4, ((0 + 0) + 98), (ErrVal)pst->event, (Txt *)"egActvTsk: Invalid event");;
                     retVal = 1;
                     break;
                  }
      ...
      default:
         {
            ...
            ,
                                            601
            , 0x4, ((0 + 0) + 101), (ErrVal)pst->event, (Txt *)"egActvTsk: Invalid event");
                                             ;
            { if (mBuf != (Buffer*) 0L) { { if (egCb.init.dbgMask & ((0x00000100 << 8))) { { { sprintf (egCb.init.prntBuf, " [%s 0x%x:%x]", "Eg", egCb.init.ent,egCb.init.inst); printf("%s\n", egCb.init.prntBuf); }; } { { sprintf (egCb.init.prntBuf, " [%s:%d]","/home/mrehwald/Desktop/Radisys/lte_enb/src/egtpu/eg_ex_ms.c",602); printf("%s\n", egCb.init.prntBuf); }; } { sprintf (egCb.init.prntBuf, " [ MEM  : %d %d %p %ld]\n", 2, 2,(Void*)mBuf, (U32)0 ); printf("%s\n", egCb.init.prntBuf); }; } }; (Void)SPutMsg(mBuf); mBuf = 0L; } };
            retVal = 1;
            break;
         }
   }
   ...
   SExitTsk();
   { if (egCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (egCb.init.prntBuf, " [%s 0x%x:%x]", "Eg", egCb.init.ent,egCb.init.inst); printf("%s\n", egCb.init.prntBuf); }; } { { sprintf (egCb.init.prntBuf, " [%s:%d]","/home/mrehwald/Desktop/Radisys/lte_enb/src/egtpu/eg_ex_ms.c",609); printf("%s\n", egCb.init.prntBuf); }; } { sprintf (egCb.init.prntBuf, " Exiting\n"); printf("%s\n", egCb.init.prntBuf); }; } return(retVal); };
}
