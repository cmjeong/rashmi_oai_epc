...
static Void rgSCHRamUlFreeAllocation (RgSchUlSf *sf,RgSchUlAlloc *alloc, RgSchCellCb *cell)
                                ;

static S16 rgSCHRamContResCrnti (RgSchCellCb *cell, RgSchUeCb *ue, RgSchRaCb *raCb, RgSchErrInfo *err)
                                                                          ;
static S16 rgSCHRamContResCcchsdu (RgSchCellCb *cell, RgSchRaCb *raCb);
...
)
{
   ...
   raIndex = (timingInfo.sfn & 1) * 10 + raRnti-1;



   for (idx = 0; idx < raReqInd->numRaReqInfo; idx++)
   {
      if (raReqInd->raReqInfoArr[idx].rapId >= 64)
      {
      ...
      if (((raReqInd->raReqInfoArr[idx].rapId) >= (cell->rachCfg.numRaPreamble)))
      {
         rgNumDedPream++;
      }
      if((rgSCHUtlAllocSBuf(cell->instIdx, (Data **)(&raReqInfo),
                        sizeof(RgSchRaReqInfo))) == 1)
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xab0001,L_ERROR,DBG_CELLID, cell->cellId,raRnti, 0, 0, 0,                                               "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_ram.c"         ,                                               252         , "rgSCHRamProcRaReq(): Allocation" " of RaReq failed RARNTI:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                ;
       err->errCause = ((((((((((0 + 1) + 4) + 1) + 7) + 1) + 2) + 1) ) + 1) );
         return(1);
      }
      ...
      raReqInfo->raReqLstEnt.next = 0L;
      raReqInfo->raReqLstEnt.prev = 0L;
      raReqInfo->raReqLstEnt.node = (U32)raReqInfo;
      ...
      if (((raReqInd->raReqInfoArr[idx].rapId) >= (cell->rachCfg.numRaPreamble)))
      {
         ((&cell->raInfo.raReqLst[raIndex])->crnt = (&cell->raInfo.raReqLst[raIndex])->first);
...

{
   ...
   if((rgSCHUtlAllocSBuf(inst, (Data **)(raCb),
                      sizeof(RgSchRaCb))) == 1)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xab0002,L_ERROR,DBG_CELLID, cell->cellId, 0, 0, 0, 0,                      "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_ram.c"      ,                      324      , "rgSCHRamCreateRaCb(): Allocation of " "RaCb failed", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                       ;
      err->errCause = ((((((((((0 + 1) + 4) + 1) + 7) + 1) + 2) + 1) ) + 1) );
      return(1);
   }
   ...
   else
   {
      ...
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xab0003,L_ERROR,DBG_CELLID, cell->cellId, 0, 0, 0, 0,                                                        "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_ram.c"      ,                                                        343      , "rgSCHRamCreateRaCb(): Allocation of " "temporary RNTI failed at MAC(CRNTI exhausted)", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                         ;


      rgSCHUtlFreeSBuf(inst, (Data **)(raCb), sizeof(RgSchRaCb));
      err->errCause = ((((((((((0 + 1) + 4) + 1) + 7) + 1) + 2) + 1) ) + 1) + 2);
      return(1);
   }
   ...
   if ((*raCb)->dlHqE == 0L)
   {
      ...
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xab0004,L_ERROR,DBG_CELLID, cell->cellId, 0, 0, 0, 0,                          "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_ram.c"      ,                          357      , "rgSCHRamCreateRaCb(): Creation of" " DL HARQ failed", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                           ;
...

{
   ...
   {
      err->errCause = ((((((((((0 + 1) + 4) + 1) + 7) + 1) + 2) + 1) ) + 1) + 1);
      *hqEnt = 0L;
      raCb->dlHqE->ue = 0L;
      return(1);
   }
...

{
   ...
   {
      if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xab0005,L_DEBUG,DBG_CELLID, cell->cellId, raCb->rntiLnk->rnti, raCb->raState, 0, 0,                            "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_ram.c"      ,                            568      , "RNTI:%d RaCb in wrong State %d Drop Msg 3", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   if(cell->rachCfg.contResTmr - cellSch->dl.msg4TxDelay > 0)
      {
         ...
         if ((cell->crntTime.subframe + ((cell->rachCfg.contResTmr - cellSch->dl.msg4TxDelay))) >= 10) expTime.sfn = (cell->crntTime.sfn + (cell->crntTime.subframe + ((cell->rachCfg.contResTmr - cellSch->dl.msg4TxDelay)))/10) % 1024; else expTime.sfn = cell->crntTime.sfn; expTime.subframe = (cell->crntTime.subframe + ((cell->rachCfg.contResTmr - cellSch->dl.msg4TxDelay))) % 10;
                                                                 ;
      }
      else
      {

         if ((cell->crntTime.subframe + (1)) >= 10) expTime.sfn = (cell->crntTime.sfn + (cell->crntTime.subframe + (1))/10) % 1024; else expTime.sfn = cell->crntTime.sfn; expTime.subframe = (cell->crntTime.subframe + (1)) % 10;;
      }
      raCb->expiryTime = expTime;
      raCb->contResTmrLnk.node = (U32)(raCb);
...

{
   ...
   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xab0006,L_DEBUG,DBG_CELLID, cell->cellId, raCb->tmpCrnti, 0, 0, 0,                          "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_ram.c"   ,                          829   , "rgSCHRamMsg4Done(): tmpCRNTI = %u", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   if(raCb->ue != 0L)
   {
      ...
      rgSCHDhmRlsHqpTb(raCb->dlHqE->msg4Proc, 0, 1);
...

{
   ...
   if (raCb->schdLnk.node == (U32)raCb)
   {
      rgSCHRamRmvFrmRaInfoSchdLst(cell, raCb);
   }

   else if(raCb->contResTmrLnk.node != 0L)
   {
   ...
   if(rlsRnti == 1)
   {
      rgSCHUtlRlsRnti(cell, raCb->rntiLnk, 0, 0);
   }


   if (raCb->dlHqE )
   {
      if (raCb->dlHqE->msg4Proc != 0L)
      {


         if ((raCb->dlHqE->msg4Proc->subFrm != 0L) &&
             (raCb->dlHqE->msg4Proc->hqPSfLnk.node != 0L))
         {
            if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xab0007,L_ERROR,DBG_CELLID, cell->cellId, raCb->tmpCrnti, 0, 0, 0,                                                          "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_ram.c"            ,                                                          929            , "TMP CRNTI:%d RACH FAILURE!! " "msg4proc  removed from SF", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                           ;
            rgSCHUtlDlHqPTbRmvFrmTx(raCb->dlHqE->msg4Proc->subFrm,
               raCb->dlHqE->msg4Proc, 0, 0);
         }
   ...
   if (raCb->msg3HqProc.reTxLnk.node)
   {
      ...
      raCb->msg3HqProc.reTxLnk.node = (U32)0L;
   }
...

{
   ...
   winGap = (rgRaPrmblToRaFrmTbl[cell->rachCfg.preambleFormat]-1)+
      (cell->rachCfg.raWinSize -1 ) + 3;

   raIdx = (((crntSfn & 1) * 10 + cell->crntTime.subframe
            + (2 + 0) - winGap)+ (2*10) )
           % (2*10);
  ...
  {
            if ((raCb->dlHqE->msg4Proc == 0L) && (raCb->dlHqE->ue == 0L))
            {
               rgSCHRamDelRaCb(cell, raCb, 1);
            }
  }
...

{
   ...
   raCb->schdLnk.node = (U32)(raCb);

   cmLListDelFrm(&cell->contResGrdTmrLst, &(raCb->contResTmrLnk));
   raCb->contResTmrLnk.node = 0L;


   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xab0008,L_DEBUG,DBG_CELLID, cell->cellId, raCb->rntiLnk->rnti, 0, 0, 0,                               "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_ram.c"   ,                               1258   , "Con Res Grd Tmr exp RNTI:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
...

{
   ...
   chkLnk = ((&(cell->contResTmrLst))->crnt = (&(cell->contResTmrLst))->first);

   for (; chkLnk; chkLnk = chkLnk->next)
   {
      raCb = (RgSchRaCb *)(chkLnk->node);

      if((((raCb->expiryTime).sfn == (cell->crntTime).sfn) && ((raCb->expiryTime).subframe == (cell->crntTime).subframe)))
      {
...

{
   ...
   chkLnk = ((&(cell->contResGrdTmrLst))->crnt = (&(cell->contResGrdTmrLst))->first);


   for (; chkLnk; chkLnk = ((&cell->contResGrdTmrLst)->crnt = ((&cell->contResGrdTmrLst)->crnt ? (&cell->contResGrdTmrLst)->crnt->next : (&cell->contResGrdTmrLst)->first)))
   {
      raCb = (RgSchRaCb *)(chkLnk->node);
...

{
   ...
   if ((raCb->expiryTime.subframe + (cellSch->dl.msg4TxDelay)) >= 10) expTime.sfn = (raCb->expiryTime.sfn + (raCb->expiryTime.subframe + (cellSch->dl.msg4TxDelay))/10) % 1024; else expTime.sfn = raCb->expiryTime.sfn; expTime.subframe = (raCb->expiryTime.subframe + (cellSch->dl.msg4TxDelay)) % 10;;
   raCb->expiryTime = expTime;
   raCb->schdLnk.node = (U32)(raCb);
