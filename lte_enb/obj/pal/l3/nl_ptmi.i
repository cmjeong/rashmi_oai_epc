...
extern S16 nlSendSonMeasRptIntervalInd ( U32 ueMeasInterval, NlCellCb *cellCb )
  ...
  ;
static LnlCfgCfm nlMiLnlCfgCfmMt[3] =
{
...
static LnlCntrlCfm nlMiLnlCntrlCfmMt[3] =
{
...
static LnlCfgUpdateInd nlMiLnlCfgUpdateIndMt[3] =
{
...
static LnlStatInd nlMiLnlAlarmIndMt[3] =
{
...
static LnlSonCfgInd nlMiLnlBrdcmSonCfgIndMt[3] =
{
...
{
   NlCfgUpdateInd *cfgUpdateInd = 0L;
   ;;
   { if (0 == SGetSBuf(nlCb.init.region, nlCb.init.pool, (Data **)(&cfgUpdateInd), (sizeof(NlCfgUpdateInd))) ) { cmMemset((U8 *)(*(&cfgUpdateInd)), 0, (sizeof(NlCfgUpdateInd))); } else { (*(&cfgUpdateInd)) = 0L; } };
   if(0L == cfgUpdateInd)
   {
...
{
   NlCfgUpdateInd *cfgUpdate = 0L;
   ;;
   { if (0 == SGetSBuf(nlCb.init.region, nlCb.init.pool, (Data **)(&cfgUpdate), (sizeof(NlCfgUpdateInd))) ) { cmMemset((U8 *)(*(&cfgUpdate)), 0, (sizeof(NlCfgUpdateInd))); } else { (*(&cfgUpdate)) = 0L; } };
   if(0L == cfgUpdate)
   {
   ...
   cfgUpdate->u.pciCfgInd.cellCfgIdx = cellCb->cellId;
...

{
   ...
   if( L_INFO < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xfd0001, L_INFO, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_ptmi.c",393, "sending BRDCM selected Tx-power indication to SM", RLOG_FILE_ID, RLOG_MODULE_NAME); };
   ;;
   { if (0 == SGetSBuf(nlCb.init.region, nlCb.init.pool, (Data **)(&cfgUpdate), (sizeof(NlCfgUpdateInd))) ) { cmMemset((U8 *)(*(&cfgUpdate)), 0, (sizeof(NlCfgUpdateInd))); } else { (*(&cfgUpdate)) = 0L; } };
   if(0L == cfgUpdate)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xfd0002, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_ptmi.c",398, "Failed to allocate memory for update Tw Power config", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
...

{
   ...
   { if (0 == SGetSBuf(nlCb.init.region, nlCb.init.pool, (Data **)(&nlMngmt), (sizeof(NlMngmt))) ) { cmMemset((U8 *)(*(&nlMngmt)), 0, (sizeof(NlMngmt))); } else { (*(&nlMngmt)) = 0L; } };
   if(0L == nlMngmt)
   {
   ...
   usta->u.stAlarm.alarm.category = 1;
   usta->u.stAlarm.alarm.event = 0;
   usta->u.stAlarm.alarm.cause = 0;
   dgn = &(usta->u.stAlarm.dgn);
   cmMemcpy((U8 *)dgn,(U8 *)alarm, sizeof(LnlUstaDgn));

   nlCb.init.lmPst.event = 6;

   NlMiLnlAlarmInd(&nlCb.init.lmPst, nlMngmt);
   return(0);
}
 S16 nlSendPrachParamUpdateInd
(
NlCellCb *cellCb
)
...

{
   ...
   if(0L == cfgUpdate)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xfd0003, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_ptmi.c",542, "Failed to allocate memory for update PCI config", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
...
)
{
   ...
   { if (0 == SGetSBuf(nlCb.init.region, nlCb.init.pool, (Data **)(&sonCfg), (sizeof(LnlSonCfg))) ) { cmMemset((U8 *)(*(&sonCfg)), 0, (sizeof(LnlSonCfg))); } else { (*(&sonCfg)) = 0L; } };
   if(0L == sonCfg)
   {
...

{
   ...
   if(0L == sonCfg)
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xfd0004, L_ERROR, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteson/src/nl_ptmi.c",652, "Failed to allocate memory for BRDCM-SON SCHD CFG", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(1);
   }
   cmMemcpy((U8 *) &(sonCfg->ecgi),(U8 *)&(cellCb->cellInfo.ecgi),
                   sizeof(LnlEcgi));
 ...
 S16 nlSendSonMeasRptIntervalInd
(
U32 ueMeasInterval,
NlCellCb *cellCb
)
