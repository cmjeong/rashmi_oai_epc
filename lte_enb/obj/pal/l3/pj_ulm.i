...

{
   ...
   if(secInfo->secAct == 1 && secInfo->cipherInfo.algoType != 0)
   {
     ...
     return(0);
         ...
         if(mBuf == 0L)
         {
            if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xf50007, L_ERROR, rxEnt->count, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ulm.c",1088, "Deciphering Req failed Rx Count [%lu]", RLOG_FILE_ID, RLOG_MODULE_NAME); };
            { CmLtePdcpId _pdcpId; Buffer *_mBuf = 0L; PjuStaIndInfo _staInd; PjPjuSapCb *_pjuSap; if (pjRbCb->rbType == 0) _pjuSap = &(gCb->u.ulCb->pjuSap[0]); else _pjuSap = &(gCb->u.ulCb->pjuSap[1]); _pdcpId.ueId = pjRbCb->ueCb->key.ueId; _pdcpId.cellId = pjRbCb->ueCb->key.cellId; _pdcpId.rbId = pjRbCb->rbId; _pdcpId.rbType = pjRbCb->rbType; switch (rxEnt->state) { case 1: case 2: { _staInd.cause = 2; break; } case 3: case 4: { _staInd.cause = 1; _mBuf = rxEnt->mBuf; rxEnt->mBuf = 0L; break; } case 5: case 6: { _staInd.cause = 3; if(pjRbCb->ulCb.discReqd == 0) { _mBuf = rxEnt->mBuf; rxEnt->mBuf = 0L; } break; } default: break; } PjUiPjuStaInd(&(_pjuSap->pst), _pjuSap->suId, &_pdcpId, &_staInd, _mBuf); };
            pjDbmDelRxEnt(gCb, &ulCb->recBuf, rxEnt->count);
            { gCb->pjGenSts.numDeciphFails++; };
            return(1);
...

{
   ...
   if(pjRbCb->ueCb->secInfo.secAct == 1 && pjRbCb->ueCb->secInfo.intInfo.algoType != 0)
   {
      ...
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xf50008, L_ERROR, rxEnt->count, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ulm.c",1201, "Integrity Verification Req failed Rx Count[%lu]", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      ...
      if(status != 0)
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xf50009, L_ERROR, rxEnt->count, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ulm.c",1233, "Integrity Verification Req failed Rx Count[%lu]", RLOG_FILE_ID, RLOG_MODULE_NAME); };
         { CmLtePdcpId _pdcpId; Buffer *_mBuf = 0L; PjuStaIndInfo _staInd; PjPjuSapCb *_pjuSap; if (pjRbCb->rbType == 0) _pjuSap = &(gCb->u.ulCb->pjuSap[0]); else _pjuSap = &(gCb->u.ulCb->pjuSap[1]); _pdcpId.ueId = pjRbCb->ueCb->key.ueId; _pdcpId.cellId = pjRbCb->ueCb->key.cellId; _pdcpId.rbId = pjRbCb->rbId; _pdcpId.rbType = pjRbCb->rbType; switch (rxEnt->state) { case 1: case 2: { _staInd.cause = 2; break; } case 3: case 4: { _staInd.cause = 1; _mBuf = rxEnt->mBuf; rxEnt->mBuf = 0L; break; } case 5: case 6: { _staInd.cause = 3; if(pjRbCb->ulCb.discReqd == 0) { _mBuf = rxEnt->mBuf; rxEnt->mBuf = 0L; } break; } default: break; } PjUiPjuStaInd(&(_pjuSap->pst), _pjuSap->suId, &_pdcpId, &_staInd, _mBuf); };
         pjDbmDelRxEnt(gCb, &ulCb->recBuf, rxEnt->count);
         { gCb->pjGenSts.numIntgVrfFails++; };
         return(1);
...

{
   ...
   if(pjRbCb->rohc.hdrCmpUsed == 1)
   {
      ...
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xf5000a, L_ERROR, rxEnt->count, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ulm.c",1300, "DeCompression Req failed Rx Count[%lu]", RLOG_FILE_ID, RLOG_MODULE_NAME); };
         { CmLtePdcpId _pdcpId; Buffer *_mBuf = 0L; PjuStaIndInfo _staInd; PjPjuSapCb *_pjuSap; if (pjRbCb->rbType == 0) _pjuSap = &(gCb->u.ulCb->pjuSap[0]); else _pjuSap = &(gCb->u.ulCb->pjuSap[1]); _pdcpId.ueId = pjRbCb->ueCb->key.ueId; _pdcpId.cellId = pjRbCb->ueCb->key.cellId; _pdcpId.rbId = pjRbCb->rbId; _pdcpId.rbType = pjRbCb->rbType; switch (rxEnt->state) { case 1: case 2: { _staInd.cause = 2; break; } case 3: case 4: { _staInd.cause = 1; _mBuf = rxEnt->mBuf; rxEnt->mBuf = 0L; break; } case 5: case 6: { _staInd.cause = 3; if(pjRbCb->ulCb.discReqd == 0) { _mBuf = rxEnt->mBuf; rxEnt->mBuf = 0L; } break; } default: break; } PjUiPjuStaInd(&(_pjuSap->pst), _pjuSap->suId, &_pdcpId, &_staInd, _mBuf); };
         pjDbmDelRxEnt(gCb, &ulCb->recBuf, rxEnt->count);
      ...
      if(mBuf == 0L)
      {
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xf5000b, L_ERROR, rxEnt->count, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ulm.c",1316, "DeCompression Req failed Rx Count[%lu]", RLOG_FILE_ID, RLOG_MODULE_NAME); };
         { CmLtePdcpId _pdcpId; Buffer *_mBuf = 0L; PjuStaIndInfo _staInd; PjPjuSapCb *_pjuSap; if (pjRbCb->rbType == 0) _pjuSap = &(gCb->u.ulCb->pjuSap[0]); else _pjuSap = &(gCb->u.ulCb->pjuSap[1]); _pdcpId.ueId = pjRbCb->ueCb->key.ueId; _pdcpId.cellId = pjRbCb->ueCb->key.cellId; _pdcpId.rbId = pjRbCb->rbId; _pdcpId.rbType = pjRbCb->rbType; switch (rxEnt->state) { case 1: case 2: { _staInd.cause = 2; break; } case 3: case 4: { _staInd.cause = 1; _mBuf = rxEnt->mBuf; rxEnt->mBuf = 0L; break; } case 5: case 6: { _staInd.cause = 3; if(pjRbCb->ulCb.discReqd == 0) { _mBuf = rxEnt->mBuf; rxEnt->mBuf = 0L; } break; } default: break; } PjUiPjuStaInd(&(_pjuSap->pst), _pjuSap->suId, &_pdcpId, &_staInd, _mBuf); };
         pjDbmDelRxEnt(gCb, &ulCb->recBuf, rxEnt->count);
         { gCb->pjGenSts.numDecmpFails++; };
         return(1);
   ...
   if((rxEnt->discFlag == 1) ||
      (rxEnt->dupEntry == 1))
   {
      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev2(0xf5000c, L_ERROR, rxEnt->discFlag, rxEnt->dupEntry, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ulm.c",1333, "Dropping packets...discflag is [%d], dupEntry is [%d]", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      return(0);
   }
...

{
   ...
   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev2(0xf5000d, L_DEBUG,pjRbCb->rbId, pjRbCb->rbType, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ulm.c",1802, "pjUlmReEstDrbAm(pjRbCb(%d,%d))", RLOG_FILE_ID, RLOG_MODULE_NAME); };
   ...
   if(pjRbCb->state == 3)
   {
      ...
      if (numBits > 0)
      {
         if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xf5000e,L_DEBUG,DBG_UEID, pjRbCb->ueCb->key.ueId, pjRbCb->rbId, 0, 0, 0,                           "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ulm.c"         ,                           1893         , "pjUlmReEstDrbAm : OutOfSeq UL Pkts are present for rbId = %d ", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   else
   {
      ...
      if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xf5000f,L_DEBUG,DBG_UEID, pjRbCb->ueCb->key.ueId, 0, 0, 0, 0,                                                           "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ulm.c"      ,                                                           1912      , "pjUlmReEstDrbAm : Sending PDCP Status Report ", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                            ;
...
)
{
   ...
   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev2(0xf50010, L_DEBUG,pjRbCb->rbId, pjRbCb->rbType, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ulm.c",1964, "pjUlmBldStaRep(pjRbCb(%d,%d))", RLOG_FILE_ID, RLOG_MODULE_NAME); };
...

{
   ...
   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev2(0xf50011, L_DEBUG, pjRbCb->rbId, pjRbCb->rbType, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ulm.c"   , 2056   , "pjUlmHdlDatFwdReq(pjRbCb(%d,%d), datFwdReq) ", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                      ;
...

{
   ...
   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev3(0xf50012, L_DEBUG, pjRbCb->rbId, pjRbCb->rbType, rxEnt->count, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ulm.c"   , 2147   , "pjUlmSubmitForDecmp(pjRbCb(%d,%d),rxEnt(%ld)) ", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                    ;
...

{
   ...
   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xf50013, L_DEBUG, pjRbCb->rbId, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ulm.c",2628, "pjUlmStartDataFrwdPerRb (%d )) ", RLOG_FILE_ID, RLOG_MODULE_NAME); };
...

{
   ...
   if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev2(0xf50014, L_DEBUG, pjRbCb->rbId, datFwdReq->numSdus, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ulm.c"   , 2800   , "pjDlmHndlDatFwdReq(pjRbCb (%d),datFwdReq(%d)) ", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                       ;
...

{
   ...
   {
      if( L_FATAL < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev0(0xf50015, L_FATAL, "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltepdcp/pj_ulm.c",2989, "Memory allocation failure", RLOG_FILE_ID, RLOG_MODULE_NAME); };
      { if (pdu != 0L) { SPutMsg(pdu); } pdu = 0L; };
   }
