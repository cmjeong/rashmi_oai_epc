...
static S16 szSendLmCfm ( Pst *pst, Header *hdr, U8 type, SzMngmt *cfm )




        ;
static S16 szGenCfg ( SzGenCfg *gen, CmStatus *status )


        ;
static S16 szValidateGenCfg ( SzGenCfg *gen )

        ;
static S16 szProtCfg ( SzProtCfg *prot, CmStatus *status )


        ;
static S16 szValidateProtCfg ( SzProtCfg *prot )

        ;
static S16 szSztSapCfg ( SzSztSapCfg *sztSap, CmStatus *status )


        ;
static S16 szValidateSztSapCfg ( SzSztSapCfg *sztSap )

        ;
static S16 szSctSapCfg ( SzSctSapCfg *sctSap, CmStatus *status )


        ;
static S16 szValidateSctSapCfg ( SzSctSapCfg *sctSap )

        ;
static S16 szPeerCfg ( SzPeerCfg *peer, CmStatus *status, Header *hdr )



        ;
static S16 szValidatePeerCfg ( SzPeerCfg *peer )

        ;
static S16 szGenCntrl ( SzCntrl *cntrl, CmStatus *status )


        ;
static S16 szSztSapCntrl ( SzCntrl *cntrl, CmStatus *status )


        ;
static S16 szValidateSztSapCntrl ( SzCntrl *cntrl )

        ;


static S16 szSctSapCntrl ( SzCntrl *cntrl, CmStatus *status )


        ;
static S16 szValidateSctSapCntrl ( SzCntrl *cntrl )

        ;
static S16 szEnaSctSap ( SpId lSapId )

        ;
static S16 szUbndSctSap ( SpId lSapId, U8 opn, U8 contEnt )
...
static S16 szPeerCntrl ( SzCntrl *cntrl, CmStatus *status )


        ;
static S16 szValidatePeerCntrl ( SzCntrl *cntrl )

        ;
static S16 szDelPeer ( SzPeerCb *peerCb )

        ;
static S16 szGetGenSts ( SzGenSts *sts, CmStatus *status, Action action )



        ;
static S16 szValidateGenSts ( SzGenSts *sts, Action action )


        ;
static S16 szGetPeerNodeSts ( SzPeerNodeSts *sts, CmStatus *status, Action action )



        ;
static S16 szValidatePNodeSts ( SzPeerNodeSts *sts, Action action )


        ;
static S16 szGetSctSapSts ( SzSctSapSts *sts, CmStatus *status, Action action )



        ;
static S16 szValidateSctSapSts ( SzSctSapSts *sts, Action action )


        ;
static S16 szGetGenSta ( SzGenSta *sta, CmStatus *status )


        ;
static S16 szGetSztSapSta ( SzSapSta *sta, CmStatus *status )


        ;
static S16 szValidateSztSapSta ( SzSapSta *sta )

        ;
static S16 szGetSctSapSta ( SzSapSta *sta, CmStatus *status )


        ;
static S16 szValidateSctSapSta ( SzSapSta *sta )

        ;
static S16 szGetPeerSta ( SzPeerSta *sta, CmStatus *status )


        ;
static S16 szValidatePeerSta ( SzPeerSta *sta )
...

{
   ...
   { if ((&szCb.init)->dbgMask & (0x00000002)) { snprintf((&szCb.init)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", szCb.nodeDbg, (&szCb.init)->procId, (&szCb.init)->ent, (&szCb.init)->inst,
                                                                "/home/mrehwald/Desktop/Radisys/lte_enb/src/s1ap/sz_mi.c"
   ,
                                                                353
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "SzMiLszCfgReq (pst, elmId (%d))\n", cfg->hdr.elmId.elmnt); SPrint((&szCb.init)->prntBuf); } }
                                                                 ;


   if ((!szCb.init.cfgDone) && (cfg->hdr.elmId.elmnt != 1))
   {
      ...
      {
         if(cfg->u.cfg.u.peer.peerInfo != 0L)
         {
            ...
            cfg->u.cfg.u.peer.peerInfo = 0L;
         }
      }
      cfgCfm.cfm.status = 1;
      cfgCfm.cfm.reason = 8;
      szSendLmCfm (pst, &cfg->hdr, 1, &cfgCfm);
...

{
   ...
   ,
                                                                    488
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "SzMiLszCntrlReq (pst, elmId (%d))\n", cntrl->hdr.elmId.elmnt); SPrint((&szCb.init)->prntBuf); } }
                                                                     ;
   ...
   szSendLmCfm(pst, &cntrl->hdr, 2, &cntrlCfm);
...

{
   ...
   ,
                                        ...
                                        624
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "SzMiLszStsReq (pst, action(%d), elmId (%d)) \n", action, sts->hdr.elmId.elmnt); SPrint((&szCb.init)->prntBuf); } }
   ...
   szSendLmCfm(pst, &sts->hdr, 3, &stsCfm);
...

{
   ...
   ,
                                                                     763
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "SzMiLszStaReq (pst, elmId (%d)) \n", sta->hdr.elmId.elmnt); SPrint((&szCb.init)->prntBuf); } }
                                                                      ;
   ...
   szSendLmCfm(pst, &sta->hdr, 4, &staCfm);
...

{
   ...
   cmMemcpy((U8 *)&usta.u.usta.dgn, (U8 *)dgn, (U32)sizeof(SzUstaDgn));
...

{
   ...
   if (mBuf != 0L)
   {
      ...
      if((ret = SCpyMsgMsg(mBuf, szCb.init.lmPst.region,
                  szCb.init.lmPst.pool, &trcBuf)) != 0)
      {




         { if (trcBuf != 0L) { SPutMsg (trcBuf); trcBuf = 0L; } };
         return(1);
      }
      ...
      if (trcLen > 0 && trcLen < ml)
      {
         ...
         if((ret = SSegMsg(trcBuf, ml, &tmpBuf)) != 0)
         {
            ...
            { if (trcBuf != 0L) { SPutMsg (trcBuf); trcBuf = 0L; } };
            { if (tmpBuf != 0L) { SPutMsg (tmpBuf); tmpBuf = 0L; } };
            return(1);
         }
...

{
   ...
   switch (type)
   {
      case 1:
         ...
         ,
                                                                        1088
         ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "szSendLmCfm: sent SzMiLszCfgCfm (cfmType: %d) \n", type); SPrint((&szCb.init)->prntBuf); } }
                                                                         ;
      ...
      case 2:
         ...
         ,
                                                                          1095
         ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "szSendLmCfm: sent SzMiLszCntrlCfm (cfmType: %d) \n", type); SPrint((&szCb.init)->prntBuf); } }
                                                                           ;
      ...
      case 3:
         ...
         ,
                                                                        1102
         ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "szSendLmCfm: sent SzMiLszStsCfm (cfmType: %d) \n", type); SPrint((&szCb.init)->prntBuf); } }
                                                                         ;
      ...
      case 4:
         ...
         ,
                                                                        1109
         ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "szSendLmCfm: sent SzMiLszStaCfm (cfmType: %d) \n", type); SPrint((&szCb.init)->prntBuf); } }
                                                                         ;
...

{
   ...
   { SGetSBuf(szCb.init.region, szCb.init.pool, (Data**)&szCb.sztSapLst, (gen->nmbSztSaps * sizeof(SzSztSapCb *))); if (szCb.sztSapLst != 0L) { cmMemset((U8*)szCb.sztSapLst, 0, (gen->nmbSztSaps * sizeof(SzSztSapCb *))); szCb.genSta.memAlloc += ((gen->nmbSztSaps * sizeof(SzSztSapCb *))); } else { SzUstaDgn _dgn; szCb.sztSapLst = 0L; _dgn.type = 2; _dgn.u.mem.region = szCb.init.region; _dgn.u.mem.pool = szCb.init.pool; szSendLmAlarm (4, 12, 15, &_dgn); szCb.genSts.nmbFailConn++; } };
   if ((SzSztSapCb *)szCb.sztSapLst == (SzSztSapCb *)0L)
   {
   ...
   { SGetSBuf(szCb.init.region, szCb.init.pool, (Data**)&szCb.sctSapLst, (gen->nmbSctSaps * sizeof(SzSctSapCb *))); if (szCb.sctSapLst != 0L) { cmMemset((U8*)szCb.sctSapLst, 0, (gen->nmbSctSaps * sizeof(SzSctSapCb *))); szCb.genSta.memAlloc += ((gen->nmbSctSaps * sizeof(SzSctSapCb *))); } else { SzUstaDgn _dgn; szCb.sctSapLst = 0L; _dgn.type = 2; _dgn.u.mem.region = szCb.init.region; _dgn.u.mem.pool = szCb.init.pool; szSendLmAlarm (4, 12, 15, &_dgn); szCb.genSts.nmbFailConn++; } };
   if ((SzSctSapCb *)szCb.sctSapLst == (SzSctSapCb *)0L)
   {
      { if (szCb.sztSapLst != 0L) { (Void)SPutSBuf(szCb.init.region, szCb.init.pool, (Data*)szCb.sztSapLst, (gen->nmbSztSaps * sizeof(SzSztSapCb *))); szCb.genSta.memAlloc -= ((gen->nmbSztSaps * sizeof(SzSztSapCb *))); szCb.sztSapLst = 0L; } };
   ...
   if(ret == 0)
   {
      offset = (U16)((U32)&conCb.lclConHlEnt - (U32)&conCb);

      ret = cmHashListInit(&szCb.lclConCp,
            (U16)szCb.genCfg.nmbConn,
            offset,
            0,
            4,
            szCb.init.region,
...

{
   ...
   szCb.nodeCb.cfgDone = 1;

   switch(szCb.nodeCb.nodeType)
   {

      case 2:
         cmMemcpy(szCb.nodeDbg, (const U8*)"S1AP: eNB", 10);
         break;


      case 3:
         cmMemcpy(szCb.nodeDbg, (const U8*)"S1AP: GW ", 10);
         break;

      default:
         cmMemcpy(szCb.nodeDbg, (const U8*)"S1AP:    ", 10);
         break;
   }
...

{
   ...
   if ((prot->nodeType != 2) && (prot->nodeType != 1) &&
       (prot->nodeType != 3))
   {
      return(13);
   }
...

{
   ...
   { SGetSBuf(szCb.init.region, szCb.init.pool, (Data**)&sztSapCb, sizeof(SzSztSapCb)); if (sztSapCb != 0L) { cmMemset((U8*)sztSapCb, 0, sizeof(SzSztSapCb)); szCb.genSta.memAlloc += (sizeof(SzSztSapCb)); } else { SzUstaDgn _dgn; sztSapCb = 0L; _dgn.type = 2; _dgn.u.mem.region = szCb.init.region; _dgn.u.mem.pool = szCb.init.pool; szSendLmAlarm (4, 12, 15, &_dgn); szCb.genSts.nmbFailConn++; } };
   if (sztSapCb == 0L)
   {
      status->status = 1;
      status->reason = 4;
   ...
   offset = (U16)((U32)&conCb.suConHlEnt - (U32)&conCb);
   ret = cmHashListInit(&sztSapCb->suConnLst,
            (U16)szCb.genCfg.nmbConn,
            offset,
            0,
            4,
...
static S16 szValidateSztSapCfg
(
SzSztSapCfg *sztSap
)
...

{
   ...
   { SGetSBuf(szCb.init.region, szCb.init.pool, (Data**)&sctSapCb, sizeof(SzSctSapCb)); if (sctSapCb != 0L) { cmMemset((U8*)sctSapCb, 0, sizeof(SzSctSapCb)); szCb.genSta.memAlloc += (sizeof(SzSctSapCb)); } else { SzUstaDgn _dgn; sctSapCb = 0L; _dgn.type = 2; _dgn.u.mem.region = szCb.init.region; _dgn.u.mem.pool = szCb.init.pool; szSendLmAlarm (4, 12, 15, &_dgn); szCb.genSts.nmbFailConn++; } };
   if (sctSapCb == 0L)
   {
      status->status = 1;
      status->reason = 4;
   ...
   sctSapCb->endpCfgDone = 0;
   sctSapCb->state = 1;


   if (sctSapCb->endpCfgDone != 1)
   {
      ...
      sctSapCb->endpCb.epState = 1;
      sctSapCb->endpCfgDone = 1;
   }
...
static S16 szValidateSctSapCfg
(
SzSctSapCfg *sctSap
)
...

{
   ...
   if (ret != 0)
   {
      if(peer->nmbPeers)
      {
         { if(peer->peerInfo != 0L) { (Void)SPutSBuf(hdr->response.mem.region, hdr->response.mem.pool, (Data*)peer->peerInfo, (sizeof(SzPeerInfo) * peer->nmbPeers)); } }
                                                                      ;
      }
      status->status = 1;
      status->reason = ret;
   ...
   for (loopCnt = 0; loopCnt < peer->nmbPeers;loopCnt++)
   {
      ...
      if ((ret = szNdbAllocPeerCb(peer->peerInfo[loopCnt].peerId.val, &peerCb)) != 0)
      {
...

{
   ...
   for (loopCnt = 0; loopCnt < peer->nmbPeers; loopCnt++)
   {
      ...
      if ( (szCb.sctSapLst[peer->peerInfo[loopCnt].sapId] == 0L) ||
           (szCb.sztSapLst[peer->peerInfo[loopCnt].uSapId] == 0L) )
      {
         return((256 + 5));
      }

      if ( ((peer->peerInfo[loopCnt].type != 2) &&
           (peer->peerInfo[loopCnt].type != 1 )) ||
      ...
      if((peer->peerInfo[loopCnt].skipUeSm != 1) &&
         (peer->peerInfo[loopCnt].skipUeSm != 0))
      {
         return(13);
      }
...

{
   ...
   if (szCb.shutdown)
   {
      ...
      ,
                                                         2332
      ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "szShutdown: Layer already SHUT DOWN) \n"); SPrint((&szCb.init)->prntBuf); } }
                                                          ;
   ...
   SDeregCfgTmr(szCb.init.ent,szCb.init.inst,(S16)szCb.genCfg.timeRes,100,(PFS16)szActvTmr)
                          ;
   ...
   {
      if (szCb.sctSapLst[loopCnt] != (SzSctSapCb *)0L)
      {

         szDelSctSap(szCb.sctSapLst[loopCnt]->suId,3);
      }
   }
   ...
   {
      if (szCb.sztSapLst[loopCnt] != (SzSztSapCb *)0L)
      {
         szDelSztSap(szCb.sztSapLst[loopCnt]->spId, 3);
      }
   }

   if (szCb.sztSapLst != (SzSztSapCb **)0L)
   {
      { if (szCb.sztSapLst != 0L) { (Void)SPutSBuf(szCb.init.region, szCb.init.pool, (Data*)szCb.sztSapLst, (szCb.genCfg.nmbSztSaps * sizeof(SzSztSapCb *))); szCb.genSta.memAlloc -= ((szCb.genCfg.nmbSztSaps * sizeof(SzSztSapCb *))); szCb.sztSapLst = 0L; } };
   }

   if (szCb.sctSapLst != (SzSctSapCb **)0L)
   {
      { if (szCb.sctSapLst != 0L) { (Void)SPutSBuf(szCb.init.region, szCb.init.pool, (Data*)szCb.sctSapLst, (szCb.genCfg.nmbSctSaps * sizeof(SzSctSapCb *))); szCb.genSta.memAlloc -= ((szCb.genCfg.nmbSctSaps * sizeof(SzSctSapCb *))); szCb.sctSapLst = 0L; } };
   }
...

{
   ...
   switch (cntrl->action)
   {
      case 31:
      {
         if ((ret = szUbndSztSap (cntrl->u.sap.id, 2)) != 0)
         {
      ...
      case 7:
      {
         if ((ret = szDelSztSap (cntrl->u.sap.id,1)) != 0)
         {
...
static S16 szValidateSztSapCntrl
(
SzCntrl *cntrl
)
...

{
   ...
   ret = szNdbDeallocAllPeerCb(sapId,1, opn);
...

{
   ...
   while (cmHashListGetNext(&szCb.peerLst, (U32)prev,
            (U32 *)&peer) == 0)
   {
      if((peer->sztSapCb != 0L) &&
   ...
   { if (szCb.sztSapLst[sapId] != 0L) { (Void)SPutSBuf(szCb.init.region, szCb.init.pool, (Data*)szCb.sztSapLst[sapId], sizeof(SzSztSapCb)); szCb.genSta.memAlloc -= (sizeof(SzSztSapCb)); szCb.sztSapLst[sapId] = 0L; } };


   return(ret);

}
static S16 szSctSapCntrl
(
SzCntrl *cntrl,
CmStatus *status
)
...

{
   ...
   switch (cntrl->action)
   {
      ...
      case 21:
      {
         if ((ret = szEnaSctSap (cntrl->u.sap.id)) != 0)
         {
      ...
      case 31:
      {
         if ((ret = szUbndSctSap (cntrl->u.sap.id,2, 0x0E)) != 0)
         {
      ...
      case 7:
      {
         if ((ret = szDelSctSap (cntrl->u.sap.id, 1)) != 0)
         {
...
static S16 szValidateSctSapCntrl
(
SzCntrl *cntrl
)
...

{
       ...
       sctSapCb->state == 3)
   {
      ...
      ,
                                                                                  2891
      ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "szEnaSctSap: SAP is already bound / waiting for bind confirm \n"); SPrint((&szCb.init)->prntBuf); } }
                                                                                   ;
   ...
   szStartTmr(sctSapCb->timers, (U32)sctSapCb, 12, &sctSapCb->tBndTmr);
...

{
       ...
       sctSapCb->state == 1)
   {
     ...
     ,
                                                       2950
     ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "szUbndSctSap: SAP already unbound \n"); SPrint((&szCb.init)->prntBuf); } }
                                                        ;
   ...
   ret = szNdbDeallocAllPeerCb(lSapId,2, opn);



   if (sctSapCb->endpCb.epState == 3)
   {
...

{
   ...
   if(szCb.sctSapLst[lSapId]->state == 2)
   {
      ret = szUbndSctSap (lSapId,opn,0x0E);
   }
   { if (szCb.sctSapLst[lSapId] != 0L) { (Void)SPutSBuf(szCb.init.region, szCb.init.pool, (Data*)szCb.sctSapLst[lSapId], sizeof (SzSctSapCb)); szCb.genSta.memAlloc -= (sizeof (SzSctSapCb)); szCb.sctSapLst[lSapId] = 0L; } };

   return(ret);

}
static S16 szPeerCntrl
(
SzCntrl *cntrl,
CmStatus *status
)
        ...
        if (cntrl->u.peer.nmbPeers > 0)
         {
            ...
            { if (cntrl->u.peer.peerId != 0L) { (Void)SPutSBuf(szCb.init.region, szCb.init.pool, (Data*)cntrl->u.peer.peerId, (sizeof(TknU32) * cntrl->u.peer.nmbPeers)); szCb.genSta.memAlloc -= ((sizeof(TknU32) * cntrl->u.peer.nmbPeers)); cntrl->u.peer.peerId = 0L; } };
            cntrl->u.peer.peerId = 0L;
         }
         break;
      }
      default:
...
static S16 szValidatePeerCntrl
(
SzCntrl *cntrl
)
...

{
   ...
   return(szLiTermAssoc(&peerCb, 0));



}
static S16 szGetGenSts
(
SzGenSts *sts,
CmStatus *status,
Action action
...

{
   ...
   if ((ret = szNdbChkPeer(sts->peerId, &peerCb)) != 0)
   {
...
static S16 szValidateSctSapSts
(
SzSctSapSts *sts,
Action action
)
...
static S16 szValidateSztSapSta
(
SzSapSta *sta
)
...

{
   ...
   {
      if (szCb.sztSapLst[sta->sapId] == 0L)
      {
         ret = 13;
      }
   }
...
static S16 szValidateSctSapSta
(
SzSapSta *sta
)
...

{
   ...
   {
      if (szCb.sctSapLst[sta->sapId] == 0L)
      {
         ret = 13;
      }
   }
...

{
   ...
   for (loopCnt = 0; loopCnt < sta->nmbPeer; loopCnt++)
   {
      ...
      if (peerCb != 0L)
      {
         ...
         if (peerCb->assocCb->assocState == 1)
         {
            sta->peerStaInfo[loopCnt].connected = 1;
         }
         else
         {
            sta->peerStaInfo[loopCnt].connected = 0;
         }
