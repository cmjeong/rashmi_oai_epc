...

{
   ...
   if(servCell->cellInfo.alarmInfo.isAlarmRaised)
   {
      for(count = 0; count < 5; count ++)
      {
         ...
         {
            servCell->cellInfo.alarmInfo.peerInfo[count].isActive = 0;
                  ...
                  cellInfo.alarmInfo.peerInfo[count].peerType)
            {
               ...
               if(0L != nlCellCb)
               {
                  if(1 == nlCellCb->cellInfo.alarmInfo.isAlarmRaised)
                  {
                     for(nghCount = 0; nghCount < 5; nghCount ++)
                     {
                        if(((NlCellCb *)nlCellCb->
                        cellInfo.alarmInfo.peerInfo[nghCount].peerAddr == servCell) &&
                        (1 == nlCellCb->cellInfo.alarmInfo.peerInfo[nghCount].isActive) &&
                        (NL_ALARM_PEER_SERV_CELL ==
                            nlCellCb->cellInfo.alarmInfo.peerInfo[nghCount].peerType))
                        {
                           nlCellCb->cellInfo.alarmInfo.peerInfo[nghCount].peerAddr = 0L;
                           nlCellCb->cellInfo.alarmInfo.peerInfo[nghCount].isActive = 0;
                           nlCellCb->cellInfo.alarmInfo.numPeerAlarm --;
                           if(0 == nlCellCb->cellInfo.alarmInfo.numPeerAlarm)
                           {
                              nlCellCb->cellInfo.alarmInfo.isAlarmRaised = 0;

                              { nlCellCb->state = NL_CELL_STATE_CONFIGURED; };
                           }
                        }
                     }
                  }
                             ...
                             cellInfo.alarmInfo.peerInfo[count].peerType)
            {
               ...
               if(0L != nghServCb)
               {
                  if(1 == nghServCb->servCellInfo.alarmInfo.isAlarmRaised)
                  {
                     for(nghCount = 0; nghCount < 5; nghCount ++)
                     {
                        if(((NlCellCb *)nghServCb->
                        servCellInfo.alarmInfo.peerInfo[nghCount].peerAddr == servCell) &&
                        (1 == nghServCb->servCellInfo.alarmInfo.peerInfo[nghCount].isActive) &&
                        (NL_ALARM_PEER_SERV_CELL ==
                            nghServCb->servCellInfo.alarmInfo.peerInfo[nghCount].peerType))
                        {
                           nghServCb->servCellInfo.alarmInfo.peerInfo[nghCount].peerAddr = 0L;
                           nghServCb->servCellInfo.alarmInfo.peerInfo[nghCount].isActive = 0;
                           nghServCb->servCellInfo.alarmInfo.numPeerAlarm --;
                           if(0 == nghServCb->servCellInfo.alarmInfo.numPeerAlarm)
                           {
                              nghServCb->servCellInfo.alarmInfo.isAlarmRaised = 0;
                           }
                        }
                     }
                  }
                          ...
                          cellInfo.alarmInfo.peerInfo[count].peerType)
            {
               ...
               if(0L != nghNghCb)
               {
                  if(1 == nghNghCb->nghNghCellInfo.alarmInfo.isAlarmRaised)
                  {
                     for(nghCount = 0; nghCount < 5; nghCount ++)
                     {
                        if(((NlCellCb *)nghNghCb->
                        nghNghCellInfo.alarmInfo.peerInfo[nghCount].peerAddr == servCell) &&
                        (1 == nghNghCb->nghNghCellInfo.alarmInfo.peerInfo[nghCount].isActive) &&
                        (NL_ALARM_PEER_SERV_CELL ==
                            nghNghCb->nghNghCellInfo.alarmInfo.peerInfo[nghCount].peerType))
                        {
                           nghNghCb->nghNghCellInfo.alarmInfo.peerInfo[nghCount].peerAddr = 0L;
                           nghNghCb->nghNghCellInfo.alarmInfo.peerInfo[nghCount].isActive = 0;
                           nghNghCb->nghNghCellInfo.alarmInfo.numPeerAlarm --;
                           if(0 == nghNghCb->nghNghCellInfo.alarmInfo.numPeerAlarm)
                           {
                              nghNghCb->nghNghCellInfo.alarmInfo.isAlarmRaised = 0;
                           }
                        }
                     }
                  }
   ...
   if(count == 5)
   {
      servCell->cellInfo.alarmInfo.isAlarmRaised = 0;
      { servCell->state = NL_CELL_STATE_CONFIGURED; };
   }
   return(0);
}
 S16 nlNghNghPeerAlarmClear
(
...

{
   ...
   if(nghNghCell->nghNghCellInfo.alarmInfo.isAlarmRaised)
   {
      for(count = 0; count < 5; count ++)
      {
         ...
         {
            nghNghCell->nghNghCellInfo.alarmInfo.peerInfo[count].isActive = 0;
   ...
   if(count == 5)
   {
      nghNghCell->nghNghCellInfo.alarmInfo.isAlarmRaised = 0;
   }
   return(0);
}
 S16 nlNghPeerAlarmClear
(
...

{
   ...
   if(nghServCell->servCellInfo.alarmInfo.isAlarmRaised)
   {
      for(count = 0; count < 5; count ++)
      {
         ...
         {
            nghServCell->servCellInfo.alarmInfo.peerInfo[count].isActive = 0;
   ...
   if(count == 5)
   {
      nghServCell->servCellInfo.alarmInfo.isAlarmRaised = 0;
   }
   return(0);
}
 S16 nlStoreNghPeerAlarmInfo
(
NlAlarmInfo *alarmInfo,
LnlDgnAlarm alarmId,
NlAlarmPeerType peerType,
...
)
{
   U8 count;
   ;;
   alarmInfo->isAlarmRaised = 1;
   for(count = 0; count < 5; count ++)
   {
      if(0 == alarmInfo->peerInfo[count].isActive)
      {
         alarmInfo->peerInfo[count].peerType = peerType;
         alarmInfo->peerInfo[count].peerAddr = peerAddr;
         alarmInfo->peerInfo[count].isActive = 1;
...

{
 ...
 S16 nlUpdateNghNghCellAlarmInfo
(
