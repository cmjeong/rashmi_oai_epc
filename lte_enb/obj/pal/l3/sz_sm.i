...

{
      ...
      switch(evnt)
      {
         ...
         case 32:
              ...
              else
              {
                 ...
                 { SztLclErrInd _errInd; cmMemset ((U8 *)&_errInd, 0, sizeof (SztLclErrInd)); _errInd.causeType = (U8) 7; _errInd.causeValue = (U8)0; _errInd.transId.pres = 1; _errInd.transId.val = conCb->cd.suConnId; SzUiSztErrInd (&peerCb->sztSapCb->pst, peerCb->sztSapCb->suId, &_errInd); }
                                                                       ;
   ...
   if(ret != 0)
   {
      if((ret == 7) ||
            (ret == 8))
      {
         { SztLclErrInd _errInd; cmMemset ((U8 *)&_errInd, 0, sizeof (SztLclErrInd)); _errInd.causeType = (U8) ret; _errInd.causeValue = (U8)0; _errInd.transId.pres = 1; _errInd.transId.val = conCb->cd.suConnId; SzUiSztErrInd (&peerCb->sztSapCb->pst, peerCb->sztSapCb->suId, &_errInd); }
                                        ;
      }
      else
      {
         { SztLclErrInd _errInd; cmMemset ((U8 *)&_errInd, 0, sizeof (SztLclErrInd)); _errInd.causeType = (U8) 10; _errInd.causeValue = (U8)0; _errInd.transId.pres = 1; _errInd.transId.val = conCb->cd.suConnId; SzUiSztErrInd (&peerCb->sztSapCb->pst, peerCb->sztSapCb->suId, &_errInd); }
                                                                     ;
      }
...

{
   ...
   if(peerCb->nodeType == 1)
   {
      ...
      if(((SztMME_UE_S1AP_ID *)mmeS1apId)->val != conCb->rmtConRefNo)
      {
         ...
         { SztLclErrInd _errInd; cmMemset ((U8 *)&_errInd, 0, sizeof (SztLclErrInd)); _errInd.causeType = (U8) 4; _errInd.causeValue = (U8)((SztMME_UE_S1AP_ID *)mmeS1apId)->val; _errInd.transId.pres = 1; _errInd.transId.val = conCb->cd.spConnId; SzUiSztErrInd (&peerCb->sztSapCb->pst, peerCb->sztSapCb->suId, &_errInd); }
                                                                                            ;
         { if (pdu != 0L) { cmFreeMem ((Ptr)(pdu)); pdu = 0L; } };
         return(ret);
      }
...

{
   ...
   ret = szNdbDeallocConCb(conCb->lclConRefNo, peerCb, 1);

   if(ret == 1)
   {
      { SztLclErrInd _errInd; cmMemset ((U8 *)&_errInd, 0, sizeof (SztLclErrInd)); _errInd.causeType = (U8) 4; _errInd.causeValue = (U8)conCb->lclConRefNo; _errInd.transId.pres = 1; _errInd.transId.val = conCb->cd.suConnId; SzUiSztErrInd (&peerCb->sztSapCb->pst, peerCb->sztSapCb->suId, &_errInd); }
                                                                                    ;
...

{
   ...
   { SztLclErrInd _errInd; cmMemset ((U8 *)&_errInd, 0, sizeof (SztLclErrInd)); _errInd.causeType = (U8) 1; _errInd.causeValue = (U8)evnt; _errInd.transId.pres = 1; _errInd.transId.val = conCb->cd.suConnId; SzUiSztErrInd (&peerCb->sztSapCb->pst, peerCb->sztSapCb->suId, &_errInd); }
                                                                   ;
...

{
   ...
   { SztLclErrInd _errInd; cmMemset ((U8 *)&_errInd, 0, sizeof (SztLclErrInd)); _errInd.causeType = (U8) 11; _errInd.causeValue = (U8)conCb->state; _errInd.transId.pres = 1; _errInd.transId.val = conCb->cd.suConnId; SzUiSztErrInd (&peerCb->sztSapCb->pst, peerCb->sztSapCb->suId, &_errInd); }
                                                                           ;
   { if (pdu != 0L) { cmFreeMem ((Ptr)(pdu)); pdu = 0L; } };

   return(0);
}
 S16 szSmSndIdleStMsg
(
SzConCb *conCb,
SzPeerCb *peerCb,
S1apPdu *pdu
...

{
   ...
   if(ret != 0)
   {
      ...
      szNdbDeallocConCb(conCb->lclConRefNo,peerCb, 1);
...

{
   ...
   retVal = szDbFillErrInd(pdu, 3, cause, &errPdu, mem, &asnErr);
   if(0 != retVal)
   {
...

{
   ...
   { cmMemset((U8*)&conInd, 0, sizeof(SztConInd)); };
   ...
   if(ret != 0 ||(12 == ret))
   {
      ...
      szNdbDeallocConCb(conCb->lclConRefNo,conCb->cd.peerCb, 1);
      { if (pdu != 0L) { cmFreeMem ((Ptr)(pdu)); pdu = 0L; } };
...

{
   ...
   { cmMemset((U8*)&conCfm, 0, sizeof(SztConCfm)); };
   ...
   if((ret == 1))
   {
      ...
      { SztLclErrInd _errInd; cmMemset ((U8 *)&_errInd, 0, sizeof (SztLclErrInd)); _errInd.causeType = (U8) 15; _errInd.causeValue = (U8)conCb->cd.suConnId; _errInd.transId.pres = 1; _errInd.transId.val = conCb->cd.suConnId; SzUiSztErrInd (&peerCb->sztSapCb->pst, peerCb->sztSapCb->suId, &_errInd); }
                                                                          ;
   ...
   if(12 == ret)
   {
      ...
      ret = cmHashListFind (&peerCb->conRefLstRem,
            (U8 *)&conCb->rmtConRefNo,sizeof(U32),0,(U32*)&dupConCb);

      cmHashListDelete (&peerCb->conRefLstRem, (U32)dupConCb);
      ret = cmHashListInsert (&peerCb->conRefLstRem, (U32)conCb,
            (U8 *)&conCb->rmtConRefNo, sizeof (SzConRefNo));
      ...
      { SztLclErrInd _errInd; cmMemset ((U8 *)&_errInd, 0, sizeof (SztLclErrInd)); _errInd.causeType = (U8) 19; _errInd.causeValue = (U8)dupConCb->cd.suConnId; _errInd.transId.pres = 1; _errInd.transId.val = dupConCb->cd.suConnId; SzUiSztErrInd (&peerCb->sztSapCb->pst, peerCb->sztSapCb->suId, &_errInd); }
                                                                         ;
...

{
   ...
   if(peerCb->nodeType == 1)
   {
      ...
      if ((mmeS1apId == 0L) || (((SztMME_UE_S1AP_ID *)mmeS1apId)->val != conCb->rmtConRefNo))
      {
         szUtilsGetMsgIdx(&evnt, pdu);
         ...
         if(evnt != 20)
         {
            ...
            szLiErrHndlr(pdu, 0, szErrCause, &errPdu, mem);


            szIncErrSts(peerCb, 0, szErrCause);
         ...
         {
            { SztLclErrInd _errInd; cmMemset ((U8 *)&_errInd, 0, sizeof (SztLclErrInd)); _errInd.causeType = (U8) 4; _errInd.causeValue = (U8)0; _errInd.transId.pres = 1; _errInd.transId.val = conCb->cd.suConnId; SzUiSztErrInd (&peerCb->sztSapCb->pst, peerCb->sztSapCb->suId, &_errInd); }
                                                           ;
         }
      }
   }
   ...
   { cmMemset((U8*)&datEvnt, 0, sizeof(SztDatEvntInd)); };
...

{
   ...
   { cmMemset((U8*)&relInd, 0, sizeof(SztRelInd)); };

   relInd.suConnId.pres = 1;
 ...
 S16 szSmRcvConnStPathSwReqFail
(
SzConCb *conCb,
SzPeerCb *peerCb,
S1apPdu *pdu
