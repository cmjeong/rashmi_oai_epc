...
static Void szBndTmrExpiry (U32 cb);
 S16 szActvTmr
(
void
)
...

{
   ...
   { if ((&szCb.init)->dbgMask & ((0x00000100 << 3))) { snprintf((&szCb.init)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", szCb.nodeDbg, (&szCb.init)->procId, (&szCb.init)->ent, (&szCb.init)->inst,
                             "/home/mrehwald/Desktop/Radisys/lte_enb/src/s1ap/sz_tmr.c"
   ,
                             209
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "szRegInitTmr: tmrRes(%d) \n", szCb.genCfg.timeRes); SPrint((&szCb.init)->prntBuf); } }
                              ;
   ...
   ret = SRegCfgTmr(szCb.init.ent,szCb.init.inst,szCb.genCfg.timeRes,100,szActvTmr);
   szCb.szTqCp.tmrLen = 64;

   for (idx = 0; idx < 64; idx++)
   {
      szCb.szTq[idx].first = 0L;
      szCb.szTq[idx].tail = 0L;
   }
...

{
   ...
   ,
                                                                276
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "szStartTmr(event(%d), wait(%d))\n", event, wait->val); SPrint((&szCb.init)->prntBuf); } }
                                                                 ;
...

{
   ...
   ,
                                                 339
   ); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf, "szStopTmr(event(%d))\n", tmr->tmrEvnt); SPrint((&szCb.init)->prntBuf); } }
                                                  ;
...

{
   ...
   { if ((&szCb.init)->dbgMask & (0x00000100)) { snprintf((&szCb.init)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", szCb.nodeDbg, (&szCb.init)->procId, (&szCb.init)->ent, (&szCb.init)->inst, "/home/mrehwald/Desktop/Radisys/lte_enb/src/s1ap/sz_tmr.c", 392); SPrint((&szCb.init)->prntBuf); sprintf (szCb.init.prntBuf,"szTmrEvnt(event(%d))\n", event); SPrint((&szCb.init)->prntBuf); } };
...

{
   ...
   if (sctSapCb->state == 3)
   {
      ...
      {
         sctSapCb->retryCnt++;
         szStartTmr(sctSapCb->timers,(U32)sctSapCb,
                      12,&sctSapCb->tBndTmr);
