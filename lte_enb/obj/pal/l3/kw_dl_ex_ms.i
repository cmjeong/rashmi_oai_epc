...
extern Void kwUtlFreeDlMem ( Void);
 ...
 S16 kwUtlDlBatchProcPkts(Void);
 S16 kwDlBatchProc(Void);
 S16 kwDlInitExt ( Void );
 S16 kwDlInitExt
(
)
...

{
   ...
   cmMemset((U8 *)tKwCb, (U8)0, sizeof(KwCb));;


   tKwCb->init.ent = ent;
   tKwCb->init.inst = inst;
   tKwCb->init.region = region;
   tKwCb->init.pool = 0;
   tKwCb->init.reason = reason;
   tKwCb->init.cfgDone = 0;
   tKwCb->init.acnt = 1;
   tKwCb->init.usta = 1;
   tKwCb->init.trc = 0;
      ...
      case 0x0E:
         {
            switch(pst->event)
            {
               ...
               default:
                  ...
                  if (pst->dstInst < 2)
                  {
                      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xc30001, L_ERROR, pst->event, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_dl_ex_ms.c"                      , 299                      , "Received Invalid Event[%d] from SM", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                       ;
                  }
                  ret = 1;
                  break;
      ...
      case 0xd7:
         {
            ...
            switch(pst->event)
            {
               ...
               case 0x10:
                  {
                     kwUtlFreeDlMemory(kwCb[pst->dstInst]);
                     break;
                  }
               ...
               default:
                  ...
                  if (pst->dstInst < 2)
                  {
                      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xc30002, L_ERROR, pst->event, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_dl_ex_ms.c"                      , 381                      , "Received Invalid Event[%d] from RLC UL", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                       ;
                  }
                  ret = 1;
                  break;
      ...
      case 0xd9:
         {
            switch(pst->event)
            {
               ...
               default:
                  ...
                  if (pst->dstInst < 2)
                  {
                      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xc30003, L_ERROR, pst->event, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_dl_ex_ms.c"                      , 438                      , "Received Invalid Event[%d] from RRC", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                       ;
                  }
                  ret = 1;
                  break;
      ...
      case 0xd8:
         {
            switch(pst->event)
            {
               ...
               default:
                  ...
                  if (pst->dstInst < 2)
                  {
                      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xc30004, L_ERROR, pst->event, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_dl_ex_ms.c"                      , 494                      , "Received Invalid Event[%d] from PDCP", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                       ;
                  }
                  ret = 1;
                  break;
      ...
      case 0xd6:
         {
            switch(pst->event)
            {
               ...
               default:
                  ...
                  if (pst->dstInst < 2)
                  {
                      if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xc30005, L_ERROR, pst->event, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_dl_ex_ms.c"                      , 552                      , "Received Invalid Event[%d] from MAC", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                       ;
                  }
                  ret = 1;
                  break;
      ...
      default:
         {
            if (pst->dstInst < 2)
            {
               ...
               if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev1(0xc30006, L_ERROR, pst->event, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterlc/kw_dl_ex_ms.c"               , 612               , "Received Invalid Source Entity[%d]", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                ;
            }
            SPutMsg(mBuf);
            ret = 1;
