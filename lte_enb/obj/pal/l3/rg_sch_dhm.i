...
extern S16 rgSCHCmnDlAllocErrIndTxRb ( RgSchCellCb *cell, RgSchDlSf *subFrm, RgSchUeCb *ue, U32 bo, U32 *effBo, RgSchDlHqProcCb *proc, RgSchCmnDlRbAllocInfo *cellWdAllocInfo )
 ...
 U32 nackSf[10];
static Void rgSCHDhmFdbkIndHndlTa (RgSchDlHqProcCb *hqP, U8 tbIdx, U8 fdbk, Bool maxHqRetxReached)
                                   ;


static S16 rgSCHDhmProcHqFdbkAckNackRep ( RgSchDlHqProcCb *hqP, RgSchDlSf *sf, U8 tbCnt, U8 *isAck )
...
static S16 rgSCHDhmUpdateAckNackHistory ( RgSchCellCb *cell, RgSchUeCb *ueCb, U8 hqfdbk, U8 tbCnt )




          ;
 S16 rgSCHDhmDlRetxAllocFail ( RgSchUeCb *ue, RgSchDlHqProcCb *proc )
...

{
   ...
   for (i=0; i < hqE->numHqPrcs; i++)
   {
      ...
      hqP->tbInfo[1].contResCe = 0;
      hqP->lnk.node = (U32)hqP;
      ...
      hqP->drxCb.rttIndx = 0xFFFF;
      hqP->drxCb.reTxIndx = 0xFFFF;
      ...
      if ((hqP->laaHqP.hqPToShiftLnk.node) &&
            (hqE->cell->rgSchLAACellCfg.isLaaCell))
      {
         cmLListDelFrm(&hqE->cell->laaCellCb.hqPToShiftChkQ[hqP->laaHqP.hqPToShiftIdx],
               &(hqP->laaHqP.hqPToShiftLnk));
         hqP->laaHqP.hqPToShiftLnk.node = 0L;
      }
...

{
   ...
   if (rgSCHUtlAllocSBuf(inst, (Data **)&hqE, sizeof(RgSchDlHqEnt)) != 0)
   {
      return(0L);
   }
   hqE->numHqPrcs = 8;
...

{
   ...
   ((((tmp) = ((&(hqE->free))->crnt = (&(hqE->free))->first)) != 0L) ? (tmp)->node : 0L);
   ...
   if (ue->dl.isSpsHqPInUse)
   {
      ...
      {
         ((((tmp) = ((&(hqE->free))->crnt = ((&(hqE->free))->crnt ? (&(hqE->free))->crnt->next : (&(hqE->free))->first)))!= 0L) ? (tmp)->node : 0L);
         if (!tmp)
         {
            break;
         }
...

{
   ...
   hqP->tbInfo[tbIdx].schdTa.pres = 0;
   hqP->tbInfo[tbIdx].contResCe = 0;
   ...
   if (hqE->ccchSduProc == hqP)
   {
      ...
      if(0L == hqP->tbInfo[tbIdx].ccchSchdInfo.retxLnk.node)
      {
         cmLListDelFrm(&hqE->inUse, &hqP->lnk);
      }
      else
      {
         hqP->tbInfo[tbIdx].ccchSchdInfo.retxLnk.node = 0L;
      }
      hqP->pdcch = 0L;
      hqP->subFrm = 0L;
   ...
   switch(hqP->tbInfo[othrTbIdx].state)
   {
      case HQ_TB_ACKED:
         ...
         if (hqP->tbInfo[tbIdx].cntrRetxAllocFail != 10)
         {
            cmLListDelFrm(&hqE->inUse, &hqP->lnk);
         }
         hqP->pdcch = 0L;
...

{
   ...
   switch(fdbk)
   {
      case 1:
         ...
         hqP->tbInfo[tbIdx].taSnt = 0;

         if (ueCb->taLnk.node == 0L)
         {
            ueCb->taLnk.node = (U32)ueCb;
            cmLListAdd2Tail(&cell->taUeLst, &ueCb->taLnk);
         }
         else
         {

            if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xf20001,L_ERROR,DBG_CELLID, cell->cellId, ueCb->ueId, 0, 0, 0,                                                 "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_dhm.c"            ,                                                 1216            , "Trying to add CRNTI:%d into TA" "ACK List twice", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                  ;
      ...
      case 0:
         ...
         if(1 == maxHqRetxReached)
         {
            hqP->tbInfo[tbIdx].taSnt = 0;
            hqP->hqE->ue->dl.taCb.state = RGSCH_TA_IDLE;

            rgSCHUtlReTxTa(cell, ueCb);
            if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xf20002,L_DEBUG,DBG_CELLID, cell->cellId, 0, 0, 0, 0,                                                         "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_dhm.c"            ,                                                         1232            , "Nack Rcvd for TA. Max Tries Attempted", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                          ;
         }
         break;
      case 4:
...

{
   ...
   tbInfo->schdTa.pres = 1;
   tbInfo->schdTa.val = ueCb->dl.taCb.ta;

   return;
}
static RgSchDlHqProcCb *rgSCHDhmHqProcByTime
(
RgSchDlHqEnt *hqE,
...
)
{
   if (hqE->msg4Proc)
   {
      if ((((hqE->msg4Proc->tbInfo[0].timingInfo).sfn == (timeInfo).sfn) && ((hqE->msg4Proc->tbInfo[0].timingInfo).subframe == (timeInfo).subframe)))
      {
          *isMsg4 = 1;
          return(hqE->msg4Proc);
      }
...
static Void rgSCHDhmHqTbTrnsFail
(
RgSchCellCb *cell,
RgSchDlHqProcCb *hqP,
U8 tbCnt,
...

{
   ...
   if (hqE->msg4Proc == hqP)
   {
      ...
      if(hqP->hqE->raCb->expiryTime.sfn == 0xffff)
      {
   ...
   if (hqP->tbInfo[tbCnt].txCntr < maxHqTx)
   {
      ...
      if((hqE->ue != 0L) && (hqE->ue->isDrxEnabled == 1))
      {
   ...
   else
   {
      ...
      else
      {
        if ( (hqP->sch != (RgSchCmnDlHqProc *)0L) &&
              ((((RgSchCmnDlHqProc *)((hqP)->sch))->isSpsSvcSchd) &&
               ((((RgSchCmnDlHqProc *)((hqP)->sch))->isSpsActv)))
           )
        {
           cmnHqDl = ((RgSchCmnDlHqProc *)((hqP)->sch));
...
)
{
   ...
   Bool hqRls=0;
   Bool hqFreed =0;
   Bool maxHqRetxReached = 0;
   RgSchCmnDlUe *ueDl = 0L;
   ...
   if (isAck == 1)
   {
      ...
      if (ue)
      {
         ...
         ue->tenbStats->stats.nonPersistent.sch[(U8)((sCell->cellId >= rgSchCb[sCell->instIdx].genCfg.startCellId) ? (sCell->cellId - rgSchCb[sCell->instIdx].genCfg.startCellId)&(8 -1): 0)].dlAckNackCnt[tbCnt]++;
   ...
   else if (isAck == 0)
   {
      ...
      { if ((&rgSchCb[inst].rgSchInit)->dbgMask & ((0x00000100 << 0))) { snprintf((&rgSchCb[inst].rgSchInit)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", "MAC Scheduler", (&rgSchCb[inst].rgSchInit)->procId, (&rgSchCb[inst].rgSchInit)->ent, (&rgSchCb[inst].rgSchInit)->inst,
                                                                  "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_dhm.c"
      ,
                                                                  2815
      ); SPrint((&rgSchCb[inst].rgSchInit)->prntBuf); sprintf (rgSchCb[inst].rgSchInit.prntBuf, "<HARQ> HqP[%d:%d] NACKED " "ue(%d)\n", hqP->procId, tbCnt, hqP->hqE->ue->ueId); SPrint((&rgSchCb[inst].rgSchInit)->prntBuf); } }
                                                                   ;
      ...
      if (ue)
      {
         ...
         ue->tenbStats->stats.nonPersistent.sch[(U8)((sCell->cellId >= rgSchCb[sCell->instIdx].genCfg.startCellId) ? (sCell->cellId - rgSchCb[sCell->instIdx].genCfg.startCellId)&(8 -1): 0)].dlAckNackCnt[tbCnt]++;
         ue->tenbStats->stats.nonPersistent.sch[(U8)((sCell->cellId >= rgSchCb[sCell->instIdx].genCfg.startCellId) ? (sCell->cellId - rgSchCb[sCell->instIdx].genCfg.startCellId)&(8 -1): 0)].dlNackCnt[tbCnt] ++;
   ...
   else
   {
      ...
      ,
                                                     2848
      ); SPrint((&rgSchCb[inst].rgSchInit)->prntBuf); sprintf (rgSchCb[inst].rgSchInit.prntBuf,"<HARQ> HqP[%d:%d] DTXED UE(%d)\n", hqP->procId, tbCnt,hqP->hqE->ue->ueId); SPrint((&rgSchCb[inst].rgSchInit)->prntBuf); } }
                                                      ;
      hqP->tbInfo[tbCnt].dtxCount += 1;
      ...
      if (ue)
      {
         ue->tenbStats->stats.nonPersistent.sch[(U8)((sCell->cellId >= rgSchCb[sCell->instIdx].genCfg.startCellId) ? (sCell->cellId - rgSchCb[sCell->instIdx].genCfg.startCellId)&(8 -1): 0)].dlDtxCnt[tbCnt]++;
      }
   }
   ...
   {
      if((rgSCHDhmProcHqFdbkAckNackRep(hqP,sf,tbCnt,&isAck)) != 0)
      {
         return(ret);
      }
   }
   else
   {
      ...
      rgSCHUtlDlHqPTbRmvFrmTx(sf, hqP, tbCnt, 0);
   }
   if (((isAck == TFU_HQ_NACK) || (isAck == TFU_HQ_ACK)) &&
         ((hqP->sch != (RgSchCmnDlHqProc *)0L) &&
          ((((RgSchCmnDlHqProc *)((hqP)->sch))->isSpsActv)))
      )
   {
   ...
   if (1 == isAck)
   {
      ...
      else
      {
         ...
         ;
         { RgSchUeCellInfo *sCellInfo = (ue->cellInfo[ue->cellIdToCellIdxMap [(U8)((sCell->cellId >= rgSchCb[sCell->instIdx].genCfg.startCellId) ? (sCell->cellId - rgSchCb[sCell->instIdx].genCfg.startCellId)&(8 -1): 0)]]); if(sCellInfo->acqiCb.aCqiCfg.pres) { rgSCHUtlUpdACqiTrigWt(ue, sCellInfo, isAck); }};
         rgSCHDhmRlsHqpTb(hqP, tbCnt, 1);
      }
      hqRls = 1;
   }
   ...
   if (hqFreed == 0 && hqP->tbInfo[tbCnt].taSnt == 1)
   {
      rgSCHDhmFdbkIndHndlTa(hqP, tbCnt, isAck, maxHqRetxReached);
   }
   return(ret);
}
static Void rgSchGetHqFdbkPos
(
 RgSchCellCb *cell,
 RgSchUeCb *ue,
 RgSchDlHqProcCb *hqP,
...
)
{
   ...
   do { S32 _subframe; _subframe = timingInfo.sfn * 10 + timingInfo.subframe; _subframe = _subframe - 4; if(_subframe < 0) { _subframe = (1024 * 10) + _subframe; } timingInfo.sfn = _subframe / 10; timingInfo.subframe = _subframe % 10; } while(0);
   ...
   else
   {
      ...
      ueDl = (&(((RgSchCmnUe *)((ue->cellInfo[ue->cellIdToCellIdxMap [(U8)((cell->cellId >= rgSchCb[cell->instIdx].genCfg.startCellId) ? (cell->cellId - rgSchCb[cell->instIdx].genCfg.startCellId)&(8 -1): 0)]])->sch))->dl));
      hqE = (ue->cellInfo[ue->cellIdToCellIdxMap [(U8)((cell->cellId >= rgSchCb[cell->instIdx].genCfg.startCellId) ? (cell->cellId - rgSchCb[cell->instIdx].genCfg.startCellId)&(8 -1): 0)]]->hqEnt);
      lnk = &ue->dl.dlSfHqInfo[sf->dlIdx].hqPLst;
      node = lnk->first;
      hqPCount = lnk->count;
      rnti = ue->ueId;
   ...
   if (ue && (ue->relPdcchFbkTiming.sfn != (1024 + 1)))
   {
      if ((((fdbkRcptTime).sfn == (ue->relPdcchFbkTiming).sfn) && ((fdbkRcptTime).subframe == (ue->relPdcchFbkTiming).subframe)))
      {
         sf = rgSCHUtlSubFrmGet(cell, timingInfo);
         {
            if (!sf->relPdcch)
            {
               if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xf20003,L_ERROR,DBG_CELLID, cell->cellId, ue->ueId, timingInfo.sfn, timingInfo.subframe, 0,                                     "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_dhm.c"               ,                                     3391               , "CRNTI:%d NO HARQ proc available for feedback: TimingInfo: " "sfn %d subframe %d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   sf = rgSCHUtlSubFrmGet(cell, timingInfo);
   if ((timingInfo.subframe + (1)) >= 10) timingInfo.sfn = (timingInfo.sfn + (timingInfo.subframe + (1))/10) % 1024; else timingInfo.sfn = timingInfo.sfn; timingInfo.subframe = (timingInfo.subframe + (1)) % 10;;
   while (hqPCount)
   {
      if(cbType != 2)
      {
...

{
   ...
   for (;;)
   {
      ...
      else
      {
         ...
         hqE = (((RgSchUeCb*)nUeCb)->cellInfo[((RgSchUeCb*)nUeCb)->cellIdToCellIdxMap [(U8)((cell->cellId >= rgSchCb[cell->instIdx].genCfg.startCellId) ? (cell->cellId - rgSchCb[cell->instIdx].genCfg.startCellId)&(8 -1): 0)]]->hqEnt);
         hqE->maxHqTx = cell->dlHqCfg.maxDlHqTx;
      }
   }
...

{
   ...
   if ((ueCb->cellInfo[ueCb->cellIdToCellIdxMap [(U8)((ueCb->cell->cellId >= rgSchCb[ueCb->cell->instIdx].genCfg.startCellId) ? (ueCb->cell->cellId - rgSchCb[ueCb->cell->instIdx].genCfg.startCellId)&(8 -1): 0)]]->hqEnt))
   {
      rgSCHDhmDelHqEnt(ueCb->cell, &((ueCb->cellInfo[ueCb->cellIdToCellIdxMap [(U8)((ueCb->cell->cellId >= rgSchCb[ueCb->cell->instIdx].genCfg.startCellId) ? (ueCb->cell->cellId - rgSchCb[ueCb->cell->instIdx].genCfg.startCellId)&(8 -1): 0)]]->hqEnt)));
   }
...

{
   ...
   do { S32 _subframe; _subframe = timingInfo.sfn * 10 + timingInfo.subframe; _subframe = _subframe - 4; if(_subframe < 0) { _subframe = (1024 * 10) + _subframe; } frm.sfn = _subframe / 10; frm.subframe = _subframe % 10; } while(0);
   ...
   while (node)
   {
      ...
      if (HQ_TB_WAITING == tmpHqProc->tbInfo[0].state)
      {
         ...
         if (tbCb->fbkRepCntr != 0)
         {
            ...
            if ((tbCb->timingInfo.subframe + (1)) >= 10) tbCb->timingInfo.sfn = (tbCb->timingInfo.sfn + (tbCb->timingInfo.subframe + (1))/10) % 1024; else tbCb->timingInfo.sfn = tbCb->timingInfo.sfn; tbCb->timingInfo.subframe = (tbCb->timingInfo.subframe + (1)) % 10;;
            rgSCHUtlDlHqPTbRmvFrmTx(sf,tmpHqProc,tbCb->tbIdx, 1);
            tbCb->fbkRepCntr--;
            continue;
         }
         rgSCHUtlDlHqPTbRmvFrmTx(sf,tmpHqProc,tbCb->tbIdx, 0);
         ...
         while (hqPNode)
         {
            ...
            for (idx = 0 ;idx < 2; idx++)
            {
               if (HQ_TB_WAITING == tmpHqProc->tbInfo[idx].state)
               {
                  ...
                  rgSCHUtlDlHqPTbRmvFrmTx(sf,tmpHqProc,idx, 0);
   ...
   while (node)
   {
      ...
      tbCb->dtxCount++;
       ...
       ue->tenbStats->stats.nonPersistent.sch[(U8)((tmpHqProc->hqE->cell->cellId >= rgSchCb[tmpHqProc->hqE->cell->instIdx].genCfg.startCellId) ? (tmpHqProc->hqE->cell->cellId - rgSchCb[tmpHqProc->hqE->cell->instIdx].genCfg.startCellId)&(8 -1): 0)].dlDtxCnt[tbCb->tbIdx] ++;
      ...
      if (tbCb->taSnt == 1)
      {
         if (maxRetx == 1)
         {
            ...
            if(ue != 0L)
            {
               ...
               rgSCHUtlReTxTa(cell, ue);
               if( L_DEBUG < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xf20004,L_DEBUG,DBG_CELLID, cell->cellId, 0, 0, 0, 0,                                                             "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_dhm.c"               ,                                                             4490               , "Nack/DTX Rcvd for TA. Max Tries Attempted", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                              ;
            }
          }
      }
...

{
    ...
    rgSCHUtlDlHqPTbRmvFrmTx(sf, hqP, tbCnt, 1);

    if (--hqP->tbInfo[tbCnt].fbkRepCntr)
    {

        if ((hqP->tbInfo[tbCnt].timingInfo.subframe + (1)) >= 10) hqP->tbInfo[tbCnt].timingInfo.sfn = (hqP->tbInfo[tbCnt].timingInfo.sfn + (hqP->tbInfo[tbCnt].timingInfo.subframe + (1))/10) % 1024; else hqP->tbInfo[tbCnt].timingInfo.sfn = hqP->tbInfo[tbCnt].timingInfo.sfn; hqP->tbInfo[tbCnt].timingInfo.subframe = (hqP->tbInfo[tbCnt].timingInfo.subframe + (1)) % 10;
                                                                ;
        return(1);
    }
...

{
   ...
   if (hqP->tbInfo[0].cntrRetxAllocFail == 10)
   {
   ...
   if (hqP->tbInfo[1].cntrRetxAllocFail == 10)
   {
      if (hqP->hqE->msg4Proc == hqP)
      {
...
)
{
   ...
   if (hqfdbk == 1)
   {
      ueDl->laCb[tbCnt].deltaiTbs += 3;
   }
   else
   {
      ueDl->laCb[tbCnt].deltaiTbs = ueDl->laCb[tbCnt].deltaiTbs - 30;
   }
...
)
{
   ...
   U8 maxiTbs = (*(RgSchCmnCqiToTbs *)(cellSch->dl.cqiToTbsTbl[0][cfi]))[16 - 1];
   ...
   if (iTbs > maxiTbs)
   {
      ...
      ueDl->mimoInfo.cwInfo[tbCnt].iTbs[0] = ((maxiTbs) <= (ueCb->cell->thresholds.maxDlItbs) ? (maxiTbs) : (ueCb->cell->thresholds.maxDlItbs));
   }
   else if (iTbs < 0)
   {
   ...
   {
      ueDl->mimoInfo.cwInfo[tbCnt].iTbs[0] = ((((ueDl->laCb[tbCnt].cqiBasediTbs + ueDl->laCb[tbCnt].deltaiTbs)/100)) <= (ueCb->cell->thresholds.maxDlItbs) ? (((ueDl->laCb[tbCnt].cqiBasediTbs + ueDl->laCb[tbCnt].deltaiTbs)/100)) : (ueCb->cell->thresholds.maxDlItbs))
