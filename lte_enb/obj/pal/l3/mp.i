...
size_t mpbits(size_t size, const mpw* data)
{
 return ((size) << 6) - mpmszcnt(size, data);
}
...
void mplshift(size_t size, mpw* data, size_t count)
{
 register size_t words = ((count) >> 6);

 if (words < size)
 {
  register short lbits = (short) (count & (64U -1));


  if (lbits)
  {
   register mpw temp, carry = 0;
   register short rbits = 64U - lbits;
  ...
  if (words)
  {
   memmove(data, data+words, ((size-words) << 3));
...
void mprshift(size_t size, mpw* data, size_t count)
{
 ...
 if (words < size)
 {
  register short rbits = (short) (count & (64U -1));


  if (rbits)
  {
   register mpw temp, carry = 0;
   register short lbits = 64U - rbits;
   register size_t i = 0;
  ...
  if (words)
  {
   memmove(data+words, data, ((size-words) << 3));
   mpzero(words, data);
  }
 }
 else
...
size_t mprshiftlsz(size_t size, mpw* data)
{
 ...
 register size_t zwords = 0;
 register short lbits, rbits = 0;
 register mpw temp, carry = 0;
...
void mpgcd_w(size_t size, const mpw* xdata, const mpw* ydata, mpw* result, mpw* wksp)
{
 ...
 {
  memcpy(wksp, xdata, ((size) << 3));
  memcpy(result, ydata, ((size) << 3));
 }
 else
 {
  memcpy(wksp, ydata, ((size) << 3));
  memcpy(result, xdata, ((size) << 3));
 }
...
void mpmod(mpw* result, size_t xsize, const mpw* xdata, size_t ysize, const mpw* ydata, mpw* workspace)
{
 ...
 memcpy(ynorm, ydata, ((ysize) << 3));
 shift = mpnorm(ysize, ynorm);
 msw = *ynorm;
 memcpy(rdata, xdata, ((xsize) << 3));
...
int hs2ip(mpw* idata, size_t isize, const char* hsdata, size_t hssize)
{
 size_t required = ((hssize + (64U >> 2) - 1) >> 4);
 ...
 if (isize >= required)
 {
  ...
  {
   register mpw w = 0;
   register size_t chunk = hssize & ((64U >> 2) - 1);
   register char ch;

   if (chunk == 0) chunk = (64U >> 2);
