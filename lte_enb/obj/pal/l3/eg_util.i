...
static S16 egUUtilDumpGmHdr ( EgUMsgHdr egGmHdr );




static S16 egUUtilDumpList ( CmLListCp *ieList );




static S16 egUUtilDumpIe ( EgUIe *egIe );
static S16 egUUtilDupIe (CmMemListCp *memCp, EgUIe *oldIe, EgUIe **dupIe );
 S16 egUUtilCmpTptAddr
(
CmTptAddr *srcAddr,
CmTptAddr *destAddr,
U16 *port
...

{
   ...
   if ((srcAddr->type != 4) && (srcAddr->type != 6))
   {
      return(1);
   }
...

{
   ...
   if (cmAllocEvnt(sizeof(EgUMsg), 1024, memInfo, (Ptr* )egMsg) != 0)
   {
      {}
                                                                                   ;
      return(1);
...

{
   ...
   { if (egCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (egCb.init.prntBuf, " [%s 0x%x:%x]", "Eg", egCb.init.ent,egCb.init.inst); printf("%s\n", egCb.init.prntBuf); }; } { { sprintf (egCb.init.prntBuf, " [%s:%d]","/home/mrehwald/Desktop/Radisys/lte_enb/src/egtpu/eg_util.c",381); printf("%s\n", egCb.init.prntBuf); }; } { sprintf (egCb.init.prntBuf," Entering\n"); printf("%s\n", egCb.init.prntBuf); }; } ;; }
   ...
   if((*egMsg)->msgHdr.msgType != EGT_GTPU_MSG_GPDU)
   {
      ...
      for (ie = (EgUIe* )((((node) = ((&(*egMsg)->u.egIeList)->crnt = (&(*egMsg)->u.egIeList)->first)) != 0L) ? (node)->node : 0L); ie != 0L;
            ie = (EgUIe* )((((node) = ((&(*egMsg)->u.egIeList)->crnt = ((&(*egMsg)->u.egIeList)->crnt ? (&(*egMsg)->u.egIeList)->crnt->next : (&(*egMsg)->u.egIeList)->first)))!= 0L) ? (node)->node : 0L))
      {
...

{
   ...
   { if (egCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (egCb.init.prntBuf, " [%s 0x%x:%x]", "Eg", egCb.init.ent,egCb.init.inst); printf("%s\n", egCb.init.prntBuf); }; } { { sprintf (egCb.init.prntBuf, " [%s:%d]","/home/mrehwald/Desktop/Radisys/lte_enb/src/egtpu/eg_util.c",468); printf("%s\n", egCb.init.prntBuf); }; } { sprintf (egCb.init.prntBuf," Entering\n"); printf("%s\n", egCb.init.prntBuf); }; } ;; }
   if (!egMsg || !egIe)
   {
      return(3);
   }
   ...
   if ( (retValue = egUUtilDupIe(&egMsg->memCp, egIe, &ie)) != 0)
   {
      {}
                                                                               ;
      return(retValue);
...

{
   ...
   { if (egCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (egCb.init.prntBuf, " [%s 0x%x:%x]", "Eg", egCb.init.ent,egCb.init.inst); printf("%s\n", egCb.init.prntBuf); }; } { { sprintf (egCb.init.prntBuf, " [%s:%d]","/home/mrehwald/Desktop/Radisys/lte_enb/src/egtpu/eg_util.c",613); printf("%s\n", egCb.init.prntBuf); }; } { sprintf (egCb.init.prntBuf," Entering\n"); printf("%s\n", egCb.init.prntBuf); }; } ;; }
   ...
   if (cmGetMem(memCp, length, (Ptr* )dupIe) != 0)
   {
      {}
                                                                       ;
      return(1);
      ...
      case EG_EXT_HDR_TYPE_LST:
         {
            ...
            if(oldIe->t.extHdrTypeLst.noOfExtHdrs <= 2)
            {
   ...
   ( (EgUIe *)(*dupIe))->node.node = (U32) (*dupIe);
...

{
   ...
   { if (egCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (egCb.init.prntBuf, " [%s 0x%x:%x]", "Eg", egCb.init.ent,egCb.init.inst); printf("%s\n", egCb.init.prntBuf); }; } { { sprintf (egCb.init.prntBuf, " [%s:%d]","/home/mrehwald/Desktop/Radisys/lte_enb/src/egtpu/eg_util.c",780); printf("%s\n", egCb.init.prntBuf); }; } { sprintf (egCb.init.prntBuf," Entering\n"); printf("%s\n", egCb.init.prntBuf); }; } ;; }
   if (!egMsg
...

{
   ...
   { if (egCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (egCb.init.prntBuf, " [%s 0x%x:%x]", "Eg", egCb.init.ent,egCb.init.inst); printf("%s\n", egCb.init.prntBuf); }; } { { sprintf (egCb.init.prntBuf, " [%s:%d]","/home/mrehwald/Desktop/Radisys/lte_enb/src/egtpu/eg_util.c",887); printf("%s\n", egCb.init.prntBuf); }; } { sprintf (egCb.init.prntBuf," Entering\n"); printf("%s\n", egCb.init.prntBuf); }; } ;; }

   cmMemset( (U8* )buff, 0, 1024);
   cmMemset( (U8* )prnBuff, 0, 1024);
   sprintf(newLine," \n");

   sprintf(buff, "   ********************* GTP-U Message *********************\n");
   sprintf(buff, "  MsgType    = %d\n", egGmHdr.msgType);
   { U16 srcIdx = 0; U16 destIdx = 0; while (prnBuff[destIdx++]); for (destIdx--; buff[srcIdx]; (prnBuff[destIdx++] = buff[srcIdx++])); };
...

{
   ...
   { if (egCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (egCb.init.prntBuf, " [%s 0x%x:%x]", "Eg", egCb.init.ent,egCb.init.inst); printf("%s\n", egCb.init.prntBuf); }; } { { sprintf (egCb.init.prntBuf, " [%s:%d]","/home/mrehwald/Desktop/Radisys/lte_enb/src/egtpu/eg_util.c",1023); printf("%s\n", egCb.init.prntBuf); }; } { sprintf (egCb.init.prntBuf," Entering\n"); printf("%s\n", egCb.init.prntBuf); }; } ;; }
   if (!ieList)
   {
      return(3);
   }
   ...
   for (egIe = (EgUIe* ) ((((node) = ((ieList)->crnt = (ieList)->first)) != 0L) ? (node)->node : 0L); egIe != 0L;
         egIe = (EgUIe* ) ((((node) = ((ieList)->crnt = ((ieList)->crnt ? (ieList)->crnt->next : (ieList)->first)))!= 0L) ? (node)->node : 0L))
   {
...

{
   ...
   { if (egCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (egCb.init.prntBuf, " [%s 0x%x:%x]", "Eg", egCb.init.ent,egCb.init.inst); printf("%s\n", egCb.init.prntBuf); }; } { { sprintf (egCb.init.prntBuf, " [%s:%d]","/home/mrehwald/Desktop/Radisys/lte_enb/src/egtpu/eg_util.c",1137); printf("%s\n", egCb.init.prntBuf); }; } { sprintf (egCb.init.prntBuf," Entering\n"); printf("%s\n", egCb.init.prntBuf); }; } ;; }
   if (!egIe)
   {
      return(3);
   }
   ...
   cmMemset( (U8* )buff, 0, 1024);
   cmMemset( (U8* )prnBuff, 0, 1024);
   cmMemset( (U8* )tmpBuff, 0, 1024 -5);
   cmMemset( (U8* )tempBuff, 0, 1024 -5);
   sprintf(newLine," \n");
   ...
   { U16 srcIdx = 0; U16 destIdx = 0; while (prnBuff[destIdx++]); for (destIdx--; "\n"[srcIdx]; (prnBuff[destIdx++] = "\n"[srcIdx++])); };
   SPrint(prnBuff);
   SPrint(newLine);
...

{
   ...
   *ieOccrCnt = 0;
   ...
   { if (egCb.init.dbgMask & ((0x00000100 << 4))) { { { sprintf (egCb.init.prntBuf, " [%s 0x%x:%x]", "Eg", egCb.init.ent,egCb.init.inst); printf("%s\n", egCb.init.prntBuf); }; } { { sprintf (egCb.init.prntBuf, " [%s:%d]","/home/mrehwald/Desktop/Radisys/lte_enb/src/egtpu/eg_util.c",1368); printf("%s\n", egCb.init.prntBuf); }; } { sprintf (egCb.init.prntBuf," Entering\n"); printf("%s\n", egCb.init.prntBuf); }; } ;; }
      for (egIe = (EgUIe* ) ((((node) = ((&egMsg->u.egIeList)->crnt = (&egMsg->u.egIeList)->first)) != 0L) ? (node)->node : 0L);
           egIe != 0L;
           egIe = (EgUIe* ) ((((node) = ((&egMsg->u.egIeList)->crnt = ((&egMsg->u.egIeList)->crnt ? (&egMsg->u.egIeList)->crnt->next : (&egMsg->u.egIeList)->first)))!= 0L) ? (node)->node : 0L))
      {
