...
private:
 ...
 U16 m_isDlfsEnabled;
   ...
   RmServingCellCqiResInfo m_stCqiResAllocInfo[LRM_UL_DL_CONFIG_MAX][32];
...
private:
 ...
 U16 m_usSRStartIdx;
   ...
   RmSrResAllocInfo m_stSrResAllocInfo[LRM_UL_DL_CONFIG_MAX][32];
...
private:
   ...
   U16 m_usMaxNumSpsUe;
   RmRcmN1PucchRes m_stN1PucchRes[32];
   U8 m_bMaxActiveUesServed;
};
...
{
   U32 bResCfgMask = 0x01 | 0x02 | 0x800;
...
S32 CRmRsrcCfgMgr::configureUeResources(CUEContext* pUE, Bool bSCellActN1ResCfgReq, RmuUeRadioResCfg & stUeRadioResCfg)
{
...
{
   Bool bIsSCellN1ToBeCfgd = 0;


   configureUeResources(pUE, bIsSCellN1ToBeCfgd, pstUeHoRsp->stUeRadioResCfg);

   if((pstUeHoRsp->stUeRadioResCfg.bRadioResDedBitMap & 4) ||
         (pstUeHoRsp->stUeRadioResCfg.bRadioResDedBitMap & 8))
   {
      U8 bIdx;
 ...
 )
{
   ...
   if(retVal == 0)
   {
      stUeRadioResCfg.stPhyCfgDed.bPhyCfgDedResBitMap |= 2;
      stUeRadioResCfg.bRadioResDedBitMap |= 2;
...
{
   if(bResCfgMask & 0x800)
   {
      U32 ueTxnMode;
      ueTxnMode = (U32)pUE->getUeTxnMode();

      stUeRadioResCfg.stPhyCfgDed.bPhyCfgDedResBitMap |= 8;
      stUeRadioResCfg.bRadioResDedBitMap |= 2;
 ...
 )
{
 ...
 if(bResCfgMask & 0x01)
 {
  ...
  stUeRadioResCfg.stPhyCfgDed.bPhyCfgDedResBitMap |= 1;
    stUeRadioResCfg.bRadioResDedBitMap |= 2;
 }
}
 ...
 if(bResCfgMask & 0x04)
 {
  ...
  if(retVal == 2)
  {
   stUeRadioResCfg.stMacMainCfg.stRmDrxCfg = pUE->getDrxCfg();
         stUeRadioResCfg.bRadioResDedBitMap |= 1;
         stUeRadioResCfg.stMacMainCfg.bMacMainCfgBitMap |= 1<<1;
  }
  else if(retVal == 3)
  {
         if( L_INFO < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x930001,L_INFO,DBG_CRNTI, (pUE->getCRNTI()), 0, 0, 0, 0,                                                          "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterrm/src/rm_resourceconfigmanager.cpp"         ,                                                          322         , "DRX Configuration Could not be done", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                           ;
  }
  else if(retVal == 1)
  {
         if( L_INFO < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0x930002,L_INFO,DBG_CRNTI, (pUE->getCRNTI()), 0, 0, 0, 0,                                                    "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterrm/src/rm_resourceconfigmanager.cpp"         ,                                                    327         , "DRX configuration is already done", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                                     ;
  }
 }
 else if (bResCfgMask & 0x20)
 {
  pUE->relDrxCfg();
  stUeRadioResCfg.stMacMainCfg.stRmDrxCfg = pUE->getDrxCfg();
  stUeRadioResCfg.bRadioResDedBitMap |= 1;
...
S32 CRmRsrcCfgMgr::rmRcmRecfgUeRes(CUEContext* pUE, RmuUeRecfgReq & stUeReCfgReq,RmuUeRecfgRsp *pstUeRecfgRsp)
{
 ...
 pstUeRecfgRsp->stUeRadioResCfg.bRadioResDedBitMap = 0;
   ...
   retVal = configureUeResources(pUE, bIsSCellN1ToBeCfgd, pstUeRecfgRsp->stUeRadioResCfg);


 if(pstUeRecfgRsp->stUeRadioResCfg.bRadioResDedBitMap)
 {
      if((pstUeRecfgRsp->stUeRadioResCfg.bRadioResDedBitMap & 4) ||
         (pstUeRecfgRsp->stUeRadioResCfg.bRadioResDedBitMap & 8))
      {
         U8 bIdx;
 ...
 bUeResAllocated = pUE->getUeRadioDedResAllocBitMap();
   ...
   if(bIsDrxEnabled == 1)
   {
      ...
      if(!(bUeResAllocated & 8))
      {
         ...
         else if(isUeEnbDrx == 2)
         {
            uiReCfgMask |= 0x04;
         }
      }
      else if(bUeResAllocated & 8)
      {
         if(isUeEnbDrx == 0)
         {
   ...
   if(!(bUeResAllocated & 2) || !(pUE->isRiCfgDone()))
 {
  uiReCfgMask |= 0x02;
 }

   if(!(bUeResAllocated & 1))
 {
  uiReCfgMask |= 0x01;
 }
   ...
   if(bIsSpsEnabled == 1)
   {
      ...
      if(!(bUeResAllocated & 16))
      {
         if((isUeSpsEnb == 1) && (bIsSpsUeRem == 1))
         {
            uiReCfgMask |= 0x200;
         }
         else
...
{
   U16 bTti;
   for (bTti = 0; (bTti < usPrdcty) && (bTti < 160); bTti++)
   {
 ...
 )
{
 ...
 if(bResCfgMask & 0x200)
 {
      m_objRcmSpsMgr.rmRcmCfgSps(pUE);
      stUeRadioResCfg.stSpsCfg = pUE->getSpsCfg();
      stUeRadioResCfg.bRadioResDedBitMap |= 4;
      stUeRadioResCfg.bRadioResDedBitMap |= 8;
   }
   else if(bResCfgMask & 0x400)
   {
