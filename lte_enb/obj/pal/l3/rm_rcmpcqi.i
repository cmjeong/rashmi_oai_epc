...
CRmRcmCqiMgr::CRmRcmCqiMgr(LrmCellConfigData &stCellCfg)
{
   ...
   m_usn2Pucch = stCellCfg.bNRbCqi * 12 +
                  ((((((stCellCfg.bNCsAn)) + ((8))-1) / ((8)))) *
                   (12 - stCellCfg.bNCsAn - 2));
...
void CRmRcmCqiMgr::rmRcmCqiPopulateCqiResTbl()
{
   ...
   if(usNumUlSf > 160)
   {
      if( L_WARNING < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev3(0x920001, L_WARNING,usNumUlSf, bCqiPrdcty, 160, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterrm/src/rm_rcmpcqi.cpp"      , 121      , "Number of UL subframes [%d] for CQI resource assignment" "for periodicity [%d] is more than Max supported [%d]." "Hence reassignig it to Max Value", RLOG_FILE_ID, RLOG_MODULE_NAME); }
   ...
   for (usSfnSf = 0; usSfnSf < usNumUlSf; usSfnSf++)
   {
      ...
      usCqiOffsetIdx = (usSfnSf * 2 + 2) % usNumUlSf;
         ...
         (usCqiResIdx < m_bNumCqiPerTti); usCqiResIdx++)
   {
      ...
      maxUlSfPerRes = usNumUlSf/2 ;
      for (usSfnSf = 0; (usSfnSf < maxUlSfPerRes ) &&
               (usUeIndex < m_bMaxActiveUesServed); usSfnSf++)
      {
         m_stCqiResAllocInfo[m_enCqiPrdcty][usUeIndex].cellCqiResInfo[0].usCqiCfgIdx =
            usCqiOffset[usSfnSf] + m_ausCqiStartOffset[m_enCqiPrdcty];



         m_stCqiResAllocInfo[m_enCqiPrdcty][usUeIndex].cellCqiResInfo[0].usRiOffset =
            ausRiOffset[usSfnSf];
         ...
         m_stCqiResAllocInfo[m_enCqiPrdcty][usUeIndex].cellCqiResInfo[0].usRiCfgIdx =
            usRiOffset + m_ausRiStartOffset[m_enRiPrdcty];

         m_stCqiResAllocInfo[m_enCqiPrdcty][usUeIndex].cellCqiResInfo[0].usCqiResIdx =
                                                   usCqiResIdx;
         usUeIndex++;
      }
   }
...
S32 CRmRcmCqiMgr::rmRcmCfgCqi(CUEContext *pobjUe)
{
   ...
   (uiTxnMode == RM_UE_TXN_MODE4))
 {
  ...
  pobjUe->setUeCqiRiIndex(uiRiIndex, 1);
 }
 else
 {
  pobjUe->setUeCqiRiIndex(0, 0);
 }
...
{
   return m_stCqiResAllocInfo[m_enCqiPrdcty][usUeIndex].cellCqiResInfo[0].usRiOffset;
}
...
)
{
 ...
 U16 usCqiResIdx;
   ...
   usCqiPmiIdx = m_stCqiResAllocInfo[m_enCqiPrdcty][usUeIndex].cellCqiResInfo[0].usCqiCfgIdx;

 usCqiResIdx = m_stCqiResAllocInfo[m_enCqiPrdcty][usUeIndex].cellCqiResInfo[0].usCqiResIdx;
