                                 ...
                                 U32 uiWindowSize)
{
   ...
   if( L_INFO < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLev2(0x9b0001, L_INFO, m_schReportingInterval, m_windowSize, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lterrm/src/rm_prbreportcollector.cpp"   , 54   , "Reporting Interval=%d WindowSize=%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
                                              ;
   return;
}
...
U32 CPRBReportCollector::getCurrentGbrUlPrbUsage(U32 uiTimePeriod) const
{
   ...
   {
      return((U32) (((m_currentGbrUlPrbUsage) + (m_windowSize)/2) / (m_windowSize)));
   }
   else if(m_curntSlotIndx)
   {
      return((U32) (((m_currentGbrUlPrbUsage) + (m_curntSlotIndx)/2) / (m_curntSlotIndx)));
   }
   return 0;
}
...
U32 CPRBReportCollector::getCurrentGbrDlPrbUsage(U32 uiTimePeriod) const
{
   ...
   if(m_areAllSlotsFilled)
   {
      return((U32) (((m_currentGbrDlPrbUsage) + (m_windowSize)/2) / (m_windowSize)));
   }
   else if(m_curntSlotIndx)
   {
      return((U32) (((m_currentGbrDlPrbUsage) + (m_curntSlotIndx)/2) / (m_curntSlotIndx)));
   }
   return 0;
}
