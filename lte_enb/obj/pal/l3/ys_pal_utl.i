...

{
   ...
   if(txMsgArr->num >= 10)
   {
      { if (ysCb.ysInit.dbgMask >= (2)) { { if ((&ysCb.ysInit)->dbgMask & (2)) { snprintf((&ysCb.ysInit)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", "Ys", (&ysCb.ysInit)->procId, (&ysCb.ysInit)->ent, (&ysCb.ysInit)->inst, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteclpal/ys_pal_utl.c", 97); SPrint((&ysCb.ysInit)->prntBuf); sprintf (ysCb.apb, "%s", "Failed: "); SPrint((&ysCb.ysInit)->prntBuf); } }; { if ((&ysCb.ysInit)->dbgMask & (2)) { snprintf((&ysCb.ysInit)->prntBuf, 255, "[%s %d:0x%x:%x] %s:%d ", "Ys", (&ysCb.ysInit)->procId, (&ysCb.ysInit)->ent, (&ysCb.ysInit)->inst, "/home/mrehwald/Desktop/Radisys/lte_enb/src/lteclpal/ys_pal_utl.c", 97); SPrint((&ysCb.ysInit)->prntBuf); sprintf (ysCb.apb, "can't queue msg\n"); SPrint((&ysCb.ysInit)->prntBuf); } }; } };
      return(1);
   }
