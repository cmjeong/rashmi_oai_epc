...

{
   ...
   if (ueCfg->ueAckNackCfg.isAckNackEnabled)
   {
      ...
      ue->ackNakRepCb.isAckNackEnabled = 1;
...

{
   ...
   if ((reCfg->isMesGapEnabled == 1) &&
         (ueMeasCb->isMesGapEnabled == 1) &&
   ...
   if (ackNackReCfg->isAckNackEnabled)
   {
      ...
      {
         ackNakRepCb->isAckNackEnabled = 1;
   ...
   {
      ackNakRepCb->isAckNackEnabled = 0;
   }
...

{
   ...
   switch (cfg->gapPrd)
   {
      case 40:
         ...
         ue->measGapCb.measQLnk.node = (U32)ue;
      ...
      case 80:
         ...
         break;
      default:
         if( L_ERROR < g_logLevel || g_modMask & RLOG_MODULE_ID){ logLevE(0xa90001,L_ERROR,DBG_CELLID, cell->cellId,ue->ueId, 0, 0, 0,                                     "/home/mrehwald/Desktop/Radisys/lte_enb/src/ltemac/rg_sch_mga.c"         ,                                     350         , "rgSCHMeasGapANRepUeRecfg() Incorrect GAP Period" "CRNTI:%d", RLOG_FILE_ID, RLOG_MODULE_NAME); }
...

{
   ...
   if (ue->measGapCb.isMesGapEnabled)
   {
      ...
      {
         rgSCHTmrStopTmr (cell, 2, ue);
      }

      ue->measGapCb.isMesGapEnabled = 0;
   }


   if (ue->dl.dlInactvMask)
   {
      if (ue->dl.dlInactvMask & 0x01)
      {
         rgSCHTmrStopTmr (cell, 6, ue);
      }
      if (ue->dl.dlInactvMask & 0x02)
      {
         rgSCHTmrStopTmr (cell, 4, ue);
      }
      ue->dl.dlInactvLnk.node = 0L;
   }
   if (ue->ul.ulInactvMask)
   {
      if (ue->ul.ulInactvMask & 0x01)
      {
         rgSCHTmrStopTmr (cell, 5, ue);
      }
      if (ue->ul.ulInactvMask & 0x02)
      {
         rgSCHTmrStopTmr (cell, 3, ue);
      }
      ue->ul.ulInactvLnk.node = 0L;
   }


   if (isUeDel & ue->ackNakRepCb.isAckNakRep)
   {
      rgSCHTmrStopTmr (cell, 1, ue);
...

{
   ...
   for (hqIdx = 0; hqIdx < hqEnt->numHqPrcs; hqIdx++)
   {
      ...
      for (repIdx = 1; repIdx < ue->ackNakRepCb.cfgRepCnt; repIdx++)
      {
         ...
         {
            if ((hqP->tbInfo[tbCnt].crntSubfrm[repIdx] != 0L) &&
                  (hqP->tbInfo[tbCnt].anRepLnk[repIdx].node != 0L))
            {
               cmLListDelFrm(&((hqP->tbInfo[tbCnt].crntSubfrm[repIdx])->
               ackNakRepQ), &hqP->tbInfo[tbCnt].anRepLnk[repIdx]);
               hqP->tbInfo[tbCnt].anRepLnk[repIdx].node = 0L;
               hqP->tbInfo[tbCnt].crntSubfrm[repIdx] = 0L;
            }
         }
      }
   }
...

{
   ...
   ;;
   offset = (cell->crntTime.sfn * 10 + cell->crntTime.subframe + 2) %
      40;
   ...
   rgSCHMeasGapANRepUtlHdlTti (cell, queue);
   do { S32 _subframe; _subframe = cell->crntTime.sfn * 10 + cell->crntTime.subframe; _subframe = _subframe - (4 - 2); if(_subframe < 0) { _subframe = (1024 * 10) + _subframe; } repTime.sfn = _subframe / 10; repTime.subframe = _subframe % 10; } while(0);
...

{
   ...
   while (node)
   {
      ...
      if ((0L != ue) && (ue->ackNakRepCb.isAckNackEnabled))
      {
         ue->ackNakRepCb.isAckNakRep = 1;
         rgSCHTmrStartTmr (cell, ue, 1,
...

{
   ...
   schedTime = cell->crntTime.sfn * 10 + cell->crntTime.subframe + (2 + 0);
   harqFdbkOffset = 4;


   offset = (schedTime + harqFdbkOffset) % 40;
   queue = &(cell->measGapCb.gapPrd40Q[offset]);
   ...
   {
      ue = (RgSchUeCb*)node->node;
      ue->dl.dlInactvMask |= 0x01;

      ue->dl.dlInactvLnk.node = (U32)ue;
      cmLListAdd2Tail (dlInactvUeLst, &(ue->dl.dlInactvLnk));

      rgSCHTmrStartTmr (cell, ue, 6,
            (6 + harqFdbkOffset));
      node = node->next;
   }


   offset = (schedTime + harqFdbkOffset) % 80;
   queue = &(cell->measGapCb.gapPrd80Q[offset]);
   ...
   if(0 == (2 + 0))
   {
     ...
     if ((cell->crntTime.subframe + ((2 + 0))) >= 10) ackNakTime.sfn = (cell->crntTime.sfn + (cell->crntTime.subframe + ((2 + 0)))/10) % 1024; else ackNakTime.sfn = cell->crntTime.sfn; ackNakTime.subframe = (cell->crntTime.subframe + ((2 + 0))) % 10;;

     do { S32 _subframe; _subframe = ackNakTime.sfn * 10 + ackNakTime.subframe; _subframe = _subframe - 1; if(_subframe < 0) { _subframe = (1024 * 10) + _subframe; } ackNakTime.sfn = _subframe / 10; ackNakTime.subframe = _subframe % 10; } while(0);
   }
   else
   {
     if ((cell->crntTime.subframe + (((2 + 0) - 1))) >= 10) ackNakTime.sfn = (cell->crntTime.sfn + (cell->crntTime.subframe + (((2 + 0) - 1)))/10) % 1024; else ackNakTime.sfn = cell->crntTime.sfn; ackNakTime.subframe = (cell->crntTime.subframe + (((2 + 0) - 1))) % 10;
                             ;
   }
   ...
   while (node)
   {
      ue = (RgSchUeCb *)(node->node);
      node = node->next;
      hqNode = ue->dl.dlSfHqInfo[dlSf->dlIdx].hqPLst.first;
      while (hqNode)
      {
...

{
...

   {
      ...
      offset = (schedTime + pdcchToPuschGap -idx + 40) % 40;
      queue = &(cell->measGapCb.gapPrd40Q[offset]);
      node = queue->first;
      while (node)
      {
         ...
         ue->ul.ulInactvLnk.node = (U32)ue;
         cmLListAdd2Tail (ulInactvUeLst, &(ue->ul.ulInactvLnk));




         rgSCHTmrStartTmr (cell, ue, 5,
               (6 + pdcchToPuschGap - idx));
      ...
      offset = (schedTime + pdcchToPuschGap - idx + 80) % 80;
      queue = &(cell->measGapCb.gapPrd80Q[offset]);
   ...
   if ((cell->crntTime.subframe + (2)) >= 10) ackNakTime.sfn = (cell->crntTime.sfn + (cell->crntTime.subframe + (2))/10) % 1024; else ackNakTime.sfn = cell->crntTime.sfn; ackNakTime.subframe = (cell->crntTime.subframe + (2)) % 10;;
...

{
   ...
   {
      case 6:
         if((ue->dl.dlInactvMask == 0)) { ue->dl.dlInactvMask &= ~0x01; } else { ue->dl.dlInactvMask &= ~0x01; if((ue->dl.dlInactvMask == 0)) { rgSCHUtlDlActvtUe(cell, ue); } };
         break;
      case 4:
         if((ue->dl.dlInactvMask == 0)) { ue->dl.dlInactvMask &= ~0x02; } else { ue->dl.dlInactvMask &= ~0x02; if((ue->dl.dlInactvMask == 0)) { rgSCHUtlDlActvtUe(cell, ue); } };
         break;
   }
   if (!ue->dl.dlInactvMask)
   {
...

{
   ...
   {
      case 5:
         if((ue->ul.ulInactvMask == 0)) { ue->ul.ulInactvMask &= ~0x01; } else { ue->ul.ulInactvMask &= ~0x01; if((ue->ul.ulInactvMask == 0)) { rgSCHUtlUlActvtUe(cell, ue); } };
         break;
      case 3:
         if((ue->ul.ulInactvMask == 0)) { ue->ul.ulInactvMask &= ~0x02; } else { ue->ul.ulInactvMask &= ~0x02; if((ue->ul.ulInactvMask == 0)) { rgSCHUtlUlActvtUe(cell, ue); } };
         break;
   }
   if (!ue->ul.ulInactvMask)
   {
...

{
   ...
   while (node)
   {
      ue = (RgSchUeCb *)(node->node);
      node = node->next;
      hqNode = ue->dl.dlSfHqInfo[crntDlSf->dlIdx].hqPLst.first;
      while (hqNode)
      {
         hqP = (RgSchDlHqProcCb *)hqNode->node;
         hqNode = hqNode->next;
