...
#else
PUBLIC S16 SPutStaticBuffer(region, pool, ptr, size, memType)
Region region;                  /* region ID */
Pool pool;                      /* pool ID */
Data *ptr;                      /* pointer to buffer */
Size size;                      /* size */
...
#endif
{
   ...
   ret = SGetSBuf(region, pool, (Data**)&allocPtr,size);
   *ptr = allocPtr;
...
#else
PUBLIC S16 SAddPstMsgMultNew(padBuf,size,sduBuf)
Buffer *padBuf;
MsgLen size;
Buffer *sduBuf;
#endif
{
   ...
   mInfo = (SsMsgInfo *)sduBuf->b_rptr; 
   ssGetDBufOfSize(mInfo->region,size, &padBuf); 
   if (padBuf == NULLP) 
   { 
      printf("padBuf alloc failed\n"); 
      RETVALUE(RFAILED);
   } 
...
PUBLIC Void SIncMsgLen(Buffer *mBuf)
{
   SsMsgInfo *minfo = (SsMsgInfo*)mBuf->b_rptr;
...
{
   S16 ret = ROK;
	ret = SCpyMsgMsg(srcBuf, dstRegion, dstPool, dstBuf);
	RETVALUE(ret);
}
