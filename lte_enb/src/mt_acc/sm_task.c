...
/*
*
*       Fun:   Register a SIGSEGV Interrupt Handler 
*
*       Desc:  This function is used to register a interrupt function 
*              for the layer. The system services will invoke the
*              function when receive SIGSEGV interrupt .
*
*       Ret:   ROK      - ok
*              RFAILED  - failed, general (optional)
*
...
#ifndef SS_MULTIPLE_PROCS
#ifdef ANSI
PUBLIC S16 SRegIntrptHdlr
(
Ent        ent,                   /* entity */
Inst       inst,                  /* instance */
PFS16      intrptActvFn           /* interrupt function, typically SActvInterrupt */
)
#else
PUBLIC S16 SRegIntrptHdlr(ent, inst, intrptActvFn)
Ent        ent;                   /* entity */
Inst       inst;                  /* instance */
PFS16      intrptActvFn;          /* interrupt function, typically SActvInterrupt */
#endif
...
#else
PUBLIC S16 SRegIntrptHdlr(proc, ent, inst, intrptActvFn)
ProcId     proc;                  /* processor */
Ent        ent;                   /* entity */
Inst       inst;                  /* instance */
PFS16      intrptActvFn;          /* interrupt function, typically SActvInterrupt */
...
#endif/*SS_MULTIPLE_PROCS*/
{
   ...
   /*Extract Tapa Task idx from entity Id and Instanace Id*/
#ifdef SS_MULTIPLE_PROCS
   idx = osCp.tTskIds[procIdIdx][ent][inst];
#else
   idx = osCp.tTskIds[ent][inst];
#endif /* SS_MULTIPLE_PROCS */
   /* check if entity and instance registered */
   if (idx == SS_TSKNC)
   {
      SS_RELEASE_ALL_SEMA(&osCp.tTskTblSem);
   ...
   /*Extract Tapa Task from idx */
   tTsk = &osCp.tTskTbl[idx];
   /*check whether already registered*/
   if(tTsk->isIntrptReg != TRUE)
   {
      /*Fill interrupt Atcitation function*/
      tTsk->isIntrptReg     = TRUE;
      tTsk->intrptActvFn    = intrptActvFn;
   ...
   else
   {
      ...
      SS_RELEASE_ALL_SEMA(&osCp.tTskTblSem);
      SSLOGERROR(ERRCLS_DEBUG, ESS395, ERRZERO,
            "Interrupt Actv func Already registered\n");
      RETVALUE(RFAILED);
   }
}
...
/main/1+     ss016.LPR sasahni   1. Added API to register callback for SIGSEGV 
                                    signal with SSI.
*********************************************************************91*/
