...
void CCellManager::procUeRecfgReq(RmuUeRecfgReq & stUeRecfgReq)
{
   ...
   /*CA_DEV*/
#ifdef LTE_ADV
   if( stUeRecfgReq.bRecfgBitMap & RMU_RECFG_REQ_SCELL_CONFIG)
   {   		
		RmuUeScellInfoReq		   *stSCellInfoReq = &stUeRecfgReq.stSCellInfoReq;		
		RmuUeSCellInfoRsp		   *stUeSCellResCfg = &pstUeRecfgRsp->stUeSCellResCfg;
		U32 idx = 0;
		
		for (idx = 0;idx < stSCellInfoReq->numOfScell; idx++)
      {

         /* Configure Aperiodic CQI for Scells*/
         CCellManager* cellManager = 
            g_rObjRRMApp.getCellCtxt(stSCellInfoReq->sCellIInfo[idx].sCellId);
         if (cellManager != NULL)
         {          
            cellManager->setScellApcqi(pUE,idx);
         }           


         if(stSCellInfoReq->sCellIInfo[idx].action != RMU_SCELL_RELEASE)
         {
            stUeSCellResCfg->sCellDedInfo[idx].stCqiRepCfg = 
               pUE->getScellCqiReport(stSCellInfoReq->sCellIInfo[idx].sCellIdx - 1);
         }
         stUeSCellResCfg->sCellDedInfo[idx].sCellIdx = stSCellInfoReq->sCellIInfo[idx].sCellIdx;
         stUeSCellResCfg->sCellDedInfo[idx].stStatusInfo.enStatus = RMU_SUCCESS;
         stUeSCellResCfg->sCellDedInfo[idx].stStatusInfo.enCause = RMU_CAUSE_NOT_APP;
         /*Store ScellIdx in Ue Context*/
         pUE->setScellIdx(idx,stSCellInfoReq->sCellIInfo[idx].sCellIdx);
         pUE->m_sCellIdx2CellIdMap[stSCellInfoReq->sCellIInfo[idx].sCellIdx] = 
            stSCellInfoReq->sCellIInfo[idx].sCellId;
         stUeSCellResCfg->numSCells++;
      }
      pstUeRecfgRsp->bRecfgBitMap |= RMU_RECFG_REQ_SCELL_CONFIG;
      pstUeRecfgRsp->stStatusInfo.enStatus = RMU_SUCCESS;
      pstUeRecfgRsp->stStatusInfo.enCause = RMU_CAUSE_NOT_APP;
      rrmSendUeRecfgRsp(stUeRecfgReq, pstUeRecfgRsp);    
      return;
...
#ifdef LTE_ADV
         pUE->setUeCaCap(stUeRecfgReq.bIsUeCaCap);
#endif
      }
...
#ifdef LTE_ADV
void CCellManager::setScellApcqi(CUEContext* pUE,U8 sCellIdx)
{
   m_objResCfgMgr.setScellApcqi(pUE,sCellIdx);
}
#endif
/********************************************************************30**
