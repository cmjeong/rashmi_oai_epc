...
LrmCause rmAppVldtRadioResCfg(LrmCellConfigData* stCellCfg)
{
   ...
   if (stCellCfg->enSrPrdcty >= LRM_SR_PRDCTY_MAX)
   {
      printf("Configuration failed : enSrPrdcty(%d) Not Supported\n",
         stCellCfg->enSrPrdcty);
   ...
   if(usTotalSrRes < usMaxNumUes)
   {
		...
		RLOG3(L_ERROR,"Total SR Configured(usTotalSrRes)=[%d]:Number SR Required(usMaxNumUes)=[%d]:RM_NUM_SF_PER_SFN=[%ld]\n",
            usTotalSrRes,usMaxNumUes,RM_NUM_SF_PER_SFN);
      
      RLOG3(L_ERROR,"NUM_SR_PER_TTI [%d],SUBFRAME_ASSIGNMENT[%d],SR_PRDCTY [%d]\n",
            bNumSrPerTti, bNumUlSf, abSrPrdcty[stCellCfg->enSrPrdcty]);
   ...
   if (stCellCfg->enCqiPrdcty >= LRM_CQI_PRDCTY_MAX)
   {
      printf("Configuration failed : enCqiPrdCty(%d) Not Supported\n",
         stCellCfg->enCqiPrdcty);
      return LRM_CAUSE_INVALID_CFG_INFO;
   }
   ...
   {
      RLOG2(L_ERROR,"Total CQI Configured(usTotalCqiRes)[%d] Number CQI Required[%d]\n", 
		usTotalCqiRes, usTotalCqiRequired);
		RLOG3(L_ERROR,"Total CQI Configured Calculation(bNumCqiPerTti)=[%d]:(bNumUlSf)=[%d]:abCqiPrdcty=%d\n",
            bNumCqiPerTti,bNumUlSf,abCqiPrdcty[stCellCfg->enCqiPrdcty]);
      retVal = LRM_CAUSE_INVALID_CFG_INFO;
   }
