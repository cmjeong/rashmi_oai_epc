...
#ifdef LTE_ADV
   /* TODO:: do this only if CA is enabled */
   /* Ppuulate the n1 resource table for scell harq ack/nack resources*/

   if(m_bIsCaEnabled)
   {
      usSrN1PucchResStart = m_objRcmSCellMgr.\
                            rmRcmSCellPopulateN1PucchResTbl(usSrN1PucchResStart);
   }
#endif
...
#ifdef LTE_ADV
   /* configure N1 resources for CA */
   /* Sprint3: Add support for F1b */
   if(bSCellActN1ResCfgReq)
   {
      m_objRcmSCellMgr.rmRcmConfigureSCellAckN1Res(stUeRadioResCfg,pUE);
   }
#endif  
...
{
   Bool bIsSCellN1ToBeCfgd = FALSE; /* TODO:: need to handle this
                                       at the time of HO activity */
	...
	)
{
   ...
   if(bResCfgMask & RM_RCM_RES_CFG_CQI)
   {
      ...
      else
      {
         ...
         if((retVal == ROK) && m_bIsCaEnabled)
         {
            retVal = m_objRcmCqiMgr.rmRcmScellCfgCqi(pUE);
         }
#endif
      }
   }
...
S32 CRmRsrcCfgMgr::rmRcmRecfgUeRes(CUEContext* pUE, RmuUeRecfgReq & stUeReCfgReq,RmuUeRecfgRsp *pstUeRecfgRsp)
{
   S32 retVal;
#ifdef LTE_ADV
   Bool bIsSCellN1ToBeCfgd = stUeReCfgReq.bRecfgBitMap & RMU_RECFG_REQ_SCELL_ACK_N1RES;
#else
   Bool bIsSCellN1ToBeCfgd = FALSE;
#endif
...
#ifdef LTE_ADV 
void CRmRsrcCfgMgr::setScellApcqi(CUEContext* pUE,U8 sCellIdx)
{
   m_objRcmCqiMgr.rmRcmCfgScellApcqi(pUE,sCellIdx);
}
#endif
/********************************************************************30**
