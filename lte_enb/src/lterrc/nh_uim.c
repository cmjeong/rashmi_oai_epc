   ...
   if (nhuCfgReqSdu->cfgInfo.ueCfgBitMsk & NHU_EXP_CFG_UE_CFG)
   {
      ...
      if(ueCb == NULLP) 
      {
         ...
         if (ROK != ret)
       {
         ...
         createUeCtxt = TRUE;
         /* Whenever UE context is createdi, RRC should store Multi SAP
          * information in its UE control block. This Sap information will e
          * used by RRC to communicate with lower layer like MAC, RLC and PDCP
          * for any UE specific message(i.e DL-DCCH) 
          */
#ifdef TENB_MULT_CELL_SUPPRT
         nhDbmCpyUeLSapInfo(&nhuCfgReqSdu->cfgInfo.uellyrSapIdInfo, ueCb);
#endif
