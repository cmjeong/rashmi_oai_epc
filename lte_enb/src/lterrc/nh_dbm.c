...
/**
 * @brief Copy UE Lsap Information which is given by RRC user.
 * 
 * @details 
 * 
 *
 * This function creates and initialises the UE context.
 * @param[in]  uellyrSapIdInfo       RLC, MAC, PDCP instance/Sap information
 * @param[out] ueCb                  UE context
 *
 * @return S16
 *  -# ROK
...
*/
#ifdef ANSI
PUBLIC Void nhDbmCpyUeLSapInfo
(
NhuUeLLyrSapIdInfo      *nhuUeLSapInfo,
NhActvUeCb              *ueCb       
)
#else /* ANSI */
PUBLIC Void nhDbmCpyUeLSapInfo(nhuUeLSapInfo, ueCb)
NhuUeLLyrSapIdInfo    *nhuUeLSapInfo;
NhActvUeCb            *ueCb;      
#endif /* ANSI */
{
  
   NhuUeLLyrSapIdInfo    *nhuSapInfo;
   TRC2(nhDbmCpyUeLSapInfo)

   nhuSapInfo  = &(ueCb->crntCfg.uellyrSapIdInfo);
   /* SuId */
   nhuSapInfo->kwuSapId = nhuUeLSapInfo->kwuSapId;
   nhuSapInfo->ckwSapId = nhuUeLSapInfo->ckwSapId;
   nhuSapInfo->crgSapId = nhuUeLSapInfo->crgSapId;
   nhuSapInfo->cpjSapId = nhuUeLSapInfo->cpjSapId;
   nhuSapInfo->pjuSapId = nhuUeLSapInfo->pjuSapId;
   nhuSapInfo->macRguUlSapId = nhuUeLSapInfo->macRguUlSapId;
   nhuSapInfo->macRguDlSapId = nhuUeLSapInfo->macRguDlSapId;

   /* SpId */
   nhuSapInfo->rlcRguSapId = nhuUeLSapInfo->rlcRguSapId;
   nhuSapInfo->rlcKwuUlSapId = nhuUeLSapInfo->rlcKwuUlSapId;
   nhuSapInfo->rlcKwuDlSapId = nhuUeLSapInfo->rlcKwuDlSapId;
