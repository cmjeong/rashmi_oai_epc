...
#ifdef LTE_ADV
   /*Fill the Sec info here from the Transport Block
    * Only if Num SCells are Greater than ZERO
    * RRC will send ModMac configuration using the UeRecfg primitives*/
   if(transCtrlBlk->ueSCellCfgInfo.numSCells > 0)
   {
      modCfgType |= NH_CRG_UE_MAC_SCELL;
   }
#endif /* LTE_ADV */
