...
#endif /* ANSI */
{
   ...
   if (nhDataReqParam->isPdcpDatReq)
   {
      ...
      NH_GET_UE_SUID_FOR_MULT_CELL_SUPPRT(suId, cellAndUeCb->ueCb, STNHPJUSAP);
...
#ifdef TIP_L3
      if(CM_LTE_LCH_CCCH == nhDataReqParam->u.rlcParam.lcType)
      {
         //kwuDatReqParam->rlcId.ueId = nhIdentity->ueId;  /* NOTE:: Aricent UE needs UE CRNTI in RLC_CCCH_MSG_REQ */
         kwuDatReqParam->rlcId.ueId = nhDataReqParam->u.rlcParam.u.ccchInfo.rnti;
      }
      else
      {
...
/**
 *
 *   @brief Function to fill RLC Cell Sap Configuration.
 *
 *   @details
 *   This function is used to fill RLC SapId for each RB.
 *            
 *
 *   @param[out] ckwCfgParam     RLC configuration patameter
 *   @param[in]  rgusapId        RGUSAP Id
 *   @return  S16
 *      -# ROK 
 *      -# RFAILED 
 **/      
#ifdef ANSI
PRIVATE S16 nhPIMFillRlcSapIdInfo 
(
CkwCfgInfo     *ckwCfgParam,
SpId            rguSapId 
)
#else /* ANSI */
PRIVATE S16 nhPIMFillRlcSapIdInfo(ckwCfgParam, rguSapId) 
CkwCfgInfo     *ckwCfgParam;
SpId           rguSapId;
#endif /* ANSI */
{
   ...
   TRC2(nhPIMFillRlcSapIdInfo)

   RLOG0(L_DEBUG, "nhPIMFillRlcSapIdInfo");
   
   for (rbCount = 0; ((rbCount < ckwCfgParam->numEnt) &&
            (rbCount < CKW_MAX_ENT_CFG)); rbCount++)
   {
      ckwCfgParam->entCfg[rbCount].rguSapId = rguSapId;
   } /* end of for loop */
   RETVALUE(ROK);
} /* end of nhPIMFillRlcSapIdInfo */
...
#endif /* ANSI */
{
   ...
   /* Get lower SAP*/
#ifdef TENB_MULT_CELL_SUPPRT
    NH_GET_UE_SUID_FOR_MULT_CELL_SUPPRT(suId, ueCb, STNHCKWSAP);
#endif /* TENB_MULT_CELL_SUPPRT */       
...
#ifdef TENB_MULT_CELL_SUPPRT
       nhPIMFillRlcSapIdInfo(ckwCfgParam, 
                               ueCb->crntCfg.uellyrSapIdInfo.rlcRguSapId);
#endif
...
#ifdef TENB_MULT_CELL_SUPPRT
            nhPIMFillRlcSapIdInfo(ckwCfgParam, cellCb->llyrSapIdInfo.rguSapId);
#endif
...
#endif /* ANSI */
{
   ...
   /* Get lower SAP */
#ifdef TENB_MULT_CELL_SUPPRT
   NH_GET_UE_SUID_FOR_MULT_CELL_SUPPRT(suId, ueCb, STNHCRGSAP);
#endif /* TENB_MULT_CELL_SUPPRT */       
...
#ifdef LTE_ADV
   if (cfgType & NH_CRG_UE_MAC_SCELL)
   {
      if (transCtrlBlk->ordrdCfg.actionMask & NH_ORD_CFG_SCELL_MAC_CFG)
      {
         /* Allocate memory for cfg Parameter */
         NH_ALLOC(NH_REG, NH_POOL, &crgCfgParam, sizeof(CrgCfgReqInfo));
         if (NH_CHK_INPUT_PARAM(crgCfgParam))
         ...
         crgCfgParam->u.recfgInfo.u.ueRecfg.cellId = ueCb->cellCb->cellId;
         crgCfgParam->u.recfgInfo.u.ueRecfg.oldCrnti = ueCb->ueCbKey.crnti;
         crgCfgParam->u.recfgInfo.u.ueRecfg.newCrnti = ueCb->ueCbKey.crnti;
         
         crgCfgParam->u.recfgInfo.u.ueRecfg.crgSCellCfg.numSCells =
            transCtrlBlk->ueSCellCfgInfo.numSCells;
         crgCfgParam->u.recfgInfo.u.ueRecfg.crgSCellCfg.isSCellCfgPres = TRUE;

         for(i = 0; i < transCtrlBlk->ueSCellCfgInfo.numSCells; i++)
         {
            crgCfgParam->u.recfgInfo.u.ueRecfg.crgSCellCfg.ueSCellCfg[i].sCellId = 
               transCtrlBlk->ueSCellCfgInfo.ueSCellCfg[i].sCellId; 
            crgCfgParam->u.recfgInfo.u.ueRecfg.crgSCellCfg.ueSCellCfg[i].macInst = 
               transCtrlBlk->ueSCellCfgInfo.ueSCellCfg[i].macInst; 
            crgCfgParam->u.recfgInfo.u.ueRecfg.crgSCellCfg.ueSCellCfg[i].rguUlSapId = 
               transCtrlBlk->ueSCellCfgInfo.ueSCellCfg[i].rguUlSapId; 
            crgCfgParam->u.recfgInfo.u.ueRecfg.crgSCellCfg.ueSCellCfg[i].rguDlSapId = 
               transCtrlBlk->ueSCellCfgInfo.ueSCellCfg[i].rguDlSapId; 
         }

         NH_GET_MAC_CFG_REQ_TRANSID(crgCfgTransId.trans, ueCb->cellCb->cellId,
               ueCb->ueCbKey.crnti, transCtrlBlk->nxTransId, CRG_UE_CFG, 
...
#ifdef TENB_MULT_CELL_SUPPRT
      crgCellConfig->rguUlSapId = cellCb->llyrSapIdInfo.rguUlSapId;
      crgCellConfig->rguDlSapId = cellCb->llyrSapIdInfo.rguDlSapId;
#endif
      /* Construct transaction ID */
      NH_GET_MAC_CFG_REQ_TRANSID(crgCfgTransId.trans, 
...
#ifdef TENB_MULT_CELL_SUPPRT
      crgCfgParam->u.cfgInfo.u.ueCfg.rguUlSapId = ueCb->crntCfg.uellyrSapIdInfo.macRguUlSapId;
      crgCfgParam->u.cfgInfo.u.ueCfg.rguDlSapId = ueCb->crntCfg.uellyrSapIdInfo.macRguDlSapId;
#endif
...
#endif /* ANSI */
{
   ...
   /* Get lower SAP */
#ifdef TENB_MULT_CELL_SUPPRT
   NH_GET_UE_SUID_FOR_MULT_CELL_SUPPRT(suId, ueCb, STNHCPJSAP);
#endif /* TENB_MULT_CELL_SUPPRT */       
...
#ifdef TENB_MULT_CELL_SUPPRT
   cpjCfgReq->rlcUlSapId=ueCb->crntCfg.uellyrSapIdInfo.rlcKwuUlSapId;
   cpjCfgReq->rlcDlSapId=ueCb->crntCfg.uellyrSapIdInfo.rlcKwuDlSapId;
#endif
   RETVALUE(ROK);
...
#endif /* ANSI */
{
  ...
  /* No need of Sending MAC UE Del Request in case of TIP, as it is taken care
   * as part of Sch  Ue Del */
#ifndef TIP_L3
   /* Get lower SAP */
#ifdef TENB_MULT_CELL_SUPPRT
      NH_GET_UE_SUID_FOR_MULT_CELL_SUPPRT(suId, ueCb, STNHCRGSAP);
...
#endif /* ANSI */
{
   ...
   switch(rbCfgInfo->dlRbCfg.dlRlcMode.mode)
   {
      case NHU_RLC_MODE_AM:
           ...
           if (NHU_POLL_PDU_INF == rbCfgInfo->dlRbCfg.dlRlcMode.m.amRlc.pollPdu)
           {
              //rbCfgInfo->dlRbCfg.dlRlcMode.m.amRlc.pollPdu = (NhuPollPdu)0xFFFF; /*NOTE:: Changed as per ARICENT requirement */
              ckwCfgParam->entCfg[rbCount].m.amInfo.dl.pollPdu = (S16)0xFFFF;
           }
           else
           {
