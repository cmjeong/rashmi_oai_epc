...
if(suId == 1)
   ...
   NhLiKwuBndCfm(&pst, suId, CM_BND_OK);
   RETVALUE(ROK);
#else
...
#endif /* ANSI */
{
   ...
   TRC3(NhLiKwuDatReq);
#ifdef TIP_L3
   if(CM_LTE_LCH_CCCH == datReqParam->lcType)
   {
      RETVALUE(tipSendCcchMsgReq(post, spId, datReqParam, datBuf));
   }
   else
   {
      RETVALUE(tipSendPcchMsgReq(post, spId, datReqParam, datBuf));
   }
#else
...
Pst   pst;                       /* Post structure */
   ...
   NhLiCkwBndCfm(&pst, suId, CM_BND_OK);
   RETVALUE(ROK);
#else
...
#endif /* ANSI */
{
   TRC3(NhLiCkwCfgReq);
#ifdef TIP_L3
   RETVALUE(tipSendRlcCfgReq(post, spId, cfgInfo));
#else
...
#ifdef TIP_L3
  RETVALUE(tipSendRlcUeIdChgReq(post, spId, transId, oldUeInfo, newUeInfo));
#else
...
Pst    pst;                       /* Post structure */
   ...
   if(suID < wrSmDfltNumCells)
   {
      NhLiCrgBndCfm(&pst, suID, CM_BND_OK);
      suID++;
   }
   else
   {
...
Pst  pst;                       /* Post structure */
   ...
   NhLiCpjBndCfm(&pst, suId, CM_BND_OK);
   RETVALUE(ROK);
#else
...
if(suId == 1)
   ...
   NhLiPjuBndCfm(&pst, suId, CM_BND_OK);
   RETVALUE(ROK);
#else
