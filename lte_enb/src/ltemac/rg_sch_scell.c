...
/************************************************************************
     ...
     Sid:      gk_sch_scell.c@@/main/2 - Sat Jul 30 02:21:48 2011
...
#endif
{
   ...
   {
      /* timer values should be timer - (time at hQfdbk - time at
       * CE is sent) 
       * */
   ...
   crntTime = (ueCb->cell->crntTime.sfn * RGSCH_NUM_SUB_FRAMES)+
...
#endif
{
   ...
   cell  = ueCb->cell;
#ifdef CA_DBG
   printf("\n  Recvd fdbk for Act CE fdbk is %d\n",fdbk);
#endif
   switch(fdbk)
   {
      case TFU_HQFDB_ACK:
...
#endif
{
   ...
   crntTime = (priCellCb->crntTime.sfn * RGSCH_NUM_SUB_FRAMES)+
                  (priCellCb->crntTime.subframe);
