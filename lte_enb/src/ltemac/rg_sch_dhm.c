...
/* LTEMAC_SPS_AN_MUX*/
  ...
  for(idx=0;idx < hqCnt; idx++)
  {
     ...
     for (tbCnt = tbStrtIdx[idx]; (tbCnt-tbStrtIdx[idx]) < numTb[idx]; tbCnt++)
     {
        ...
        if(NULLP != ue)
        {
           uciFrmtTyp = ue->dl.dlSfHqInfo[sf->dlIdx].uciFrmtTyp;
        }
...
#endif
{
...
#ifdef LTE_ADV
   sCellActCePres = hqP->tbInfo[tbCnt].schdSCellActCe.pres; 
#endif
      ...
      (sCellActCePres == TRUE))
   {
      ...
      * sec cell. THis needs to be considered
      * while UL_CA*/
     rgSCHSCellHndlFdbkInd(hqP, tbCnt,
           isAck, maxHqRetxReached);
   }
#endif
   RETVALUE(ret);
 ...
 **/
#ifdef ANSI
PRIVATE Void rgSchGetHqFdbkPos
(
 RgSchCellCb          *cell,
 RgSchUeCb            *ue,
 RgSchDlHqProcCb      *hqP,
...
#else
PRIVATE Void rgSchGetHqFdbkPos(cell,ue,hqP,uciFrmtTyp,isAck,fdbk)
 RgSchCellCb          *cell;
 RgSchUeCb            *ue;
 RgSchDlHqProcCb      *hqP;
 RgrSchFrmt1b3TypEnum uciFrmtTyp;
