...
typedef struct rgLaaTbReq
{
   ...
   CmLListCp         laaTbInfoLst;  /*!< List of type RgLaaTbInfo */
} RgLaaTbReq;
...
/** 
  * @brief
  * Saved Hq Proc TB information
  */
typedef struct rgDlSavedHqProcInfo
{
   CmLList         savedHqPLnk;  /*!< Used to add to savedProcs list */
   U16             saveId;       /*!< Uniquely identify a saved HqP info */
#ifndef L2_OPTMZ
   Buffer          *tb[RG_MAX_TB_PER_UE];    /*!< Multiplexed Pdu */
#else
   RgTfuDatReqTbInfo  tb[RG_MAX_TB_PER_UE];   /* TB to be sent to CL/PHY*/
#endif
}RgDlSavedHqProcInfo;
...
struct rgDlHqEnt
{
   ...
   CmLListCp    savedProcLst[RG_MAX_DL_HARQ_NUM]; /*!< Saved TBs for the release Hq Procs*/
};
...
EXTERN S16 rgDHMSndConsolidatedStaInd ARGS ((RgCellCb *cell,RgInfUeInfo *ueInfo,
      CmLteTimingInfo timingInfo, RgErrInfo *err, RgLaaTbReq
      **laaCellHqPLst));
#else
EXTERN S16 rgDHMSndConsolidatedStaInd ARGS ((RgCellCb *cell,RgInfUeInfo *ueInfo,
...
EXTERN S16 RgSchMacHndlRelReq ARGS (( RgCellCb *cell, RgUeCb *ue, RgInfUeHqInfo  *ueHqInfo));
EXTERN Void rgDHMFreeSavedHqP ARGS (( Inst inst, RgDlHqEnt *hqE, U8 procId));
EXTERN Void rgLaaChkAndReqTbs  ARGS((RgDlSf *dlSf,RgLaaTbReq** laaCellHqPLst,
                                    Inst pMacInst));
EXTERN Bool rgLaaChkAllRxTbs ARGS
((
 RgDlSf   *dlSf
 ));
EXTERN S16 rgLaaPushHqPToScellLst ARGS
((
RgInfUeAlloc  *allocInfo,
RgLaaTbReq**    laaCellHqPLst,
CmLteTimingInfo   timingInfo
));
EXTERN Void rgLaaInitTbInfoLst ARGS
((
