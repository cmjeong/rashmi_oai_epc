...
PRIVATE Void rgSCHDlfsType0RbAssgn ARGS((
        RgSchDlfsCellCb *, RgSchDlfsSf *, RgSchCellCb *, RgSchUeCb *, 
        RgSchDlRbAlloc *, U32 *, U32 *, U32 *));
...
#endif
{
   S16             ret       = ROK;
   Inst            instIdx   = cell->instIdx;
   RgSchDlfsUeCb   *dlfsUe   = RG_SCH_DLFS_GET_UE(ue, cell);
   RgSchDlfsCellCb *dlfsCell = RG_SCH_DLFS_GET_CELL(cell);
...
#else
PUBLIC Void rgSCHDlfsDlCqiIndHndlr(cell, ue, isPucchInfo, cqiIndInfo, timingInfo)
RgSchCellCb          *cell;
RgSchUeCb            *ue;
Bool                 isPucchInfo;
Void                 *cqiIndInfo;
...
#endif
{
   ...
   /*LAA: DLFS not supported on LAA SCell currently*/
   if(cell->rgSchLAACellCfg.isLaaCell)
   {
      RLOG_ARG0(L_ERROR,DBG_CELLID,cell->cellId,
            "DLFS not supported for LAA SCell");
      RETVOID;
   }
...
#endif
{
   ...
   dlfsUe   = RG_SCH_DLFS_GET_UE(ue, cell);
...
#else
PRIVATE Void rgSCHDlfsType0RbAssgn(dlfsCell, dlfsSf, cell, ue, allocInfo, 
                totAllocRbs, pbchOvrlpRbs, dlAllocBitMask)
RgSchDlfsCellCb *dlfsCell;
RgSchDlfsSf     *dlfsSf;
RgSchCellCb        *cell;
...
#else
PUBLIC Void rgSCHDlfsOdrUeForRbAssgnmt(cell, lCp, hqP)
RgSchCellCb *cell;
CmLListCp          *lCp;
RgSchDlHqProcCb    *hqP;
#endif
{
   RgSchDlfsUeCb    *dlfsUe = RG_SCH_DLFS_GET_UE(hqP->hqE->ue, cell);
   ...
   while(lstNode)
   {
      ...
      lstDlfsUe = RG_SCH_DLFS_GET_UE(lstHqP->hqE->ue, cell);
      allocInfo = RG_SCH_CMN_GET_ALLOCCB_FRM_UE(lstHqP->hqE->ue, cell);
