...
typedef struct rgSchUeCatTbl
{
   U32    maxUlBits;/*Maximum number of
                      ...
                      TTI*/
   U32    maxDlBits[2];/*Maximum number of 
                      bits of a DLSCH
                      transport block 
...
/* LTE_ADV_FLAG_REMOVED_END */
   ...
   CmLListCp         laaSCellUeLst;/*!< LAA_SCELL: UE List which are scheduled in Perticular SF on LAA SCell */
...
/**
  @brief LAA related TB details. */
typedef struct rgSchLaaTBInfo
{
   CmLList           laaTbsLnk;   /*!< To link to the errInd list */
   U8                procId;      /*!< Harq process ID */
   U8                numTBs;      /*! Number of TBs*/
   U32               tbSz[2];     /*!< TB size scheduled for this TB */
   U16               hqPStamp;    /*! reqd for identifying TBS in the
                                      SCell when the TBs have been
                                      queued from the same HqP in the
                                      PCell multiple times*/
   RgSchUeCb         *ue;         /*!< Ue control block*/
   RgSchCellCb       *laaSCell;   /*!< Pointer to the LAA SCELL cell cb*/
}RgSchLaaTBInfo;

/**
  @brief RB Alloc info reqd for scheduling LAA related TB details. */
typedef struct rgSchLaaDlRbAlloc 
{
   Bool isLaaTB;                  /*!< Flag to indicate LAA TB moved from
                                    SCell. Will be used in the PCell*/
   RgSchLaaTBInfo *laaTBInfo;     /*!< Control block containing the moved TB
                                    details*/
}RgSchLaaDlRbAlloc;
...
#endif
/*LAA_PCELL*/
   RgSchLaaDlRbAlloc laaDlRbAlloc; /*!< LAA UE Alloc info*/
} RgSchDlRbAlloc;
...
/**
  * @brief
  * Downlink HARQ process information for LAA UE.
  */

typedef struct rgSchLaaHqP
{
   CmLList              hqPToShiftLnk;     /*!< To link to the hqPToShiftChkQ lists
                                            reqd on SCell*/
   CmLList              errIndHqpLnk;     /*!< To link to the errInd list,
                                            reqd on SCell*/
   U8                   procId;           /*!< SCell procId reqd when
                                            scheduling on PCell*/ 
   U8                   sCellMacInstId;   /*! MAC instId of the LAA SCelli,
                                            reqd when scheduling on PCell*/
   U8                   tbId;             /*! reqd when scheduling on PCell,
                                            in the case only one of 2 TBs can
                                           be scheduled due to RI*/
   U16                  hqPStamp;         /*! reqd for identifying TBS in the
                                            SCell when the TBs have been
                                            queued from the same HqP in the
                                            PCell multiple times*/
   U8                   overflowCycles;   /*!< This is set when expiry time is more than
                                             RG_SCH_MAX_HQP_SHIFT_Q_SZ */
   U8                   hqPToShiftIdx;    /*!< Expiry index into hqPToShiftChkQ for this
                                             hqP */
}RgSchLaaHqP;
...
/*f1b_Sprint3*/
   ...
   U32               isPuschFdbk;
#endif
   /*LAA: harq proc info for LAA processing*/
   RgSchLaaHqP laaHqP;             /*!< Harq proc info required for processing
                                     on PCell and SCell */
};
...
/*f1b_Sprint3*/
   ...
   CmLList                dlSfLaaUeLnk; /*!<LAA_SCELL: list entry into dlsf->ueLst */
...
/**
  * @brief
  * Control block for LAA cell.
  */
typedef struct rgSchLaaCellCb
{
   CmLListCp       errIndHqpLst;          /*!< List of hqPs having error ind */
   CmLListCp       laaTbsToBeSchedLst;    /*!< List of info for LAA TBs to be
                                            scheduled on PCell*/
   CmLListCp       hqPToShiftChkQ[RG_SCH_MAX_HQP_SHIFT_Q_SZ]; /*!< List of hqPs having error ind 
                                                            indexed based on max time to try sched
                                                            in LAA SCELL*/
}RgSchLaaCellCb;
...
/* LTE_ADV_FLAG_REMOVED_END */
   ...
   U32              totalTime;
#endif
   /*LAA: Adding LAA flag and LAA cell info*/
   RgrLteUCfg  rgSchLAACellCfg; /*!< LAA_SCELL: LAA SCell Configuration information */
   RgSchLaaCellCb        laaCellCb;   /*!< LAA_SCELL: LAA SCell specific Control block for the cell */
};
...
struct _rgSchCb
{
   ...
   CmLListCp     laaMovedTBNodeLst;          /*!< List holding nodes to be
                                               used for LAA moved TB info*/ 
};
...
EXTERN S16 rgSCHUtlRgrCfgCfm ARGS ((Inst inst, SpId spId,
                                  RgrCfgTransId transId,U8 status, U8 action, U8 cfgType));
#else
EXTERN S16 rgSCHUtlRgrCfgCfm ARGS ((Inst inst, SpId spId,
                                  RgrCfgTransId transId,U8 status));
#endif
...
EXTERN S16 rgSCHLaaErrInd ARGS ((
RgSchCellCb     *cell,
TfuErrIndInfo    *errInd
));

EXTERN Void rgSCHLaaSch ARGS ((
RgSchCellCb     *cell
));

EXTERN Bool rgSCHLaaSCellEnabled ARGS ((
 RgSchCellCb        *cell
 ));

EXTERN S16 rgSCHLaaSCellCbInit ARGS ((
 RgSchCellCb        *cell
 ));

EXTERN Void rgSCHLaaSCellCbDeInit ARGS ((
 RgSchCellCb        *cell
 ));
