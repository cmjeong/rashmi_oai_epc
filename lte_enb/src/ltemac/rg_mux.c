...
#endif
{
   ...
   if (NULLP != pdu->contResId)
   {
      if(pdu->schdTbSz >= RG_CRES_ELM_LEN)
      {
         ...
         if(ret != ROK)
         {
            ...
            RGDBGERRNEW(inst,(rgPBuf(inst), "RGERR_MUX_BLD_CEHDR_FAIL \n"));
            RETVALUE(RFAILED);
         }
   ...
   if (TRUE == pdu->ta.pres)
   {
      if(pdu->schdTbSz >= RG_TA_ELM_LEN)
      {
         ...
         {
            err->errCause = RGERR_MUX_BLD_CE_FAIL;
            RGDBGERRNEW(inst,(rgPBuf(inst), "RGERR_MUX_BLD_CE_FAIL \n"));
            RETVALUE(RFAILED);
         }
   ...
   if(TRUE == pdu->sCellActCe.pres)
   {
      if(pdu->schdTbSz >= RG_SCELL_CE_ELM_LEN)
      {
         ...
         if(pdu->schdTbSz > RG_SCELL_CE_ELM_LEN)
         {
            RG_PACK_CE_SHDR(subHdr, RG_SCELL_LCID_IDX, TRUE, ceShdrBuf, ret);
         }
         else
         {
            RG_PACK_LAST_CE_SHDR(subHdr, RG_SCELL_LCID_IDX, ceShdrBuf, ret);
         }
...
#else
PRIVATE S16 rgMUXInsSdu(inst,schdTbSz, lcId, sdu, sHdrBuf, sduBuf, err)
Inst           inst;
MsgLen         *schdTbSz;
U8             lcId;
Buffer         *sdu;
...
#else
PRIVATE S16 rgMUXAddPadd(inst,schdTbSz, sHdrBuf, sduBuf, isRar, err)
Inst           inst;
MsgLen         *schdTbSz;
Buffer         *sHdrBuf;
Buffer         *sduBuf;
...
#else
PRIVATE S16 rgMUXAddSdus(inst,pdu, sHdrBuf, sduBuf, err)
Inst           inst;
RgBldPduInfo   *pdu;
Buffer         *sHdrBuf;
Buffer         *sduBuf;
...
#endif
{
   ...
   if(rgMUXAddPadd(inst,&pdu->schdTbSz, sHdrBuf, sduBuf, FALSE, err) != ROK)
   {
...
#endif
{
   ...
   if(rgMUXAddSdus(inst, pdu, mBuf1, mBuf2, err) != ROK)
   {
      RG_FREE_MSG(mBuf1);
      RG_FREE_MSG(mBuf2);
      err->errType = RGERR_MUX_BLD_PDU;
...
//               printf("2222Received Type = %d %d \n", pdu->reqType, pdu->schdTbSz);
   ...
   if(rgMUXAddPadd(inst, &pdu->schdTbSz, sHdrBuf, NULLP, FALSE, err) != ROK)
   {
...
#endif
{
   ...
   */
   ...
   if ((ret = rgGetMsg(inst,&hdrBuf)) != ROK)
   {
      /* Buffer couldnt get allocated. Return a failure */
      err->errCause = RGERR_MUX_MEM_ALLOC_FAIL;
   ...
   if(rgMUXAddPadd(inst,&schdTbSz, hdrBuf, datBuf, TRUE, err) != ROK)
   {
      RG_FREE_MSG(hdrBuf);
      RG_FREE_MSG(datBuf);
