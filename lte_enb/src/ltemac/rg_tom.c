...
#endif
{
   ...
   if(cellCb->ulSf[subframe].ueUlAllocInfo != NULLP)
   {
      /*ccpu00117052 - MOD - Passing double for proper NULLP
                             assignment */
      rgFreeSBuf(inst,(Data **)&(cellCb->ulSf[subframe].ueUlAllocInfo), 
...
#else
PRIVATE S16 rgHndlSchdUe(cell, timingInfo, ueInfo, err, laaCellHqPLst) /* laa_ut_fix */
RgCellCb            *cell;
CmLteTimingInfo     timingInfo;
RgInfUeInfo         *ueInfo;
RgErrInfo           *err;
...
#ifdef LTE_ADV
   rgDHMSndConsolidatedStaInd(cell, ueInfo, timingInfo, err,laaCellHqPLst);
#else
...
#endif
{
   ...
   if((ulSf->ueUlAllocInfo == NULLP) && (ueInfo->numUes > 0))
   {
       ...
       if((ret = rgAllocSBuf(inst,(Data**)&(cell->ulSf[timingInfo.subframe].
...
#endif
{
   ...
   Inst           inst;
#ifdef LTE_ADV
   RgLaaTbReq*    laaCellHqPLst[RG_MAX_SCELL_PER_UE]; 
#endif
...
#ifdef LTE_ADV
   rgLaaInitTbInfoLst(laaCellHqPLst);
#endif
...
#ifdef LTE_ADV
   rgHndlSchdUe(cell, sfInfo->timingInfo, &sfInfo->ueInfo, &err,laaCellHqPLst);
   if (RG_TIMEINFO_SAME(cell->crntTime, dlSf->schdTime))
   {
      rgLaaChkAndReqTbs(dlSf,laaCellHqPLst, inst);
   }
