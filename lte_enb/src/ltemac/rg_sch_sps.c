...
(((RgSchCmnUe *)(((ue)->cellInfo[0])->sch))->dl.dlSpsInfo).dlSpsStatus = stateVal;\
...
(((RgSchCmnUe *)(((ue)->cellInfo[0])->sch))->dl.dlSpsInfo).dlSpsStatus = state;
#endif
...
TfuDciFormat rgSchCmnSpsRaTypeToDciFrmt[9][3] = {
   {TFU_DCI_FORMAT_1, TFU_DCI_FORMAT_1, TFU_DCI_FORMAT_1A},
   {TFU_DCI_FORMAT_1, TFU_DCI_FORMAT_1, TFU_DCI_FORMAT_1A},
   {TFU_DCI_FORMAT_2A,TFU_DCI_FORMAT_2A, TFU_DCI_FORMAT_1A},
   {TFU_DCI_FORMAT_2, TFU_DCI_FORMAT_2, TFU_DCI_FORMAT_1A},
...
#else /* ANSI */
PUBLIC Void rgSCHCmnSpsDlCqiIndHndlr(cell, ue, timingInfo)
RgSchCellCb          *cell;
RgSchUeCb            *ue;
CmLteTimingInfo      timingInfo;
#endif /* ANSI */
...
#endif /* ANSI */
{
   ...
   while (dlSf->remUeCnt && (node != NULLP))
   {
      ...
      dlRbAllocInfo =  RG_SCH_CMN_GET_ALLOCCB_FRM_UE(ue, cell);
...
#endif /* ANSI */
{
   ...
   if (RG_SCH_CMN_IS_UE_SPS_SCHDLD(ue, cell, timingInfo))
   {
      RETVALUE(TRUE);
   }
...
#endif /* ANSI */
{
   ...
   while (listNode)
   {
      ...
      if (!dlSpsUe->isRelPdcchSent)
      {
         ...
         relPdcch->dlDai = rgSCHCmnUpdDai(ue, &ue->relPdcchFbkTiming,
                                 ue->relPdcchValm, TRUE,NULLP,NULLP);
#endif
...
#endif /* ANSI */
{
   RgSchDlRbAlloc    *dlRbAllocInfo = NULLP;
   CmLList          *node;
   RgSchCmnDlCellSpsInfo *dlSpsCell = RG_SCH_CMN_SPS_GET_DL_CELL(cell);
   RgSchUeCb        *ue;
   RgSchDlHqProcCb  *hqP;
...
#endif /* ANSI */
{
   ...
   anInfo = rgSCHUtlGetUeANFdbkInfo(ue, 
                     &ue->relPdcchFbkTiming,RGSCH_PCELL_INDEX);
   if(anInfo)
   {
...
#else /* ANSI */
PRIVATE U8 rgSCHCmnSpsUlGetITbs(cell, ue, isEcp)
RgSchCellCb     *cell;
RgSchUeCb        *ue;
Bool             isEcp;
#endif /* ANSI */
...
#endif /* ANSI */
{
   ...
   for(;ueLnk; ueLnk= ueLnk->next)
   {
...
#endif
      ue->ul.ulSpsInfo.state = RGSCH_SPS_REL_SENT;
      RLOG_ARG3(L_DEBUG,DBG_CELLID,cell->cellId,
      ...
      {
         ue->ul.ulSpsInfo.state = RGSCH_SPS_IDLE;
         ue->ul.explicitRelCnt = 0;
         ...
         if (ue)
         {
            do {
               ...
               RGSCHDBGSPSSTATS("DL SPS:CRNTI %d SPSRNTI %d STATE %u:Cqi/ScCqi[%d/%d]"
                     " SpsSchd %lu DynSchd %lu Act %lu Rel %lu React %lu \n",
                     ue->ueId,
                     ue->spsRnti, 
                     dlSpsUe->dlSpsStatus,
                     cmnUeDl->mimoInfo.cwInfo[0].cqi,
