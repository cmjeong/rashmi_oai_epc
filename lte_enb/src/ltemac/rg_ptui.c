...
#ifdef TIP_LOWER_L2
   RETVALUE(tipSendMacAppTtiInd(pst, suId, ttiInd));
#else
...
#else
PUBLIC S16 RgUiRgrCfgCfm(pst, suId, transId, status, action, cfgType)
Pst* pst;
SuId suId;
RgrCfgTransId transId;
U8 status;
...
#endif
{
   ...
   TRC3(RgUiRgrCfgCfm);
   RETVALUE(tipSendMacCfgRsp(pst, suId, transId, status, action, cfgType));
...
#ifdef  TIP_LOWER_L2
#if 1
   RETVALUE(tipSendMacSiCfgRsp(pst, suId, transId, status));
#else
   if(status != RGR_CFG_CFM_OK)
   {
      printf("RgUiRgrSiCfgCfm  status is NOT OK\n");
      RETVALUE(RFAILED);
   }
   RETVALUE(ROK);
#endif
...
#endif
{
   ...
   TRC3(RgUiRgrStaInd);
#ifdef TIP_LOWER_L2
   /* In TIP, RLF or UE SYNC Loss is found at LL2. Hence no need of sending CQI
    * rpt to Application */
   RETVALUE(ROK);
#else
