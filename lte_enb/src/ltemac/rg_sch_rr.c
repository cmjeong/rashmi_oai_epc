...
PRIVATE Void rgSCHRrDlAddUeToSchdLst ARGS(( RgSchRrCListCp *lcp,
                                            RgSchUeCb      *ueToBeInsrtd, RgSchCellCb *cell));
...
#else
PUBLIC Void rgSCHRrDlCqiInd(cell, ue, isPucchInfo, dlCqi)
RgSchCellCb        *cell;
RgSchUeCb          *ue;
Bool               isPucchInfo;
Void               *dlCqi;
...
#else
PUBLIC S16 rgSCHRrDlUeHqEntInit(cell, hqEnt)
RgSchCellCb      *cell;
RgSchDlHqEnt     *hqEnt;
#endif
{
   ...
   if(rgSCHUtlAllocSBuf(cell->instIdx,
      (Data**)&(hqEnt->sch),
      (hqEnt->numHqPrcs * sizeof(RgSchRrDlHqProc))) != ROK)
   {
 ...
 **/
#ifdef ANSI
PUBLIC S16 rgSCHRrSCellDlUeCfg
(
RgSchCellCb      *sCell,
RgSchUeCb        *ue,
RgSchErrInfo     *err
...
#else
PUBLIC S16 rgSCHRrSCellDlUeCfg(sCell, ue, err)
RgSchCellCb      *sCell;
RgSchUeCb        *ue;
RgSchErrInfo     *err;
#endif
{
   ...
   //RgSchDlLcCb *dlLc;
   RgSchCmnUe *schCmnUe;
   RgSchCmnUe *pCellSchCmnUe;
   RgSchRrDlUe  *rrDlUe;
   RgSchRrDlUe  *pCellRrDlUe; 
   U32 lcId = 0;
   TRC2(rgSCHRrSCellDlUeCfg);

   schCmnUe = RG_SCH_CMN_GET_UE(ue, sCell); 
   if(rgSCHUtlAllocSBuf(sCell->instIdx,
            (Data**)&(schCmnUe->dl.schSpfc), (sizeof(RgSchRrDlUe))) != ROK)
   {
      RGSCHDBGERRNEW(sCell->instIdx, (rgSchPBuf(sCell->instIdx), 
               "Memory allocation FAILED"));
      RETVALUE(RFAILED);
   }
   ...
   pCellSchCmnUe = RG_SCH_CMN_GET_UE(ue, ue->cell); 
   pCellRrDlUe = (RgSchRrDlUe *)(pCellSchCmnUe->dl.schSpfc);

   for (lcId = 0; lcId < RGSCH_MAX_LC_PER_UE;lcId++)
   {
   	if (ue->dl.lcCb[lcId])
	{
         	rgSCHRrDlLcCfg(sCell, ue, ue->dl.lcCb[lcId],NULLP,err);
	}
   }
 ...
 **/
#ifdef ANSI
PUBLIC S16 rgSCHRrSCellDlUeDel
(
RgSchUeCellInfo *sCellInfo,
RgSchUeCb        *ue
)
#else
PUBLIC S16 rgSCHRrSCellDlUeDel(sCell, ue, err)
RgSchUeCellInfo *sCellInfo;
RgSchUeCb        *ue;
#endif
{
   ...
   for (lcId = 0; lcId < RGSCH_MAX_LC_PER_UE;lcId++)
   {
      if (ue->dl.lcCb[lcId])
      {
         rgSCHUtlFreeSBuf(sCellInfo->cell->instIdx,
               (Data**)(&(RG_SCH_CMN_GET_LC_SCH_SPFC(ue, ue->dl.lcCb[lcId],\
                                            sCellInfo->cell))), (sizeof(RgSchRrDlLc)));
      }
   }
...
/**
 * @brief Scell Deactivation function for RR
 *
 * @details
 *
 *     Function : rgSCHRrDlSCellDeactv
 *
 *     Processing Steps:
 ...
 **/
#ifdef ANSI
PUBLIC S16 rgSCHRrDlSCellDeactv
(
 RgSchCellCb      *cell,
 RgSchUeCb        *ue
)
#else
PUBLIC S16 rgSCHRrDlSCellDeactv(cell, ue)
 RgSchCellCb      *cell;
 RgSchUeCb        *ue;
#endif
{

   TRC2(rgSCHRrDlSCellDeactv); 
 
   RETVALUE(ROK);
}

/**
 * @brief Scell Activation function for RR
 *
 * @details
 *
 *     Function : rgSCHRrDlSCellActv
 *
 *     Processing Steps:
 ...
 **/
#ifdef ANSI
PUBLIC S16 rgSCHRrDlSCellActv
(
 RgSchCellCb      *cell,
 RgSchUeCb        *ue
)
#else
PUBLIC S16 rgSCHRrDlSCellActv(cell, ue)
 RgSchCellCb      *cell;
 RgSchUeCb        *ue;
#endif
{
   TRC2(rgSCHRrDlSCellActv); 
...
#endif
{
   ...
   if(rgSCHUtlAllocSBuf(cell->instIdx,
      (Data**)&(RG_SCH_CMN_GET_LC_SCH_SPFC(ue,dlLc,cell)), (sizeof(RgSchRrDlLc))) != ROK)
   {
...
#endif
{
   ...
   RgSchRrDlLc *rrDlLc = RG_SCH_CMN_GET_LC_SCH_SPFC(ue,dlLc,cell);
   TRC2(rgSCHRrDlLcDel);

   if(schCmnDlLc->schSpfc == NULLP || rrDlLc == NULLP)
   {
      RETVOID;
   }
   ...
   for proper NULLP assignment*/
   rgSCHUtlFreeSBuf(cell->instIdx,
   (Data**)(&(RG_SCH_CMN_GET_LC_SCH_SPFC(ue,dlLc,cell))), (sizeof(RgSchRrDlLc)));
   RETVOID;
}
...
#endif
{
   ...
   if((dlLc->bo) && (rrDlLc->lnk.node == NULLP))
   {
      if((dlLc->lcId >= RG_RR_MIN_DCCH_LCID) &&
         (dlLc->lcId <= RG_RR_MAX_DCCH_LCID))
      {
         /* All SRBs Should be Scheduled in P-Cell only*/
#ifdef LTE_ADV
         if ((rgSchCb[cell->instIdx].genCfg.forceCntrlSrbBoOnPCel == FALSE) ||
               (cell == ue->cell))
#endif
         {
...
#endif
{
   ...
   while((node=rrDlUe->sigLst.first) != NULLP)
   {
      ...
      rrDlLc = RG_SCH_CMN_GET_LC_SCH_SPFC(ue,dlLc,cell);
      rgSCHRrCListDelFrm(&(rrDlUe->sigLst), &(rrDlLc->lnk));
      rrDlLc->lnk.node = NULLP;
      node = node->next;
   }
...
#endif
{
...
#ifdef LTE_ADV
               /*f1b_Sprint */
               rgSchFreeTpcIdxForSCell(ue, schCmnUe->dl.proc, sf);
               /*f1b_Sprint */
#endif
   ...
   while(node)
   {
      ...
      /* Schedule RLC Control PDU only on P-cell*/
#ifdef LTE_ADV
 ...
 * @details
 *
 *     Function : rgSCHRrDlSchdSecCellActCe
 *
 *     Processing Steps:
...
#ifdef ANSI
PRIVATE S16 rgSCHRrDlSchdSecCellActCe
(
RgSchCellCb             *cell,
RgSchCmnDlRbAllocInfo   *allocInfo
)
#else
PRIVATE S16 rgSCHRrDlSchdSecCellActCe(cell, allocInfo)
RgSchCellCb             *cell;
RgSchCmnDlRbAllocInfo   *allocInfo;
#endif
{
   ...
   TRC2(rgSCHRrDlSchdSecCellActCe);
   ...
   while(secCellCeNode && dlSf->remUeCnt)
   {
      ...
      if(rgSCHCmnDlAllocTxRb(cell, dlSf, ue,
               RGSCH_SCELL_ACT_CE_SIZE, &allocBo, schCmnUe->dl.proc,\
               allocInfo) != ROK)
      {
...
#ifdef LTE_ADV
         /*f1b_Sprint */
         rgSchFreeTpcIdxForSCell(ue, schCmnUe->dl.proc, dlSf);
         /*f1b_Sprint */
#endif
         rgSCHRrRlsDlHqProc(schCmnUe->dl.proc);
         schCmnUe->dl.proc = NULLP;
      ...
      if(allocBo)
      {
         rrDlUe->schdSCellActCe = TRUE;
         /* Added the handling for updating the totBtyes.*/
         cmnHqDl->totBytes += allocBo;
         
         if(ue->totalBo)
         {/* Add to the list only if some BO is present */
...
#endif
{
   ...
   {
      ue = (RgSchUeCb *)(node->node);
      cmnUe = RG_SCH_CMN_GET_UE(ue, cell); 
...
#else 
PRIVATE Void rgSCHRrDlAddUeToSchdLst(toBeSchedLst, ueToBeInsrtd, cell)
RgSchRrCListCp *toBeSchedLst;
RgSchUeCb      *ueToBeInsrtd;
RgSchCellCb             *cell;
#endif
{
   RgSchCmnUe *schCmnUe             = RG_SCH_CMN_GET_UE(ueToBeInsrtd, cell); 
   RgSchRrDlUe   *rrDlUe            = (RgSchRrDlUe *)(schCmnUe->dl.schSpfc);
   ...
   else
   {
      ...
      while(tmpNode != (toBeSchedLst->first))
      {
         ...
         tmpSchCmnUe  = RG_SCH_CMN_GET_UE(tmpUe, cell); 
...
#endif
{
   ...
   do
   {
            ...
            (rrDlUe->dataLst.count))
      {
         ...
         if(rrDlUe->schdTa == FALSE)
#else
          /* Avail harq proc only if TA and Scell act CE is not 
          * scheduled */
         if((rrDlUe->schdTa == FALSE) &&
            (rrDlUe->schdSCellActCe == FALSE))
#endif
         {
            ...
            if(RG_SCH_IS_CELL_SEC(ue,cell))
            {
               ...
               if(ROK != rgSCHSCellIsActive(cell,ue))
               {/* Skipping new tx if scell is not active state ,
                   Avoiding tx if the deact was triggered*/
                  rgSCHRrRlsDlHqProc(schCmnUe->dl.proc);
                  schCmnUe->dl.proc = NULLP;
                  continue;
               }
...
#ifdef CA_DBG
      extern U32 gPcellZeroBoOcc,gScellZeroBoOcc;
      if (cell->cellId == 1)
         gPcellZeroBoOcc++;
      else
         gScellZeroBoOcc++;
 ...
 **/
#ifdef ANSI
PUBLIC Void rgSCHRrDlCeSchd
(
RgSchCellCb             *cell,
RgSchCmnDlRbAllocInfo   *allocInfo
)
#else
PUBLIC Void rgSCHRrDlCeSchd(cell, allocInfo)
RgSchCellCb             *cell;
RgSchCmnDlRbAllocInfo   *allocInfo;
#endif
{
 ...
 **/
#ifdef ANSI
PUBLIC Void rgSCHRrDlRetxSchd
(
RgSchCellCb             *cell,
RgSchCmnDlRbAllocInfo   *allocInfo
)
#else
PUBLIC Void rgSCHRrDlRetxSchd(cell, allocInfo)
RgSchCellCb             *cell;
RgSchCmnDlRbAllocInfo   *allocInfo;
#endif
{
   TRC2(rgSCHRrDlRetxSchd);
 ...
 **/
#ifdef ANSI
PUBLIC Void rgSCHRrDlNewSchd
(
RgSchCellCb             *cell,
RgSchCmnDlRbAllocInfo   *allocInfo
)
#else
PUBLIC Void rgSCHRrDlNewSchd(cell, allocInfo)
RgSchCellCb             *cell;
RgSchCmnDlRbAllocInfo   *allocInfo;
#endif
{
   TRC2(rgSCHRrDlNewSchd);
 ...
 **/
#ifdef ANSI
PUBLIC Void rgSCHRrDlPreSchd
(
 RgSchCellCb    *cell
)
#else
PUBLIC Void rgSCHRrDlPreSchd(cell)
 RgSchCellCb    *cell;
#endif
{
   TRC2(rgSCHRrDlPreSchd);
 ...
 **/
#ifdef ANSI
PUBLIC Void rgSCHRrDlPstSchd
(
 Inst       schInst
)
#else
PUBLIC Void rgSCHRrDlPstSchd(schInst)
 Inst       schInst
#endif
{
   TRC2(rgSCHRrDlPstSchd);
   ...
   while((*node) && (*effAlloc > 0))
   {
      ...
      *node = (*node)->next;
      ...
      if ((rgSchCb[cell->instIdx].genCfg.forceCntrlSrbBoOnPCel) &&
            (cell != ue->cell))
      {
         /* dlLc->bo will always be greator than dlLc->staPduBo for Scell.
          * Deduct the staPduBo from LcBo as it is not schd on Scell*/
         if (bo > dlLc->staPduBo)
         {
            bo -= dlLc->staPduBo;
         }
      ...
      *effAlloc -= bo;
      ...
      if(dlLc->bo == 0)
      {
         ...
         if (ue->numSCells)
         {
            rgSCHSCellDlDedBoUpd(cell, ue, dlLc);
         }
#endif
      }
...
#endif
{
   ...
   while(node)
   {
      ...
      rgSCHCmnDlUeResetTemp(ue, hqP);
#ifdef TENB_STATS
      ue->tenbStats->stats.nonPersistent.sch[RG_SCH_CELLINDEX(hqP->hqE->cell)].dlTxOccns++; 
#endif
...
#ifdef LTE_ADV
   rgSchDlApis->rgSCHRgrSCellDlUeCfg  = rgSCHRrSCellDlUeCfg;
   rgSchDlApis->rgSCHRgrSCellDlUeDel  = rgSCHRrSCellDlUeDel;
   rgSchDlApis->rgSCHDlSCellDeactv    = rgSCHRrDlSCellDeactv;
   rgSchDlApis->rgSCHDlSCellActv      = rgSCHRrDlSCellActv;
#endif
...
#endif
{
   ...
   if((ue->ul.effBsr) && (rrUlUe->contResLnk.node == NULLP))
   {
      if(rrUlUe->ueTxLnk.node == NULLP)
      {
...
#endif
{
   ...
   RgSchRrUlUe  *rrUlUe = (RgSchRrUlUe *)(schCmnUe->ul.schSpfc);
#ifdef LTE_ADV
   RgSchRrUlCell *rrUlSCell;
   RgSchCmnUe *sCellSchCmnUe;
   RgSchRrUlUe  *rrSCellUlUe;
#endif
   ...
   */
   ...
   if (ue->ul.isUlCaEnabled  && ue->numSCells)
   {
      for(U8 sCellIdx = 1; sCellIdx <= RG_SCH_MAX_SCELL ; sCellIdx++)
      {
         ...
         {
            rrUlSCell = (RgSchRrUlCell *)(((RgSchCmnCell *)(cell->sc.sch))->ul.schSpfc);
            sCellSchCmnUe = RG_SCH_CMN_GET_UE(ue, cell); 
            rrSCellUlUe = (RgSchRrUlUe *)(sCellSchCmnUe->ul.schSpfc);
            rgSCHRrUlDelUeTxLst(ue,rrUlSCell,rrSCellUlUe);
         }
      }
   }
#endif 
...
#endif
{
   ...
   do
   {
      ...
      if ((ue->ul.ulInactvMask) ||
          (RG_SCH_CMN_IS_UL_UE_RETX(ue, cell)))
      {
         continue;
      }
      schCmnUe = RG_SCH_CMN_GET_UE(ue, cell); 
...
#endif
{
   ...
   if (ue->ul.isUlCaEnabled  && ue->numSCells)
   {
      for(U8 sCellIdx = 0; sCellIdx <= RG_SCH_MAX_SCELL ; sCellIdx++)
      {
         if((ue->cellInfo[sCellIdx] != NULLP) && (cell != ue->cellInfo[sCellIdx]->cell) &&
               (ue->cellInfo[sCellIdx]->sCellState == RG_SCH_SCELL_ACTIVE))
         {
            sCell =  ue->cellInfo[sCellIdx]->cell;
            rrUlSCell = (RgSchRrUlCell *)(((RgSchCmnCell *)(sCell->sc.sch))->ul.schSpfc);
            sCellSchCmnUe = RG_SCH_CMN_GET_UE(ue, sCell); 
            rrSCellUlUe = (RgSchRrUlUe *)(sCellSchCmnUe->ul.schSpfc);
            rgSCHRrUlDelUeTxLst(ue,rrUlSCell,rrSCellUlUe);
         }
      }
