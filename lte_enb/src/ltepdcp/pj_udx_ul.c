...
#ifdef TIP_UPPER_L2
PUBLIC S16 pjTipChkSndReestCmpInd
(
PjCb         *tPjCb,
PjCfgInfo    *cfgInfo
)
{
   ...
   for (idx = 0; idx < cfg->numEnt && idx < CPJ_MAX_CFG; idx++)
   {
      ...
      switch (entCfg->cfgType)
      {
         ...
         {
            PJ_DBM_FETCH_UL_RBCB(entCfg->rbId, entCfg->rbType, ueCb, ulRbCb);
            if(!ulRbCb)
            {
               printf("\ncould not find RB CB for %d type %d\n",
                     entCfg->rbId, entCfg->rbType);
               RLOG_ARG2(L_DEBUG, DBG_UEID, cfg->ueId,
                     "Could not find RB CB for %d type %d",
                     entCfg->rbId, entCfg->rbType);
               continue;
            }

            if(ulRbCb->rbType != PJ_SRB)
            {
               RLOG_ARG1(L_DEBUG, DBG_UEID, cfg->ueId,
                     "Processing local reestablishment confirm for %d",
                     entCfg->rbId);
...
#ifdef TIP_UPPER_L2
   /* process RLC reestablishment cfm indication */
