...
/* External security library includes */
...
#ifdef TIP_UPPER_L2
   if(libTrans->intAlgoType == CPJ_SEC_ALG_SNOW3G)
   {
      ret = pjTipIntegProtVerifyPkt(libTrans, &mBuf, macI);
   }
   else if (libTrans->intAlgoType == CPJ_SEC_ALG_NULL)
   {
       //Do Nothing
   }
   else
   {
     RLOG_ARG1(L_ERROR,DBG_UEID,libTrans->ueId,"Integrity Verification Algorithm Type %d not supported !!", libTrans->intAlgoType);
   }
#endif
   RETVALUE(ret);
}
...
#ifdef TIP_UPPER_L2
      if(libTrans->ciphAlgoType == CPJ_SEC_ALG_SNOW3G)
      {
         ret = pjTipSendToDeCipher(libTrans, mBuf, opSdu);
      }
      else if (libTrans->ciphAlgoType == CPJ_SEC_ALG_NULL)
      {
         //Do Nothing
      }
      else
      {
         RLOG_ARG1(L_ERROR,DBG_UEID,libTrans->ueId,"Deciphering Algorithm Type %d not supported !!", libTrans->ciphAlgoType);
      }
#endif
   RETVALUE(ret);
...
#endif
{
   ...
   if((pjSendToTipKeyDeriveFunc(libTrans, kdfCfm->secKey)) != ROK)
   {
      RLOG0(L_ERROR, "In pjLibObdPrcKenbStarCfg: pjTipKDF failed");
      ret = RFAILED;
   }/*End of if(ret!=ROK)*/
...
{
   TRC3(PjLibObdIntProtReq);
#if defined(LTE_PAL_ENB) && !defined(TIP_UPPER_L2)
   Buffer *mBuf1;
   ...
   *mBuf = mBuf1;
      ...
      {
         RLOG_ARG1(L_ERROR,DBG_UEID,libTrans->ueId,"Integrity Algorithm Type %d is not supported !!", libTrans->intAlgoType);
      }
#endif
   RETVALUE(ROK);
}
...
#endif
{
   ...
   {
     RLOG_ARG1(L_ERROR,DBG_UEID,libTrans->ueId," Ciphering Algorithm Type %d is not supported !!", libTrans->ciphAlgoType);
   }
#endif 
