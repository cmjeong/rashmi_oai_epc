...
PRIVATE S16 wrUmmRrcFillMacCfg
(
...
)
{
   ...
   wrFillTknU32(&asCfg->antennaInfoCmn.antennaPortsCount, numAntPorts);
 ...
 *  @return 
 */
PUBLIC S16 wrUmmRrcFillUePhyCfgDedInfo
(
   CmMemListCp        *memCp,
...
)
{
         ...
         & (U8)RM_RCM_PHY_DED_RES_CFG_CQI)
   {
      ...
      WR_FILL_TKN_UINT(cqiRprtng->nomPDSCH_RS_EPRE_Offset, \
            cellCb->cqiRptCfg.nomPdschRsEpreOff); /* 0, MIMO */

      /* NhuCQI_ReportPeriodicsetup - empty */
      cqiRprtng->cqi_ReportPeriodic.choice.pres = NOTPRSNT;

      wrFillTknS32(&(cqiRprtng->nomPDSCH_RS_EPRE_Offset),\
            (S32)(cellCb->cqiRptCfg.nomPdschRsEpreOff));
 ...
 *  @return 
 */
PUBLIC S16 wrUmmRrcFillUeRadioResDedInfo
(
   /* SPS changes starts */
   CmMemListCp                      *memCp,
   WrUeCb                           *ueCb,
...
/*Removed DRX flag*/
   ...
   {   
      wrUmmRrcFillUePhyCfgDedInfo(memCp,rrcCfgDed, rrmUeRecfgResp, ueCb->cellId);
   }
   /* SPS changes starts */
