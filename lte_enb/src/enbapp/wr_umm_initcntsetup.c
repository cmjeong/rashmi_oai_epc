...
#ifdef LTE_ADV_UECAP
EXTERN S16 wrUmmStoreUeCapR10(NhuUE_EUTRA_Cap_v1020_IEs *ueCapNonCritExtn_v1020,
                            NhuUE_EUTRA_Cap_v1020_IEs *nhuDecNonCriticalExtn_v1020,WrUeCapInfo *ueCap);
EXTERN S16 wrUmmStoreUeCapRel10PhyLyr(NhuUE_EUTRA_Cap_v1020_IEs *ueCapNonCritExtn_v1020, 
                           NhuUE_EUTRA_Cap_v1020_IEs *nhuDecNonCriticalExtn_v1020,WrUeCapInfo *ueCap);
EXTERN S16 wrUmmStoreUeCapRel10RfParams(NhuUE_EUTRA_Cap_v1020_IEs *ueCapNonCritExtn_v1020,
                           NhuUE_EUTRA_Cap_v1020_IEs *nhuDecNonCriticalExtn_v1020,WrUeCapInfo *ueCap);
EXTERN S16  wrUmmStoreUeCapv1060(NhuUE_EUTRA_Cap_v1060_IEs *ueCap_v1060,NhuUE_EUTRA_Cap_v1060_IEs *nhuDec_v1060,
WrUeCapInfo *ueCap);
EXTERN S16 wrUmmChkUeCASupp(WrUeCb *ueCb);
#endif /* LTE_ADV_UECAP */
...
)
{
   ...
   if(nhuDecNonCriticalExtn_v940->pres.pres == PRSNT_NODEF)
   {
      ...
      if(nhuDecNonCriticalExtn_v1020->pres.pres == PRSNT_NODEF)
      {
         ...
         /*CA Features*/
#ifdef LTE_ADV_UECAP
         wrUmmStoreUeCapR10(ueCapNonCritExtn_v1020,nhuDecNonCriticalExtn_v1020,ueCap);
#endif /* LTE_ADV_UECAP */
      }
   }
...
)
{
   ...
   WR_UMM_GET_CELLCB(cellCb, ueRecfg->cellId);
   if(cellCb == NULLP)
   {
      RLOG_ARG0(L_ERROR, DBG_CELLID, (U32)ueRecfg->cellId, "Cell Cb is NULL ");
      RETVALUE(RFAILED);
   }
   ...
   if ((initCtxt->dlAmbr != 0) || (initCtxt->ulAmbr != 0))
   {
      ...
      ueRecfg->ueUlHqRecfg.maxUlHqTx = cellCb->macMainCfg.ulSchCfg.maxHARQ_Tx;
   ...
   wrUmmSchdFillUeCategory(&ueRecfg->ueCatEnum, transCb->ueCb);
   ueRecfg->ueRecfgTypes |= RGR_UE_UE_ACCESS_STRATUM_REL_RECFG;
   wrUmmSchdFillUeAccessStratumRls(&ueRecfg->accessStratumRls, transCb->ueCb);
   /* SPS changes starts */
   ...
   WrIfmSchdCfgUe(cfgReq, transId, transCb->ueCb->cellId);
   RETVALUE(ROK);
}
...
)
{
   ...
   lchCfg->dlInfo.dlQos.qci = WR_SRB2_QCI;
   //lchCfg->dlInfo.dlQos.qci = 5;
   lchCfg->ulLchQciInfo.lcId = WR_PDCP_ID_SRB2;
   lchCfg->ulLchQciInfo.qci = WR_SRB2_QCI;
...
)
{
   ...
   if (transCb->msg->u.schdCfm.status == RGR_CFG_CFM_OK)
   {
      initCtxt->erabList[idx].schdCfgState = (U8)WR_INIT_CTXT_SCHD_CFM_SUCC; /*TIP_DEBUG: Valgrind FIX */
...
)
{
   ...
   if(initCtxt->csFallBackInd.pres == TRUE)
   {
      ret = wrUmmMeasInitialUeCfg (&recfgMsg->memCp, ueCb, 
            &initCtxt->measTransCb, nhuMeasCfg, transCb ,WR_MEAS_CONFIG_NO_EVT_ANR);
   }
   else
   {
      ret = wrUmmMeasInitialUeCfg (&recfgMsg->memCp, ueCb, 
            &initCtxt->measTransCb, nhuMeasCfg, transCb, WR_MEAS_CONFIG_EVT_ANR);
   }
   if(ROK != ret)
   {
...
#else
      erabItem->e_RABlevelQoSParams.gbrQosInform.pres.pres = TRUE;
      erabInfo[rbIdx].qci           = 4;
#endif
...
#else
         erabInfo[rbIdx].qos.dlMbr = 300000000;
         erabInfo[rbIdx].qos.dlGbr = 300000000;
         wrGetBitRate(&gbrIe->e_RAB_MaxBitrateUL, &erabInfo[rbIdx].qos.ulMbr);
         wrGetBitRate(&gbrIe->e_RAB_GuaranteedBitrateUL,
                                                  &erabInfo[rbIdx].qos.ulGbr);
#endif
...
)
{
   ...
   for (idx = 0; idx < initCtxtCb->numErabs; idx++)
   {
    ...
    tnlInfo->hoType = WR_HO_TYPE_NONE;
         wrEmmGetDatAppAddr(ueCb->cellId, &initCtxtCb->lclAddr, rabInfo->sgwAddr.type);
...
)
{
   ...
   WR_UMM_GET_CELLCB(cellCb, transCb->ueCb->cellId);
   WR_EMM_GET_CELLCB(emmCellCb, transCb->ueCb->cellId);
   ...
   switch (msg->msgTyp)
   {
...
#ifdef LTE_ADV
                     /*CA-Dev*/
                     /*Based on whether Ue is Rel10 capable, need to call this function.
                      * Ue Capability check to be taken in sprint 2*/
                     if((TRUE == wrEmmCb.isCaEnabled) 
                           &&(FALSE == cellCb->measCellCfg->isMeasReqForSCellAdd)
#ifdef LTE_ADV_UECAP
                           &&(TRUE == ueCb->isUeCaCap)
#endif/* LTE_ADV_UECAP*/
                       )
      ...
      case WR_UMM_SCHD_CFG:
      {
         ...
         {
            RLOG0(L_DEBUG,"A4:Received Schd Gap Setup Cfm\n");
#ifdef WR_RSYS_KPI 
      ...
      case WR_UMM_DAM_CFM:
      {
         ...
         {
#ifdef LTE_ADV
            if((TRUE == wrEmmCb.isCaEnabled) && (TRUE == cellCb->measCellCfg->isMeasReqForSCellAdd)
#ifdef LTE_ADV_UECAP
                  && (TRUE == ueCb->isUeCaCap)
#endif
              )
            {
               ...
               {
                  wrUmmMeasInterCfgMeasGapForA4(ueCb, &transCb->u.measTransCb.addMeasInfo,
                        cellCb, &isGapCfgd);
                  if(isGapCfgd == TRUE)
                  {
                     WrUmmMeasGapStatus      measGapStatusForScellAdd = WR_UMM_MEAS_GAP_NONE;
                     wrUmmMeasAddMeasGap(WR_UMM_MEAS_GAP_SCELL_ADD,cellCb, ueCb,
                           &transCb->u.measTransCb.measGapStatus, transCb->transId);
                     measGapStatusForScellAdd = transCb->u.measTransCb.measGapStatus;

                     if((ueCb->measCfg->gapMeasInfo.latestTxId ==
                              transCb->transId) &&
                           !(( WR_UMM_MEAS_GAP_NONE == transCb->u.measTransCb.measGapStatus) &&
                              ( WR_UMM_MEAS_GAP_NONE == measGapStatusForScellAdd)))
                     {
                        /* configure Meas Gap so sending messge to SCH */
                        transCb->u.measTransCb.measGapStatus = WR_UMM_MEAS_GAP_INIT_SCH_CONFIG;
...
)
{
   ...
   if(ueCb->bResCfgBitMap &  WR_UE_RADIO_RES_CFG_CQI)
   {
     ...
     if(ueCb->cqiRptCfg.riPres == TRUE)/*revanth fix*/
     /*TENB_BRDCMTDD_TM4*/
     ...
     {   
      ...
      RLOG4(L_ALWAYS,"wrUmmCqiSrRadResModPrepRrcMsgAndCfg UE ID %d ricfg pres %d cfg idx %d txmode %lu \n",
      ueCb->crnti,
      ueCb->cqiRptCfg.riPres,
...
)
{
   ...
   {   
      wrUmmRrcFillUePhyCfgDedInfo(&recfgMsg->memCp, rrcCfgDed, rrmUeRecfgInd, ueCb->cellId);
   }
...
#ifdef LTE_ADV_UECAP
/** @brief This function is used to copy the CA supported IEs into the
 *   local structure
 *         
 *
 * 
 * Function:   wrUmmStoreUeCapR10
 *        
 * @param[in]  ueCapNonCritExtn_v1020 : UeCapablility IE version 1020
 *             nhuDecNonCriticalExtn_v1020 : Decoded UeCapablility IE version 1020
 *             ueCap                       : UE Capability Information Structure
 *
 *
 * @return S16
 * -#Success : ROK
...
PUBLIC S16 wrUmmStoreUeCapR10
(
 NhuUE_EUTRA_Cap_v1020_IEs              *ueCapNonCritExtn_v1020,
 NhuUE_EUTRA_Cap_v1020_IEs              *nhuDecNonCriticalExtn_v1020,
 WrUeCapInfo                            *ueCap
)
{

   wrUmmStoreUeCapRel10PhyLyr(ueCapNonCritExtn_v1020,nhuDecNonCriticalExtn_v1020,ueCap); 
   wrUmmStoreUeCapRel10RfParams(ueCapNonCritExtn_v1020,nhuDecNonCriticalExtn_v1020,ueCap);

   NhuUE_EUTRA_Cap_v1060_IEs *ueCap_v1060 = &ueCapNonCritExtn_v1020->nonCriticalExtn;
   NhuUE_EUTRA_Cap_v1060_IEs *nhuDec_v1060 = &nhuDecNonCriticalExtn_v1020->nonCriticalExtn;

   wrUmmStoreUeCapv1060(ueCap_v1060,nhuDec_v1060,ueCap);

   RETVALUE(ROK);
}
/** @brief This function is used to copy the CA supported IEs Rel10 Phy Layer
 *  param into the local structure
 *
 *
 *
 * Function:   wrUmmStoreUeCapRel10PhyLyr
 *
 * @param[in]  ueCapNonCritExtn_v1020 : UeCapablility IE version 1020
 *             nhuDecNonCriticalExtn_v1020 : Decoded UeCapablility IE
 *             version 1020
 *             ueCap                       : UE Capability Information
 *             Structure
 *
 *
 * @return S16
...
PUBLIC S16 wrUmmStoreUeCapRel10PhyLyr
(
 NhuUE_EUTRA_Cap_v1020_IEs              *ueCapNonCritExtn_v1020,
 NhuUE_EUTRA_Cap_v1020_IEs              *nhuDecNonCriticalExtn_v1020,
 WrUeCapInfo                            *ueCap
)
{
   U32 numComp = 0; 
   U32 idx = 0;
   if(PRSNT_NODEF == nhuDecNonCriticalExtn_v1020->phyLyrParams_v1020.pres.pres)
   {
      wrFillTknU8(&(ueCapNonCritExtn_v1020->phyLyrParams_v1020.pres),PRSNT_NODEF);

      wrFillTknU32(&(ueCapNonCritExtn_v1020->phyLyrParams_v1020.twoAntennaPortsForPUCCH_r10),
            nhuDecNonCriticalExtn_v1020->phyLyrParams_v1020.twoAntennaPortsForPUCCH_r10.val);
     
      wrFillTknU32(&(ueCapNonCritExtn_v1020->phyLyrParams_v1020.tm9_With_8Tx_FDD_r10),
            nhuDecNonCriticalExtn_v1020->phyLyrParams_v1020.tm9_With_8Tx_FDD_r10.val);

      wrFillTknU32(&(ueCapNonCritExtn_v1020->phyLyrParams_v1020.pmi_Disabling_r10),
            nhuDecNonCriticalExtn_v1020->phyLyrParams_v1020.pmi_Disabling_r10.val);

      wrFillTknU32(&(ueCapNonCritExtn_v1020->phyLyrParams_v1020.crossCarrierScheduling_r10),
            nhuDecNonCriticalExtn_v1020->phyLyrParams_v1020.crossCarrierScheduling_r10.val);

      wrFillTknU32(&(ueCapNonCritExtn_v1020->phyLyrParams_v1020.simultaneousPUCCH_PUSCH_r10),
            nhuDecNonCriticalExtn_v1020->phyLyrParams_v1020.simultaneousPUCCH_PUSCH_r10.val);

      wrFillTknU32(&(ueCapNonCritExtn_v1020->phyLyrParams_v1020.multiClusterPUSCH_WithinCC_r10),
            nhuDecNonCriticalExtn_v1020->phyLyrParams_v1020.multiClusterPUSCH_WithinCC_r10.val);

      wrFillTknU16(&(ueCapNonCritExtn_v1020->phyLyrParams_v1020.nonContiguousUL_RA_WithinCC_Lst_r10.noComp),
            nhuDecNonCriticalExtn_v1020->phyLyrParams_v1020.nonContiguousUL_RA_WithinCC_Lst_r10.noComp.val);

      WR_GET_MEM(&ueCap->memCp, sizeof(NhuNonContiguousUL_RA_WithinCC_r10) * \
            nhuDecNonCriticalExtn_v1020->phyLyrParams_v1020.nonContiguousUL_RA_WithinCC_Lst_r10.noComp.val,
            &ueCapNonCritExtn_v1020->phyLyrParams_v1020.nonContiguousUL_RA_WithinCC_Lst_r10.member);

      if(NULLP == ueCapNonCritExtn_v1020->phyLyrParams_v1020.nonContiguousUL_RA_WithinCC_Lst_r10.member)
      {
         RLOG0(L_FATAL, "Failed to allocate memory for NhuNonContiguousUL_RA_WithinCC_r10");
         RETVALUE(RFAILED);

      }


      NhuNonContiguousUL_RA_WithinCC_Lst_r10 *ueCapnonContUL_RA_WithnCC_Lst_r10_v1020 = \
      &ueCapNonCritExtn_v1020->phyLyrParams_v1020.nonContiguousUL_RA_WithinCC_Lst_r10;

      NhuNonContiguousUL_RA_WithinCC_Lst_r10 *nhuDecnonContUL_RA_WithnCC_Lst_r10_v1020 = \
      &nhuDecNonCriticalExtn_v1020->phyLyrParams_v1020.nonContiguousUL_RA_WithinCC_Lst_r10;
    
      numComp = nhuDecnonContUL_RA_WithnCC_Lst_r10_v1020->noComp.val; 
      if(numComp > 0)
      {
         for(idx = 0; idx < numComp ; idx++)
         {
            if(PRSNT_NODEF == nhuDecnonContUL_RA_WithnCC_Lst_r10_v1020->member[idx].pres.pres)
            {
               wrFillTknU8(&(ueCapnonContUL_RA_WithnCC_Lst_r10_v1020->member[idx].pres),PRSNT_NODEF);
               wrFillTknU32(&(ueCapnonContUL_RA_WithnCC_Lst_r10_v1020->member[idx]. \
                        nonContiguousUL_RA_WithinCC_Info_r10),nhuDecnonContUL_RA_WithnCC_Lst_r10_v1020-> \
                     member[idx].nonContiguousUL_RA_WithinCC_Info_r10.val);
            }
         }
      }
...
/** @brief This function is used to copy the CA supported IEs Rel10 RF params into the
 *   local structure
 *
 *
 *
 * Function:   wrUmmStoreUeCapRel10RfParams
 *
 * @param[in]  ueCapNonCritExtn_v1020 : UeCapablility IE version 1020
 *             nhuDecNonCriticalExtn_v1020 : Decoded UeCapablility IE
 *             version 1020
...
PUBLIC S16 wrUmmStoreUeCapRel10RfParams
(
 NhuUE_EUTRA_Cap_v1020_IEs              *ueCapNonCritExtn_v1020,
 NhuUE_EUTRA_Cap_v1020_IEs              *nhuDecNonCriticalExtn_v1020,
 WrUeCapInfo                            *ueCap
 )
{
   ...
   /*Copying Rel 10 rf params*/
   if(PRSNT_NODEF == nhuDecNonCriticalExtn_v1020->rf_Params_v1020.pres.pres)
   {
      NhuRF_Params_v1020 *nhuDecRfParams_v1020 = &nhuDecNonCriticalExtn_v1020->rf_Params_v1020;
      NhuRF_Params_v1020 *rfParams_v1020 = &ueCapNonCritExtn_v1020->rf_Params_v1020;

      wrFillTknU8(&(rfParams_v1020->pres), PRSNT_NODEF);
      wrFillTknU16(&(rfParams_v1020->supportedBandCombination_r10.noComp),
            nhuDecRfParams_v1020->supportedBandCombination_r10.noComp.val);

      WR_GET_MEM(&ueCap->memCp, sizeof(NhuBandCombinationParams_r10) * \
            nhuDecRfParams_v1020->supportedBandCombination_r10.noComp.val,\
            &rfParams_v1020->supportedBandCombination_r10.member);

      if(NULLP == rfParams_v1020->supportedBandCombination_r10.member)
      {
         RLOG0(L_FATAL, "Failed to allocate memory for supported Band Combination r10");
         RETVALUE(RFAILED);

      }

      numComp = nhuDecRfParams_v1020->supportedBandCombination_r10.noComp.val;
      if(numComp > 0)
      {
         for(idx = 0; idx < numComp ; idx++)
         {
            wrFillTknU16(&(rfParams_v1020->supportedBandCombination_r10.member[idx].noComp),
                  nhuDecRfParams_v1020->supportedBandCombination_r10.member[idx].noComp.val);
            WR_GET_MEM(&ueCap->memCp, sizeof(NhuBandParams_r10) * nhuDecRfParams_v1020-> \
                  supportedBandCombination_r10.member[idx].noComp.val,&rfParams_v1020-> \
                  supportedBandCombination_r10.member[idx].member);

            if(NULLP == rfParams_v1020->supportedBandCombination_r10.member[idx].member)
            {
               RLOG0(L_FATAL, "Failed to allocate memory for Nhu Band Params r10");
               RETVALUE(RFAILED);
            }

            numComp1 = nhuDecRfParams_v1020->supportedBandCombination_r10.member[idx].noComp.val;
            if(numComp1 > 0)
            {
               for(idx1 = 0; idx1 < numComp1 ; idx1++)
               {
                  if(PRSNT_NODEF == nhuDecRfParams_v1020->supportedBandCombination_r10. \
                        member[idx].member[idx1].pres.pres)
                  {
                     wrFillTknU8(&(rfParams_v1020->supportedBandCombination_r10.member[idx].member[idx1].pres), 
                           PRSNT_NODEF);
                     cmMemcpy((U8*)&rfParams_v1020->supportedBandCombination_r10.member[idx]. \
                           member[idx1].bandEUTRA_r10,(U8*)&nhuDecRfParams_v1020-> \
                           supportedBandCombination_r10.member[idx].member[idx1].bandEUTRA_r10, \
                           sizeof(NhuFreqBandIndicator));


                     NhuBandParamsUL_r10 *ueCapbandParamsUL_r10 = &rfParams_v1020->supportedBandCombination_r10. \
                                                                  member[idx].member[idx1].bandParamsUL_r10;
                     NhuBandParamsUL_r10 *nhuDecbandParamsUL_r10 = &nhuDecRfParams_v1020->supportedBandCombination_r10. \
                                                                   member[idx].member[idx1].bandParamsUL_r10;


                     wrFillTknU16(&(ueCapbandParamsUL_r10->noComp),nhuDecbandParamsUL_r10->noComp.val);
                     WR_GET_MEM(&ueCap->memCp,sizeof(NhuCA_MIMO_ParamsUL_r10) * nhuDecbandParamsUL_r10-> \
                           noComp.val,&ueCapbandParamsUL_r10->member);

                     if(NULLP == ueCapbandParamsUL_r10->member)
                     {
                        RLOG0(L_FATAL, "Failed to allocate memory for NhuCA_MIMO_ParamsUL_r10");
                        RETVALUE(RFAILED);
                     }

                     numComp2 = nhuDecbandParamsUL_r10->noComp.val;
                     if(numComp2 > 0)
                     {
                        for(idx2 = 0; idx2 < numComp2 ; idx2++)
                        {
                           if(PRSNT_NODEF == nhuDecbandParamsUL_r10->member[idx2].pres.pres)
                           {
                               wrFillTknU8(&(ueCapbandParamsUL_r10->member[idx2].pres), PRSNT_NODEF);

                              wrFillTknU32(&(ueCapbandParamsUL_r10->member[idx2].ca_BandwidthClassUL_r10),
                                    nhuDecbandParamsUL_r10->member[idx2].ca_BandwidthClassUL_r10.val);

                              wrFillTknU32(&(ueCapbandParamsUL_r10->member[idx2].supportedMIMO_CapUL_r10),
                                    nhuDecbandParamsUL_r10->member[idx2].supportedMIMO_CapUL_r10.val);

                              RLOG2(L_DEBUG, "ca_BandwidthClassUL_r10[%lu] supportedMIMO_CapUL_r10[%lu]",
                                    ueCapbandParamsUL_r10->member[idx2].ca_BandwidthClassUL_r10.val, \
                                    ueCapbandParamsUL_r10->member[idx2].supportedMIMO_CapUL_r10.val );
                     ...
                     NhuBandParamsDL_r10  *ueCapbandParamsDL_r10 = &rfParams_v1020->supportedBandCombination_r10. \
                                                                   member[idx].member[idx1].bandParamsDL_r10;
                     NhuBandParamsDL_r10 *nhuDecbandParamsDL_r10 = &nhuDecRfParams_v1020->supportedBandCombination_r10. \
                                                                   member[idx].member[idx1].bandParamsDL_r10;
                     wrFillTknU16(&(ueCapbandParamsDL_r10->noComp),nhuDecbandParamsDL_r10->noComp.val);
                     WR_GET_MEM(&ueCap->memCp,sizeof(NhuCA_MIMO_ParamsDL_r10) * nhuDecbandParamsDL_r10-> \
                           noComp.val,&ueCapbandParamsDL_r10->member);

                     if(NULLP == ueCapbandParamsDL_r10->member)
                     {
                        RLOG0(L_FATAL, "Failed to allocate memory for NhuCA_MIMO_ParamsDL_r10");
                        RETVALUE(RFAILED);
                     }

                     numComp2 = nhuDecbandParamsDL_r10->noComp.val;
                     if(numComp2 > 0)
                     {
                        for(idx2 = 0; idx2 < numComp2 ; idx2++)
                        {
                           if(PRSNT_NODEF == nhuDecbandParamsDL_r10->member[idx2].pres.pres)
                           {
                              wrFillTknU8(&(ueCapbandParamsDL_r10->member[idx2].pres), PRSNT_NODEF);
                              wrFillTknU32(&(ueCapbandParamsDL_r10->member[idx2].ca_BandwidthClassDL_r10),
                                    nhuDecbandParamsDL_r10->member[idx2].ca_BandwidthClassDL_r10.val);

                              wrFillTknU32(&(ueCapbandParamsDL_r10->member[idx2].supportedMIMO_CapDL_r10),
                                    nhuDecbandParamsDL_r10->member[idx2].supportedMIMO_CapDL_r10.val);

                              RLOG2(L_DEBUG, "ca_BandwidthClassDL_r10[%lu] supportedMIMO_CapDL_r10[%lu]",
                                    ueCapbandParamsDL_r10->member[idx2].ca_BandwidthClassDL_r10.val, \
                                    ueCapbandParamsDL_r10->member[idx2].supportedMIMO_CapDL_r10.val );
                           }
 ...
 *
 *
 * Function:   wrUmmStoreUeCapv1060
 *
 * @param[in]  ueCapNonCritExtn_v1060 : UeCapablility IE version 1060
 *             nhuDecNonCriticalExtn_v1060 : Decoded UeCapablility IE
 *             version 1060
 *             ueCap                       : UE Capability Information
 *             Structure
 *
...
PUBLIC S16 wrUmmStoreUeCapv1060
(
 NhuUE_EUTRA_Cap_v1060_IEs            *ueCap_v1060,
 NhuUE_EUTRA_Cap_v1060_IEs            *nhuDec_v1060,
 WrUeCapInfo                          *ueCap
 )
{
   U32 idx = 0;
   U32 numComp = 0;
   if(PRSNT_NODEF == nhuDec_v1060->pres.pres)
   {
      wrFillTknU8(&(ueCap_v1060->pres),PRSNT_NODEF);

      /* fdd_Add_UE_EUTRA_Capabilities_v1060 */
      if(PRSNT_NODEF == nhuDec_v1060->fdd_Add_UE_EUTRA_Capabilities_v1060.pres.pres)
      {
         wrFillTknU8(&(ueCap_v1060->fdd_Add_UE_EUTRA_Capabilities_v1060.pres),PRSNT_NODEF);

         NhuUE_EUTRA_CapAddXDD_Mode_v1060 *ueCapfdd_v1060 = &ueCap_v1060->fdd_Add_UE_EUTRA_Capabilities_v1060;
         NhuUE_EUTRA_CapAddXDD_Mode_v1060 *nhuDecfdd_v1060 = &nhuDec_v1060->fdd_Add_UE_EUTRA_Capabilities_v1060;
         if(PRSNT_NODEF == nhuDec_v1060->fdd_Add_UE_EUTRA_Capabilities_v1060.phyLyrParams_v1060.pres.pres)
         {
            wrFillTknU8(&(ueCapfdd_v1060->phyLyrParams_v1060.pres),PRSNT_NODEF);
            NhuPhyLyrParams_v1020 *ueCapphyLyrParams_v1060 = &ueCapfdd_v1060->phyLyrParams_v1060;
            NhuPhyLyrParams_v1020 *nhuDecphyLyrParams_v1060 = &nhuDecfdd_v1060->phyLyrParams_v1060;

            wrFillTknU32(&(ueCapphyLyrParams_v1060->twoAntennaPortsForPUCCH_r10),
                  nhuDecphyLyrParams_v1060->twoAntennaPortsForPUCCH_r10.val);
            
            wrFillTknU32(&(ueCapphyLyrParams_v1060->tm9_With_8Tx_FDD_r10),
                  nhuDecphyLyrParams_v1060->tm9_With_8Tx_FDD_r10.val);

            wrFillTknU32(&(ueCapphyLyrParams_v1060->pmi_Disabling_r10),
                  nhuDecphyLyrParams_v1060->pmi_Disabling_r10.val);

            wrFillTknU32(&(ueCapphyLyrParams_v1060->crossCarrierScheduling_r10),
                  nhuDecphyLyrParams_v1060->crossCarrierScheduling_r10.val);

            wrFillTknU32(&(ueCapphyLyrParams_v1060->simultaneousPUCCH_PUSCH_r10),
                  nhuDecphyLyrParams_v1060->simultaneousPUCCH_PUSCH_r10.val);

            wrFillTknU32(&(ueCapphyLyrParams_v1060->multiClusterPUSCH_WithinCC_r10),
                  nhuDecphyLyrParams_v1060->multiClusterPUSCH_WithinCC_r10.val);

            wrFillTknU16(&(ueCapphyLyrParams_v1060->nonContiguousUL_RA_WithinCC_Lst_r10.noComp),
                  nhuDecphyLyrParams_v1060->nonContiguousUL_RA_WithinCC_Lst_r10.noComp.val);

            WR_GET_MEM(&ueCap->memCp, sizeof(NhuNonContiguousUL_RA_WithinCC_r10) * \
                  nhuDecphyLyrParams_v1060->nonContiguousUL_RA_WithinCC_Lst_r10.noComp.val,
                  &ueCapphyLyrParams_v1060->nonContiguousUL_RA_WithinCC_Lst_r10.member);

            if(NULLP == ueCapphyLyrParams_v1060->nonContiguousUL_RA_WithinCC_Lst_r10.member)
            {
               RLOG0(L_FATAL, "Failed to allocate memory for ueCapphyLyrParams_v1060-> \
                     nonContiguousUL_RA_WithinCC_Lst_r10");
               RETVALUE(RFAILED);
            }


            NhuNonContiguousUL_RA_WithinCC_Lst_r10 *ueCapnonContUL_RA_WithnCC_Lst_r10 = \
                      &ueCapphyLyrParams_v1060->nonContiguousUL_RA_WithinCC_Lst_r10;
            NhuNonContiguousUL_RA_WithinCC_Lst_r10 *nhuDecnonContUL_RA_WithnCC_Lst_r10 = \
                      &nhuDecphyLyrParams_v1060->nonContiguousUL_RA_WithinCC_Lst_r10;

            numComp = nhuDecnonContUL_RA_WithnCC_Lst_r10->noComp.val;
            if(numComp > 0)
            {
               for(idx = 0; idx < numComp; idx++)
               {
                  if(PRSNT_NODEF == nhuDecnonContUL_RA_WithnCC_Lst_r10->member[idx].pres.pres)
                  {
                     wrFillTknU8(&(ueCapnonContUL_RA_WithnCC_Lst_r10->member[idx].pres),PRSNT_NODEF);
                     wrFillTknU32(&(ueCapnonContUL_RA_WithnCC_Lst_r10->member[idx]. \
                              nonContiguousUL_RA_WithinCC_Info_r10),nhuDecnonContUL_RA_WithnCC_Lst_r10-> \
                           member[idx].nonContiguousUL_RA_WithinCC_Info_r10.val);
                  }
               }
            }
         ...
         /* Copying featureGroupIndRel10_v1060 */
         if(nhuDecfdd_v1060->featureGroupIndRel10_v1060.pres == PRSNT_NODEF)
         {
            ueCapfdd_v1060->featureGroupIndRel10_v1060.pres = PRSNT_NODEF;
            WR_GET_MEM(&ueCap->memCp, sizeof(U32),
                  &ueCapfdd_v1060->featureGroupIndRel10_v1060.val);

            if(NULLP == ueCapfdd_v1060->featureGroupIndRel10_v1060.val)
            {
               RLOG0(L_FATAL, "Failed to allocate memory for featureGroupIndRel10_v1060");
               RETVALUE(RFAILED);

            }


            if( ueCapfdd_v1060->featureGroupIndRel10_v1060.val == NULLP)
            {
               RLOG0(L_FATAL, "Failed to allocate memory for feature group indicator Rel10_v1060");
               RETVALUE(RFAILED);
            }
            ueCapfdd_v1060->featureGroupIndRel10_v1060.len = nhuDecfdd_v1060->featureGroupIndRel10_v1060.len;
            cmMemcpy((U8*)ueCapfdd_v1060->featureGroupIndRel10_v1060.val,
                  (U8*)nhuDecfdd_v1060->featureGroupIndRel10_v1060.val,sizeof(U32));
         }
         else
         {
            ueCapfdd_v1060->featureGroupIndRel10_v1060.pres = NOTPRSNT; 

         }

         /*Copying interRAT_ParamsCDMA2000_v1060*/

         if(nhuDecfdd_v1060->interRAT_ParamsCDMA2000_v1060.pres.pres == PRSNT_NODEF)
         {
            wrFillTknU8(&(ueCapfdd_v1060->interRAT_ParamsCDMA2000_v1060.pres),PRSNT_NODEF);
            wrFillTknU32(&(ueCapfdd_v1060->interRAT_ParamsCDMA2000_v1060.e_CSFB_dual_1XRTT_r10),
                  nhuDecfdd_v1060->interRAT_ParamsCDMA2000_v1060.e_CSFB_dual_1XRTT_r10.val);
         }

         /*Copying interRAT_ParamsUTRA_TDD_v1060*/

         if(nhuDecfdd_v1060->interRAT_ParamsUTRA_TDD_v1060.pres.pres == PRSNT_NODEF)
         {
            wrFillTknU8(&(ueCapfdd_v1060->interRAT_ParamsUTRA_TDD_v1060.pres),PRSNT_NODEF);
            wrFillTknU32(&(ueCapfdd_v1060->interRAT_ParamsUTRA_TDD_v1060.e_RedirectionUTRA_TDD_r10),
                  nhuDecfdd_v1060->interRAT_ParamsUTRA_TDD_v1060.e_RedirectionUTRA_TDD_r10.val);
         }


      }/*if end for fdd_Add_UE_EUTRA_Capabilities_v1060 */

      /*Copying tdd_Add_UE_EUTRA_Capabilities_v1060*/
      NhuUE_EUTRA_CapAddXDD_Mode_v1060 *ueCaptdd_v1060 = &ueCap_v1060->tdd_Add_UE_EUTRA_Capabilities_v1060;
      NhuUE_EUTRA_CapAddXDD_Mode_v1060 *nhuDectdd_v1060 = &nhuDec_v1060->tdd_Add_UE_EUTRA_Capabilities_v1060; 
      if(PRSNT_NODEF == nhuDectdd_v1060->pres.pres)
      {
         wrFillTknU8(&(ueCaptdd_v1060->pres),PRSNT_NODEF);
         if(PRSNT_NODEF == nhuDectdd_v1060->phyLyrParams_v1060.pres.pres)
         {
            wrFillTknU8(&(ueCaptdd_v1060->phyLyrParams_v1060.pres),PRSNT_NODEF);

            wrFillTknU32(&(ueCaptdd_v1060->phyLyrParams_v1060.twoAntennaPortsForPUCCH_r10),
                  nhuDectdd_v1060->phyLyrParams_v1060.twoAntennaPortsForPUCCH_r10.val);

            wrFillTknU32(&(ueCaptdd_v1060->phyLyrParams_v1060.tm9_With_8Tx_FDD_r10),
                  nhuDectdd_v1060->phyLyrParams_v1060.tm9_With_8Tx_FDD_r10.val);

            wrFillTknU32(&(ueCaptdd_v1060->phyLyrParams_v1060.pmi_Disabling_r10),
                  nhuDectdd_v1060->phyLyrParams_v1060.pmi_Disabling_r10.val);

            wrFillTknU32(&(ueCaptdd_v1060->phyLyrParams_v1060.crossCarrierScheduling_r10),
                  nhuDectdd_v1060->phyLyrParams_v1060.crossCarrierScheduling_r10.val);

            wrFillTknU32(&(ueCaptdd_v1060->phyLyrParams_v1060.simultaneousPUCCH_PUSCH_r10),
                  nhuDectdd_v1060->phyLyrParams_v1060.simultaneousPUCCH_PUSCH_r10.val);

            wrFillTknU32(&(ueCaptdd_v1060->phyLyrParams_v1060.multiClusterPUSCH_WithinCC_r10),
                  nhuDectdd_v1060->phyLyrParams_v1060.multiClusterPUSCH_WithinCC_r10.val);

            wrFillTknU16(&(ueCaptdd_v1060->phyLyrParams_v1060.nonContiguousUL_RA_WithinCC_Lst_r10.noComp),
                  nhuDectdd_v1060->phyLyrParams_v1060.nonContiguousUL_RA_WithinCC_Lst_r10.noComp.val);

            WR_GET_MEM(&ueCap->memCp, sizeof(NhuNonContiguousUL_RA_WithinCC_r10) * \
                  nhuDectdd_v1060->phyLyrParams_v1060.nonContiguousUL_RA_WithinCC_Lst_r10.noComp.val,
                  &ueCaptdd_v1060->phyLyrParams_v1060.nonContiguousUL_RA_WithinCC_Lst_r10.member);

            if(NULLP == ueCaptdd_v1060->phyLyrParams_v1060.nonContiguousUL_RA_WithinCC_Lst_r10.member)
            {
               RLOG0(L_FATAL, "Failed to allocate memory for nonContiguousUL_RA_WithinCC_Lst_r10 tdd v1060");
               RETVALUE(RFAILED);

            }

            numComp = nhuDectdd_v1060->phyLyrParams_v1060.nonContiguousUL_RA_WithinCC_Lst_r10.noComp.val;
            if(numComp > 0)
            {
               for(idx = 0; idx < numComp; idx++)
               {
                  if(PRSNT_NODEF == nhuDectdd_v1060->phyLyrParams_v1060. \
                        nonContiguousUL_RA_WithinCC_Lst_r10.member[idx].pres.pres)
                  {
                     wrFillTknU8(&(ueCaptdd_v1060->phyLyrParams_v1060. \
                              nonContiguousUL_RA_WithinCC_Lst_r10.member[idx].pres),PRSNT_NODEF);

                     wrFillTknU32(&(ueCaptdd_v1060->phyLyrParams_v1060.nonContiguousUL_RA_WithinCC_Lst_r10. \
                              member[idx].nonContiguousUL_RA_WithinCC_Info_r10),nhuDectdd_v1060-> \
                           phyLyrParams_v1060.nonContiguousUL_RA_WithinCC_Lst_r10.member[idx]. \
                           nonContiguousUL_RA_WithinCC_Info_r10.val);

                  }
               }
            }

         }/*NhuPhyLyrParams_v1020 ends*/

         /* Copying featureGroupIndRel10_v1060 */
         if(nhuDectdd_v1060->featureGroupIndRel10_v1060.pres == PRSNT_NODEF)
         {
            ueCaptdd_v1060->featureGroupIndRel10_v1060.pres = PRSNT_NODEF;
            WR_GET_MEM(&ueCap->memCp, sizeof(U32),
                  &ueCaptdd_v1060->featureGroupIndRel10_v1060.val);
            if( ueCaptdd_v1060->featureGroupIndRel10_v1060.val == NULLP)
            {
               RLOG0(L_FATAL, "Failed to allocate memory for feature group indicator Rel10_v1060");
               RETVALUE(RFAILED);
            }
            ueCaptdd_v1060->featureGroupIndRel10_v1060.len = nhuDectdd_v1060->featureGroupIndRel10_v1060.len;
            cmMemcpy((U8*)ueCaptdd_v1060->featureGroupIndRel10_v1060.val,
                  (U8*)nhuDectdd_v1060->featureGroupIndRel10_v1060.val,sizeof(U32));
         }
         else
         {
            ueCaptdd_v1060->featureGroupIndRel10_v1060.pres = NOTPRSNT;

         }

         /*Copying interRAT_ParamsCDMA2000_v1060*/

         if(nhuDectdd_v1060->interRAT_ParamsCDMA2000_v1060.pres.pres == PRSNT_NODEF)
         {
            wrFillTknU8(&(ueCaptdd_v1060->interRAT_ParamsCDMA2000_v1060.pres),PRSNT_NODEF);
            wrFillTknU32(&(ueCaptdd_v1060->interRAT_ParamsCDMA2000_v1060.e_CSFB_dual_1XRTT_r10),
                  nhuDectdd_v1060->interRAT_ParamsCDMA2000_v1060.e_CSFB_dual_1XRTT_r10.val);
         }

         /*Copying interRAT_ParamsUTRA_TDD_v1060*/

         if(nhuDectdd_v1060->interRAT_ParamsUTRA_TDD_v1060.pres.pres == PRSNT_NODEF)
         {
            wrFillTknU8(&(ueCaptdd_v1060->interRAT_ParamsUTRA_TDD_v1060.pres),PRSNT_NODEF);
            wrFillTknU32(&(ueCaptdd_v1060->interRAT_ParamsUTRA_TDD_v1060.e_RedirectionUTRA_TDD_r10),
                  nhuDectdd_v1060->interRAT_ParamsUTRA_TDD_v1060.e_RedirectionUTRA_TDD_r10.val);
         }


      } /* tdd_Add_UE_EUTRA_Capabilities_v1060 ends */

      /* Copying rf_Params_v1060 */
      if(PRSNT_NODEF == nhuDec_v1060->rf_Params_v1060.pres.pres)
      {
         wrFillTknU8(&(ueCap_v1060->rf_Params_v1060.pres),PRSNT_NODEF);
         wrFillTknU16(&(ueCap_v1060->rf_Params_v1060.supportedBandCombinationExt_r10.noComp),
               nhuDec_v1060->rf_Params_v1060.supportedBandCombinationExt_r10.noComp.val);
        WR_GET_MEM(&ueCap->memCp, sizeof(NhuBandCombinationParamsExt_r10) * \
               nhuDec_v1060->rf_Params_v1060.supportedBandCombinationExt_r10.noComp.val,\
               &ueCap_v1060->rf_Params_v1060.supportedBandCombinationExt_r10.member);

         if(NULLP == ueCap_v1060->rf_Params_v1060.supportedBandCombinationExt_r10.member)
         {
            RLOG0(L_FATAL, "Failed to allocate memory for NhuSuppBandCombinationExt_r10");
            RETVALUE(RFAILED);

         }


         NhuSuppBandCombinationExt_r10 *ueCapsuppdBndCombnExt_r10 = &ueCap_v1060-> \
                rf_Params_v1060.supportedBandCombinationExt_r10;

         NhuSuppBandCombinationExt_r10 *nhuDecsuppdBndCombnExt_r10 = &nhuDec_v1060-> \
                rf_Params_v1060.supportedBandCombinationExt_r10;

         numComp = nhuDecsuppdBndCombnExt_r10->noComp.val;
         if(numComp > 0)
         {
            for(idx = 0; idx < numComp; idx++)
            {
               if(PRSNT_NODEF == nhuDecsuppdBndCombnExt_r10->member[idx].pres.pres)
               {
                   wrFillTknU8(&(ueCapsuppdBndCombnExt_r10->member[idx].pres),
                         PRSNT_NODEF);
                   if(nhuDecsuppdBndCombnExt_r10->member[idx].supportedBandwidthCombinationSet_r10.\
                         pres == PRSNT_NODEF)
                      {
                         ueCapsuppdBndCombnExt_r10->member[idx].supportedBandwidthCombinationSet_r10.\
                            pres = PRSNT_NODEF;
                      
                  WR_GET_MEM(&ueCap->memCp, sizeof(U32),&ueCapsuppdBndCombnExt_r10-> \
                        member[idx].supportedBandwidthCombinationSet_r10.val);
                  if(NULLP == ueCapsuppdBndCombnExt_r10->member[idx].supportedBandwidthCombinationSet_r10.val)
                  {
                     RLOG0(L_FATAL, "Failed to allocate memory for supportedBandwidthCombinationSet_r10");
                     RETVALUE(RFAILED);
                  }
                  ueCapsuppdBndCombnExt_r10->member[idx].supportedBandwidthCombinationSet_r10.len = \
                        nhuDecsuppdBndCombnExt_r10->member[idx].supportedBandwidthCombinationSet_r10.len;

                  cmMemcpy((U8*)ueCapsuppdBndCombnExt_r10->member[idx]. \
                        supportedBandwidthCombinationSet_r10.val,(U8*)nhuDecsuppdBndCombnExt_r10-> \
                        member[idx].supportedBandwidthCombinationSet_r10.val,sizeof(U32));
                      }
                   else
                      {
                         ueCapsuppdBndCombnExt_r10->member[idx].supportedBandwidthCombinationSet_r10.\
