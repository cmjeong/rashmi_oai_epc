...
)
{
   ...
   if((cellCb->measCellCb->anrRptCfg != WR_UMM_STRONG_CELL_CFG)    &&
         ...
         (cellCb->measCellCb->anrRptCfg != WR_UMM_EVENT_A4_A5_INTER_CFG)) 
   {
...
#ifdef LTE_ADV
   cellCb->measCellCfg->isMeasReqForSCellAdd = rcvMeasCfgParams->isMeasReqForSCellAdd; 
#endif
...
)
{
   ...
   {
      RLOG0(L_ERROR, "Measurement object Id Node is NULL");
      RETVALUE(WR_UMM_MEAS_INVALID_MEAS_ID);
   }
   RETVALUE(ROK);
}
...
)
{
   ...
   {
      RLOG0(L_ERROR, "Measurement ID not found");
      RETVALUE(WR_UMM_MEAS_INVALID_MEAS_ID);
   }
   RETVALUE(ROK);
}
...
#ifdef LTE_ADV
      case WR_UMM_MEAS_GAP_SCELL_ADD:
#endif
...
)
{
   ...
   {
      RLOG0(L_ERROR, "Measurement Id is not Available");
      RETVALUE(RFAILED);
   }
...
#ifdef LTE_ADV
/** @brief This function is responsible for to fill Event A4 report config
 * Object.
 *
 *  @details
 *  Function:wrUmmMeasFillEvntA4RptObj
 *      
 *  Processing steps:
 *      - Fill the report config for event A4 report config object
 *      
 *   @param[out] rptCfgEutra: pointer to Eutra report config Object.
 *   @param[in] thrshldTyp: threshold type.
 *   @param[out] evntTyp: pointer to Event Type structure.
 *   @param[in] a4EvntRptCfg: pointer to measurement config.
 *   @return S16
 *       SUCCESS : ROK
 *       FAILURE :RFAILED
*/
PRIVATE S16 wrUmmMeasFillEvntA4RptObj
(
NhuReportConfigEUTRA                *rptCfgEutra,
WrThrshldTyp                        thrshldTyp,
NhuReportConfigEUTRAtriggerTypevent *evntTyp, 
LwrMeasEvntRptCfg                    *a4EvntRptCfg
)
{
   TRC2(wrUmmMeasFillEvntA4RptObj)

   wrFillTknU8(&rptCfgEutra->pres, 1);
   WR_FILL_TKN_UINT(rptCfgEutra->triggerTyp.choice, TRIGGERTYP_EVENT)
   wrFillTknU8(&evntTyp->pres, 1);;
   WR_FILL_TKN_UINT(evntTyp->eventId.choice, EVENTID_EVENTA4)
   wrFillTknU8(&evntTyp->eventId.val.eventA4.pres, 1);
   if(thrshldTyp == WR_UMM_MEAS_THRESHOLD_RSRP)
   {
      WR_FILL_TKN_UINT(evntTyp->eventId.val.eventA4.a4_Threshold.choice,
                                        THRESHOLDEUTRA_THRESHOLD_RSRP)
      WR_FILL_TKN_UINT(evntTyp->eventId.val.eventA4.a4_Threshold.val.threshold_RSRP,
                                         a4EvntRptCfg-> threshold_RSRP)
   } 
   else 
   {
      WR_FILL_TKN_UINT(evntTyp->eventId.val.eventA4.a4_Threshold.choice,
                                         THRESHOLDEUTRA_THRESHOLD_RSRQ)
      WR_FILL_TKN_UINT(evntTyp->eventId.val.eventA4.a4_Threshold.val.threshold_RSRQ,
                                         a4EvntRptCfg-> threshold_RSRQ)
   }
   WR_FILL_TKN_UINT(evntTyp->hysteresis , a4EvntRptCfg->hysteresis)
   WR_FILL_TKN_UINT(evntTyp->timeToTrigger , a4EvntRptCfg->timeToTrigger)
   if(thrshldTyp == WR_UMM_MEAS_THRESHOLD_RSRP)
   {
      WR_FILL_TKN_UINT(rptCfgEutra->triggerQuantity ,THRESHOLDEUTRA_THRESHOLD_RSRP)
   } 
   ...
   WR_FILL_TKN_UINT(rptCfgEutra->reportQuantity , a4EvntRptCfg->reportQty)
   WR_FILL_TKN_UINT(rptCfgEutra->maxReportCells , a4EvntRptCfg->maxRptCells)
   WR_FILL_TKN_UINT(rptCfgEutra->reportInterval , a4EvntRptCfg->reportInterval)
   WR_FILL_TKN_UINT(rptCfgEutra->reportAmount , a4EvntRptCfg->reportAmount)
...
#ifdef LTE_ADV
      case WR_UMM_MEAS_EVENT_A4:
      {
         rptCfgEutra = &rptCfg->val.eventA4;
         eutraEvntTyp = &rptCfgEutra->triggerTyp.val.event; 
         
         wrUmmMeasFillEvntA4RptObj(rptCfgEutra, rptCfgInfo->thrshldTyp, eutraEvntTyp,
              &measCellCfg->measA4EvtRptCfg[WR_MEAS_EVENTA4_SCELL_ADD]);
         break;
      }
#endif
...
)
{
   ...
   TRC2(wrUmmMeasRptAndMeasIdCfg)
      ...
      if(ROK != wrUmmMeasFindRptCfgID(&rptCfgId, rptCfgInfo, &measUeCb->rptCfgLst))
      {
         ...
         {
            RLOG0(L_ERROR, "Invalid report config Id");
            RETVALUE(RFAILED);
         }
      }
...
#ifdef LTE_ADV
/** @brief This function is responsible for to configure measurement config 
 *  for receiving Event A4 measurement report.
 *
 *  @details*
 *  Function:wrUmmMeasInterCfgForScellAdd
 *    
 *  Processing steps:
 *      - For Inter frequency Objects Configure Event A4 
 *  
 *   @param[in] measUeCb: pointer to measurement ue Cb.
 ...
 *       FAILED  : RFAILED
 */
PRIVATE S16 wrUmmMeasInterCfgForScellAdd
(
 WrUeCb                      *ueCb,
...
)
{
   ...
   U16                          idx = 0;
   Bool                         isScellANeighbour = FALSE;

   TRC2(wrUmmMeasInterCfgForScellAdd)
   ...
   while (NULLP != interMeasObj)
   {
      ...
      RLOG1(L_DEBUG,"A4:Num of Cells=%d\n",wrEmmCb.numCells);

      for(idx=0; idx<wrEmmCb.numCells; idx++)
      {
         /* only process for active cells */
         if(NULLP == wrEmmCb.cellCb[idx])
            continue;

         RLOG2(L_DEBUG,"InterFreq:A4:carrFreq=%lu,wrEmmCb.cellCb[idx]->dlCarrierFreq=%d\n",eutraCellObj->carrFreq,wrEmmCb.cellCb[idx]->dlCarrierFreq);
         if(eutraCellObj->carrFreq == wrEmmCb.cellCb[idx]->dlCarrierFreq)
         {
            isScellANeighbour = TRUE;
            RLOG1(L_DEBUG,"Potential Scell is a neighbour with carrFreq=%lu\n",eutraCellObj->carrFreq);
            break;
         }
      }
      if(TRUE == isScellANeighbour)
      {
         ...
         if(isUeSupported == TRUE)
         {
                     ...
                     carrFreq, measUeCb, &measObjId, &eutraCellObj))
            {
               ...
               if(measObjId != WR_UMM_MEAS_INVALID_MEAS_ID)
               {
                  ...
                  RLOG1(L_DEBUG,"A4:Configuring Inter frequency measurements for UE Crnti [%d] \n", ueCb->crnti); 
               }
               else
               { 
            ...
            {
               /*ccpu00133426 - inter freq ho requirement - end*/
               rptCfgInfo.rptCfgTyp = WR_UMM_MEAS_EVENT_A4;
               if(!measCellCfg->measA4EvtRptCfg[WR_MEAS_EVENTA4_SCELL_ADD].triggerQty)
               {
            ...
            {
               RLOG0(L_DEBUG, " BIT 14 in FGI is Zero,Hence A4 not configured\n");
            }
            /* Report Config for Event A4*/
            if(ROK != wrUmmMeasRptAndMeasIdCfg(WR_UMM_MEAS_TYPE_SCELL_ADD, measObjId,
                     &rptCfgInfo, ueCb, measCellCfg, addMeasInfo))
            {
               RLOG0(L_ERROR, "Invalid report and Meas config Id");
               RETVALUE(RFAILED);
            }
            RLOG1(L_ALWAYS,"A4 Event Configured for UE Crnti [%d] \n", ueCb->crnti); 
         }
      }
...
)
{
   ...
   if(((measCellCb->anrRptCfg == WR_UMM_EVENT_A3_CFG) || 
       (measCellCb->anrRptCfg == WR_UMM_EVENT_A4_A5_INTER_CFG)) && 
...
#ifdef LTE_ADV
   /* Report Config for Event A4 */
   RLOG3(L_ALWAYS,"isCaEnabled=%d:isMeasReqForSCellAdd=%d:isUeCaCap=%d\n",
         wrEmmCb.isCaEnabled,cellCb->measCellCfg->isMeasReqForSCellAdd,ueCb->isUeCaCap);
     ...
     )
   {
      ...
      if(TRUE == wrUmmUtilFeatureGrpInd(ueCb->cellId, ueCb->crnti,
               WR_UMM_A4_A5_MEAS_SUPPORTED))
      {
         if(ROK != wrUmmMeasInterCfgForScellAdd(ueCb, addMeasInfo, cellCb))
         {
            RLOG0(L_DEBUG,"Event A4 NOT Configured for SCell Addition\n");
         }
         else
         {
            measCfgInfo->measGapStatus = transCb->u.measTransCb.measGapStatus; 
         }
      }
      else
      {
         RLOG0(L_DEBUG,"FGI Bit 14 not supported\n");
      }
   }
#endif
   RETVALUE(ROK);
         ...
         if(ueUtraMeasObj->arfcn == cellUtraMeasObj->arfcn)
         {
            ...
            else
            {
               ...
               if((wrUmmUtraDynUeMeasObjCfg(cellUtraMeasObj, ueUtraMeasObj, ueCb, 
                     &nrUpdtNode->utraNode[nrUpdtNode->numUtra++]))== RFAILED)
               {
...
#ifdef LTE_ADV
         if (rptCfgObj->rptCfgType == WR_UMM_MEAS_EVENT_A4)
         {
            WR_FILL_TKN_UINT(nhuRptCfgObj->choice, REPORTCONFIG_REPORTCONFIGEUTRA)
            cmMemcpy((U8*)&nhuRptCfgObj->val.reportConfigEUTRA,(U8*)&rptCfgObj->val.
                  eventA4, sizeof(NhuReportConfigEUTRA));
            break;
         }
#endif
...
)
{
   ...
   if((NULLP != nhuMeasCfg) && (NULLP != measCfgInfo))
   {
      ...
      wrUmmMeasCreatIntraInitalCfgMsg(cellCb, measCfgInfo, ueCb, cause, transCb);
      /* Configuring Meas Objects and report configuration List*/
      if(ROK != wrMeasFillNhuMsg (memCp, nhuMeasCfg, ueCb, measCfgInfo, measCellCfg, transCb->transId))
      {
...
/* LTE_ADV ends */
   ...
   measCellCfg = cellCb->measCellCfg;
/* LTE_ADV starts */
   WR_EMM_GET_CELLCB(emmCellCb, measUeCb->cellId)
   if(emmCellCb == NULLP)
   {
...
#ifdef LTE_ADV
/** @brief This function is responsible for adding Scell and 
 *  Configuring measurements for UE with A2 for Scell Relese
 * 
 * @details
 *
 * Function:wrUmmMeasEvntA4MeasConfig
 *
 *   Processing steps:
 *    - Add Scell and Configure A2 for SCell Release
 ...
 *       FAILED  : RFAILED
 **/
PRIVATE S16 wrUmmMeasEvntA4MeasConfig
(
 U32                          recvMeasId,
 WrUmmTransCb                 *transCb,
 U16                          sCellId
)
{
   ...
   TRC2(wrUmmMeasEvntA4MeasConfig)
   ...
   WR_UMM_GET_CELLCB(cellCb, cellId)
   WR_EMM_GET_CELLCB(emmCellCb, cellId)
   if((NULLP == cellCb) || (emmCellCb == NULLP))
   {
      RLOG0(L_ERROR, "cellCb is NULL");
      RETVALUE(RFAILED);
   }
   /*Delete Event A4 Meas ID*/
   ...
   for ( idx = 0; idx < measIdRmvCnt; idx++ )
   {
      ...
      {
         RLOG0(L_WARNING, "MultipleA4 report- already received an \
               A4 report and processing it, hence ignoring\n");
         RETVALUE(ROK);
      }
   }
   ...
   wrUmmMeasRelMeasGap (WR_UMM_MEAS_GAP_SCELL_ADD,
   ...
   {
       sCellIdLst[idx] = sCellId;
   }
   RLOG0(L_ALWAYS, "Triggering Scell addition based on A4\n");

   wrUmmSCellTrans(ueCb, WR_UMM_SCELL_ADD, 1, sCellIdLst);

   /*TODO:Configure A2 for SCell Release*/
   RETVALUE(ROK);
}
#endif
...
#ifdef LTE_ADV
/** @brief This function is responsible for measurement report
 * received from UE based on Evenet A4.
 * 
 * @details
 *
 * Function:wrUmmMeasProcessEvntA4
 *
 *   Processing steps:
 *    - Check A4 threshold value.
 *    - Check the PCI if it belongs to the intra enode cell.
 *
 * @param[in]  measRprt pointer to Measurement report 
 * @param[out] selectedCellId cellId of the cell reported  
 * @return S16
 *       SUCCESS : ROK
 *       FAILED  : RFAILED
 */
PRIVATE S16 wrUmmMeasProcessEvntA4 
(
NhuMeasResults               *measRprt,
U16                          *selectedCellId
)
{
   U16 idx = 0;
   U32 phyCellId = 0;
   if(0 != measRprt->measResultNeighCells.val.measResultLstEUTRA.member[0].measResult.rsrpResult.val)
   {
      for(idx=0; idx<wrEmmCb.numCells; idx++)
      {
         /* only process for active cells */
         ...
         if(PRSNT_NODEF == measRprt->measResultNeighCells.val.measResultLstEUTRA.member[0].physCellId.pres)
         {
            phyCellId = measRprt->measResultNeighCells.val.measResultLstEUTRA.member[0].physCellId.val;
         }
         if(phyCellId == wrEmmCb.cellCb[idx]->physCellId)
         {
            RLOG1(L_DEBUG, "[OTA] PCI(%lu) is received in A4 MeasurementReport\n",phyCellId);
            *selectedCellId = wrEmmCb.cellCb[idx]->cellId;
            RETVALUE(ROK);
         }
         else
         {
            RLOG1(L_DEBUG, "[OTA] PCI(%lu) Received in A4 MeasurementReport \
                  does not belong to this eNodeb\n",phyCellId);
            continue;
         }
      }
   }
   ...
   {
      RLOG1(L_EVENT,"[OTA] Ghost Cell reported in A4 = %d\n",
            measRprt->measResultNeighCells.val.measResultLstEUTRA.member[0].measResult.rsrpResult.val);
      RETVALUE(RFAILED);
   }
   RETVALUE(RFAILED);
}
...
)
{
   ...
   switch (rptCfgTyp)
   {
      ...
      case WR_UMM_MEAS_EVENT_A4:
         {
            U16 selectedCellId;
            if(ROK == wrUmmMeasProcessEvntA4(measRprt, &selectedCellId))
            {
               RLOG1(L_ALWAYS,"[OTA] Received A4 MeasurementReport with threshold = %d\n",
                     measRprt->measResultNeighCells.val.measResultLstEUTRA.member[0].measResult.rsrpResult.val);
               if(ROK != wrUmmMeasEvntA4MeasConfig(recvMeasId, transCb, selectedCellId))
               {
                  RLOG_ARG0(L_EVENT, DBG_CRNTI,ueCb->crnti,"[OTA] Adding Scell failed\n");
                  wrUmmTransComplete(transCb);
                  RETVALUE(RFAILED);
               }
            }
 ...
 )
{
   ...
   for(freqCnt = 0; freqCnt < nrUpdtInfo->eutran.numFreq; freqCnt++)
   {
      ...
      if((ueCb->hoRstrctInfo != NULLP)&&(ueCb->hrlMeasCfg.intraMeasObj != NULLP))
      {
         if((ueCb->hrlMeasCfg.intraMeasObj != NULLP) &&
