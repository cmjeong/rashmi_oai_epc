...
/* LTE_ADV ends */
   ...
   if (wrSmDfltNumUnLicnsdCells != 0)
   {
      static U32 cellCount = 0;
      if (cellCount >= (wrSmDfltNumCells - wrSmDfltNumUnLicnsdCells))
      {
         lwrMng->t.protoCfg.t.cellCfg.t.addCell.cellCfgGrp[cfgGrpIndex].grpType = WR_CFG_TYPE_UNLICNSD_CELL;
         lwrMng->t.protoCfg.t.cellCfg.t.addCell.cellCfgGrp[cfgGrpIndex].t.lteUCfgGrp = smCfgCb.lteUCfg;
         cfgGrpIndex++;
      }
      cellCount++;
   }
