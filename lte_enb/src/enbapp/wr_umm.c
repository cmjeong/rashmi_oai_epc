...
#ifdef LTE_ADV
/** @brief This function returs the pointer to UE control block based on
 *         cell id and UE CRNTI.
 *
 * @details
 *
 *     Function: wrUmmGetUeFrmUeCrnti
 *
 *         Processing steps:
 *         - Get the Cell control Block using cellId
 *         - If the location of ue index in the Array of UeCbs id not NULL,
 *           - validate that the ue CRNTI is for this ue
 *         - Return the address of the UE Control Block
 *
 * @param WrUeCb     ueCb 
 * @param WrCellId   cellId 
 ...
 * @return ROK 
 */ 
PUBLIC S16 wrUmmGetUeFrmUeCrnti
(
WrUeCb                       **ueCb,
WrCellId                     cellId,
WrCrnti                      crnti
)
{
   ...
   *ueCb = NULLP;
   ...
   for(ueIdx = 0; ueIdx < WR_MAX_UES_PER_CELL; ueIdx++)
   {
      if ((cellCb->ueCbs[ueIdx] != NULL) &&(cellCb->ueCbs[ueIdx]->crnti == crnti))
      {
         *ueCb = cellCb->ueCbs[ueIdx];
         RETVALUE(ROK);
      }
   }
   RETVALUE(RFAILED);
...
)
{
   ...
   /* Get EMM CellCb*/
   WR_EMM_GET_CELLCB(emmCellCb, ueCb->cellId);
...
/* LTE_ADV_ABS ends */
   ...
   /* CSG_DEV Ends */
#ifdef LTE_ADV
   for(numOfSCells  = 0; numOfSCells  < WR_MAX_SCELL_PER_UE; numOfSCells ++)
   {
      if(ueCb->sCellInfo[numOfSCells ] != NULLP)
      {
         WR_FREE(ueCb->sCellInfo[numOfSCells], sizeof(WrSCellInfo));
      }
   }
#endif
    /*
...
#ifdef LTE_ADV
         /*CA-Dev*/ 
         else if((msg->u.internalMsg->intMsgType == WR_UMM_INTMSG_SCELL_TRANS))
         {
            WrUmmIntSCellTransMsg  *sCellTransInfo = NULLP;
            sCellTransInfo = msg->u.internalMsg->u.sCellInfo;
            if(sCellTransInfo != NULLP)
            {
               WR_FREE(sCellTransInfo, sizeof(WrUmmIntSCellTransMsg));
            }
         } 
#endif
         /*CA-Dev*/ 
...
)
{
   ...
   switch(msgType)
   {
      ...
      /*CA-Dev*/
#ifdef LTE_ADV
      case WR_UMM_INTMSG_SCELL_TRANS:
         {
           *transTyp = WR_UMM_SCELL_TRANS;
         }
         break;
#endif
      /*CA-Dev*/
 ...
 )
{
   ...
   else if (transCb->state == WR_UMM_TRANS_RELEASE_CALL)
   {
      ...
      {
         RLOG0(L_WARNING,"UE Release due to transCb->state == WR_UMM_TRANS_RELEASE_CALL \n");
#ifdef UE_REL_DBG
         printf("\nUE Release due to transCb->state == WR_UMM_TRANS_RELEASE_CALL \n");
#endif
   ...
   switch(wrUmmTmrExpEvnt)
   {
      ...
      case WR_TMR_DETECT_RLF:
      {
         ...
         RLOG_ARG0(L_WARNING, DBG_CRNTI,ueCb->crnti, "CQI Rept Timer Expired "
          ...
          cmLListDelFrm(&(ueCb->tmrLst), &(cb->tmrLnk));
#ifdef UE_REL_DBG
          printf("\n CQI Rept Timer Expired RLF Detected \n");
#endif
#ifndef BRDCM
...
)
{
   ...
   /* ccpu00148952 */
   RLOG_ARG1(L_ALWAYS, DBG_CRNTI, ueCb->crnti, "mobCtrlState[%d] "
         , (U32)ueCb->mobCtrlState);

   RLOG_ARG1(L_ALWAYS, DBG_CRNTI, ueCb->crnti, "hoType[%d] "
         , (U32)ueCb->hoType);
...
#endif
#if 1
   params->rlcParams.u.am.ul.pollByte      = NhuPollBytekB100Enum;
   params->rlcParams.u.am.ul.pollPdu       = NhuPollPDUp4Enum;
   params->rlcParams.u.am.ul.pollRetxTmr   = NhuT_PollRetransmitms100Enum;
   params->rlcParams.u.am.ul.maxRetxThrsh  = NhuUL_AM_RLCmaxRetxThresholdt32Enum;
#else
   params->rlcParams.u.am.ul.pollByte      = NhuPollBytekBinfinityEnum;
   params->rlcParams.u.am.ul.pollPdu       = NhuPollPDUp16Enum;
   params->rlcParams.u.am.ul.pollRetxTmr   = NhuT_PollRetransmitms400Enum;
...
#if 1
   params->rlcParams.u.am.dl.statusProhTmr = NhuT_StatusProhibitms20Enum;
#else
...
#if 1
   params->rlcParams.u.am.dl.reorderingTmr = NhuT_Reorderingms5Enum;//cellCb->rlcCfg.dlModeInfo.drbtReordring;
#else
   params->rlcParams.u.am.dl.reorderingTmr = cellCb->rlcCfg.dlModeInfo.drbtReordring;
#endif
...
#endif
#if 1
   params->rlcParams.u.am.dl.pollByte      = NhuPollBytekB100Enum;
   params->rlcParams.u.am.dl.pollPdu       = NhuPollPDUp4Enum;
   params->rlcParams.u.am.dl.pollRetxTmr   = NhuT_PollRetransmitms100Enum;
   params->rlcParams.u.am.dl.maxRetxThrsh  = NhuUL_AM_RLCmaxRetxThresholdt32Enum;
#else
   params->rlcParams.u.am.dl.pollByte      = NhuPollBytekBinfinityEnum;
   params->rlcParams.u.am.dl.pollPdu       = NhuPollPDUp16Enum;
   params->rlcParams.u.am.dl.pollRetxTmr   = NhuT_PollRetransmitms400Enum;
...
)
{
   ...
   wrUmmCqiRptCfg(cellCb, &cfgAdd->cqiRptCfg);
   wrUmmPdcpCfg(cellCb, &cfgAdd->pdcpInfo);
   wrUmmMacMainCfg(cellCb, &cfgAdd->macMainCfg);
   wrUmmSchReqCfg(cellCb, &cfgAdd->schReqCfg);
   wrUmmPuschDedCfg(cellCb, &cfgAdd->puschDedicated);
   wrUmmPucchPwrCfg(cellCb, &cfgAdd->pucchPwrCfg);
   wrUmmPdschPwrCfg(cellCb, &cfgAdd->pdschCfgDedicated);
   wrUmmQosCfg(cellCb, &cfgAdd->qosCfg);
...
)
{
   ...
   RLOG0(L_WARNING,"UE Release due to wrUmmPdcpDatCfm \n");
#ifdef UE_REL_DBG
   printf("\nUE Release due to wrUmmPdcpDatCfm \n");
#endif
...
)
{
   ...
   cellCb->cqiRptCfg.aperCqiEnbld = cqiRpt->aperCqiEnbld;
   cellCb->cqiRptCfg.cqiAperiodicMode = cqiRpt->cqiAperiodicMode;
   cellCb->cqiRptCfg.nomPdschRsEpreOff = cqiRpt->nomPdschRsEpreOff;
   cellCb->cqiRptCfg.periodRptSetupPres = cqiRpt->periodRptSetupPres;
   cellCb->cqiRptCfg.cqiPucchRsrcIdx = cqiRpt->cqiPucchRsrcIdx;
   cellCb->cqiRptCfg.cqipmiCfgIdx = cqiRpt->cqipmiCfgIdx;
   cellCb->cqiRptCfg.subbndCqik = cqiRpt->subbndCqik;
   cellCb->cqiRptCfg.fmtInd = cqiRpt->fmtInd;
   cellCb->cqiRptCfg.riCfgIdx = cqiRpt->riCfgIdx;
   cellCb->cqiRptCfg.simulAckNackAndCQI = cqiRpt->simulAckNackAndCQI;
   cellCb->cqiRptCfg.periodicmode = cqiRpt->periodicmode;
   cellCb->cqiRptCfg.riPres = cqiRpt->riPres;
...
)
{
   ...
   cellCb->macMainCfg.timeAlignmentTimerDedicated =
                                 macMainCfg->timeAlignmentTimerDedicated;
   cellCb->macMainCfg.ulSchCfg.maxHARQ_Tx =
                                 macMainCfg->ulSchCfg.maxHARQ_Tx;
   cellCb->macMainCfg.ulSchCfg.periodicBSR_Timer = 
                                 macMainCfg->ulSchCfg.periodicBSR_Timer;
   cellCb->macMainCfg.ulSchCfg.retxBSR_Timer = 
                                 macMainCfg->ulSchCfg.retxBSR_Timer;
   cellCb->macMainCfg.ulSchCfg.ttiBundling = 
                                 macMainCfg->ulSchCfg.ttiBundling;
   cellCb->macMainCfg.ulSchCfg.deltaHqOffset = 
                                 macMainCfg->ulSchCfg.deltaHqOffset;
   cellCb->macMainCfg.phr_Config.periodicPHR_Timer =
                                macMainCfg->phr_Config.periodicPHR_Timer;
   cellCb->macMainCfg.phr_Config.prohibitPHR_Timer = 
                                 macMainCfg->phr_Config.prohibitPHR_Timer;
   cellCb->macMainCfg.phr_Config.dl_PathlossChange =
...
)
{
   ...
   cellCb->pdcpInfo.discardTimer =
                                pdcpInfo->discardTimer;
   cellCb->pdcpInfo.statRprtReqrd = 
                                 pdcpInfo->statRprtReqrd;
   cellCb->pdcpInfo.umSnSize = 
                                 pdcpInfo->umSnSize;
   RETVOID;
}
...
)
{
   ...
   cellCb->schReqCfg.srPucchRsrcIdx =
                                schReqCfg->srPucchRsrcIdx;
   cellCb->schReqCfg.dsrTransMax = 
...
)
{
   ...
   cellCb->puschDedicated.ackIdx =
                                puschDedicated->ackIdx;
   cellCb->puschDedicated.riIdx = 
                                puschDedicated->riIdx;
   cellCb->puschDedicated.cqiIdx = 
                                puschDedicated->cqiIdx;
   cellCb->puschDedicated.puschPwrCfg.pres = 
                     puschDedicated->puschPwrCfg.pres;
   cellCb->puschDedicated.puschPwrCfg.isFmt3a = 
...
)
{
   ...
   cellCb->uePucchPwr.pres =
                                pucchPwrCfg->pres;
   cellCb->uePucchPwr.isFmt3a = 
                                pucchPwrCfg->isFmt3a;
...
/* LTE_ADV ends */
      ...
      case WR_UMM_INTMSG_SCELL_TRANS:
         {
            (*intMsg)->u.sCellInfo = (WrUmmIntSCellTransMsg *)msg;
            break;
         }
      /*CA-Dev*/
#endif
...
)
{
   ...
   WR_EMM_GET_CELLCB(cellCb, wrCellId);
   if(NULLP == cellCb)
   {
      /*127802*/
...
)
{
   ...
   {
      RLOG0(L_WARNING,"UE Release due to wrUmmInitalCtxRspTimerExp \n");
#ifdef UE_REL_DBG
      printf("\nUE Release due to wrUmmInitalCtxRspTimerExp \n");
#endif
 ...
 * @details
 *
 *     Function: wrUmmMeasGapReqdInterfreqR10
 *
 *         Processing steps:
 *         - build message
 *
 ...
 *       -# RFAILED
 */
PRIVATE S16 wrUmmMeasGapReqdInterfreqR10
(
 WrUeCb                       *ueCb,
 U8                           servingBand,
 U8                           targetBand,
 ...
 )
{
   ...
   U8          targetBandIdx  = 0;
   U8          numBandComb    = 0;
   Bool        bandCombPrsnt  = FALSE;
   U16         numEutraBands  = 0;
   U32         idx = 0;
   U32         gapIdx = 0;
   NhuBandCombinationParams_r10  *bandCombinationParamsR10 = NULLP; 
   NhuBandParams_r10             *bandParamsR10 = NULLP;
   NhuSuppBandEUTRA              *supportedBandEutra = NULLP;
   if(ueCb->ueCap->eUTRA_Cap.nonCriticalExtn.nonCriticalExtn.nonCriticalExtn.
         rf_Params_v1020.supportedBandCombination_r10.noComp.pres 
         != NOTPRSNT)
   {
      numBandComb = ueCb->ueCap->eUTRA_Cap.nonCriticalExtn.nonCriticalExtn.nonCriticalExtn.
         rf_Params_v1020.supportedBandCombination_r10.noComp.val;
      for(idx=0; idx<numBandComb; idx++)
      {
         bandCombinationParamsR10 = &(ueCb->ueCap->eUTRA_Cap.nonCriticalExtn.nonCriticalExtn.nonCriticalExtn.
               rf_Params_v1020.supportedBandCombination_r10.member[idx]);
         numEutraBands = bandCombinationParamsR10->noComp.val;
         for(servingBandIdx = 0; servingBandIdx < numEutraBands; servingBandIdx++)
         {
            bandParamsR10 = &(bandCombinationParamsR10->member[servingBandIdx]);
            if(bandParamsR10->bandEUTRA_r10.val == servingBand)
            {
               for(targetBandIdx=0;targetBandIdx < numEutraBands;targetBandIdx++)
               {
                  bandParamsR10 = &(bandCombinationParamsR10->member[targetBandIdx]);
                  if(bandParamsR10->bandEUTRA_r10.val == targetBand)
                  {
                     RLOG2(L_DEBUG, "\nwrUmmMeasGapReqdInterfreqR10:targetBand(%d) is at idx = %d\n",targetBand,targetBandIdx);
                     bandCombPrsnt = TRUE;
                     break;
                  }
               }
               if(TRUE == bandCombPrsnt)
               {
                  numEutraBands = ueCb->ueCap->eUTRA_Cap.rf_Params.supportedBandLstEUTRA.noComp.val;
                  for(gapIdx = 0; gapIdx < numEutraBands; gapIdx++)
                  {
                     supportedBandEutra = &(ueCb->ueCap->eUTRA_Cap.rf_Params.supportedBandLstEUTRA.member[gapIdx]);
                     if(targetBand == supportedBandEutra->bandEUTRA.val)
                     {
#ifndef TENB_ACC
                        *result = ueCb->ueCap->eUTRA_Cap.nonCriticalExtn.nonCriticalExtn.nonCriticalExtn.
                           measParams_v1020.bandCombinationLstEUTRA_r10.member[idx].
                           interFreqBandLst.member[gapIdx].interFreqNeedForGaps.val;
#else
                        *result = TRUE;
#endif
                        break;
   ...
   }/*supportedBandCombination_r10.noComp.pres*/
   else
   {
      RLOG0(L_ERROR, "Supported band combination list is not present\n");
      RETVALUE(RFAILED);
   }
   RLOG1(L_DEBUG, "\nwrUmmMeasGapReqdInterfreqR10: measGap as per UeCap is (%d)\n",*result);
   RETVALUE(ROK);
}/*wrUmmMeasGapReqdInterfreqR10*/
#endif
/** @brief This function is responsible for finding out if measurement gap
 *         is required for a band class(interfreq)  
 * @details
...
)
{
   ...
   if ((ueCb->s1ConCb) && (ueCb->s1ConCb->s1apConnState == WR_S1AP_CONNECTED))
   {
      RLOG0(L_WARNING,"UE Release due to wrUmmPrcRrmUeRelInd \n");
#ifdef UE_REL_DBG
      printf("\nUE Release due to wrUmmPrcRrmUeRelInd \n");
#endif
 ...
 *
 *          Processing steps:
 *          - update the CQI Report Cfg info into UE context/SCell transCb
 *
 *  @param [in] ueCqiRptCfgPtr  : pointer to the UE CB's/SCell transCb CQI Report config Info 
 *  @param [in] rrmCqiRepCfgPtr : pointer to CQI Report config Info in RRM Ue Recfg
 *                                Resp
 *  @return Void
 */

PUBLIC S16 wrUmmCommitUeCqiCfgInfo
(
...
)
{
   ...
   ueCqiRptCfgPtr->nomPdschRsEpreOff = cellCb->cqiRptCfg.nomPdschRsEpreOff;
   ...
   ueCqiRptCfgPtr->riPres = rrmCqiRepCfgPtr->stPrdCqiRepCfg.riConfigIndex.bPres;
   cellCb->cqiRptCfg.riPres = rrmCqiRepCfgPtr->stPrdCqiRepCfg.riConfigIndex.bPres;
   if(TRUE == rrmCqiRepCfgPtr->stPrdCqiRepCfg.riConfigIndex.bPres)
   {
      ueCqiRptCfgPtr->riCfgIdx =
         rrmCqiRepCfgPtr->stPrdCqiRepCfg.riConfigIndex.usRiConfigIndex;
      cellCb->cqiRptCfg.riCfgIdx = 
         rrmCqiRepCfgPtr->stPrdCqiRepCfg.riConfigIndex.usRiConfigIndex;
   }
   RLOG2(L_ALWAYS,"wrUmmCommitUeCqiCfgInfo:riPres = %d and riCfgIdx = %d******\n",
         ueCqiRptCfgPtr->riPres,
         ueCqiRptCfgPtr->riCfgIdx);
...
)
{
   ...
   {
      wrUmmCommitUeCqiCfgInfo(&(ueCb->cqiRptCfg), &(stPhyCfgDed->stCqiRepCfg), ueCb->cellId);
...
#ifdef LTE_ADV
   /* Store PUCCH resource Info in UeCb*/
   if((stUeRadioResCfg->stPhyCfgDed.bPhyCfgDedResBitMap)
         & (U8)RM_RCM_PHY_DED_RES_CFG_SCELL_ACK_N1_RES)
   {
      ueCb->pucchCfgDedPres = TRUE;
      WR_MEM_COPY((&ueCb->pucchCfgDed),(&stUeRadioResCfg->stPhyCfgDed.pucchCfgDed),
            sizeof(RmuUePucchCfg));
   }
#endif
} /* end of wrUmmCommitUePhyCfgDedResInfo */  
...
)
{
         ...
         RM_RCM_PHY_DED_RES_CFG_CQI)
   {
      wrUmmCommitUeCqiCfgInfo(&(ueCb->cqiRptCfg),
            &(rrmUeHoResp->stUeRadioResCfg.stPhyCfgDed.stCqiRepCfg), ueCb->cellId);
   }
...
)
{
   ...
   if(WR_UMM_STRONG_CELL_CFG == cellCb->measCellCb->anrRptCfg) 
   {
      ...
      wrUmmStartPeriodicAnrTrans(ueCb);
           ...
           (WR_UMM_EVENT_A4_A5_INTER_CFG == cellCb->measCellCb->anrRptCfg))
   {
...
)
{
   ...
   switch(ueStaInd->status)
   {
      ...
      {
         RLOG0(L_ALWAYS,"wrUmmSchdUeStaInd:Rcvd - RGR_UESTA_MAC_CRNTI_CE_RECVD\n");
...
)
{
   ...
   if(NULLP == ummcellCb)
   {
      RLOG_ARG0(L_ERROR, DBG_CELLID, cellId, "UMM Cell Cb is NULL ");
      RETVALUE(RFAILED);
   }
 ...
 /* Reset the Radio resource configuraiotn bitmap */
   ...
   ueCb->uePucchPwr.pres                   = ummcellCb->uePucchPwr.pres;
   ueCb->uePucchPwr.isFmt3a                = ummcellCb->uePucchPwr.isFmt3a;
   ...
   /*PDSCH */
   ueCb->pdschDedCfg.p_a                   = ummcellCb->pdschCfgDedicated.pA;
   ...
   /* PUSCH */
   ueCb->puschDedCfg.ackIdx                = ummcellCb->puschDedicated.ackIdx;
   ueCb->puschDedCfg.riIdx                 = ummcellCb->puschDedicated.riIdx;
   ueCb->puschDedCfg.cqiIdx                = ummcellCb->puschDedicated.cqiIdx;
   ...
   ueCb->cqiRptCfg.aperCqiEnbld            = ummcellCb->cqiRptCfg.aperCqiEnbld;
   ueCb->cqiRptCfg.cqiAperiodicMode        = ummcellCb->cqiRptCfg.cqiAperiodicMode;
   ueCb->cqiRptCfg.nomPdschRsEpreOff       = ummcellCb->cqiRptCfg.nomPdschRsEpreOff; 
   ueCb->cqiRptCfg.periodRptSetupPres      = ummcellCb->cqiRptCfg.periodRptSetupPres;
   ueCb->cqiRptCfg.cqiPucchRsrcIdx         = ummcellCb->cqiRptCfg.cqiPucchRsrcIdx;
   ...
   *  UEIdx 0, 1, 2, 3 would get 78, 80, 82, 84. */
   ...
   ueCb->cqiRptCfg.subbndCqik              = ummcellCb->cqiRptCfg.subbndCqik;
   ueCb->cqiRptCfg.fmtInd                  = ummcellCb->cqiRptCfg.fmtInd;
   ueCb->cqiRptCfg.riCfgIdx                = ummcellCb->cqiRptCfg.riCfgIdx;
   ueCb->cqiRptCfg.simulAckNackAndCQI      = ummcellCb->cqiRptCfg.simulAckNackAndCQI;
   ueCb->cqiRptCfg.periodicmode            = ummcellCb->cqiRptCfg.periodicmode;
   ueCb->cqiRptCfg.riPres                  = ummcellCb->cqiRptCfg.riPres;

   {
      WR_EMM_GET_NUM_TX_ANT(numTxAnt, cellId);
      if (numTxAnt == CTF_AP_CNT_1)
      {
         ueCb->antennaInfo.txmode            = CTF_UE_TX_ANT_MODE_TM1;
      }
      else
      {
         ueCb->antennaInfo.txmode            = CTF_UE_TX_ANT_MODE_TM2;

         /*ccpu00129642*/
         if(ueCb->ueCat > CM_LTE_UE_CAT_1)
         {
            ueCb->antennaInfo.txmode = CTF_UE_TX_ANT_MODE_TM3;
         }
     }
   }
   ueCb->antennaInfo.codebookpres          = FALSE;
   ueCb->pdcpCfg.discardTmr                = ummcellCb->pdcpInfo.discardTimer;
   ueCb->pdcpCfg.statusRptReq              = ummcellCb->pdcpInfo.statRprtReqrd;
   ueCb->pdcpCfg.snSize                    = ummcellCb->pdcpInfo.umSnSize;

   ueCb->harqinfo.ulMaxHarqTx              = ummcellCb->macMainCfg.ulSchCfg.maxHARQ_Tx;
   ueCb->harqinfo.deltaHarqOffset          = ummcellCb->macMainCfg.ulSchCfg.deltaHqOffset;

   WR_GET_SR_CFGINDX(ueCb->srCfg.srCfgIdx, ueCb);
   printf ("SR Config for UE IDX %d sridx %d \n",ueCb->ueIdx,ueCb->srCfg.srCfgIdx);
   ueCb->srCfg.srPUCCHResIdx               = ummcellCb->schReqCfg.srPucchRsrcIdx; 
   ueCb->srCfg.enDsrTransMax               = ummcellCb->schReqCfg.dsrTransMax;

   ueCb->qosCfg.ambrPres                   = FALSE;
   ueCb->qosCfg.dlAmbr                     = 0;
   ueCb->qosCfg.ueBr                       = ummcellCb->qosCfg.ueBr;
   if(ummcellCb->macMainCfg.timeAlignmentTimerDedicated
   ...
   else
   {
      ...
      if(wrUtlGetDedTaTmrVal(ummcellCb->macMainCfg.timeAlignmentTimerDedicated,
   ...
   ueCb->ueUlHqCfg.maxUlHqTx               = ummcellCb->macMainCfg.ulSchCfg.maxHARQ_Tx;
   ueCb->ueUlHqCfg.deltaHqOffst            = ummcellCb->macMainCfg.ulSchCfg.deltaHqOffset;
...
)
{
   ...
   {
      RLOG0(L_WARNING,"UE Release due to wrUmmPrcEgtpRabRelS1Err \n");
#ifdef UE_REL_DBG
      printf("\nUE Release due to wrUmmPrcEgtpRabRelS1Err \n");
#endif
   ...
   else
   {
      ...
      /* ccpu00149547 */
      lcrabrls->cause[0] = SztCauseRadioNwrelease_due_to_eutran_generated_reasonEnum;
      lcrabrls->causeTyp = WR_CAUSE_RADIONW;
 ...
 *         for SCell ADD/MOD/DEL
 *  @details
 * Function:wrUmmSCellTrans
 * 
 *   Processing steps:
 ...
 * @param[in]  intTransTyp: Internal message transaction type. 
 * @return S16
 *       SUCCESS : ROK
 *       FAILED  : RFAILED
**/
PUBLIC S16 wrUmmSCellTrans
(
   WrUeCb          *ueCb,
...
)
{
   ...
   WrUmmIncMsg               *msg;
   Void                      *transMsg = NULLP;
   WrUmmIntSCellTransMsg     *sCelltransMsg = NULLP;
   U32                       idx=0;
   U32                       idxScell=0;

   TRC2(wrUmmSCellTrans)
   RLOG0(L_ALWAYS, "wrUmmSCellTrans Called");
   ...
   WR_ALLOC(&sCelltransMsg, sizeof(WrUmmIntSCellTransMsg));
   if (NULLP == sCelltransMsg)
   {
      RLOG0(L_FATAL, "Failed to allocate memory.");
      RETVALUE(RFAILED);
   }
   sCelltransMsg->numSCells = numSCells;
   if(WR_UMM_SCELL_ADD == action)
   {
      for(idx=0; idx<numSCells; idx++)
      {
         sCelltransMsg->sCellUpdtReq[idx].action  = action;
         sCelltransMsg->sCellUpdtReq[idx].sCellId = sCellId[idx];
      }
   }

   else if(WR_UMM_SCELL_RELEASE == action)
   {
      for(idx = 0; idx < numSCells; idx++)
      {
         for(idxScell = 0; idxScell < WR_MAX_SCELL_PER_UE; idxScell++)
         {
            if((ueCb->sCellInfo[idxScell] != NULLP)&&
                  (ueCb->sCellInfo[idxScell]->sCellId == sCellId[idx]))
            {
               sCelltransMsg->sCellUpdtReq[idx].sCellId = sCellId[idx];
               sCelltransMsg->sCellUpdtReq[idx].action  = action;
               sCelltransMsg->sCellUpdtReq[idx].sCellIdx = ueCb->sCellInfo[idxScell]->sCellIdx;
               break;
            }
         }
         if(WR_MAX_SCELL_PER_UE == idxScell)
         {
            RLOG_ARG1(L_ALWAYS, DBG_CRNTI, ueCb->crnti, "SCellId  %d is Not Configured in UE", sCellId[idx]);
            WR_FREE(sCelltransMsg, sizeof(WrUmmIntSCellTransMsg));
            RETVALUE(RFAILED);
         }
      }
   }
   ...
   {
        RLOG0(L_ERROR, "Unknow TriggerType received");
   }
   sCelltransMsg->triggerTyp = action;
   transMsg = (Void *)sCelltransMsg;
   if (ROK != wrUmmBldInternalMsg(&internalMsg, WR_UMM_INTMSG_SCELL_TRANS,
               transMsg))
   {
        RLOG0(L_ERROR, "creation of internal message Failed");
        RETVALUE(RFAILED);
...
)
{
   ...
   TRC2(wrUmmSCellIntTrans)

   /* Call a function to get list of all secondary cells for this UE */
   wrEmmGetSCellIdLst(ueCb->cellId, &numOfSCells, sCellIdLst);

   RLOG1(L_INFO, "numOfSCells = %d\n", numOfSCells);
   /* Call Create transaction function with correct argumnet list */
   if(0 != numOfSCells)
   {
       wrUmmSCellTrans(ueCb, action, numOfSCells, sCellIdLst);
   }
   RETVALUE(ROK);
}
 ...
 )
{
   ...
   if (ueRecfgReq == NULLP)
   {
      RLOG0(L_FATAL, "Alloc for RrmWrUeRecfgReqInfo failed");
      RETVALUE(RFAILED);
   }
   ...
   /* Change the TX mode in case of ueCat > 1  and num of Tx antenna at enodeb
    * is > 1
    */
   WR_EMM_GET_NUM_TX_ANT(numTxAnt, transCb->ueCb->cellId);
   if ((CTF_AP_CNT_1 < numTxAnt) &&
         (ueCap->eUTRA_Cap.ue_Category.pres == PRSNT_NODEF)
         &&
         (ueCap->eUTRA_Cap.ue_Category.val > 1))
   {
      /* Store the information in the UE CB */
      transCb->ueCb->antennaInfo.txmode =
   ...
   ueRecfgReq->bRecfgBitMap |= RMU_RECFG_REQ_UE_CAP;
#ifdef LTE_ADV
   ueRecfgReq->bIsUeCaCap    = FALSE;
   /* SPrint 3:: TODO:: Add chck for CA capability of the UE*/
   /* Move this code out from this file to the proper place */
#ifdef LTE_ADV_UECAP
   if(wrEmmCb.caBandCfg.isEnbCaCap)
   {
         if(TRUE == wrUmmChkUeCASupp(transCb->ueCb))
#endif/* LTE_ADV_UECAP*/
         {
            ueRecfgReq->bRecfgBitMap |= RMU_RECFG_REQ_SCELL_ACK_N1RES;
            transCb->ueCb->isUeCaCap = TRUE;
#ifdef LTE_ADV_ACQI_SUPP
            ueRecfgReq->bIsUeCaCap   = TRUE;
#endif
...
/** @brief This function is responsible for to configure measurement gap for
 *  configuring A4 measurement report.
 *
 *  @details*
 *  Function:wrUmmMeasInterCfgMeasGapForA4
 *    
 *  Processing steps:
 *      - For Configuring Measurement Gap for A4 event 
 *  
 *   @param[in] ueCb: pointer to ue control block.
 *   @param[in] addMeasInfo: pointer to measurement transaction Cb.
 *   @param[in] cellCb: pointer to cell control block.
 *   @param[out] isGapRqd: is measurement gap required.
 *   @return S16
 *       SUCCESS : ROK
 *       FAILED  : RFAILED
 */
PUBLIC S16 wrUmmMeasInterCfgMeasGapForA4
(
 WrUeCb                      *ueCb,
...
)
{
   ...
   TRC2(wrUmmMeasInterCfgMeasGapForA4)
...
#endif/*WR_RSYS_KPI*/ 
   /* Check UE capabilites, before configuring A4 measurement report. */ 
   isUeCapable = wrUmmAnrChkUeCap(ueCb, WR_UMM_MEAS_INTER_FREQ , WR_UMM_EVENT_A4_A5_INTER_CFG);
...
#ifdef LTE_ADV_UECAP
            wrUmmMeasGapReqdInterfreqR10(ueCb,servingBand,targetBand,isGapRqd);
#else
            *isGapRqd = TRUE;
#endif
      }
