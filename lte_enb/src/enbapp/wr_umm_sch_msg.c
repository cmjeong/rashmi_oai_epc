...
{
   if(bResCfgBitMap & WR_UE_RADIO_RES_CFG_CQI)
   {
      ueCfg->aprdCqiCfg.pres = FALSE;
      /*TODO TFU_UPGRADE  */
      ueCfg->aprdCqiCfg.aprdModeEnum = cqiRptCfg->cqiAperiodicMode; 
      /* Fix: tfu upgrade changes */
#ifndef TFU_UPGRADE
      ueCfg->prdCqiCfg.pres = cqiRptCfg->periodRptSetupPres;
      ueCfg->prdCqiCfg.prdModeEnum = cqiRptCfg->periodicmode;
      /* TFU_UPGRADE  TODO*/
      ...
      ueCfg->prdCqiCfg.cqiOffst = cqiOffst;
      /* TODO: Take correct value from UE CB */
      ueCfg->prdCqiCfg.k = cqiRptCfg->subbndCqik;
      ...
      ueCfg->prdCqiCfg.k = 1;
      ueCfg->prdCqiCfg.cqiPmiCfgIdx = cqiRptCfg->cqipmiCfgIdx;
#else
      if (cqiRptCfg->aperCqiEnbld)
      {
         ueCfg->aprdCqiCfg.pres = TRUE;
         ueCfg->aprdCqiCfg.aprdModeEnum = \
            (RgrAprdCqiMode) cqiRptCfg->cqiAperiodicMode;

#ifdef LTE_ADV
         ueCfg->aprdCqiCfg.triggerSet1 = cqiRptCfg->aCqiTriggerSet1;
         ueCfg->aprdCqiCfg.triggerSet2 = cqiRptCfg->aCqiTriggerSet2;
         printf("\n APP_ACQI_CFG::Trigger1/Trigger2:: (%x/%x)\n",
               ueCfg->aprdCqiCfg.triggerSet1,
               ueCfg->aprdCqiCfg.triggerSet2);
#endif
      }
      ...
      ueCfg->prdCqiCfg.cqiSetup.cqiPResIdx = cqiRptCfg->cqiPucchRsrcIdx; /*!< cqi-PUCCH-ResourceIndex (0.. 1185) */
      ueCfg->prdCqiCfg.cqiSetup.cqiPCfgIdx = cqiRptCfg->cqipmiCfgIdx; /*!< cqi-pmi-ConfigIndex (0..1023) */
      /*129311 : CQI format check*/
      if(cqiRptCfg->fmtInd ==  WR_UE_CQI_FMT_WB)
      {
      ...
      else
      {
         ...
         ueCfg->prdCqiCfg.cqiSetup.k = cqiRptCfg->subbndCqik;
      }

      /*!< Rand Indicator is Enabled TRUE(1) FALSE(0) */
      if(cqiRptCfg->riPres == TRUE)
      {
         ueCfg->prdCqiCfg.cqiSetup.riEna = TRUE; /*!< Rand Indicator is Enabled TRUE(1) FALSE(0) */
         ueCfg->prdCqiCfg.cqiSetup.riCfgIdx = cqiRptCfg->riCfgIdx; /*!< ri-ConfigIndex    (0..1023)  */
      }
      else
      {
         ueCfg->prdCqiCfg.cqiSetup.riEna = FALSE; /*!< Rand Indicator is Enabled TRUE(1) FALSE(0) */
      }
      ueCfg->prdCqiCfg.cqiSetup.sANCQI = cqiRptCfg->simulAckNackAndCQI;  /*!< simultaneousAckNackAndCQI TRUE(1) FALSE(0) */
      ueCfg->prdCqiCfg.cqiSetup.prdModeEnum = (RgrPrdCqiMode)cqiRptCfg->periodicmode; /*!< Peiodic CQI reporting mode */
      /*ccpu00129641*/
#endif
   }
#ifdef TFU_UPGRADE
 ...
 )
{
   ...
   /* UE DL CQI config */
   if (wrUmmSchFillRgrUeDlCqiCfg(&ueCfg->ueDlCqiCfg,&ueCb->cqiRptCfg, ueCb->cqiOffst, ueCb->bResCfgBitMap) != ROK)
   {
   ...
   /* UE category */
   wrUmmSchdFillUeCategory(&ueCfg->ueCatEnum, ueCb);/*CA dev changes-Passing ueCb*/
   wrUmmSchdFillUeAccessStratumRls(&ueCfg->accessStratumRls, ueCb);
 ...
 )
{
   ...
   if (WrIfmSchdCfgUe(cfgInfo, transId, cellId) != ROK)
   {
      RETVALUE(ROK);
   }
 ...
 )
{
   ...
   /* Dispatch the prepared message to scheduler.                         */
   WrIfmSchdCfgUe(cfgReq, transId, cellId);
   RETVALUE(ROK);
 ...
 )
{
   ...
   if((ueRecfgRespPtr->stUeRadioResCfg.bRadioResDedBitMap) & (U8)RM_RCM_PHY_DED_RES_CFG)
   {
      ...
      {
         /* Fill the RGR struct here */
         ueRecfg->ueRecfgTypes |= RGR_UE_SCELL_PUCCH_RECFG;
         ueSCellN1ResCfg = &ueRecfgRespPtr->stUeRadioResCfg.stPhyCfgDed.pucchCfgDed;

         ueRecfg->sCellAckN1ResCfg.pucchFormatType = 1; /* TODO::Add enum for 1b and CS */
         ueRecfg->sCellAckN1ResCfg.sCellAckN1ResTb1Count = ueSCellN1ResCfg->sCellAckN1ResTb1Count;
         ueRecfg->sCellAckN1ResCfg.sCellAckN1ResTb2Count = ueSCellN1ResCfg->sCellAckN1ResTb2Count;

         for(idx = 0 ; idx < ueRecfg->sCellAckN1ResCfg.sCellAckN1ResTb1Count; idx++)
         {
            ueRecfg->sCellAckN1ResCfg.sCellAckN1ResTb1[idx] = ueSCellN1ResCfg->sCellAckN1ResTb1[idx];
         }
         for(idx = 0 ; idx < ueRecfg->sCellAckN1ResCfg.sCellAckN1ResTb2Count; idx++)
         {
            ueRecfg->sCellAckN1ResCfg.sCellAckN1ResTb2[idx] = ueSCellN1ResCfg->sCellAckN1ResTb2[idx];
         }
      }
#endif
...
)
{
   *ueCatEnum = CM_LTE_UE_CAT_1;
   if(NULLP != ueCb->ueCap)
   {
      switch(ueCb->ueCatV1020)
      {
         case 6: 
            *ueCatEnum = CM_LTE_UE_CAT_6;
            break;
         case 7: 
            *ueCatEnum = CM_LTE_UE_CAT_7;
            break;
         case 8: 
            *ueCatEnum = CM_LTE_UE_CAT_8;
            break;
         default:
            { 
               switch(ueCb->ueCat)
   ...
   /*ccpu00127802*/
   RLOG1(L_DEBUG, "Filled UE Category value[%d]\n", *ueCatEnum);
}/* End of wrUmmSchdFillUeCategory */

/**
 *  @brief This function fills the Access Stratum Release configuration info
 *         into the Scheduler UE Cfg info.
 *
 *      Function: wrUmmSchdFillUeAccessStratumRls
 *
 *          Processing steps:
 *          - In case of UE configuring to Scheduler
 *             - update the UE Access Startum Release value
 *
 *
 *  @param [in] ueCatEnum  : pointer to UE Access Stratum Release value to be sent to Scheduler
 *  @param [in] ueCb       : ue control block
 *  @return Void
 */

PUBLIC Void wrUmmSchdFillUeAccessStratumRls
(
   RgrAccessStratumRls        *accessStratumRls,
   WrUeCb                     *ueCb
)
{
   *accessStratumRls = RGR_REL_8;
   if(NULLP != ueCb->ueCap)
   {
      switch(ueCb->ueCap->eUTRA_Cap.accessStratumRls.val)
      {
         case NhuAccessStratumRlsrel8Enum: 
            *accessStratumRls = RGR_REL_8;
            break;
         case NhuAccessStratumRlsrel9Enum: 
            *accessStratumRls = RGR_REL_9;
            break;
         case NhuAccessStratumRlsrel10Enum: 
            *accessStratumRls = RGR_REL_10;
            break;
         case NhuAccessStratumRlsrel11Enum: 
            *accessStratumRls = RGR_REL_11;
            break;
         case NhuAccessStratumRlsspare4Enum: 
            *accessStratumRls = RGR_REL_SPARE_4;
            break;
         case NhuAccessStratumRlsspare3Enum: 
            *accessStratumRls = RGR_REL_SPARE_3;
            break;
         case NhuAccessStratumRlsspare2Enum: 
            *accessStratumRls = RGR_REL_SPARE_2;
            break;
         case NhuAccessStratumRlsspare1Enum: 
            *accessStratumRls = RGR_REL_SPARE_1;
            break;
         default:
            break;
      }
   }/*ueCb->ueCap*/
   RLOG1(L_DEBUG, "Filled UE Access Stratum Value[%u]", *accessStratumRls);
}/* End of wrUmmSchdFillUeAccessStratumRls */
...
#ifdef LTE_ADV
/** @brief This function fills TxMode in Scheduler Ue-Recfg
 *
 * @details
 *     Function: wrUmmFillSchdUeTxMode
 *
 *         Processing steps:
 *         - Fills the Txmode, TransState and UeCodeBookRstRecfg in Ue
 *         reconfiguration 
 *
 * @param WrUeCb        ueCb        UE Control Block
 * @param RgrUeRecfg       ueRecfg     scheduler ue reconfiguration
 * @param RgrUeSecCellCfg ueSCellDedCfg scell cfg 
 * @return ROK/RFAILED
 */
PUBLIC Void wrUmmFillSchdUeTxMode
(
   WrUeCb      *ueCb,
   RgrUeRecfg  *ueRecfg,
...
)
{
    ...
    * is > 1
    */
   WR_EMM_GET_NUM_TX_ANT(numTxAnt, sCellId);

   if (CTF_AP_CNT_1 == numTxAnt)
   {
      ueRecfg->ueRecfgTypes |= RGR_UE_TXMODE_RECFG;
      ueSCellDedCfg->txMode.pres = PRSNT_NODEF;
      ueSCellDedCfg->txMode.txModeEnum = RGR_UE_TM_1;
      ueSCellDedCfg->txMode.tmTrnstnState = RGR_TXMODE_RECFG_CMPLT;
      ueRecfg->ueCodeBookRstRecfg.pres = NOTPRSNT;
   }
   else if ((CTF_AP_CNT_1 < numTxAnt) &&
         (ueCb->ueCap->eUTRA_Cap.ue_Category.pres == PRSNT_NODEF) &&
         (ueCb->ueCap->eUTRA_Cap.ue_Category.val > 1))
   {
      ueRecfg->ueRecfgTypes |= RGR_UE_TXMODE_RECFG;
      ueSCellDedCfg->txMode.pres = PRSNT_NODEF;
      WR_GET_RGR_TM(ueCb->antennaInfo.txmode, mode);
      ueSCellDedCfg->txMode.txModeEnum = mode;
      ueSCellDedCfg->txMode.tmTrnstnState = RGR_TXMODE_RECFG_CMPLT;
      ueRecfg->ueCodeBookRstRecfg.pres = PRSNT_NODEF;
      ueRecfg->ueCodeBookRstRecfg.pmiBitMap[0] = 0xc0000000;
      ueRecfg->ueCodeBookRstRecfg.pmiBitMap[1] = 0x0;
   }
   else
   {
      ueRecfg->ueRecfgTypes |= RGR_UE_TXMODE_RECFG;
      ueSCellDedCfg->txMode.pres = PRSNT_NODEF;
      ueSCellDedCfg->txMode.txModeEnum = RGR_UE_TM_2;
      ueSCellDedCfg->txMode.tmTrnstnState = RGR_TXMODE_RECFG_CMPLT;
      ueRecfg->ueCodeBookRstRecfg.pres = NOTPRSNT;
   }
   RETVOID;
}/* End of wrUmmFillSchdUeTxMode */
#endif
