...
/********************************************************************20**
     ...
     Sid:      fc_smm_s1ap_t2k.c@@/main/TeNB_Main_BR/5 - Mon Aug 11 16:52:30 2014
...
#endif
{
   ...
   cfg->nmbConn = (U32)(WRSM_LSZ_MAX_PEER * (lteeNodeBparams[0].lteSmCellCfg.maxUeSupp + WR_MAX_SR_CQI_NOT_CONFIGURED_UES));
...
#endif
{
   ...
   cfg->srcAddr.sctPort = lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteEnbProtoCfg.sctpPortNumS1;
   if(TRUE == fapParameters.lteEnodebIpAddr.ipv4pres)
   {
      cfg->srcAddr.ipAddr.nAddr[idx].type = CM_IPV4ADDR_TYPE;
      cfg->srcAddr.ipAddr.nAddr[idx].u.ipv4NetAddr = fapParameters.lteEnodebIpAddr.enbIpAddr;
      idx++;
      cfg->srcAddr.ipAddr.nmb = idx;
   }
   if(TRUE == fapParameters.lteEnodebIpAddr.ipv6pres)
   {
      cfg->srcAddr.ipAddr.nAddr[idx].type = CM_IPV6ADDR_TYPE;
      cmMemcpy((U8*)cfg->srcAddr.ipAddr.nAddr[idx].u.ipv6NetAddr,
...
#endif
{
   ...
   protCfg->srcAddr.sctPort = lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteEnbProtoCfg.sctpPortNumS1;
   if(TRUE == fapParameters.lteEnodebIpAddr.ipv4pres)
   {
   ...
   if(TRUE == fapParameters.lteEnodebIpAddr.ipv6pres)
   {
      ...
      cmMemcpy((U8*)protCfg->srcAddr.ipAddr.nAddr[idx].u.ipv6TptAddr.ipv6NetAddr,
            (U8*)fapParameters.lteEnodebIpAddr.enbIpv6Addr, sizeof(CmInetIpAddr6));
      idx++;
      protCfg->srcAddr.ipAddr.nmb = idx;
   }
...
#endif
{
   ...
   for (cnt=0; cnt < cfg->nmbPeers; cnt++)
   {
     ...
     wrCpyTptToNetAddr(&cfg->peerInfo[cnt].assocCfg.priDstAddr, &mmeCfg->mmeAddr[0]);
      ...
      cfg->peerInfo[cnt].assocCfg.dstPort            =  lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteEnbProtoCfg.sctpPortNumS1;
...
#endif
{
   ...
   RLOG1(L_DEBUG, "S1AP Cfg Cfm received for the element %d",
      ...
      if (wrSzCfg == WR_SM_SZ_CONFIGURED)
      {
         ...
         WR_FILL_PST(szPst,SM_SM_PROC, SM_SZ_PROC, ENTSM, ENTWR,0,0, EVTWREGTPCFG, WR_POOL, SM_SELECTOR_LC);
#else
         WR_FILL_PST(szPst,SM_SM_PROC, SM_SZ_PROC, ENTSM, ENTWR,0,0, EVTWRX2APCFG, WR_POOL, SM_SELECTOR_LC);
#endif
...
#endif /* ANSI */
{
   ...
   if (cntrl->cfm.status == LCM_PRIM_OK)
   {
      if (smCb.smState == WR_SM_STATE_X2AP_SHUTDWN_DONE)
      {
         ...
         WR_FILL_PST(szPst,SM_SM_PROC, SM_SZ_PROC, ENTSM, ENTWR,0,0, EVTWRSCTPSHUT, WR_POOL, SM_SELECTOR_LC);
...
/********************************************************************30**
           ...
           End of file:     fc_smm_s1ap_t2k.c@@/main/TeNB_Main_BR/5 - Mon Aug 11 16:52:30 2014
