...
#ifdef LTE_ADV_UECAP
#define WR_EMM_CELL_BW_20MHZ                20
#endif /*LTE_ADV_UECAP*/
/* CSG_DEV */
...
#ifdef TIP_L3
     upperL2Addr.type = CM_TPTADDR_IPV4;
     upperL2Addr.u.ipv4TptAddr.address = tipFetchUl2EgtpIpAddrFromCellId(cellId);
     upperL2Addr.u.ipv4TptAddr.port = WR_DFLT_EGTP_PORT;
     *addr = &upperL2Addr;
#else
...
#ifdef LTE_ADV_UECAP
PUBLIC Void wrEmmFillCaCfg
(
 WrCellCb                  *cellCb
);

PUBLIC S16 wrEmmVldtCellCont
(
...
PUBLIC S16 wrEmmGetBndAndBwInfo
(
 WrCaBandCfg   *caBandCfg
);

PUBLIC Void wrEmmSortBndAndBw
(
 U8 *band,
 U8 *bandwidth
);

PUBLIC U8 wrEmmGetBw
(
 U8 dlBWRb
);
#endif /*LTE_ADV_UECAP*/
...
#ifdef TIP_L3
/*
*       Fun:   wrGetSchedEnbCfg
*
*       Desc:  get Sch ENB Cfg 
*
*       Ret:   ROK
*
*       Notes: None
*/
PUBLIC LwrSchedEnbCfg *wrGetSchedEnbCfg
(
)
{
   RETVALUE(&wrEmmCb.schedEnbCfg);
}/*end of wrGetSchedEnbCfg*/
...
#else
   if(!wrEmmCb.isSchedEnbCfgDone)
   {
      wrEmmCb.isSchedEnbCfgDone = TRUE;
      wrEmmSendSchedEnbCfg(&wrEmmCb.schedEnbCfg);
   }
...
{
   RLOG0(L_DEBUG, 
         "wrEmmSchdEnbCfgCfm: EnodeB Scheduler Configuration Received");
   if (status != WR_CFG_OK)
   {
      /* Raise an alarm here for cell configuration failure               */
      RLOG0(L_FATAL,"ALARM!! EnodeB Scheduler Configuration Failed");
      RETVALUE(RFAILED);
   }
   RETVALUE(ROK);
}
...
)
{
   ...
   WR_EMM_GET_CELLCB(cellCb, transId);
   if(NULLP == cellCb)
   {
   ...
   if (wrEmmCellCfgDam(cellCb) != ROK)/*MultiCell*/
   {
      RLOG0(L_FATAL,"ALARM!! Data application configuration error");
      RETVALUE(RFAILED);
   }

   if ((smCb.reCfg == TRUE)&&(smCb.prem == FALSE))
   {
...
)
{
   ...
   if (status == WR_CFG_OK)
   {
      ...
      /*add enbcfg here sprint4 UECap start*/
#ifdef LTE_ADV_UECAP
      	if(wrEmmCb.isCaEnabled == TRUE)
      	{
           wrEmmFillCaCfg(cellCb);
      	}
#endif/* LTE_ADV_UECAP*/
...
{
//#ifndef TIP_L3 /*TODO need to find how SI modification or updation or add(SIB8) can we send how the interface should be*/
   wrPbmTTIInd(ttiInd);
//#endif
...
)
{
  ...
  /* avoid cell up during SI modificatio */
...
)
{
   ...
   if(NULLP == cellCb)
   {
      RLOG_ARG0(L_ERROR, DBG_CELLID, cellId, "Invalid cellId received");
      RETVALUE(RFAILED);
   }
   RLOG1(L_INFO,"Number of TX antenna = %d",cellCb->numTxAntenna);
   RETVALUE(cellCb->numTxAntenna);
}
/** @brief This function is used to fetch the PCI for the corresponding cellId
 *
 * @details
...
)
{
   ...
   WR_EMM_GET_CELLCB(emmCellCb, redirectInfo->cellId);
   if (NULLP == emmCellCb)
   {
 ...
 *    Fun: wrEmmFillCaCfg
 *  
 *    Desc: This function will fill CA related UeCap
 *    parameter(band,class,bandwidth,bandwidthcombset) to each config 
 *    cell based on uecapability table .
 *          
 *    inputs: Cell CB
 * 
 **/
...
)
{
   ...
   S16          ret = RFAILED;
   caBandCfg->wrCellParams.cellId[caBandCfg->numOfAssocCells] = cellCb->cellId;
   caBandCfg->numOfAssocCells++;
   /* checking No of assoc cell with enodeb is equal to total no of config
    * cell*/
   if(caBandCfg->numOfAssocCells == wrEmmCb.numCells)      
   {
      ret = wrEmmGetBndAndBwInfo(caBandCfg);  
      if(ret == ROK)
      {
         wrEmmChkCaCap(caBandCfg);
      }
      else
      {
...
/**
 *    Fun: wrEmmVldtCellCont
 *  
 *    Desc: This function will validate if the cells 
 *          are contiguous or not and than fill enbcfg for CA
 *          
 *    inputs: caBandCfg : contains band related info
 *            band : contains the bands supported by the cells
 *            bandwidth : contains the bandwidth supported by the cells
 * 
 **/
PUBLIC S16 wrEmmVldtCellCont
(
...
)
{
   ...
   U8 EnbSuppClass = 0;
   U8 ContId = 0;
   /*checking if both cells are contiguous or not*/
   if(wrEmmCb.isCellFreqCont != TRUE)                
   {
      /* sorting band and bandwidth value in ascending order to mao with table*/
      wrEmmSortBndAndBw(band,bandwidth);             
      for(idx = 0 ; idx < WR_MAX_CELLS  ; idx++)
      {
         caBandCfg->wrBandComb.band[idx].band = band[idx];
         caBandCfg->wrBandComb.band[idx].bwClass = NhuCA_BandwidthClass_r10aEnum;
         caBandCfg->wrCellParams.cellSuppBw[idx] = bandwidth[idx];
         caBandCfg->wrBandComb.numBandComb++;
      }
   }
   else
   {
      /* checking if both bands are same or not for intra contiguous */
      if(band[idx] == band[idx+1])                   
      {
         for(idx = 0 ; idx < WR_MAX_CELLS; idx++)
         {
            bw = bw + bandwidth[idx];
         }
         /* if addition of both cells num of RB's are less than or equal to 100(<=20Mhz)
          * than it is class B else class C*/
         if(bw <= WR_EMM_CELL_BW_20MHZ)             
         {
            EnbSuppClass = NhuCA_BandwidthClass_r10bEnum;
         }
         else
         {
            EnbSuppClass = NhuCA_BandwidthClass_r10cEnum;
         }
         caBandCfg->wrBandComb.band[ContId].band = band[ContId];
         caBandCfg->wrBandComb.band[ContId].bwClass = EnbSuppClass;
         caBandCfg->wrCellParams.cellSuppBw[ContId] = bandwidth[ContId];
         caBandCfg->wrCellParams.cellSuppBw[ContId +1] = bandwidth[ContId +1];
         caBandCfg->wrBandComb.numBandComb++;
      }
      else
      {
...
/**
 *    Fun: wrEmmGetBndAndBwInfo
 *  
 *    Desc: This function will get band and bandwidth parameter for each cell.
 *          
 *    inputs: caBandCfg : contains the band information of Enb
 * 
 **/
PUBLIC S16 wrEmmGetBndAndBwInfo
(
...
{
   S16      ret = RFAILED;
   U8       band[WR_MAX_CELLS]={0};
   U8       bandwidth[WR_MAX_CELLS]={0};
   WrCellCb *cellCb = NULLP;
   U16      idx = 0;
   /* kw changes */
   for(idx = 0; idx < WR_MAX_CELLS ; idx++)
   {
      WR_EMM_GET_CELLCB(cellCb,caBandCfg->wrCellParams.cellId[idx]);
      ...
      band[idx] = cellCb->sib1.freqBandIndicator;             
      /*getting bandwidth value based on enum value get from mib*/
      bandwidth[idx] =  wrEmmGetBw(cellCb->mib.dlBw);
   }
   ret=wrEmmVldtCellCont(caBandCfg,band,bandwidth);
   RETVALUE(ret);
}

/** 
 *    Fun: wrEmmSortBndAndBw
 * 
 *    Desc: This function will sort the band and bandwidth 
 *          arrays in ascending order
 *          
 *   
 *    inputs: band and bandwidth values to be sorted
 * */
PUBLIC Void wrEmmSortBndAndBw
(
 U8 *band,
...
)
{
   ...
   for(idx = 0;idx < WR_MAX_CELLS; idx++)
   {
      for(idx1 = idx+1; idx1 < WR_MAX_CELLS ; idx1++)
      {
         if(band[idx] > band[idx1])
         {
            temp1 = band[idx];
            temp2 = bandwidth[idx];
            band[idx] = band[idx1];
            bandwidth[idx] = bandwidth[idx1];
            band[idx1] = temp1;
            bandwidth[idx1]=temp2;
          }
       }
    }
}
 ...
 *    Fun: wrEmmGetBw
 * 
 *    Desc: This function will get the BW for the given RB
 *          
 *    inputs:number of RBs 
 *
 *    output : Bandwidth for the given RB
 * */
PUBLIC U8 wrEmmGetBw
(
 U8 dlBWRb
)
{
   ...
   switch(dlBWRb)
   {
      case WR_BANDWIDTH_6RBS:
           bandwidth = 1.4;
           break;

      case WR_BANDWIDTH_15RBS:
           bandwidth = 3;
           break;
            
      case WR_BANDWIDTH_25RBS:
           bandwidth = 5;
           break;
           
      case WR_BANDWIDTH_50RBS:
           bandwidth = 10;
           break;

      case WR_BANDWIDTH_75RBS:
           bandwidth = 15;
           break;

      case WR_BANDWIDTH_100RBS:
           bandwidth = 20;
           break;
