...
#define WR_UE_RADIO_RES_CFG_SCELL_ACK_N1_RES 1<< 4
#endif
...
/* Structure to store Secondary cell Informaton details */ 
typedef struct wrSCellInfo
{
   U8                    sCellIdx; /* SCell Index to be used in RRC message and 
                                      at RMU interface */
   WrCellId              sCellId;  /* This will be secondary cellId */
	U8                    bResCfgBitMap;
   Bool                  isScellAdded; /* Set to TRUE when this SCELL is successfully configured
                                         for this UE */
   WrCqiRptCfg           cqiRptCfg; /* This will store dedicated info 
                                              for a particular secondar cell */
#if 0
   WrAntInfoDed          dedAntInfo; /* Stores Dedicated Antenna Info */
#endif
}WrSCellInfo;
...
#ifdef LTE_ADV
   U8                        numSCells;
   WrSCellInfo               *sCellInfo[WR_MAX_SCELL_PER_UE]; /*!< This is array of pointer of SCells. This information will be used while
                                                                    filling RRC message IE for SCell and while hadling the Mod/Del request */
   Bool                      pucchCfgDedPres;     /*!<for Re-use in Re-est */
   RmuUePucchCfg             pucchCfgDed;         /* Added support for Format 1B 
                                                     PCCH Resource for Scell*/
   U8                        apcsiTrigSet1;       /* aperiodic CSI Trigger set one */
   U8                        apcsiTrigSet2;       /* aperiodic CSI Trigger set two */
#endif
   Bool                     isUeCaCap;
};
...
typedef struct wrUmmCell
{
   ...
   CmLListCp                 ueCbSCellLst;    /* List of ueCb addresses(type *WrUeCb) for which this cell is a secondary cell */
...
#endif
#ifdef LTE_ADV
typedef enum wrUmmScellAction
{
   WR_UMM_SCELL_ADD,
   WR_UMM_SCELL_MOD_COMMON,
   WR_UMM_SCELL_MOD_DEDICATED,
   WR_UMM_SCELL_RELEASE,
   WR_UMM_SCELL_NA
}WrUmmScellAction;
#endif
...
#ifdef LTE_ADV
EXTERN S16 wrUmmGetUeFrmUeCrnti(WrUeCb **ueCb, WrCellId cellId, WrCrnti crnti);
#endif
...
#ifdef LTE_ADV   
EXTERN S16 wrUmmSCellIntTrans
(
 WrUeCb                       *ueCb,
 WrUmmScellAction             action
);

EXTERN S16 wrUmmSCellTrans
(
 WrUeCb          *ueCb,
 WrUmmScellAction   action,
 U8              numSCells,
...
EXTERN S16 wrUmmCliTriggerScellAction
(
 U8 action
);
EXTERN Void wrUmmFillSchdUeTxMode
(
   WrUeCb      *ueCb,
   RgrUeRecfg  *ueRecfg,
...
EXTERN S16 wrUmmMeasInterCfgMeasGapForA4
(
 WrUeCb                      *ueCb,
 WrUmmMeasAddInfo            *addMeasInfo,
 WrUmmCellCb                 *cellCb,
