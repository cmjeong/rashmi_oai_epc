...
/********************************************************************20**
     ...
     Sid:      fc_smm_x2ap_t2k.c@@/main/TeNB_Main_BR/5 - Mon Aug 11 16:52:38 2014
...
#endif
   cfg->maxNmbPeer = (U32) lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteSmCellCfg.maxX2Peers;
...
#endif
{
   ...
   {
      protCfg->srcAddr.ipAddr.nAddr[idx].type = CM_IPV4ADDR_TYPE;
      protCfg->srcAddr.ipAddr.nAddr[idx].u.ipv4NetAddr = fapParameters.lteEnodebIpAddr.enbIpAddr;
      idx++;
      protCfg->srcAddr.ipAddr.nmb = idx;
   }
   if(TRUE == fapParameters.lteEnodebIpAddr.ipv6pres)
   {
      protCfg->srcAddr.ipAddr.nAddr[idx].type = CM_IPV6ADDR_TYPE;
      cmMemcpy((U8*)protCfg->srcAddr.ipAddr.nAddr[idx].u.ipv6NetAddr,
            (U8*)fapParameters.lteEnodebIpAddr.enbIpv6Addr, sizeof(CmInetIpAddr6));
      idx++;
      protCfg->srcAddr.ipAddr.nmb = idx;
   }

   protCfg->tRELOCprep = (U16)lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteProcTimerCfg.x2PrepTimerVal;
   protCfg->tX2RELOCOverall = (U16)lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteProcTimerCfg.x2OvrAllTimerVal;
   protCfg->timeToWait = (U16)lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteProcTimerCfg.x2TimeToWaitTimerVal;
...
#endif
{
   ...
   /* TODO: MultiCell Implementation, CZT PEER Cfg being done only for Cell 1 using DFLT Cell Indx
    * to discuss with architects and change this.*/

   for(idx=0; idx < wrSmDfltNumCells; idx++)
   {
      for(cnt=0; cnt<lteeNodeBparams[idx].lteNeighCellCfg.numEutraCells; cnt++)
      {
         if(TRUE != lteeNodeBparams[idx].lteNeighCellCfg.t.neighAddCfg[cnt]. \
                    eutraCell.isNewNbrEnb)
            continue;
         numOfNewNbrEnb++;
      }
   }
   cfg->nmbPeers = numOfNewNbrEnb;
   /* TODO */
   ...
   for(idx=0; idx < wrSmDfltNumCells; idx++)
   {
      for(cnt=0; cnt<lteeNodeBparams[idx].lteNeighCellCfg.numEutraCells; cnt++)
      {
         if(TRUE != lteeNodeBparams[idx].lteNeighCellCfg.t.neighAddCfg[cnt]. \
            ...
            continue;
#ifdef CZ_ENB   
         cfg->peerCfg[numOfNewNbrEnb].peerId   =  lteeNodeBparams[idx]. \
              lteNeighCellCfg.t.neighAddCfg[cnt].eutraCell.peerId;
         cfg->peerCfg[numOfNewNbrEnb].tSapId   =  0; 
         cfg->peerCfg[numOfNewNbrEnb].uSapId   =  0;  
         cfg->peerCfg[numOfNewNbrEnb].eAValue  =  UNKNOWN_VALUE;
         cfg->peerCfg[numOfNewNbrEnb].iPAValue =  UNKNOWN_VALUE;
         cfg->peerCfg[numOfNewNbrEnb].assocCfg.locOutStrms =  WRSM_LCZ_OUT_STRMS;
         cfg->peerCfg[numOfNewNbrEnb].assocCfg.priDstAddr.type    =  CM_IPV4ADDR_TYPE;
         cfg->peerCfg[numOfNewNbrEnb].assocCfg.priDstAddr.u.ipv4NetAddr   =  \
         lteeNodeBparams[idx].lteNeighCellCfg.t.neighAddCfg[cnt].eutraCell.enbAddr;

         cfg->peerCfg[numOfNewNbrEnb].assocCfg.dstAddrLst.nmb     =  1;
         cfg->peerCfg[numOfNewNbrEnb].assocCfg.dstAddrLst.nAddr[0].type   =  CM_IPV4ADDR_TYPE;
         cfg->peerCfg[numOfNewNbrEnb].assocCfg.dstAddrLst.nAddr[0].u.ipv4NetAddr = \
              lteeNodeBparams[idx].lteNeighCellCfg.t.neighAddCfg[cnt].eutraCell.enbAddr;

         cfg->peerCfg[numOfNewNbrEnb].assocCfg.dstPort            =  WRSM_LCZ_ENB_SCTPORT;/*Peer eNB's port number.*/
         numOfNewNbrEnb++;
...
#endif
{
   ...
   /* TODO: MultiCell Implementation, CZT PEER Cfg being done only for Cell 1 using DFLT Cell Indx
    * to discuss with architects and change this.*/
      cfg->peerCfg[cnt].peerId = lteeNodeBparams[WR_SM_DFLT_CELL_INDX]. \
                 lteNeighCellCfg.t.neighAddCfg[nbrIndex - 1].eutraCell.peerId; 
      cfg->peerCfg[cnt].tSapId   =  0; 
      cfg->peerCfg[cnt].uSapId   =  0;  
      cfg->peerCfg[cnt].eAValue  =  UNKNOWN_VALUE;
      /* cfg->peerCfg[cnt].iPAValue =  0; */
      ...
      cfg->peerCfg[cnt].assocCfg.priDstAddr.u.ipv4NetAddr   =
         lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteNeighCellCfg.t. \
         neighAddCfg[nbrIndex - 1].eutraCell.enbAddr;
      cfg->peerCfg[cnt].assocCfg.dstAddrLst.nmb     =  1;
      cfg->peerCfg[cnt].assocCfg.dstAddrLst.nAddr[0].type   =  CM_IPV4ADDR_TYPE;
      cfg->peerCfg[cnt].assocCfg.dstAddrLst.nAddr[0].u.ipv4NetAddr = \
         lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteNeighCellCfg.t. \
         neighAddCfg[nbrIndex - 1].eutraCell.enbAddr;
      cfg->peerCfg[cnt].assocCfg.dstPort            =  WRSM_LCZ_ENB_SCTPORT;/*Peer eNB's port number.*/
...
#endif
{
   ...
   if (cfm->cfm.status == LCM_PRIM_OK)
   {
      ...
      if (((wrCzCfg == WR_SM_CZ_CONFGRD_WO_PEER) && \
               (WR_VAL_ZERO == lteeNodeBparams[WR_DFLT_CELL_IDX].lteNeighCellCfg. \
                numEutraCells)) \
...
#endif
#ifdef RM_INTF
                  WR_FILL_PST(czPst, SM_SM_PROC, SM_CZ_PROC, ENTSM, ENTWR,0,0, EVTWRRRMCFG, WR_POOL, SM_SELECTOR_LC);
#else
                  WR_FILL_PST(czPst, SM_SM_PROC, SM_CZ_PROC, ENTSM, ENTWR,0,0, EVTWRAPPCFG, WR_POOL, SM_SELECTOR_LC);
#endif /*RM_INTF*/
...
#endif /* ANSI */
{
...
#ifdef RM_INTF
         WR_FILL_PST(czPst, SM_SM_PROC, SM_CZ_PROC, ENTSM, ENTWR,0,0, EVTWRS1APSHUT, WR_POOL, SM_SELECTOR_LC);
#else
         WR_FILL_PST(czPst, SM_SM_PROC, SM_CZ_PROC, ENTSM, ENTWR,0,0, EVTWRS1APSHUT, WR_POOL, SM_SELECTOR_LC);
#endif /*RM_INTF*/
   ...
   if(cntrl->hdr.elmId.elmnt == STCZSCTSAP)
   {
      ...
      if (smCb.smState == WR_SM_STATE_RRC_STK_UNBND_DONE)
      {
         if (cntrl->cfm.status == LCM_PRIM_OK)
         {
            ...
            WR_FILL_PST(czPst, SM_SM_PROC, SM_WR_PROC, ENTSM, ENTWR,0,0, EVTWREGTPSTKUNBND,
   ...
   if ( STCZPEER == cntrl->hdr.elmId.elmnt)
   {
...
/********************************************************************30**
           ...
           End of file:     fc_smm_x2ap_t2k.c@@/main/TeNB_Main_BR/5 - Mon Aug 11 16:52:38 2014
