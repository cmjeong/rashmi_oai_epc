...
#endif
{
   ...
   if((smCb.smState == WR_SM_STATE_INIT) && (wrNlCfg == SM_NL_CONFIGURED))
   {
      ...
      WR_FILL_PST(nlPst,SM_SM_PROC, SM_NL_PROC, ENTSM, ENTWR,0,0, EVTWRTUCLCFG, WR_POOL, SM_SELECTOR_LC);
...
#endif
{
   ...
   if (cfm->cfm.status == LCM_PRIM_OK)
   {
      if (smCb.smState == WR_SM_STATE_APP_SHUTDWN_DONE)
      {
          ...
          WR_FILL_PST(nlPst, SM_SM_PROC,SM_NL_PROC, ENTSM, ENTWR,0,0, EVTWRRRCSHUT, nlMemMngr.nlPool, SM_SELECTOR_LC);
...
#endif
{
   ...
   switch(smCb.smState)
   {
      case WR_SM_STATE_INIT :
      {
         if(LCM_PRIM_OK == cfm->cfm.status)
         {
            switch(cfm->hdr.transId)
            {
               ...
               {
                  cellCfgCnt++;
                  if(cellCfgCnt == wrSmDfltNumCells)
                  {
                     cellCfgCnt = 0;
                     wrNlCfg |= SM_NL_CELL_CFG;
                     for(cellCfgIdx = 0; cellCfgIdx < wrSmDfltNumCells; cellCfgIdx++)
                     {
                        smSendNlNghCellCfg(cellCfgIdx);
                     }
                  }
               }
               break;
      ...
      default :
      {
         RLOG1(L_ERROR,"Configuration Cfm in Invalid SM state:[%d]", smCb.smState);
      }
   }
...
#else
PUBLIC Void smSendCellUpIndToSon(cellCfgIndx)
U8    cellCfgIndx; 
#endif /* ANSI */
{
   ...
   usta->u.stEnbStaInfo.u.cellId = lteeNodeBparams[cellCfgIndx].lteCellSib1CfgGrp.cellAccessInfo.cellId;
...
#ifdef DEBUGP
   WR_DBGP(DBGMASK_SM, (SM_PRNT_BUF, "NL Gen Cfg Req sent\n"));
#endif
   /* Send the request to NL */
...
#else
PRIVATE  S16 smSendNlCellCfg (cellCfgIdx)
U8      cellCfgIdx;
#endif
{
   ...
   if(ROK != smBuildNlCellCfg(cellCfg, cellCfgIdx))
   {
      RLOG0(L_FATAL," Building cell configuration failed \n");
      SPutSBuf(smCb.init.region, smCb.init.pool,
                                  (Data*)nlMngmt, sizeof(NlMngmt));
...
#else
PRIVATE  S16 smSendNlNghCellCfg (cellCfgIdx)
 U8      cellCfgIdx;
#endif
{
   ...
   smBuildNlNghCellCfg(nghCellCfg, cellCfgIdx);
