...
)
{
   ...
   if (WrIfmSchdCfgUe(cfgInfo, transId, ueCb->cellId) != ROK)
   {
...
)
{
   ...
   wrUmmUeMultiLSapId(&(conSetupSdu->sdu.ueCfg.uellyrSapIdInfo), ueCb->cellId);
...
)
{
...
#ifdef UE_REL_DGB
      printf("\nMaximum UE Limit reached, redirecting UE\n");
#endif
  ...
  ueCb->rrcConSetupCmplTm);
   ...
   if (wrUmmRrcConSelectMme(transCb) != ROK)
   {
      ...
      RLOG0(L_WARNING, "No MME to serve the UE. Release the connection and go away");
#ifdef UE_REL_DBG
      printf("\nNo MME to serve the UE. Release the connection and go away \n");
#endif
      transCb->state = WR_UMM_TRANS_RELEASE_CALL; 
   ...
   if (wrUmmRrcConS1InitUe(transCb) != ROK)
   {
      ...
      RLOG0(L_WARNING, "Failed to send initial UE message to MME, Initiate Local Ctxt Rel");
#ifdef UE_REL_DBG
      printf("\n Failed to send initial UE message to MME, Initiate Local Ctxt Rel \n");
#endif
      transCb->state = WR_UMM_TRANS_RELEASE_CALL; 
      wrEmmUpdateStat(WR_EMM_RRC_STATS,transCb->ueCb->cellId,WR_EMM_RRC_CON_FAIL,1,transCb->ueCb->establishCause);
      RETVALUE(ROK);
...
)
{
   ...
   lchCfg->dlInfo.dlQos.qci = WR_SRB1_QCI;
   ...
   lchCfg->ulLchQciInfo.qci = WR_SRB1_QCI;
            ...
            case WR_TMR_RRC_RRCCON:
               {
                  ...
                  RLOG0(L_WARNING, "RRC Connection Setup Timer Exp");
#ifdef UE_REL_DBG
                  printf("\n RRC Connection Setup Timer Exp \n");
#endif
                  transCb->state = WR_UMM_TRANS_RELEASE_CALL; 
