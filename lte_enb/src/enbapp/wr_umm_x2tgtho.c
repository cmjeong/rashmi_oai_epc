...
)
{
   ...
   for(rbIdx = 0; (rbIdx < x2HoTrans->numOfRbCfgd) && (rbIdx1 < noOfBearer); rbIdx++)
   {
      ...
      if(rbInfo[rbIdx].dlFwding == TRUE)
      {
          ...
          */
         wrEmmGetDatAppAddr(transCb->ueCb->cellId, &lclAddr, x2HoTrans->peerAddr.type);
    ...
    */
   ret = SFndLenMsg(transCb->msg->u.encCfm->sdu.mBuf, (MsgLen *)&len);
   if((len == 0) || (ret != ROK))
   {
...
)
{
   ...
   for(erabIdx = 0; erabIdx < x2TgtHoTrans->numOfRbCfgd; erabIdx++)
   {
      ...
      wrEmmGetDatAppAddr(transCb->ueCb->cellId, &lclAddr, mmeCb->mmeAddr[0].type);      
...
)
{
    ...
    */
   wrEmmGetDatAppAddr(ueCb->cellId, &lclAddr, peerAddr->type);
...
)
{
   ...
   numTxAntenna =  wrEmmGetNumTxAnt(ueCb->cellId);
   /**
