...
#ifdef LTE_ADV_UECAP
PUBLIC S16 wrUmmGetSuppBwCombSet
(
 NhuUE_EUTRA_Cap_v1020_IEs   *ueCapv1020,
 U16                          idx,
 U8                          *bwComSetLen,
 U8                          **bwCombSet 
);

PUBLIC S16 wrUmmGetBwCombSet
(
 U8    *val,
 U16   len,
 U8    *bwComSetLen,
...
/* struct wrCaUeCapVerTab
   {
      ...
      U8 class2;
      U8 noOfComb;
      U8 bandwidth[WR_MAX_COMB];
      U8 bdwthCombSet;
   }*/
 WrCaUeCapVerTab wrCaUeCapVerTab[WR_CA_MAX_ENTRIES] = 
 {
   {1,2,0,0,2,
      {
         15,15,
   ...
   /* bandwidth combination sets are 0,1,2,3,4*/
   {4,0,12,0,8,
      {
         1.4,5,
         1.4,10,
...
#endif
{
   cmInetAddr((S8 *)lteeNodeBparams[0].lteGostLog.ipAddress,&(wrSmDfltWiresharkDstIpAddr));
   wrSmDfltWiresharkDstIpAddr = CM_INET_HTON_U32(wrSmDfltWiresharkDstIpAddr);
   if( (0 == lteeNodeBparams[0].lteGostLog.portNumber) ||
   ...
   g_serv_addr.sin_port   = htons(lteeNodeBparams[0].lteGostLog.portNumber);
   /* TODO: Need to take care of IPv6 */
   g_serv_addr.sin_addr.s_addr = htonl(fapParameters.lteEnodebIpAddr.enbIpAddr);
   bind(g_sockfd, &g_serv_addr, sizeof(g_serv_addr));

   g_serv_addr1.sin_family = AF_INET;
   g_serv_addr1.sin_port   = htons(lteeNodeBparams[0].lteGostLog.portNumber);
...
#endif
{
    ...
    * */
   //printf("\n in wr_utils.c wrCb.enblSIAndPagngLog = %d\n",wrCb.enblSIAndPagngLog);
   if(wrCb.enblSIAndPagngLog)
   {
      if(ueId == 65534)
      {
      /*   printf("wireshark logs for paging CL->APP\n");
         SPrntMsg(mBuf, 0, 0);*/
         statBuffer[bufOffset++] = 5;
      }
      if(ueId == 65535)
      {
      /*   printf("wireshark logs for SI CL->APP\n");
         SPrntMsg(mBuf, 0, 0);*/
...
)
{
   switch(ratType)
   {
      case CDMA2000_1XRTT:
         {
            *baseId = (U64)((cId & 0x00007fff80000000LL) >> 31);
            *sId = (U16)((cId & 0x000000007fff0000LL) >> 16);
            *nId = (U16)((cId & 0x000000000000ffffLL));
            break;
         }
      case CDMA2000_HRPD:
         {
...
/**
 *  @brief This function will fill the SapIds for below interfaces which RRC
 *  will give PER UE to MAC, RLC and PDCP 
 *
 *      Function: wrUmmUeMultiLSapId
 *
 *          Processing steps:
 *             - Fill the SapIds for all interfaces which will be used for multi
 *             Sap/instance support
 *
 *  @param[in]  noOfCells      :  Number of cells that can be configured
 */
PUBLIC Void wrUmmUeMultiLSapId
(
   NhuUeLLyrSapIdInfo  *ueLSapInfo,
   U8                  cellId
)
{
      ueLSapInfo->kwuSapId = wrCb.sapIds[cellId - 1].kwuSapId;
      ueLSapInfo->ckwSapId = wrCb.sapIds[cellId - 1].ckwSapId;
      ueLSapInfo->crgSapId = wrCb.sapIds[cellId - 1].crgSapId;
      ueLSapInfo->cpjSapId = wrCb.sapIds[cellId - 1].cpjSapId;
      ueLSapInfo->pjuSapId = wrCb.sapIds[cellId - 1].pjuSapId;

       /* SuId configured at MAC for RLC-UL and RLC-DL*/
      ueLSapInfo->macRguUlSapId = WR_KWDL_INST;
      ueLSapInfo->macRguDlSapId = WR_KWUL_INST;

      /* SapId configured between RLC and that MAC instance */
      ueLSapInfo->rlcRguSapId =  wrCb.sapIds[cellId - 1].rguSapId; 

      /* SpId configured at RLC for PDCP-UL and PDCP-DL */
      ueLSapInfo->rlcKwuUlSapId = WR_KWUL_INST;  
      ueLSapInfo->rlcKwuDlSapId = WR_KWDL_INST;
 ...
 *  @param[out]  *sapIdLst    :  sapId list to be filled by App
 *  @return Bool 
 */
PUBLIC Void wrFillLSapIds
(
...
)
{
   ...
   sapIdLst->kwuSapId = wrCb.sapIds[cellId - 1].kwuSapId;
   sapIdLst->ckwSapId = wrCb.sapIds[cellId - 1].ckwSapId;
   sapIdLst->crgSapId = wrCb.sapIds[cellId - 1].crgSapId;
   sapIdLst->cpjSapId = wrCb.sapIds[cellId - 1].cpjSapId;
   sapIdLst->pjuSapId = wrCb.sapIds[cellId - 1].pjuSapId;

   sapIdLst->rguUlSapId = wrCb.sapIds[cellId - 1].rguUlSapId;
   sapIdLst->rguDlSapId = wrCb.sapIds[cellId - 1].rguDlSapId;

   sapIdLst->rguSapId = wrCb.sapIds[cellId - 1].rguSapId;
   RETVOID;
}
#endif
/**
...
#ifdef UE_REL_DGB
   printf("\nWR_UMM_TRANS_RELEASE_CALL called with Cause WR_UMM_CTXT_CSFB_REL\n");
#endif
...
#ifdef LTE_ADV_UECAP
/** @brief This function is used to check if the Ue is CA capable
 *  or not
 *  
 *   @details
 * 
 *   Function: wrUmmChkUeCASupp
 *  
 *   Processing steps:
 *      1) check if the UE capability supports CA bands
 *         and check the FGI bits for rel 1020.Check 
 *         if the enb cfg for CA and the UECap matches.
 *      
 *   @param[in]  ueCb: Ue control block
 *   @return
 *         -#Success : TRUE
...
{
   NhuUE_EUTRA_Cap_v1020_IEs *ueCapv1020 = NULLP;
   NhuSuppBandCombination_r10 *ueCapSupBndCmb_r10 = NULLP;

   U16 numComp1 = 0;
   U16 numComp2 = 0;
   U16 idx1 = 0;
   S16 ret = RFAILED;
   ueCapv1020 = &ueCb->ueCap->eUTRA_Cap.nonCriticalExtn.nonCriticalExtn.nonCriticalExtn;
   U8 bands[WR_MAX_CELLS] = {0};
   U8 class[WR_MAX_CELLS] = {0};
   U8 numComp3 = 0;
   U16 idx2 = 0;
   U16 idx3 = 0;
   U8 *bwCombSet = NULLP;
   ...
   if(wrEmmCb.caBandCfg.isEnbCaCap == TRUE)
   {
      /* Check if the UE is Rel10 and above or not */
      if(ueCb->ueCap->eUTRA_Cap.accessStratumRls.val >= NhuAccessStratumRlsrel10Enum)
      {
         if(PRSNT_NODEF == ueCapv1020->rf_Params_v1020.pres.pres)
         {
            numComp1 = ueCapv1020->rf_Params_v1020.supportedBandCombination_r10.noComp.val;
            ueCapSupBndCmb_r10 = &ueCapv1020->rf_Params_v1020.supportedBandCombination_r10;
            if(numComp1 > 0)
            {
               for(idx1 = 0 ;idx1 < numComp1 ; idx1++)
               {
                  numComp2 = ueCapSupBndCmb_r10->member[idx1].noComp.val;
                  if(numComp2 > 0)
                  {
                     //ret = TRUE;
                     for(idx2 = 0; idx2 < numComp2; idx2++)
                     {
                        NhuBandParams_r10 *ueCapBandParams_r10 = \
                                                                 &ueCapSupBndCmb_r10->member[idx1].member[idx2];
                        if(PRSNT_NODEF == ueCapBandParams_r10->pres.pres)
                        {
                           bands[idx2]= ueCapBandParams_r10->bandEUTRA_r10.val;
                           numComp3 = ueCapBandParams_r10->bandParamsDL_r10.noComp.val;
                           if(numComp3 > 0)
                           {
                              for(idx3 = 0 ; idx3 < numComp3 ; idx3++)
                              {
                                 if(PRSNT_NODEF == 
                                       ueCapBandParams_r10->bandParamsDL_r10.member[idx3].pres.pres)
                                 {
                                    class[idx2] = ueCapBandParams_r10->bandParamsDL_r10.member[idx3]. \
                                                  ca_BandwidthClassDL_r10.val;
                                 }
                              }
                           }
                        }
                     }
                     /* Get SupportedBandCombinationSet-r10 */
                     ret = wrUmmGetSuppBwCombSet(ueCapv1020,idx1,&bwComSetLen,&bwCombSet);
                     if(bwCombSet != NULLP && ret == ROK)
                     {
                        ret = wrUmmChkWthEnbCfg(bands,class,bwCombSet,numComp2,bwComSetLen);
                        WR_FREE(bwCombSet,(sizeof(U8)*bwComSetLen));
                     }   
                     if(ret == ROK)
                     {
                        RETVALUE(TRUE);
...
/** @brief This function is used to get the supported Bandwidth
 *         combination set from UECap.
 *  
 *   @details
 * 
 *   Function: wrUmmGetSuppBwCombSet
 *  
 *   Processing steps:
 *      1) get the supported bandwidth combination set from v1060
 *         IE of UECapability
 *         
 *      
 *   @param[in]  ueCapv1020: UE Capability v1020 IE
 *   @return
 *          
 *         -#Failure : FALSE
 */
...
{
   NhuRF_Params_v1060 *ueCapRfParam1060 = NULLP;
   ueCapRfParam1060 = &ueCapv1020->nonCriticalExtn.rf_Params_v1060;
   U8 numComp1 = 0;
   S16 ret = RFAILED;
   if(PRSNT_NODEF == ueCapRfParam1060->pres.pres)
   {
     numComp1 = ueCapRfParam1060->supportedBandCombinationExt_r10.noComp.val;
     NhuSuppBandCombinationExt_r10 *suppBandCombExt = \
                             &ueCapRfParam1060->supportedBandCombinationExt_r10;
     if(numComp1 > 0)
     {
        /* checking that particular index ie is present or not */
        if(PRSNT_NODEF == suppBandCombExt->member[idx].supportedBandwidthCombinationSet_r10.pres)
        { 
          ret =  wrUmmGetBwCombSet(
           suppBandCombExt->member[idx].supportedBandwidthCombinationSet_r10.val,
           suppBandCombExt->member[idx].supportedBandwidthCombinationSet_r10.len,
           bwComSetLen,bwCombSet);
          if(ret != ROK)
          {
             RETVALUE(RFAILED);
          }
        }
        /* if not present then it is supporting bandwdth comb set 0 */
        else
        {
           *bwComSetLen = 1;

           WR_ALLOC(bwCombSet,sizeof(U8));
           if(NULLP == *bwCombSet)
           {
              RLOG0(L_FATAL,"Error in memory allocation");
              RETVALUE(RFAILED);
           }
           **bwCombSet = 0;
        }
...
/** @brief This function is used to get the supported Bandwidth
 *         combination set by processing the bit map
 *  
 *   @details
 * 
 *   Function: wrUmmGetBwCombSet
 *  
 *   Processing steps:
 *      1) get the supported bandwidth combination set from v1060
 *         IE of UECapability
 *         
 *      
 *   @param[in]  val: bit map pointer
 *               len: length of the bit map      
 *   @return    
 *         -#Success : TRUE
 *         -#Failure : FALSE
 */
...
{
   U8 idx1 = 0;
   S8 set[WR_MAX_BW_COMB_SET]={0};
   U8 pos = 0;
   S8 idx2 = 0;
   U8  val2 = 0;
   U8  noOfSetBit = 0;

   for(idx1 = 0 ; idx1 < (len/8) ; idx1++)
   {
      val2 = val[idx1];
      if(val2 == 0x00)
      {
      ...
      else
      {
         ...
         {
            /* checking bitstring that particular  index bit is set or not 
              to retrieve bandwdth comb set sent  by ue and  storing that bit pos*/
            if((val2 >> idx2) & 1)
            {
               set[noOfSetBit++] = pos++;
            }
   ...
   WR_ALLOC(bwCombSet,sizeof(U8)*noOfSetBit);
   if(NULLP == *bwCombSet)
   {
      RLOG0(L_FATAL,"Error in memory allocation");
      RETVALUE(RFAILED);
   }
   cmMemcpy((U8*)(*bwCombSet), (U8*)&set,sizeof(U8)*noOfSetBit);
   *bwComSetLen = noOfSetBit;
    RETVALUE(ROK);
}


/** @brief This function is used to check whether ENB is CA capable or not by
 * Checking it with Table entries
 *    
 *  
 *   @details
 * 
 *   Function: wrEmmChkCaCap
 *  
 *   @param[in]  val: bit map pointer
 *               len: length of the bit map      
 *   @return    
...
PUBLIC Void  wrEmmChkCaCap
(
 WrCaBandCfg *caBandCfg
)
{
   U8 idx  = 0;
   U8 idx1 = 0;
   U8 temp = 0;
      ...
      for(idx = 0 ;idx < WR_CA_MAX_ENTRIES; idx++)
      {
         /* checking cell supp band and class with table entries to retrieve
          * bandwdth comb set value */
         if((caBandCfg->wrBandComb.band[temp].band == wrCaUeCapVerTab[idx].band1) && 
            (caBandCfg->wrBandComb.band[temp].bwClass == wrCaUeCapVerTab[idx].class1) &&
            (caBandCfg->wrBandComb.band[temp+1].band == wrCaUeCapVerTab[idx].band2) &&
            (caBandCfg->wrBandComb.band[temp+1].bwClass == wrCaUeCapVerTab[idx].class2))
         {
            for(idx1 = 0 ;idx1 < (2*(wrCaUeCapVerTab[idx].noOfComb)) ; idx1++)
            { 
               /* checking cell supported bandwidth with table entries */
               if((wrCaUeCapVerTab[idx].bandwidth[idx1++] == caBandCfg->wrCellParams.cellSuppBw[temp]) 
                     && (wrCaUeCapVerTab[idx].bandwidth[idx1] == caBandCfg->wrCellParams.cellSuppBw[temp+1]))
               {
                  /* storing supported bandwdth comb set for enodeb */
                  caBandCfg->wrBwCombSet.bwCombSet[caBandCfg->wrBwCombSet.noOfBwCombSet] = 
                     wrCaUeCapVerTab[idx].bdwthCombSet;  
                  caBandCfg->wrBwCombSet.noOfBwCombSet++;
      ...
      if(caBandCfg->wrBwCombSet.noOfBwCombSet > 0)
      {
         caBandCfg->isEnbCaCap = TRUE;
         for (idx1 = 0; idx1<caBandCfg->wrBandComb.numBandComb; idx1++)
         {
            RLOG2(L_ALWAYS, "LTE_ADV_UECAP:caBandCfg->band = %d, caBandCfg->bwClass = %d \n", 
                  caBandCfg->wrBandComb.band[idx1].band, caBandCfg->wrBandComb.band[idx1].bwClass);

            printf("LTE_ADV_UECAP:caBandCfg->band = %d, caBandCfg->bwClass = %d \n", 
                  caBandCfg->wrBandComb.band[idx1].band, caBandCfg->wrBandComb.band[idx1].bwClass);
         }
      }
      else
      {
         caBandCfg->isEnbCaCap = FALSE;
      }
}

/** @brief This function is used to check if the enb supports the Ue 
 *  by validating it with the UE cap sent.
 *    
 *  
 *   @details
 * 
 *   Function: wrUmmChkWthEnbCfg
 *  
 *   @param[in]  bands : bands supported by UE 
 *               class : classes supported by UE
 *               bwCombSet : bandwidth cmbination sets
 *                           supported by UE for the given combination
 *               bwComSetLen : number of sets supported by the UE
 *   @return    
 *         -#Success : TRUE
 *         -#Failure : FALSE
 */
...
)
{
   U8 idx1 = 0;
   U8 idx2 = 0;
   WrCaBandCfg *caBandCfg = &wrEmmCb.caBandCfg;
   U8 numBandComb = caBandCfg->wrBandComb.numBandComb;
   /* checking no Of bands in each comb sent by Ue and no of band supported by
    * enodeb are same pr not */
   if((ueCapNumBnd == numBandComb) && (ueCapNumBnd <= WR_MAX_CELLS))
   {
      for(idx1 = 0 ; idx1 < ueCapNumBnd ; idx1++)
      {
         /* checking bands and class supported by enb and send by ue in ue
          * capability */
         if((bands[idx1] == caBandCfg->wrBandComb.band[idx1].band) &&
           (class[idx1] == caBandCfg->wrBandComb.band[idx1].bwClass))
         {
            continue;
         }
         else
      ...
      if(idx1 == ueCapNumBnd)
      {
         for(idx1 = 0 ; idx1 < bwComSetLen ; idx1++)
         {
            for(idx2 = 0 ; idx2 < caBandCfg->wrBwCombSet.noOfBwCombSet; idx2++)
            {
               if(bwCombSet[idx1] == caBandCfg->wrBwCombSet.bwCombSet[idx2])
               {
                  RETVALUE(ROK);
               }
            }
