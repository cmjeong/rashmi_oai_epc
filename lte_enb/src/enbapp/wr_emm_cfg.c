...
#ifdef TIP_L3
EXTERN CmTptAddr upperL2Addr[WR_MAX_CELLS];
...
PRIVATE Void wrSendEnbCfgTransfer(LwrNeighCellCfg* nghCellcfg);
 ...
 )
{
   ...
   {
      /* In SIB 1 CSG Indication shall be TRUE and 
       * CSG Id shall be broadcast only if eNodeB is configured as Femto. */
   ...
   {
      /* In SIB 1 CSG Indication shall be FALSE for MACRO eNodeB Cfg. */
      sib1->csgInd               = FALSE;
   }
...
)
{
   ...
   if((WR_ENODEB_TYPE_MACRO != wrEmmCb.enbType) 
         && (sib4CfgGrp->csgInfo.isCsgPciCfgd))
   {
...
)
{
   ...
   WR_EMM_GET_CELLCB(cellCb, rrmWrCellRecfgInd->bCellId);
   if(cellCb == NULLP)
   {
...
/* LTE_ADV_DSFR ends */
               ...
               (PTR)neighCb,(U8 *)&neighCb->enbId, sizeof(neighCb->enbId)) != ROK)
      {
         ...
         /* This eNB might be for another cell of the eNB i.e when multiCell is cfgd. */
         cmHashListFind(&(wrEmmCb.neighEnbLstCp), 
               (U8 *)&neighCb->enbId,sizeof(neighCb->enbId),
               0,(PTR *)&neighEnCb);  
         if(neighEnCb->cellId == neighCb->cellId)
         {
...
)
{
   ...
   {
      if(NULLP != wrEmmCb.cellCb[cellIdx])
...
/*
*
*       Fun:   wrEmmSendSchedEnbCfg
*
*       Desc:  This function for filling eNodeB SCHED Configuration
*
*       Ret:   ROK
*
*       Notes: None
...
*
*/
PUBLIC S16 wrEmmSendSchedEnbCfg
(
 LwrSchedEnbCfg        *schedEnbCfg
)
{
   ...
   cfgReq->u.cfgInfo.cfgType = RGR_ENB_CFG;
...
//   cfg->cellId = (U16)cellId;
   /* MAC instance */
   //TODO
//   cfg->macInst              = schedEnbCfg.macInst;
   /* Number of Transmission antenna ports */
   cfg->numTxAntPorts        = schedEnbCfg->numAntPorts; 
   RLOG2(L_DEBUG,"wrEmmSendSchedEnbCfg:cfg->numTxAntPorts=%d:schedEnbCfg->numAntPorts=%d\n",cfg->numTxAntPorts,schedEnbCfg->numAntPorts);
   /* DL scheduler to be used */
   /*SCH Type|FullyFair|MaxCI|QoS-RR|QoS-PFS|
    *TpFact  |   0     | 10  |  0   |  10   |
    *FairFact|  10     |  0  |  0   |  10   |
    */
   cfg->dlSchdType           = schedEnbCfg->dlSchType;
   cfg->dlSchInfo.dlPfs.tptCoeffi = schedEnbCfg->schDlPfs.tptCoeffi;
   cfg->dlSchInfo.dlPfs.fairCoeffi = schedEnbCfg->schDlPfs.fairCoeffi;
   for(idx = 0; idx < LWR_MAX_NUM_QCI; idx++)
   {
      cfg->dlSchInfo.dlPfs.qciWgt[idx] =
         schedEnbCfg->schDlPfs.dlQciWgtList[idx];
   }   
    ...
    *FairFact|  10     |  0  |  0   |  10   |
    */
   cfg->ulSchdType           = schedEnbCfg->ulSchType;
   cfg->ulSchInfo.ulPfs.tptCoeffi= schedEnbCfg->schUlPfs.tptCoeffi;
   cfg->ulSchInfo.ulPfs.fairCoeffi= schedEnbCfg->schUlPfs.fairCoeffi;
   for(idx = 0; idx < LWR_MAX_NUM_QCI; idx++)
   {
      cfg->ulSchInfo.ulPfs.qciWgt[idx] =
         schedEnbCfg->schUlPfs.ulQciWgtList[idx];
   }   
   /* DLFS scheduler to use. Only 1 DLFS schd implementation
    * supported */

   printf("\nwrEmmSendSchedEnbCfg:: numTxAntPorts=%d\t ulSchdType=%d\t ulPfs.tptCoeffi=%d\t ulPfs.fairCoeffi=%d\n",\
         cfg->numTxAntPorts, cfg->ulSchdType, cfg->ulSchInfo.ulPfs.tptCoeffi, cfg->ulSchInfo.ulPfs.fairCoeffi); 
   
   /*TODO - To assign proper assignment of access mode during feature interaction with CA*/
   cfg->accsMode           = WR_ENB_OPEN_ACCESS;

   if(ROK != WrIfmSchdEnbCfgReq(cfgReq, (U32)1 /*transId*/))
   {
      RLOG0(L_FATAL, "WrSndRgrCfgReq failed.");
      RETVALUE(RFAILED);
   }

    RETVALUE(ROK);
} /* wrEmmSendSchedEnbCfg */
...
)
{
...
#ifdef TIP_L3
   wrCpyCmTptAddr(&upperL2Addr[0], &(cfgAdd->protoCfg).enbIpAddr[WR_IPV4_IDX]);
   //upperL2Addr.u.ipv4TptAddr.address = inet_addr("172.27.1.177");
   //upperL2Addr.u.ipv4TptAddr.address = inet_addr("192.168.1.12");
   //upperL2Addr.u.ipv4TptAddr.address = UL2_IP_ADDR;
   upperL2Addr[0].u.ipv4TptAddr.address = UL2_GTP_IP_ADDR;
   //upperL2Addr.u.ipv4TptAddr.address =
     // CM_INET_HTON_U32(upperL2Addr.u.ipv4TptAddr.address);
   upperL2Addr[0].u.ipv4TptAddr.port = WR_DFLT_EGTP_PORT;
#endif
   ...
   /*sprint4 UECap start*/
#ifdef LTE_ADV
   wrEmmCb.isCaEnabled = cfgAdd->isCaEnabled;
#ifdef LTE_ADV_UECAP
   wrEmmCb.isCellFreqCont = cfgAdd->isCellFreqCont;
#endif /* LTE_ADV_UECAP*/
#endif /* LTE_ADV*/
   /*sprint4 UECap end*/

   /* Added SCH Config Function which shall 
    * send Scheduler Specific Configuratoins to MAC-SCH
    * on the RGR interface, the Confirmation from the 
    * SCH is handled in the wrMiLwrSchedCfgCfm()
    * */
   wrEmmCb.isSchedEnbCfgDone = FALSE;
   cmMemcpy((U8 *)&wrEmmCb.schedEnbCfg, (U8 *)&cfgAdd->schedEnbCfg, sizeof(LwrSchedEnbCfg));
...
#if 0
/*CHINNA: Currently RIM procedure is commented */
   /* RIM Initilisation */
...
)
{
...
#ifdef LTE_ADV
   cellCb->sCellDeActTmr.pres = cfgAdd->sCellDeActTmr.pres;
   cellCb->sCellDeActTmr.val = cfgAdd->sCellDeActTmr.val;
#endif /* LTE_ADV */
   /* sps - HOFIX */
   cellCb->numTxAntenna = cfgAdd->numTxAntenna;
   cellCb->numAntPorts = cfgAdd->numTxAntenna;
   RLOG1(L_DEBUG,"In wrEmmCfgCellAdd, cellCb->numTxAntenna:%d\n",cellCb->numTxAntenna);
   ...
   /* CSG_DEV end */
   /*Moved wrUmmInitRabParams after FOR loop*/
   ...
   for (grpIndx = 0; grpIndx < cfgAdd->numOfGrps; grpIndx++)
   {
      switch (cfgAdd->cellCfgGrp[grpIndx].grpType)
      {
         ...
         case WR_CFG_TYPE_UNLICNSD_CELL:
            if((WR_DFLT_CELL_INDX-1) == cfgAdd->cellIndex)
            {   
               cellCb->isUnliCell = TRUE;
               cellCb->lteUCfg = cfgAdd->cellCfgGrp[grpIndx].t.lteUCfgGrp;
            }
            break;
#endif
         default:
   ...
   wrUmmInitRabParams(cellCb);/*DO NOT remove*/
 ...
 */
 ...
 )
 {
    ...
    {
       eutranNeighCell =(WrEutranNeighCellCb *)list->node;
       if ((eutranNeighCell->pci == pci) && (neignEnb->cellId == cellId))
       {
          RETVALUE(ROK); 
       }
       CM_LLIST_NEXT_NODE(&(neignEnb->srvdCellLst),list);
...
)
{
   ...
   if(wrEmmCb.neighEnbLstCp.hashFunc == NULLP)
   {
      RETVALUE(RFAILED);
   }
               ...
               case WR_NEIGH_CELL_EUTRA:
                  {
                     ...
                     if (wrEmmFindServedNeighCell(neighEnCb, neighCellCb->pci, cellId) ==  ROK)
                     {
...
)
{
   ...
   if x2 is enbale it will be enabled for all neighbor or it will be disabled for all neighbor*/
      if((neighEnbCb->isX2Reqd == TRUE) && (neighEnbCb->srvdCellLst.count != 0)
            && (neighEnbCb->cellId == cellCb->cellId) && (neighEnbCb->enbCfgTransTriggerd == FALSE))
        {
...
/** @brief This function used send enb config transfer to get x2 peerId
 *  
 *  @details
 *
 *  Function: wrSendEnbCfgTransfer 
 *
 *  Processing steps: 
 *     - Handle the Api
 *
 *  @param[in] numCells : Number of cells
 *             neighCellAdd : pointer to LwrNeighCellCfg object
 *
 *  @return U16
 *     -# ROK       : Success
 *     -# RFAILED   : Failure
...
PRIVATE Void wrSendEnbCfgTransfer(LwrNeighCellCfg* nghCellcfg)
{
   ...
   U16                idx;
   U8                 sndCfgTra = FALSE; 
   U8                 isIpAddrCfg = FALSE;
   /* Not configured ip address value */ 
   U8                 ipAddrVal[16] = {0};  
   eNodCfgParams.numEnb = 0;

   for(idx =0; idx < nghCellcfg->numCells; idx++)
   {
     enbId = nghCellcfg->t.neighAddCfg[idx].t.eutraCell.enbId;

     ret = cmHashListFind(&(wrEmmCb.neighEnbLstCp),
                      (U8 *)&enbId,sizeof(enbId),0,(PTR *)&neighEnCb);
     /*trigger enb config transfer only if x2 need to enable 
      if the ip is not configured from the SM
      check for the same enobed is already enbconfig is triggerd or not
     */
     if(CM_TPTADDR_IPV4 == neighEnCb->neighEnbAddr.type)
     {
         if(neighEnCb->neighEnbAddr.u.ipv4TptAddr.address != 0)
         {
            isIpAddrCfg = TRUE;
         } 
     }
     else if(CM_TPTADDR_IPV6 == neighEnCb->neighEnbAddr.type)
     {
         if(ROK != cmMemcmp((U8 *)&neighEnCb->neighEnbAddr.u.ipv6TptAddr.ipv6NetAddr,
            (U8 *)&ipAddrVal, sizeof(CmInetIpAddr6)))
         {
            isIpAddrCfg = TRUE;
         } 
     }

     if((ret == ROK) && (neighEnCb->isX2Reqd == TRUE) &&
      (isIpAddrCfg == FALSE) && (neighEnCb->enbCfgTransTriggerd == FALSE))
     {
         /*macro can have multiple cells but only once enbCfgTransfer should send*/
          eNodCfgParams.neighEnbCb[eNodCfgParams.numEnb] = neighEnCb;
          eNodCfgParams.numEnb ++;
          sndCfgTra = TRUE;
          neighEnCb->enbCfgTransTriggerd = TRUE;
     }
   }
   if(sndCfgTra == TRUE)
   {
     wrEmmMmeBldEnbConfigTransfer(&eNodCfgParams,nghCellcfg->cellId);
   }
}/*end of wrSendEnbCfgTransfer*/
...
//   cellCb = wrEmmCb.cellCb[WR_DFLT_CELL_IDX];
   cellCb = wrEmmCb.cellCb[cfg->cellIdx];
   
   if(WR_CELL_STATE_UP != cellCb->cellState)
   {
      RLOG1(L_ERROR,"Recived Dynamic configuration for cell %d which is not up",
             cfg->cellIdx);
   ...
   switch (cfg->cfgType)
   {
      ...
      case LWR_DYN_EVNT_NEIGH_EUTRA_CELL_CFG:
      {
         ...
         /*to trigger enbConfig transfer message at least one serving cell is needed 
       that why we are calling this funtion after adding neighbor cell*/
         if(neighCellCfg->action == WR_ACT_ADD)
         {
           wrSendEnbCfgTransfer(neighCellCfg);
         }
         WR_FREE(cfg->dynCfg, sizeof(LwrNeighCellCfg));
       } 
      break;
...
PUBLIC S16 wrEmmSendAddGeranNghInd(WrGeranCellCb *geranNgh, LwrActType actType, U8 cellId)
{
...
PUBLIC S16 wrEmmSendAddEutraNghInd(WrEutranNeighCellCb *eutraNgh,LwrActType actType,U8 cellId)
{
...
PUBLIC S16 wrEmmSendUtraNghInd(WrUtraNeighCellCb *utraNgh,LwrActType actType, U8 cellId)
{
