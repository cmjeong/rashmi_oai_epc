...
/*WR_EMM_CELL_GET_RGR_TX_ANT removed in CA codebase*/
...
/** @brief This structure contains the following members
 - U16          tac              One of the TAC entries to be sent in S1 Setup.
 - U8           numPlmnIds       Number of PlmnIds supported in this TAC by the eNodeB.
 - WrPlmnId     plmns[WR_MAX_PLMN_IDS]    The Plmns supported in this TAC by the eNodeB.
 */
typedef struct wrEmmSuppTAs{
   U16         tac;
   U8          numPlmnIds;
   WrPlmnId    plmns[WR_MAX_PLMN_IDS];
}WrEmmSuppTAs;

/** @brief This structure contains the following members
 - U16          numTAs              Number of TAs supported by the eNodeB, to be sent in S1 Setup.
 - WrEmmSuppTAs   taInfo[WR_EMM_MAX_S1_SETUP_TAs]    The TAs supported by the eNodeB, to be sent in S1 Setup.
 */
typedef struct wrEmmS1SetpSuppTAs{
   U16         numTAs;
   WrEmmSuppTAs   taInfo[WR_EMM_MAX_S1_SETUP_TAs];
}WrEmmS1SetpSuppTAs;
...
typedef struct WrCellCb
{
   ...
   U16                       physCellId;
   U16                       dlCyclicprefix;
   U8                        numAntPorts;//check whether to remove
   U8                        numTxAntenna;
   U32                       ttiCnt;
   WrMibParams               mib;
   WrSib1Params              sib1;
...
/* CNM_DEV */
   ...
   Bool                       isUnliCell;  /*!< Presence of Unlicensed cell- LTE-U/LAA*/
   LwrLteUCfg                 lteUCfg;     /*!< Unlicensed Configuration */
#endif
...
typedef struct wrNeighEnbCb
{
   ...
   Bool                      isX2Connected;
   Bool                      isX2Reqd;
   Bool                      isX2ReqSent;          /* Used to identify whether X2 Request has been sent from ENBAPP */
   Bool                      isAnrDetected;
   U8                        enbType;
   U8                        numGuGrpIds;
   WrGuGrpIdLst              guGrpIdLst[WR_MAX_GU_GRP_IDS];
...
#define WR_EMM_GET_CELLCB(_cellCb, _cellId){\
   U32 _idx;\
   _cellCb = NULLP;\
...
#define WR_EMM_GET_NUM_TX_ANT(_numTxAnt, _cellId) {\
   WrCellCb       *ncellCb;\
   WR_EMM_GET_CELLCB(ncellCb, _cellId);\
      if(ncellCb != NULLP)\
      {\
         _numTxAnt = ncellCb->numAntPorts;\
      }\
}

#define WR_EMM_SET_TTI_CNT(_cellId) {\
   WrCellCb       *ncellCb;\
   WR_EMM_GET_CELLCB(ncellCb, _cellId);\
      if(ncellCb != NULLP)\
      {\
...
#define WR_EMM_GET_TTI_CNT(_cellId, _ttiCnt) {\
   WrCellCb       *ncellCb;\
   WR_EMM_GET_CELLCB(ncellCb, _cellId);\
      if(ncellCb != NULLP)\
      {\
         _ttiCnt = ncellCb->ttiCnt;\
      }\
}
...
EXTERN S16 wrEmmCellCfgDam(WrCellCb *cellCb);/*MultiCell*/
