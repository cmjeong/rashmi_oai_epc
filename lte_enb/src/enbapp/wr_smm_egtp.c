...
/********************************************************************20**
     ...
     Sid:      fc_smm_egtp_t2k.c@@/main/TeNB_Main_BR/5 - Mon Aug 11 16:52:07 2014
...
#endif
{
   ...
   RLOG1(L_DEBUG, "EGTP Tapa Task successfully registered and attached to %d",sysTskId);
...
#endif
{
   ...
   cntrl.hdr.response.selector   = SM_SELECTOR_LC;
...
#endif
{
   ...
   cfg->lmPst.dstProcId  = SM_UL2_SM_PROC;
#else
...
#else
PUBLIC Void smFillEgIpInfo(spId, ipInfo)
SpId           spId;
EgIpInfo       *ipInfo;        
#endif
{
...
#ifdef TIP_L3
   //cmInetAddr((S8 *)"172.27.1.177", &(upperL2IpAddr));
   //upperL2IpAddr = CM_INET_HTON_U32(upperL2IpAddr);
   ipInfo->srcIpAddr.type = CM_TPTADDR_IPV4;
   //cmInetAddr("172.27.1.177", &ipInfo->srcIpAddr.u.ipv4TptAddr.address);
   cmInetAddr("192.168.1.12", &ipInfo->srcIpAddr.u.ipv4TptAddr.address);
   ipInfo->srcIpAddr.u.ipv4TptAddr.address = 
      CM_INET_HTON_U32(ipInfo->srcIpAddr.u.ipv4TptAddr.address);
   //ipInfo->srcIpAddr.u.ipv4TptAddr.address = inet_addr("172.27.1.177");
   ...
   {
     ipInfo->srcIpAddr.type = CM_TPTADDR_IPV4;
     ipInfo->srcIpAddr.u.ipv4TptAddr.address = fapParameters.lteEnodebIpAddr.enbIpAddr;
     ipInfo->srcIpAddr.u.ipv4TptAddr.port = 0;
   }
   else if(TRUE == fapParameters.lteEnodebIpAddr.ipv6pres)
   {
      ipInfo->srcIpAddr.type = CM_TPTADDR_IPV6;
      cmMemcpy((U8*)ipInfo->srcIpAddr.u.ipv6TptAddr.ipv6NetAddr,
...
//CHINNA:TODO workaround
   fapParameters.lteEnodebIpAddr.ipv4pres = TRUE;
   //fapParameters.lteEnodebIpAddr.enbIpAddr = UL2_IP_ADDR;
   fapParameters.lteEnodebIpAddr.enbIpAddr = UL2_GTP_IP_ADDR;
   ...
   if(TRUE == fapParameters.lteEnodebIpAddr.ipv4pres)
   {
      smFillEgIpInfo(spId, &cfg->ipInfo[idx]);
      idx++;
   } 
   if(TRUE == fapParameters.lteEnodebIpAddr.ipv6pres)
   {
...
#else
PUBLIC S16 smBuildTServerCfg(Void)
#endif
{
   EgMngmt  egMgt;
   Pst      pst;
   ...
   //cmInetAddr("172.27.1.177", &cfg->tptAddr.u.ipv4TptAddr.address);
   cmInetAddr("192.168.1.12", &cfg->tptAddr.u.ipv4TptAddr.address);
   cfg->tptAddr.u.ipv4TptAddr.address = 
      CM_INET_HTON_U32(cfg->tptAddr.u.ipv4TptAddr.address);
   //cfg->tptAddr.u.ipv4TptAddr.address = inet_addr("172.27.1.177");
   cfg->tptAddr.u.ipv4TptAddr.port = WR_DFLT_EGTP_PORT; 
#else
   if(TRUE == fapParameters.lteEnodebIpAddr.ipv6pres)
   {
      cfg->tptAddr.type = CM_TPTADDR_IPV6;
      cmMemcpy((U8*)cfg->tptAddr.u.ipv6TptAddr.ipv6NetAddr,
            (U8*)fapParameters.lteEnodebIpAddr.enbIpv6Addr, sizeof(CmInetIpAddr6));
   ...
   {
      cfg->tptAddr.type = CM_TPTADDR_IPV4;
      cfg->tptAddr.u.ipv4TptAddr.address = fapParameters.lteEnodebIpAddr.enbIpAddr;  /* 0xAC1A001C */
      cfg->tptAddr.u.ipv4TptAddr.port = WR_DFLT_EGTP_PORT; 
   }
#endif
...
#endif
{
   ...
   RLOG2(L_DEBUG, "EGTP Cfg status %d elmnt: %d",
   ...
   if (cfm->cfm.status == LCM_PRIM_OK)
   {
       ...
       if (wrEgCfg == WR_SM_EG_CONFIGURED)
       {
...
#ifdef TIP_L3_UL2_LL2_SPLIT
          WR_FILL_PST(egPst, SM_UL2_SM_PROC, SM_EG_PROC, ENTSM, ENTEG,0,0, EVTWRPDCPCFG, WR_POOL, SM_SELECTOR_LC);
#else
          WR_FILL_PST(egPst, SM_SM_PROC, SM_EG_PROC, ENTSM, ENTWR,0,0, EVTWRX2APCFG, WR_POOL, SM_SELECTOR_LC);
#endif 
#else
#ifdef RM_INTF
#ifdef TIP_L3_UL2_LL2_SPLIT
          WR_FILL_PST(egPst, SM_UL2_SM_PROC, SM_EG_PROC, ENTSM, ENTEG,0,0, EVTWRPDCPCFG, WR_POOL, SM_SELECTOR_LC);
#else
          WR_FILL_PST(egPst, SM_SM_PROC, SM_EG_PROC, ENTSM, ENTWR,0,0, EVTWRRRMCFG, WR_POOL, SM_SELECTOR_LC);
#endif 
#else
#ifdef TIP_L3_UL2_LL2_SPLIT
          WR_FILL_PST(egPst, SM_UL2_SM_PROC, SM_EG_PROC, ENTSM, ENTEG,0,0, EVTWRPDCPCFG, WR_POOL, SM_SELECTOR_LC);
#else
          WR_FILL_PST(egPst, SM_SM_PROC, SM_EG_PROC, ENTSM, ENTWR,0,0, EVTWRAPPCFG, WR_POOL, SM_SELECTOR_LC);
#endif 
#endif /*RM_INTF*/
...
#endif /* ANSI */
{
   ...
   if (cntrl->cfm.status == LCM_PRIM_OK)
   {
      if (smCb.smState == WR_SM_STATE_MAC_SHUTDWN_DONE)
      {
         ...
         wrEgIpCfgCnt = 0;
#ifdef LTE_HO_SUPPORT
         WR_FILL_PST(egPst, SM_SM_PROC, SM_EG_PROC, ENTSM, ENTWR,0,0, EVTWRX2APSHUT, WR_POOL, SM_SELECTOR_LC);
#else
#ifdef RM_INTF
         WR_FILL_PST(egPst, SM_SM_PROC, SM_EG_PROC, ENTSM, ENTWR,0,0, EVTWRX2APSHUT, WR_POOL, SM_SELECTOR_LC);
#else
   ...
   if (cntrl->cfm.status == LCM_PRIM_OK && cntrl->hdr.elmId.elmnt == STEGTSAP)
   {
      ...
      if (smCb.smState == WR_SM_STATE_X2AP_STK_UNBND_DONE)
      {
            ...
            WR_FILL_PST(egPst,  SM_SM_PROC,SM_WR_PROC, ENTSM, ENTWR,0,0, EVTWRS1APSTKUNBND,
...
#ifdef TIP_L3_UL2_LL2_SPLIT
      smBindUnBindDamToEgtSap(ABND_ENA);
#else
   ...
   {
      RLOG0(L_FATAL, "SmMiLegCntrlCfm failed");
   }
...
/********************************************************************30**
           ...
           End of file:     fc_smm_egtp_t2k.c@@/main/TeNB_Main_BR/5 - Mon Aug 11 16:52:07 2014
