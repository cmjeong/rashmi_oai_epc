...
)
{
   ...
   //printf("\n WrIfmEgtpEguLclTnlMgmtReq: Selector=%d\n",pst->selector);
...
#ifdef LC_EGTP_THREAD
void egWrReadRingBuf(SsRngBufId ringId)
{
#if 0
   S16          ret           = ROK;
   U8           rngBufDeqIndx = 0;
#endif
   Void         *elmIndx      = NULLP;
   EgWrFreeInfo *freeBufInfo  = NULLP;


   elmIndx = SRngGetRIndx(ringId);

   while(NULLP != elmIndx)
   {
      freeBufInfo  = (EgWrFreeInfo *)elmIndx;

      SsRngInfoTbl[ringId].nPktProc++;

      if((freeBufInfo->tmpBuf != NULLP) && 
            (freeBufInfo->buf != NULLP))
      {
         wrDamFbDatInd(freeBufInfo->tmpBuf,freeBufInfo->buf,
               freeBufInfo->bufLen);
      }
      ...
      if(rngBufDeqIndx >= SS_FREE_MGR_MAX_FREE)
         break;
#endif
      if((elmIndx = SRngGetRIndx(ringId)) == NULLP)
         break;
   }
