...
#define EVTLWRCELLCFGREQ                          70       /* Cell Config request */
#define EVTLWRCELLCFGCFM                          71       /* Cell Config Confirm */
#define EVTLWRCELLCNTRLREQ                        72       /* Cell Control Request */
#define EVTLWRCELLCNTRLCFM                        73       /* Cell Control Confirm */
...
#define LWR_CAUSE_DAM_SAP_BOUND          (LCM_CAUSE_LYR_SPECIFIC + 11)
...
#define LWR_TRANSID_DAMCFG                       (LWR_TRANSID_GEN + 30)
/***************************************************************
 *                       Element Macros                        *
 ***************************************************************/
#define STWRGEN                                   1
...
#define STWRSCELLRELEASE                          33
#define STWRSCELLADD                              34
#define STWRSCELLACT                              35
#define STWRSCELLDEACT                            36
#define STWRDAMSAP                                37
...
typedef struct lwrLteUCfg
{
   U8      coExistMethod; /*!< Defines the type of coexistence, like continuous 
						       transmission or fixed duty cycle without LBT or 
                               LBT, etc */         
   U8      lteOnPeriod;   /*!< Defines the percentage of LTE on duration */
   U8      caeMethod;     /*!< Channel sensing method */
   U8      adaptiveTx;    /*!< If enabled, the duty cycle can vary dynamically */
   U16     transPeriod;   /*!< Defines the duty cycle in ms */ 
   U16     listenPrd;     /*!< Time duration in micro sec for which the clear 
							   channel assesment (CCA) is performed */
   U16     energyTh;      /*!< The energy threshold for CCA */ 
   U16     scanTimePrd;   /*!< Time period for periodic channel scan */ 
   U16     activityTh;    /*!< Threshold interms of percentage of activity from 
							   other WI-FI or LTE-U transmissions */
   S32     txPowerLimit;  /*!< Max RSRP for the unlicensed cell(s) */
   U8      numFreq;       /*!< Total number of configured EARFCN in unlicensed */ 
   U32     earfcn[LWR_LTEU_MAX_EARFCN];   /*!< List of unlicensed EARFCN's */
...
#ifdef LTE_ADV
#define LWR_MEAS_MAX_EVENT_A4_CFG         2
#else
 ...
 * -# Scell addition Specific for Event A4
 * -# Reserved1
 * -# Reserved2
 */
...
#ifdef LTE_ADV
   WR_MEAS_EVENTA4_SCELL_ADD,//suhas_sprint12
#endif
...
#ifdef LTE_UNLICENSED
      LwrLteUCfg             lteUCfgGrp; /*!< Unlicensed cell Cfg parameters */
#endif
   }t;
} LwrCellCfgGrp;
...
#ifdef LTE_ADV
   U8              aCqiTriggerSet1;    /*!< Trigger set one*/
   U8              aCqiTriggerSet2;    /*!< Trigger set two*/
#endif
      U16      		 cqiPucchRsrcIdx;
      U16      		 cqipmiCfgIdx;
...
typedef struct lwrAddCellCfg
{
   ...
   LwrCellCfgGrp  cellCfgGrp[WR_CELL_CFG_MAX_GRPS];   /*!< Array of groups of
                                                     configuration */
...
typedef struct lwrNeighAddModCfg
{
   ...
   CmTptAddr         secIpAddr;  /*!< Secondary IP address */
   LwrNeighEnbState  state;      /*!< State of the neighbour cell */
   LwrX2ConFlg       isX2Reqd;       /*!< Whether X2 connection should be established(FALSE) or Not(TRUE) */
/* LTE_ADV_DSFR starts */
...
/** @brief This structure contains the Enodeb level DL PFS Scheduler configuration coming from the
 * OAM interface. 
 */
...
/** @brief This structure contains the Enodeb level UL Scheduler configuration coming from the
 * OAM interface. 
 */
...
/** @brief This structure contains the Enodeb level Scheduler configuration coming from the
 * OAM interface. 
 */
typedef struct lwrSchedEnbCfg
{
   U8                  macInst;     /* Scheduler Instance */
   U8                  numAntPorts; /* number of ANtena Ports */
   U8                  dlSchType;  /* Indicates which DL scheduler to use, range
                                      * is 0..(number of schedulers - 1) */
   LwrEnbSchDlPfs      schDlPfs;    /* DL PFS Config */
   U8                  ulSchType;  /* Indicates which UL scheduler to use, range
                                     * is 0..(number of schedulers - 1) */
   LwrEnbSchUlPfs      schUlPfs;    /* UL PFS Config */
...
typedef struct lwrENodeBCfg
{
   ...
   LwrSchedEnbCfg  schedEnbCfg;   /* Scheduler Specific Paramters */
} LwrENodeBCfg;
...
typedef struct lwrSmCellCfg
{
   ...
   U16 duplexMode;
   U16  maxUeSupp; /* maximum number of UEs supported by each eNodeB */
   /* PLMN Id */
   TknU8 mcc[3];
