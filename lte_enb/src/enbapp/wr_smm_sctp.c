/********************************************************************20**
     ...
     Sid:      fc_smm_sctp_t2k.c@@/main/TeNB_Main_BR/5 - Mon Aug 11 16:52:32 2014
...
#endif /* ANSI */
{
   ...
   {
      cfg->srcNAddrLst.nAddr[idx].type = CM_IPV4ADDR_TYPE;
      cfg->srcNAddrLst.nAddr[idx].u.ipv4NetAddr = fapParameters.lteEnodebIpAddr.enbIpAddr;
      idx++;
      cfg->srcNAddrLst.nmb = idx;
   }
   if(TRUE == fapParameters.lteEnodebIpAddr.ipv6pres)
   {
      cfg->srcNAddrLst.nAddr[idx].type = CM_IPV6ADDR_TYPE;
      cmMemcpy((U8*)cfg->srcNAddrLst.nAddr[idx].u.ipv6NetAddr,
            (U8*)fapParameters.lteEnodebIpAddr.enbIpv6Addr, sizeof(CmInetIpAddr6));
      idx++;
      cfg->srcNAddrLst.nmb = idx;
   }

   cfg->srcNAddrLst.nAddr[0].u.ipv4NetAddr = fapParameters.lteEnodebIpAddr.enbIpAddr;
...
#endif /* ANSI */
{
   ...
   cfg->reConfig.maxAckDelayDg = 2;

/* Amit: SCTP Param Configuration Support */
#ifdef WR_RSYS_OAM
   cfg->reConfig.rtoInitial = lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteSctpCfgParams.rtoInitial/SCTP_TIME_FROM_MS_TO_SEC;
   cfg->reConfig.rtoMin     = lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteSctpCfgParams.rtoMin/SCTP_TIME_FROM_MS_TO_SEC;
   cfg->reConfig.rtoMax     = lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteSctpCfgParams.rtoMax/SCTP_TIME_FROM_MS_TO_SEC;
   cfg->reConfig.intervalTm = lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteSctpCfgParams.heartBeatInterval/SCTP_TIME_FROM_MS_TO_SEC;    /* Heartbeat timer,5 seconds */
#else
  cfg->reConfig.rtoInitial = lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteSctpCfgParams.rtoInitial;
   cfg->reConfig.rtoMin     = lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteSctpCfgParams.rtoMin;
   cfg->reConfig.rtoMax     = lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteSctpCfgParams.rtoMax;
   cfg->reConfig.intervalTm = lteeNodeBparams[WR_SM_DFLT_CELL_INDX].lteSctpCfgParams.heartBeatInterval;    /* Heartbeat timer,5 seconds */
#endif
...
#endif
            WR_FILL_PST(sbPst, SM_SM_PROC, SM_SB_PROC, ENTSM, ENTWR,0,0, EVTWRS1APCFG, WR_POOL, SM_SELECTOR_LC);
...
#endif /* ANSI */
{
   ...
   {
      if (smCb.smState == WR_SM_STATE_CL_SHUTDWN_DONE)
      {
      smCb.smState = WR_SM_STATE_SCTP_SHUTDWN_DONE;
         RLOG0(L_ALWAYS,"SCTP SHUTDOWN SUCCESSFULLY PERFORMED.");
      WR_FILL_PST(sbPst, SM_SM_PROC, SM_SZ_PROC, ENTSM, ENTWR,0,0, EVTWRTUCLSHUT, WR_POOL, SM_SELECTOR_LC);
   ...
   if (cntrl->hdr.elmId.elmnt == STSBTSAP) 
   {
      ...
      if (smCb.smState == WR_SM_STATE_EGTP_STK_UNBND_DONE)
      {
         if (cntrl->cfm.status == LCM_PRIM_OK)
         {
            ...
            WR_FILL_PST(sbPst, SM_SM_PROC, SM_WR_PROC, ENTSM, ENTWR, 0,0,EVTWRRRMSHUT,
   ...
   else if(cntrl->cfm.status == LCM_PRIM_NOK)
   {
...
/********************************************************************30**
           ...
           End of file:     fc_smm_sctp_t2k.c@@/main/TeNB_Main_BR/5 - Mon Aug 11 16:52:32 2014
