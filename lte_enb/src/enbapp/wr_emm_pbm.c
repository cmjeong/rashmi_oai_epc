...
#ifdef TIP_L3
/** @brief This function is used to Activate the PLMN information SIB1
 *
 * @details
 *
 *     Function: wrEmmPbmActivatePlmnInSib1
 *
 *         Processing steps:
 ...
 *    -# Success : ROK 
 */
PUBLIC S16 wrEmmPbmActivatePlmnInSib1
(
 WrMmeCb                     *mmeCb
)
{
   ...
   for(plmnIdx = 0; plmnIdx < mmeCb->numPlmnIds; plmnIdx++)
   {
      for(cellIdx = 0; cellIdx < WR_MAX_CELLS; cellIdx++)
      {
         ...
         /* Currently PLMN Check happening for primary PLMN Only
          * Need to add same logic for other PLMN ID's*/
         if ((cellCb != NULLP) && 
             (wrUtlPlmnsEqual(&cellCb->sib1.plmns[0].plmnId, &mmeCb->plmnIds[plmnIdx]) == TRUE))
         {
            cellCb->sib1.plmns[plmnIdx].activated = TRUE;
         }
      }
   }
   RETVALUE(ROK);
...
)
{
   ...
   for(loopIndx = 0; loopIndx < numPagRcrds; loopIndx++)
   {
      ...
      {
         if(cellCb->ttiCnt >= pbmInfo->pendPagMsg.pagRec[loopIndx].arrvTimeInfo)
         {
            timeDiff = cellCb->ttiCnt - 
   ...
   if(isPagingRecSlectd)
   {
      ...
      pbmInfo->pendPagMsg.pagRec[selPagIndex].arrvTimeInfo = 
         (cellCb->ttiCnt % WR_MAX_PAGING_TTI_TIME);
   ...
   while (pbmInfoLnk)
   {
            ...
            (pbmInfo->pendPagMsg.po == uePagingInfo->po))
      {
         if(pbmInfo->pendPagMsg.numPagRcrds < WR_PBM_MAX_PAGING_RECORDS)
         {
            ...
            wrPbmUpdatePagingRecord(cellCb, uePagingInfo, pbmInfo);
            RETVALUE(ROK);
         }
         else 
         {
...
)
{
   ...
   if(numUpdatedSis)
   {
      ...
      {
#ifdef TIP_L3
         /* only for TIP interface, to indicate number of SIs that are 
          * being reconfigured */
         ...
         cellReCfg->cellId = (U16)cellCb->cellId;
         cellReCfg->recfgTypes |= RGR_CELL_SI_RECFG;
         wrEmmCellSchdSiCfg(&cellReCfg->siReCfg, cellCb);
         /* update numSi to only the number of SIs that will
          * be reconfigured */
         cellReCfg->siReCfg.numSi = numUpdatedSis;

         if(ROK != WrIfmSchdCfgCell(cfgReq, (U32)cellCb->cellId))
         {
            RLOG_ARG0(L_ERROR, DBG_CELLID, cellCb->cellId,
                  "Failed to send cell configuration to TIP interface");
            RETVALUE(RFAILED);
         }
#endif
         /* Ph04_CMAS : Added last parameter. */
...
//  for (cellIdx = 0; cellIdx < wrEmmCb.numCells; cellIdx++)
//  {
   WR_EMM_GET_CELLCB(cellCb, tmngInfo->cellId);
   if (cellCb == NULLP)
   {
      RLOG1(L_ERROR,"Invalid Cell Id [%d] received in measurement report CGI handler", \
      tmngInfo->cellId);
      RETVALUE(RFAILED);
   }
   //  cellCb = wrEmmCb.cellCb[cellIdx];
