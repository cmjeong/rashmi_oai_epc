...
)
{
   ...
   /*printf("\n\ntnl cfg cfm LclTnlID[%ld] action: %d status: %d\n\n",
         eguMsg->u.tnlMgmt.teId, eguMsg->u.tnlMgmt.action,
         eguMsg->u.tnlMgmt.cfmStatus.status);*/

   RLOG_ARG3(L_DEBUG, DBG_UEIDX, ueIdx, "TnlCfgCfm LclTnlID[%ld] action: %d"
         "status: %d", eguMsg->u.tnlMgmt.teId, eguMsg->u.tnlMgmt.action,
         eguMsg->u.tnlMgmt.cfmStatus.status);
...
)
{
...
#if 0
   WrDamPrntCellTpt          *damPrntCellTpt = NULLP;
#endif
   U32                       idx;
   ...
   cellCb->cellInUse = TRUE;
#if 0
   WR_ALLOC_DATA_APP(&damPrntCellTpt,  sizeof(WrDamPrntCellTpt));
   printf("WR_DAM_STATS_TMR: exp cellId %d\n", damPrntCellTpt->cellId);
   if(damPrntCellTpt == NULLP)
   {
     printf("Failed to alloc Memeory for timer\n");
     RETVALUE(ROK);
   }
   printf("Staring STATS timer cellId %d\n", cellId);
   damPrntCellTpt->cellId = cellId;
   cmInitTimers(&damPrntCellTpt->statsTmr, 1);
   wrDamStartTmr((PTR)damPrntCellTpt, WR_DAM_STATS_TMR, 3000);
#endif
   RETVALUE(ROK);
}
...
#ifdef TIP_UPPER_L2
   /* In TIP case: Ticks for Egtp is generated based on timer expiry*/
   RLOG0(L_ALWAYS, "Thread to create ticks for EGTP is initiated");
   strtPst = 1;
#endif
   RETVALUE(ROK);
}
...
/*MultiCell - end*/
       ...
       (*ueCb)->ueId       = rnti;
       (*ueCb)->ueIdx      = ueIdx;
       (*ueCb)->cellId     = cellCb->cellId;/*MultiCell*/
       (*ueCb)->dataRcvd   = FALSE;
       (*ueCb)->numTunnels = 0;
...
)
{
   ...
   if ((cellCb = wrDamGetCell(cellId)) == NULLP)
   {
      RLOG0(L_FATAL, "Cell Cb not available for Cell Id.");
      RETVALUE(RFAILED);
   }
   if (!(WR_DAM_VALID_RNTI(cellCb, rnti)))
   {
   ...
   if (tnlType == WR_TNL_NORMAL)
   {
       if(ueCb == NULLP)
       {
          if (wrDamAddUe(cellCb, rnti, &ueCb, tnlInfo->isFullCfg) != ROK) /* RRC-REL10-Upgrade */
          {
   /*MultiCell - end*/
             RETVALUE(RFAILED);
          }
   ...
   if(ROK == wrFillEgtpDatMsg(drbCb->dlFwdTnl, &eguEvtMsg, msgType))
   {
      if(msgType == EGT_GTPU_MSG_GPDU)
      {
         ...
         if(pktDiscard == FALSE)
         {
            ...
            SCpyMsgMsg(egtpBufInfo->data, 1, 0, &tempMBuf);
...
)
{
   ...
   for (idx = 0; idx < WR_DAM_MAX_CELLS; idx++)
   {
      wrDamCb.cells[idx].cellInUse = FALSE;
   }
   RETVALUE(ROK);
}
...
)
{
   ...
   wrDamCb.wrDamToRrmPst.dstProcId = 10;
...
)
{
   ...
   switch (cfg->hdr.elmId.elmnt)
   {
      case STWRGEN:
      {
          ...
          /* Allocate memory */
          WR_ALLOC_DATA_APP(&wrDamCb.appSap, (sizeof(WrLiSapCb *) * WR_NUM_DAM_SAPS));
          if(NULLP == wrDamCb.appSap)
          {
              RLOG0(L_FATAL, "Memory allocation failed.");
              retVal = RFAILED;
              break;
...
)
{
   ...
   switch(cfg->hdr.elmId.elmnt)
   {
      ...
      case STWRDAMSAP:
      {
         WR_ALLOC_DATA_APP(&wrDamCb.appSap[0], sizeof(WrLiSapCb));
         if(NULLP == wrDamCb.appSap[0])
         {
            RLOG0(L_FATAL, "Memory allocation failed.");
            status->status = LCM_PRIM_NOK;
            status->reason = LCM_REASON_INVALID_PAR_VAL;
      ...
      case WR_DAM_DRB_STATE_BUFFERING:
         {
            if (drbCb->state == WR_DAM_DRB_STATE_NORMAL)
            {
               RLOG_ARG2(L_INFO, DBG_UEID, ueCb->ueId, "Tunnel state changed from (%d) to (%d)",
 ...
 *    -#Success : ROK
 */
PUBLIC Void wrDamPrintThroughput (WrCellId cellId)
{
   WrDamUeCb                 *ueCb;
   WrDamUeIdx                *ueIdx;
   WrDamCellCb               *cellCb; 
   ...
   totalIcpuPkts = droppedIcpuPkts = 0;
      ...
      len+=sprintf(logBufTemp+len,"\nIcpuRate=[%ld] IcpuDropRate=[%ld] PktCount@EGTP:[%lu] PktsDroped@EGTP:[%lu]\n", 
		      ((SsRngInfoTbl[0].pktRate*12)/30),((SsRngInfoTbl[0].nWriteFail*12)/30), pktCntegtp,pktDropategtp);
      printf("\nIcpuRate=[%ld] IcpuDropRate=[%ld] PktCount@EGTP:[%lu] PktsDroped@EGTP:[%lu]\n", 
		      ((SsRngInfoTbl[0].pktRate*12)/30),((SsRngInfoTbl[0].nWriteFail*12)/30), pktCntegtp,pktDropategtp);
