...
)
{
   ...
   while(lnk != NULLP)
   {
      ...
      WR_ONLY_FREE(lnk->node, sizeof(WrGeranCellCb));
      lnk = lst->last;
   }
   ...
   while (node != NULLP)
   {
      ...
      while (node1 != NULLP)
      {
         ...
         while (node2 != NULLP)
         {
            ...
            WR_ONLY_FREE(node2->node, sizeof(WrCdma2kNeighCellCb));
         ...
         WR_ONLY_FREE(node1->node, sizeof(WrNrCdmaFreqCb));
         node1 = freqLstCp->last;
      } /* Ends for frequency list */
      cmLListDelFrm(bandLstCp, node);
      WR_ONLY_FREE(node->node, sizeof(WrNrCdmaBandCb));
      node = bandLstCp->last;
   } /* Ends for band list */
   RETVOID;
...
#ifdef WR_RSYS_OAM
         if( wrEmmSendAddEutraNghInd(newNr,WR_ACT_ADD,cellCb->cellId) != ROK)
         {
...
/** @brief This function will fetch ulArfcn
 *
 *  @details
 *
 *  Function:  wrEmmGetNbrUtraUlArfcn
 *
 *  Processing steps:
 ...
 *              earfcnDl: Neighbor Cell Dl arfcn
 *        [out] earfcnUl: DL arfcn for neighbor cell
 *
 *  @return Void
 */
PRIVATE Void wrEmmGetNbrUtraUlArfcn
(
  WrCellCb   *cellCb,
  U16        arfcnDl, 
  U16        *arfcnUl
)
{
   ...
   while(neighFreqLnk != NULLP)
   {
      ...
      if(neighFreqCb->t.utraFddFreq.arfcnDl == arfcnDl) 
      {
         /* Delete the Freq Details */
         *arfcnUl = neighFreqCb->t.utraFddFreq.arfcnUl;
         break;
      }
      neighFreqLnk = neighFreqLnk->next;
   }
   RETVOID;
}/*End of wrEmmGetNbrUtraUlArfcn */
...
)
{
   ...
   if(freqNode != NULLP)
   {
      if(freqNode->nrWlLst.count < WR_MAX_CELL_PER_FREQ)
      {
         ...
         wrEmmGetNbrUtraUlArfcn(cellCb, newNr->t.utraFdd.arfcnDl,
               &newNr->t.utraFdd.arfcnUl);

         cellCb->nrCb->nrUpdtInfo->freqTypeBitMap |=
            WR_ANR_UTRA_FREQ_BIT;
#ifdef WR_RSYS_OAM
        if((wrEmmSendUtraNghInd(newNr,WR_ACT_ADD,cellCb->cellId)) != ROK)
        {
...
)
{
   ...
   if(freqNode != NULLP)
   {
      ...
      /*cmLListAdd2Tail(&cellCb->nrCb->geranCb.geranNrCellLst, (CmLList *)newNr);*/
#ifdef WR_RSYS_OAM
      if( wrEmmSendAddGeranNghInd(newNr, WR_ACT_ADD,cellCb->cellId) != ROK)
      {
         RLOG0(L_ERROR,"Failed to send an indication to OAM");
         ret = RFAILED;
      }
...
#ifdef WR_RSYS_OAM 
   if(wrEmmSendAddEutraNghInd(nrNode,WR_ACT_DEL,cellCb->cellId) != ROK) 
   { 
 ...
 *           given nrType. and decrements the corresponding counters.
 *
 *      Function: wrEmmAnrRemEutranNodeFrmNrLst
 *
 *          Processing steps:
 *          - get the list in which the node is present
 *          - remove the node from the list
...
PRIVATE CmLList* wrEmmAnrRemEutranNodeFrmNrLst
(
   WrEutranNeighCellCb           *neighCellInfo
)
{
   ...
   TRC2(wrEmmAnrRemEutranNodeFrmNrLst);

   freqNode = neighCellInfo->freqNode;
   /* Verify whether the frequency node exists in the neighbour */
   if (freqNode == NULLP)
   {
       RLOG0(L_ERROR,"wrEmmAnrRemEutranNodeFrmNrLst: Failed to get FREQ NODE ");
       RETVALUE(NULLP);
   }
   ...
   RETVALUE(entryLnk);
} /* end of wrEmmAnrRemEutranNodeFrmNrLst */ 

/**
 *  @brief This function deletes the Eutran NR from the NRT list and 
 *         NCL/HO Black List
 *
 *      Function: wrEmmAnrDelFrmEutranNr
 *
 *          Processing steps:
 *          - initializes the structure for sending the nr update info to
 *            measurement module.
 ...
 *  
 */
PUBLIC CmLList* wrEmmAnrDelFrmEutranNr
(
   WrCellCb                      *cellCb, 
   WrEutranNeighCellCb           *nrNode
)
{
   TRC2(wrEmmAnrDelFrmEutranNr);
   RETVALUE(wrEmmAnrRemEutranNodeFrmNrLst( nrNode));
} /* end of wrEmmAnrDelFrmEutranNr*/

/**
 *  @brief This function updates the ANR freq type bit mapping
 *
 *      Function: wrEmmAnrUpdFrqTypBitMap
 *
 *          Processing steps:
 *          - initializes the structure for sending the nr update info to
 *            measurement module.
 ...
 *  @param [in] nrNode   : Pointer to the cell Cb's to be
 *                         deleted
 *  @param [in] isNull   : flag which tells whether the EUTRAN
 *                         ANR from NRT list has been removed or not.
 *  @return S16
    -# Success : ROK
    -# Failure : RFAILED
 *  
 */
PUBLIC Void wrEmmAnrUpdFrqTypBitMap
(
   WrCellCb                      *cellCb, 
   WrEutranNeighCellCb           *nrNode,
   Bool                          isNull
...
/**
 *  @brief This function updates WL/BL info.
 *
 *  @details The funciton updates the ANR inter frequency 
 *  whitelist and black list information.
 *
 *      Function: wrEmmAnrUpdInterFreqWlBlInfo
 *
 *
 *  @param  [in] cellCb        : pointer to cell control Block
 *  @param  [in] neighCellInfo : list entry to be deleted
...
PUBLIC CmLList* wrEmmAnrUpdInterFreqWlBlInfo
(
   WrCellCb                      *cellCb,
   WrEutranNeighCellCb           *neighCellInfo
)
{
   ...
   TRC2(wrEmmAnrUpdInterFreqWlBlInfo);
   ...
   RETVALUE(entryLnk);
} /* end of wrEmmAnrUpdInterFreqWlBlInfo */ 
...
#ifdef WR_RSYS_OAM 
   if((wrEmmSendAddEutraNghInd(modNr,WR_ACT_DEL,cellCb->cellId))!= ROK) 
   { 
      RLOG0(L_ERROR,"Failed to send notification to OAM"); 
      RETVALUE(RFAILED); 
   } 
