...
)
{
   ...
   /*Increment KPI for Successful RRC connection re-establishments*/
	...
	wrEmmUpdateStat(WR_EMM_RRC_REEST_STATS, transCb->ueCb->cellId, 
         WR_EMM_RRC_REESTAB_CON_SUCCESS, 1, 
         transCb->u.rrcReEstabTransCb.reestabCause);
   RETVALUE(ROK);
}
...
)
{
   ...
   if (idx == WR_UMM_RRC_REESTAB_UERST_TRANSID)
   {
      ...
      {
         RLOG0(L_DEBUG, " Re-est: Received Schd CFM for UE RESET ");
         /* Send TRUE to fill only for Re-est */
   ...
   else if (idx == WR_UMM_RRC_REESTAB_SCH_UEID_CHG_TRANSID)
   {
      if (schdCfm->status == RGR_CFG_CFM_OK)
      {
         ...
         {
            RLOG0(L_DEBUG, " Re-est: Received Schd CFM for UE ID Change ");
   ...
   else if (idx == WR_UMM_RRC_REESTAB_UERECFG_TRANSID)
   {
      if (schdCfm->status == RGR_CFG_CFM_OK)
      {
         RLOG0(L_DEBUG, " Re-est: Received Schd CFM : RRC_REESTAB_UERECFG ");
         reestabTrans->schdUeCfgState = WR_UMM_RRC_RESTB_CFG_SUCC;
      }
      else
      {
   ...
   else if(idx == WR_UMM_SCH_TRANS_SCELL_READY)
   {
      if (schdCfm->status == RGR_CFG_CFM_OK )
      {
         RLOG2(L_ALWAYS, "Re-est:SCELL: Activation Success for cell[%d] UeId[%d]", 
...
/* HO_REEST */
   ...
   */
   ...
   for (idx = 0; idx < WR_MAX_SCELL_PER_UE; idx++)
   {
      ...
      if ((sCellInfo != NULLP) && (sCellInfo->isScellAdded == TRUE ))
      {
         numScell++;
      }
   }
   if(numScell > 0)
   {
      WR_GET_MEM((&recfgMsg->memCp), ((sizeof(NhuSCellToAddMod_r10) * numScell)),
            &(sCellToAddModLst_r10->member));
      wrFillTknU16(&sCellToAddModLst_r10->noComp, numScell);
   }
   /* to re-use the variable resetting here */
   numScell = 0;
   /* Add all the Scells in Reconfig */
   for (idx = 0; idx < WR_MAX_SCELL_PER_UE; idx++)
   {
      sCellInfo = ueCb->sCellInfo[idx];
      if ((sCellInfo != NULLP) && (sCellInfo->isScellAdded == TRUE ))
      {
         RLOG2(L_ALWAYS, "Filling SCell = [%d] in Rest", 
               sCellInfo->sCellIdx, sCellInfo->sCellId);

         NhuSCellToAddMod_r10 *member = &sCellToAddModLst_r10->member[numScell];
         if (ROK != wrUtlFillAddModSCell(member, sCellInfo, ueCb, recfgMsg))
         {	
   ...
   if (numScell > 0)
   {
      transCb->u.rrcReEstabTransCb.sCellState = TRUE; 
   }
#endif
 ...
 )
{
    ...
    else
    {
      ...
      */
       RLOG0(L_WARNING, "UE Release because RRC Re-est while UE Attach Scenario ");
#ifdef UE_REL_DBG
       printf("\nUE Release because RRC Re-est while UE Attach Scenario\n");
#endif
...
)
{
   ...
   switch(transCb->msg->msgTyp)
   {
      ...
      case WR_UMM_PDCP_DATA_RESUME_CFM:
      {
         ...
         //chandan
#ifdef LTE_ADV
         /* Send Scell Activation to SCH after Re-established Procedure */
         if (reestabTrans->sCellState == TRUE)
         {
            RLOG0(L_DEBUG, "Sending SCell Activation After RRC Re-establishment")
            /* Send TRUE to fill only for Re-est */
