 ...
 )
{
   ...
   if(enbnghCb->srvdCellLst.count)
   {
      ...
      while(NULLP != nrLnk)
      {
         ...
         for ( cellIdx = 0; cellIdx < wrEmmCb.numCells; cellIdx++ )
         {
            wrEmmAnrUpdInterFreqWlBlInfo(wrEmmCb.cellCb[cellIdx], nghSrvdCellCb);
         } 
         if(NULLP != wrEmmAnrDelFrmEutranNr(wrEmmCb.cellCb[cellIdx], nghSrvdCellCb))
         {
            isNull = FALSE;
         }
         for ( cellIdx = 0; cellIdx < wrEmmCb.numCells; cellIdx++ )
         { 
            wrEmmAnrUpdFrqTypBitMap(wrEmmCb.cellCb[cellIdx], nghSrvdCellCb, isNull);
         }
 ...
 )
{
   ...
   enbNghCb->peerId        = peerId;
   enbNghCb->x2Connection = WR_NEIGH_ENB_STATE_UP;
   enbNghCb->isX2ReqSent   = FALSE; /* Since X2 Setup is now esteblished, resetting this flag. */
 ...
 )
{
   ...
   for (idx=0; idx < numSrvdCellModified; idx++)
   {
      ...
      WR_EMM_GET_CELLCB(crntCellCb,crntCellId);
      if (crntCellCb == NULLP )
      {
         RLOG0(L_ERROR," WR_EMM_GET_CELLCB failed");
         RETVALUE(RFAILED);
      }
   ...
   for (idx1 = 0; idx1 < noComp; idx1++)
   {
      ...
      if (mdfCell->member[idx1].servedCellInfo.pres.pres == TRUE)
      {
         ...
         for ( cellIdx = 0; cellIdx < wrEmmCb.numCells; cellIdx++ )
         {
            ...
            wrEmmAnrUpdtEutranModInfo(wrEmmCb.cellCb[cellIdx], neServCellCb);
         }
      }
   }
