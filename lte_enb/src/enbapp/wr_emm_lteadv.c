...
)
{
   ...
   eEUTRAN_CellId |= (U32)eUTRANcellIdentifier->val[0];
   eEUTRAN_CellId <<= 8;
   eEUTRAN_CellId |= (U32)eUTRANcellIdentifier->val[1];
   eEUTRAN_CellId <<= 8;
   eEUTRAN_CellId |= (U32)eUTRANcellIdentifier->val[2];
   eEUTRAN_CellId <<= 8;
   eEUTRAN_CellId |= (U32)eUTRANcellIdentifier->val[3];
...
)
{
   ...
   WR_EMM_GET_CELLCB(emmCellCb, cellCb->cellId);
   if((emmCellCb == NULLP))
   {
       RLOG0(L_ERROR, " CELLCB not found ");
       RETVALUE(RFAILED);   
...
)
{
   ...
   WR_EMM_GET_CELLCB(cellCb, evnt->u.cellEvent.cellId);
   if(cellCb == NULLP)            
   {    
...
)
{
   ...
   WR_EMM_GET_CELLCB(cellCb, schLoadInfo->cellId);
   if(cellCb == NULLP)           
   {    
...
)
{
   ...
   WR_EMM_GET_CELLCB(cellCb, evnt->u.absUeEvent.cellId);
   if(NULLP ==cellCb)
   {
      RETVALUE(ROK);
   }
...
)
{
   ...
   switch(evnt->eventId)
   {
      case WR_EMM_ABS_DISABLED_EVENT:
      {
         wrEmmCb.lteAdvCb.absInfo.state = WR_EMM_ABS_DISABLED_STATE; 
         /*ABS_TESTING start */
         WR_EMM_GET_CELLCB(cellCb, evnt->u.absCfgEvent.cellId);
