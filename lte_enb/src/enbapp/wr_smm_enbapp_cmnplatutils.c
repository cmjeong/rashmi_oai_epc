...
#endif
{
   ...
   for(idx = 0; idx<lteeNodeBparams.cdmaBandClassParams.numBandClass; idx++)
   {
            ...
            bandClass1XRtt.bandClass == msmNghCdma1xCell->bandClass)
      {
         ...
         lwrNghCdma1xCell->baseId        = (U16)baseId;
...
/*ccpu00138869 start*/
   ...
   RLOG2(L_DEBUG,"smFillNghEnbParam:enbType = %lu,enbId = %lu",
...
#endif
{
   ...
   /*Do not do right shift by 8 for MACRO as it is done in EMM module*/
   RLOG3(L_DEBUG,"smFillNghEutraCellParam: enbType = %lu,cid = %lu, enbid = %lu",
...
#endif
{
   ...
   {
      RLOG0(L_ERROR, "cmHashListInit failed");
      RETVALUE(RFAILED);
   }
   ...
   for(idx = 0; idx < lteeNodeBparams.lteNeighFreqCfg.numEutraFreq; idx++)
   {
      ...
      RLOG1(L_ALWAYS,"earfcnDL is %d",nghEutraFreqList->nghEutraFreqCfg.earfcnDl);
   } 
...
/*         RETVALUE(RFAILED);*/
   ...
   for(idx = 0; idx < lteeNodeBparams.lteNeighCellCfg.numEutraCells; idx++)
   {
      ...
      RLOG2(L_INFO,"Neighbor %d added with DL frequency %d", idx+1,msmNghEutraCell->earfcnDl);
      RLOG3(L_INFO,"PLMN: MCC[0], MCC[1],MCC[2] %d %d %d",msmNghEutraCell->plmnId[0].mcc[0],
            msmNghEutraCell->plmnId[0].mcc[1],
            msmNghEutraCell->plmnId[0].mcc[2]);
      RLOG2(L_INFO,"PLMN: MNC[0], MNC[1] %d %d",msmNghEutraCell->plmnId[0].mnc[0],
            msmNghEutraCell->plmnId[0].mnc[1]);
               ...
               sizeof(msmNghEutraCell->earfcnDl), 0,(PTR*) &nghEutraFreqList))
      {
         RLOG1(L_ERROR, "can't add cell because corresponding frequeancy is not"
            " configured arfcn = %d", msmNghEutraCell->earfcnDl);
         continue;
      }
   ...
   for(idx = 0; idx < lteeNodeBparams.lteNeighCellCfg.numEutraCells; idx++)
   {
      ...
      {
         RLOG1(L_ERROR, "cmHashListInsert failed Two eutra cells configured same"
            " plmnId and cellId CID = %ld", 
            (U32)lteeNodeBparams.lteNeighCellCfg.t.neighAddCfg[idx].eutraCell.cid);
         WR_FREE(nghEutraCellList, sizeof(SmNeigEutraCellList));
         continue;
        /* RETVALUE(RFAILED);*/
   ...
   for(idx = 0; idx < lteeNodeBparams.lteNeighCellCfg.numUtraCells; idx++)
   {
      ...
      {
         RLOG1(L_ERROR, "can't add cell because corresponding frequeancy is not"
            " configured earfcn = %d", msmNghUtraCell->arfcnDl);
         continue;
      }
               ...
               (PTR)nghUtraCellList, (U8 *)&nghUtraCellList->hashKey, sizeof(smNghCellHasKey)) != ROK)
      {
         RLOG1(L_ERROR, "cmHashListInsert failed Two Utra cells configured same"
            "plmnId and cellId CID = %d", msmNghUtraCell->cId);
         WR_FREE(nghUtraCellList, sizeof(SmNeigUtraCellList));
/*         RETVALUE(RFAILED);*/
      }
   }
   ...
   for(idx = 0; idx < lteeNodeBparams.lteNeighCellCfg.numUtraTddCells; idx++)
   {
      ...
      {
         RLOG1(L_ERROR, "cmHashListInsert failed Two Utra Tdd cells configured same"
               "plmnId and cellId CID = %d", msmNghUtraCell->cId);
         WR_FREE(nghUtraCellList, sizeof(SmNeigUtraCellList));
         /*         RETVALUE(RFAILED);*/
      }
   ...
   for(idx = 0; idx < lteeNodeBparams.cdmaBandClassParams.numBandClass; idx++)
   {
      ...
      {
         RLOG1(L_ERROR, " cmHashListInsert failed bandClass = %d ", 
   ...
   for(idx = 0; idx < lteeNodeBparams.lteNeighCellCfg.numCdma1xCells; idx++)
   {
      ...
      {
         RLOG1(L_ERROR, " cmHashListInsert failed nrIndex = %d ", 
   ...
   for(idx = 0; idx < lteeNodeBparams.lteNeighFreqCfg.numGeranFreq; idx++)
   {
      ...
      {
         RLOG1(L_ERROR, "cmHashListInsert failed bcchArfcn %d",
   ...
   for(idx = 0; idx < lteeNodeBparams.lteNeighCellCfg.numGeranCells; idx++)
   {
      ...
      {
         RLOG1(L_ERROR, "can't add cell because corresponding frequeancy is not"
            "configured bcchArfcn %d", msmNghGeranCell->bcchArfcn);
         continue;
      }
      ...
      {
         RLOG1(L_ERROR, "cmHashListInsert failed Two geran cells configured same"
            " plmnId and cellId CI = %ld", 
            (U32)lteeNodeBparams.lteNeighCellCfg.t.neighAddCfg[idx].geranCell.ci);
         WR_FREE(nghGeranCellList, sizeof(SmNeigGeranCellList));
         continue;
        /* RETVALUE(RFAILED);*/
...
{ 
   ...
   for(idx = 0; idx < smCfgCb.noOfCfg; idx++)
   {
      ...
      if(ROK == ret)
      {
         if(lteeNodeBparams.lteeNodeBIPparams.lteSiglinkserverlist[idx1].ipv4pres == TRUE)
         {   
...
#endif
{
   ...
   if(WR_ENB_OPEN_ACCESS != accessMode && WR_ENODEB_TYPE_MACRO == wrSmDfltEnbType)
   {
      ...
      RLOG3(L_ERROR, "servingCellPci [%d] and it should be in the range from [%d] to [%d]",
              servingCellPci,
              lteeNodeBparams.lteAddCellCfg.startPci,
...
PUBLIC S16 MsmEnodeBinitialCfgComplete(Void)
{
   ...
   RLOG0(L_ALWAYS," Calling MsmEnodeBinitialCfgComplete \n");
...
PUBLIC S16 smStaticCfgReq(Void)
{
   ...
   else if(lteeNodeBparams.lteEnodebIpAddr.ipv4pres == TRUE)
  {
   ...
   {
      RLOG0(L_ERROR, "smInitNghHashList Failed");
      RETVALUE(RFAILED);
   }

   if(ROK != smInitMmeCfg())
   {
      RLOG0(L_ERROR, "unable to smInitMmeCfg");
      RETVALUE(RFAILED);
   }
...
/* ccpu00135220 */
   ...
   if(ROK != smInitDbgCfg())
   {
      RLOG0(L_ERROR, "unable to smInitDbgCfg");
      RETVALUE(RFAILED);
   }
...
)
{
   ...
   RLOG2(L_DEBUG, "smBuildWrEnbCfg:enbCfg->protoCfg.enbType = %lu,enbCfg->protoCfg.enbId = %lu",
...
)
{
   ...
   while((retVal = cmHashListGetNext(&(smCfgCb.neighEutraFreqLstCp), prevEntry, (PTR *)&eutrFreqList)) == ROK)
   {
     ...
     RLOG1(L_ALWAYS,"earfcnDl is %d", eutraFeqCfg->earfcnDl);
...
#endif /* end of ANSI*/
{
   ...
   {
      RLOG1(L_ERROR, "enb Not Configurured at SM enbId = %ld", enbId);
      RETVALUE(RFAILED);
   }
...
#endif /* end of ANSI*/
{
   ...
   for(U8 idx = 0; idx < MAX_NUM_QCI; idx++)
   {
    ...
    {
       RLOG0(L_ERROR,"DLFSS is not supported in INTEL FDD,So Disabling it for cellCfg");
   ...
   rgrCfg->dfltIsDlFreqSel = lteeNodeBparams.lteCellDlSchdCfgGrp.dlfsSchd;
    ...
    {
       RLOG0(L_ERROR,"DLFSS is not supported in INTEL FDD,So Disabling it for rgCfg");
   ...
   RLOG1(L_INFO, "smBuildCellCfg:eCSFB value from oam = %d",lteeNodeBparams.csfbCfgGrp.ecsfbSupport);
/*RRC Rel10 Upgrade - start*/
   if (TRUE == lteeNodeBparams.csfbCfgGrp.dualRxTxSup)
   {
   		if(LWR_ECSFB_NOT_SUPPORTED == csfbGrp->eCsfbMeasSup) 
   		{
			...
			"DualRxTx Cfg(%d)",csfbGrp->eCsfbMeasSup,lteeNodeBparams.csfbCfgGrp.dualRxTxSup);
			
			RETVALUE(RFAILED);
		}
   }
   RLOG1(L_INFO, "smBuildCellCfg:dualRxTx value from oam = %d",lteeNodeBparams.csfbCfgGrp.dualRxTxSup);
...
#endif /* ANSI */
{
   ...
   {
      RLOG1(L_ERROR, "enb Not Configurured at SM enbId = %ld",
...
#endif /* ANSI */
{
  ...
  {
     RLOG1(L_ERROR, "Add cell is not supported for neghbor cell with cfgType=%d",
...
#endif /* ANSI */
{
   ...
   { 
      ...
      RLOG2(L_DEBUG,"SmHdlNghEurtaAddCellCfg:enbType = %lu,enbId = %lu",
...
#endif /* ANSI */
{
   ...
   {
      RLOG1(L_ERROR, "cmHashListInsert failed Two Utra cells configured same "
         "plmnId and cellId CID = %d", utranCell->cId);
      WR_FREE(nghUtraCellList, sizeof(SmNeigUtraCellList));
      RETVALUE(RFAILED);
   }
