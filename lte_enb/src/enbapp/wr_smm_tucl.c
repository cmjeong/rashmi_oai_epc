/********************************************************************20**
     ...
     Sid:      fc_smm_tucl_t2k.c@@/main/TeNB_Main_BR/5 - Mon Aug 11 16:52:35 2014
...
#endif
{
   ...
   if (SRegTTsk((Ent)ENTHI, (Inst)0, (Ttype)TTNORM, (Prior)PRIOR1,
...
#endif /* ANSI */
{
   ...
   cfg->lmPst.region = HI_MEM_REGION;
 ...
 /* Fill Header */
   ...
   hiMgt.hdr.response.mem.region = HI_MEM_REGION;
...
#endif /* ANSI */
{
   ...
   cfg->uiMemId.region =  HI_USAP_MEM_REGION;
...
#endif
         WR_FILL_PST(hiPst, SM_SM_PROC, SM_HI_PROC, ENTSM, ENTWR,0,0, EVTWRSCTPCFG, WR_POOL, SM_SELECTOR_LC);
...
#endif
{
   ...
   if (cfm->cfm.status == LCM_PRIM_OK)
   {
      ...
      {
/* Raghu DIP changes fo common code */
         smCb.smState = WR_SM_STATE_TUCL_SHUTDWN_DONE;
         RLOG0(L_ALWAYS, "TUCL SHUTDOWN SUCCESSFULLY PERFORMED.");
         WR_FILL_PST(hiPst, SM_SM_PROC, SM_SZ_PROC, ENTSM, ENTWR,0,0, EVTWRTUCLSHUT, WR_POOL, SM_SELECTOR_LC);
   ...
   {
      RLOG0(L_INFO,"Control Request is in progress");
   }
   else
   {
...
/********************************************************************30**
           ...
           End of file:     fc_smm_tucl_t2k.c@@/main/TeNB_Main_BR/5 - Mon Aug 11 16:52:35 2014
