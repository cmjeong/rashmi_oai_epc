...
//suhas_sprint12
#ifdef LTE_ADV
#define MSM_MEAS_MAX_EVENT_A4_CFG           2
#else
#define MSM_MEAS_MAX_EVENT_A4_CFG           1
#endif
//suhas_sprint12
...
#define MSM_MAX_CELLS                     12           
#define MSM_ABS_PATTERN_LEN               40
...
#define MSM_LTEU_MAX_EARFCN   10
#endif /*LTE_UNLICENSED*/
...
/*automation*/
/** 
 * @brief enum to indicate dynamic neighbour changes status
 */
typedef enum{
   MSM_WR_CFG_FOUND,
   MSM_WR_CFG_DONE,
...
/* MsmLteMeasUtraEvntB1RptCfg    measUtraEventB1RptCfg[LWR_MEAS_MAX_UTRA_PERD_CFG];*/
   ...
   MsmLteMeasEvntRptCfg          measA4EvtRptCfg[MSM_MEAS_MAX_EVENT_A4_CFG]; /*!< Event A4 configuration *///suhas_sprint12
#endif
...
/* U8           modType;    */   /* Modulation Type */
   ...
   S16          maxUeSupp;     /* maximum number of UEs supported by each eNodeB */
   /* RRM_SP1_START */
   U8           maxAvgGbrPrbUsage;   /* maximum Average GBR PRB Usage in % */ 
   U16           rrmQci1TmngReq;      /* QCI-1 Timing Requirements in ms */ 
...
/* TknU8        mcc[3];
   ...
   /*Removed U8             freqBandInd;   Frequency Band Indicator */
...
/*sprint8: newly added*/
   ...
   Bool          isIpv6Pres; 
   U32           peerId;      /* Peer ID */
   Bool        isNewNbrEnb; /* To identify the new Ngh eNB at the eNodeB level
                               i.e if in multiCell all the cells have same 
                               ngh eNB, at eNodeB level it should be 1 ngh eNB only. */
...
{
   MsmActType        actType;
   U16                cqi_PUCCH_ResourceIdx;/*!< Position of PUCCH*/
...
{
   MsmActType        actType;
   /*Removed         enbIpAddr;*/    /*!< eNB IP Address */
...
/* U32               bhLinkCapacity; */ /*!< Total backhaul link of Mhe eNB */
   ...
   U8                numOfCells; /*number of Cells supported*/
} MsmLteEnbProtoCfg;
...
/** @brief This structure holds the Tunnel info params **/
typedef struct msmLteTunnelInfoParams
{
   MsmActType        actType;
   Bool              enable;                   /*!< Enables or disable LTE Tunnel */
   U16               numOfTunnelEntries;       /*!< number of TunnelEntries */
   U8                tunnelRef[MAX_NUM_CHARS]; /*!< reference to an IPsec tunnel */
   MsmLtePlmnId      tunnelPlmnId;             /*!< PLMM ID associated with the tunnel */
...
typedef struct msmLteX2ConfigParams
{
   ...
   /*sprint8 this is moved to ProcTmrCfg */
   //U32               x2TimeToWaitTimerVal;   
   /**< Value for the time to wait IE.
...
typedef struct msmAbsSfrCfg
{
   U8    neighCIO;          
   U32   absPatternType;      
   U8    absPattern[MSM_ABS_PATTERN_LEN];             
...
typedef struct msmLteUCfg
{
   ...
   U32     earfcn[MSM_LTEU_MAX_EARFCN];   /*!< List of unlicensed EARFCN's */
...
{
   U8      gtpuEchoIntvl;  /*<! TIme interval between GTPU echo request */
}MsmS1UCfg;
#endif /*TIP_OAM*/
...
{
   U32 cellId;                   /*!<Cell Id for MultiCell */
...
/*sprint8*/
/*S16 msmSetX2ConfigParameters(const MsmLteX2ConfigParams
 * *lteX2ConfigParams);*/
...
#endif
#ifdef LTE_ADV
S16 msmSetCAParameters(MsmCAGenCfg *caGenCfg,U16 cellIdx);
#endif
