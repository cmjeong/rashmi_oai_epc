...
/* MultiCell - */
/** @brief This function fills the PLMN present in the TA. 
 *
 * @details This function fills the Supported TAs PLMN List.
 *          Access all the PLMNs present for this TA and fill it.
 *
 *     Function: wrEmmFillSuppTAsPlmn
 *
 * @param[in]  WrEmmS1SetpSuppTAs: suppTAs
 * @param[in]  WrCellCb: cellCb.
 * @return 
 */
PRIVATE Void wrEmmFillSuppTAsPlmn
(
 ...
 )
{
   ...
   for(sibIndx = 0; sibIndx < cellCb->sib1.numPlmnIds; sibIndx++)
   {
      suppTAs->taInfo[suppTAs->numTAs].plmns[sibIndx] 
         = cellCb->sib1.plmns[sibIndx].plmnId;
   }
   suppTAs->taInfo[suppTAs->numTAs].numPlmnIds = sibIndx;
}

/** @brief This function fills the tracking area code. 
 *         Access each Cells supported by the eNodeB and
 *         populate the TA supported by each cell if its different 
 *         else if all cells support sam TA, then fill the same TA.
 *
 *     Function: wrEmmGetSuppTAs
 *
 * @param[in]  WrEmmS1SetpSuppTAs: suppTAs
 * @return 
 */
PRIVATE S16 wrEmmGetSuppTAs
(
 WrEmmS1SetpSuppTAs     *suppTAs
 )
{
   ...
   /* Multi Cells. */
   for(cellIndx = 0; cellIndx < wrEmmCb.numCells; cellIndx++)
   {
      cellCb = wrEmmCb.cellCb[cellIndx];
#if 0
      for(suppTaIdx = 0; suppTaIdx < suppTAs->numTAs; suppTaIdx++)
      {
         if(cellCb->sib1.tac == suppTAs->taInfo[suppTaIdx].tac)
         {
            break;
         }
      } 
   
      if (suppTAs->numTAs == suppTaIdx)
      {   
#endif
      if (cellCb != NULLP)
      {   
         wrEmmFillSuppTAsPlmn(suppTAs, cellCb);
         suppTAs->taInfo[suppTAs->numTAs++].tac = cellCb->sib1.tac;
      }
//      }   
   }
   RETVALUE(ROK);
...
)
{
   ...
   *pdu   = NULLP;
   ...
   for(cellIdx = 0; cellIdx < wrEmmCb.numCells; cellIdx++)
   {
      ...
      if((cellCb != NULLP) && (cellCb->sib1.csgInd))
      {
         csgPrsnt++;
      }
   }
   ...
   if(csgPrsnt != 0)
   {
      ...
      for(cellIdx = 0; cellIdx < wrEmmCb.numCells; cellIdx++)
      {
         ...
         if((cellCb != NULLP) && (cellCb->sib1.csgInd))
         {
            ...
            csgItem = &(ie->value.u.sztCSG_IdLst.member[csgCnt++]);
            wrFillTknU8(&(csgItem->pres), cellCb->sib1.csgId.pres);
