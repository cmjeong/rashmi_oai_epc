...
)
{
         ...
         || (0 != cellInfo->numGrpIdDeleted))
   {
      for(cellIdx = 0; cellIdx < wrEmmCb.numCells; cellIdx++)
      {
         ...
         */
         if((cellCb != NULLP) && (TRUE == cellCb->sib1Modified))
         {
...
)
{
 ...
 //CHINNA testing purpose
#if 1 
#ifdef TIP_L3
   wrEmmPbmActivatePlmnInSib1(mmeCb);
#endif
...
)
{
   ...
   RrmWrEnbCfgReqInfo             *rrmWrEnbCfgReq = NULLP;
#endif /*RM_INTF*/
/* CHINNA: RIM procedure not handling for TIP Currently */
#if 0
   S16                        retVal = RFAILED;
#endif
  ...
  wrAlarmEvt(WR_ALARM_ID_S1_SETUP_FAIL, WR_ALARM_SEVERITY_CLEARED, NULLP, NULLP);
  wrAlarmEvt(WR_ALARM_ID_SCTP_LNK_FAIL, WR_ALARM_SEVERITY_CLEARED, NULLP, NULLP);
