...
#ifdef TIP_L3
   RETVALUE(tipSendCtfCfgReq(pst, spId, transId, cfgReqInfo));
#else
   ...
   { 
     ...
     else if(CTF_CONFIG == transId.trans[1])
     {
        ...
        else
        {
           ...
           if (wrEmmCellSchdCfg(lTransId) != ROK)
           {
              RLOG1(L_ERROR, "Scheduler Cell configuration failed for cellid  %d", lTransId);
              RETVALUE(RFAILED);
           }
#else
#ifdef ENABLE_CNM
...
/*#endif*/
           ...
           (transId.trans[8] == WR_MS_TX_PWR_UP_CFM) )
     {
      ...
      else
      {
         ...
         WR_EMM_GET_CELLCB(cellCb, lTransId);
         if(cellCb == NULLP)
         {
