 ...
 */
PUBLIC Void wrEmmCpyMultiLSapId
(
   U8       noOfCells
)
{
   ...
   for(idx=0; idx < noOfCells; idx++)
   {
      wrCb.sapIds[idx].kwuSapId = WR_KWU_UL_SUID;
      wrCb.sapIds[idx].ckwSapId = WR_KWU_UL_SUID;
      wrCb.sapIds[idx].crgSapId = idx;
      wrCb.sapIds[idx].cpjSapId = idx;
      wrCb.sapIds[idx].pjuSapId = (idx * 2); 

      wrCb.sapIds[idx].rguUlSapId = WR_SM_PJUL_INST;
      wrCb.sapIds[idx].rguDlSapId = WR_SM_PJDL_INST;
       
      wrCb.sapIds[idx].rguSapId   = idx;
...
)
{
   ...
   wrFillLSapIds(&nhuCellCfgReqSdus->sdu.u.cfgInfo.llyrSapIdInfo, cellCb->cellId);
#endif
...
)
{
   ...
   /* SpId info for MAC and SCH interface */
   cellCfgReq->u.cfg.u.cellCfg.macSapId = cellId - 1;
   cellCfgReq->u.cfg.u.cellCfg.schSapId = wrEmmCb.numCells + cellId - 1;
   ...
   /* Antenna config */
   cellCfgReq->u.cfg.u.cellCfg.antennaCfg.antPortsCnt =  cellCb->numTxAntenna;
...
)
{
   ...
   /* Antenna config */
   cellReCfg->antennaCfg.antPortsCnt =  cellCb->numTxAntenna;
...
)
{
   ...
   WR_GET_PAGING_CYCLE(lteeNodeBparams[WR_SM_DFLT_CELL_INDX].ltePCCHCfgCommon.defaultPagingCycle, dfltDrxCycle);
...
)
{
   ...
   wrEmmCellSchdPreambleCfg(cellCb->sib2.rachCfg.noRaPreambles, 
                                                    &cfg->numRaPreamble);
