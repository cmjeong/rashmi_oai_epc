...
#define WR_ONLY_FREE(_buf, _size)          \
{\
...
typedef enum wrUmmMeasRptCfgType
{
   ...
   WR_UMM_EVENT_A4_A5_INTER_CFG, /* Inter-Freq ANR: This Algo has A5 meas for inter-freq and A3 meas for Intra-freq */
...
#ifdef LTE_ADV_UECAP
#define MAX_BAND_COMB          2
#define WR_MAX_BW_COMB_SET     6
typedef struct wrBwCombSet
{ 
   U8              noOfBwCombSet;                       /* no of bandwidth comb set*/
   U8              bwCombSet[WR_MAX_BW_COMB_SET];       /* storing bwCombSet value in array*/
}WrBwCombSet;
typedef struct wrBandCfg
{
   U8                              band;                /* cell supported band */
   EnumNhuCA_BandwidthClass_r10    bwClass;             /* cell supported class*/
}WrBandCfg;

typedef struct wrBandComb
{
   U8          numBandComb;                             /* number of band in combination*/
   WrBandCfg   band[MAX_BAND_COMB];                     /* array to store bands*/
}WrBandComb;

typedef struct wrCellParams
{
   U16             cellId[WR_MAX_CELLS];                /* cell Id */
   U8              cellSuppBw[WR_MAX_CELLS];            /* cell supported bandwidth*/
}WrCellParams;

typedef struct wrCaBandCfg
{
   Bool            isEnbCaCap;                      /* validating flag for eNB is cacap or not*/
   WrCellParams    wrCellParams;
   WrBandComb      wrBandComb;
   U8              numOfAssocCells;                     /* no of cell that can be config by eNB*/
   WrBwCombSet     wrBwCombSet;
}WrCaBandCfg;
#endif /*LTE_ADV_UECAP*/
...
EXTERN S16 smUpdateNlPciVal(NlUpdatePciCfgInd       *pciCfgInd);//july4_suhas
