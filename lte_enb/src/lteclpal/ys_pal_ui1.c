...
/*   tfuSap = &(ysCb.tfuSap); */
   ...
   if ((ROK != ret) || (NULLP == tfuSap) || (spId != tfuSap->spId))
   {
#if (ERRCLASS & ERRCLS_INT_PAR)      
...
#endif
{
 ...
 /*  schTfuSap = &(ysCb.schTfuSap); */
   ...
   if ((ROK != ret) || (NULLP == schTfuSap) || (spId != schTfuSap->spId))
   {
#if (ERRCLASS & ERRCLS_INT_PAR)      
...
#endif
{
   ...
   {
      for(indx = 0; indx < YS_PAL_MAX_CELLS; indx++)
      {
         sap = ysCb.tfuSapLst[indx];
         if((NULLP != sap) && (sap->spId == spId))
         {
            break;
         }
         sap = NULLP;
