...
/********************************************************************20**
     ...
     Sid:      pt_ss.c@@/main/tenb_5.0_SIB_BINWIN/1 - Tue Nov 17 15:17:54 2015
...
PRIVATE Void *pthreadCreateHdlr ARGS((void *)); 
...
/* mt005.301 : Cavium changes */
   ...
   act.sa_flags = 0;
#ifdef SS_SEGV_SIG_HDLR
   if (sigaction(SIGSEGV, &act, NULLP) != 0)
   {
      RETVALUE(RFAILED);
   }
#endif
...
/*
*
*       Fun:   pthreadCreateHdlr
*
*       Desc:  This function calls the start_routine
*
*       Ret:   ROK      - ok
*
*       Notes:
...
*       Fun:   SCreatePThread
*
*       Desc:  This function creates thread
*
*       Ret:   ROK      - ok
*
*       Notes:
...
#endif
{
   ...
   int                   procIdNo;
#endif
/*calling all Interrupt hadlling functions registered with SSI*/
   SS_ACQUIRE_SEMA(&osCp.tTskTblSem, ret);
   if (ret != ROK)
   {
...
#ifndef SS_MULTIPLE_PROCS
   for (entNo = 0; entNo < SS_MAX_ENT; entNo++)
   {
      for (instNo = 0;  instNo < SS_MAX_INST;  instNo++)
      {
         idx = osCp.tTskIds[entNo][instNo];
#else /* SS_MULTIPLE_PROCS */
   for (procIdNo = 0;  procIdNo < SS_MAX_PROCS;  procIdNo++)
   {
      for (entNo = 0;  entNo < SS_MAX_ENT;  entNo++)
      {
         for (instNo = 0;  instNo < SS_MAX_INST;  instNo++)
         {
             idx = osCp.tTskIds[procIdNo][entNo][instNo];
#endif /* SS_MULTIPLE_PROCS */
             /*If task is registered and Interrupt handler registerd */
             if(idx != SS_TSKNC)
             {
                tTsk = &osCp.tTskTbl[idx];
                if(tTsk->isIntrptReg ==TRUE )
                {
                   tTsk->intrptActvFn();
                }
             }
...
/*ccpu00129785*/
#ifdef ANSI
PUBLIC S16 SChekMemUtilization
(
Region region,
Bool *memAlarm
)
#else
PUBLIC S16 SChekMemUtilization(region, memAlarm)
Region region;
Bool *memAlarm;
#endif
{
   ...
   for (idx = 0; idx < mtCMMRegCb[region]->numBkts; idx++)
   {
      memBkt = mtCMMRegCb[region]->bktTbl[idx];
      bkt[idx]=((memBkt.numBlks - memBkt.numAlloc)*10)/memBkt.numBlks;
      if (bkt[idx] <= MT_MAX_MEM_THRESHOLD)
      {
         *memAlarm = TRUE;
         RETVALUE(ROK);
      }
...
/********************************************************************30**
         ...
         End of file:     pt_ss.c@@/main/tenb_5.0_SIB_BINWIN/1 - Tue Nov 17 15:17:54 2015
...
/main/1+    mt015.301 akapoor   1. Enclosed all timer activation functions in
                                ...
                                2. Initialized the timer activation functions with
                                   NULLP 
/main/1+    mt016_LPR sasahni   1. SIGSEGV signal catch handler introduced.
                                2. Function to call all registered callbacks for
                                   SIGSEGV signal.
**********************************************************************91*/
