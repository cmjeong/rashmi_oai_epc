...
/********************************************************************20**
     ...
     Sid:      sm_gen.c@@/main/tenb_5.0_SIB_BINWIN/1 - Tue Nov 17 15:17:52 2015
...
/* ss001.301: additions */
#ifdef SS_LICENSE_CHECK 
#include "cm_key.h"           /* For license validation */
#endif /* SS_LICENSE_CHECK */
...
#ifdef SS_VW
#ifdef ANSI
PUBLIC Void lActvTmr
(
U32 res
)
#else
PUBLIC Void lActvTmr(res)
U32 res;
#endif
#else
#ifdef ANSI
...
#endif /* SS_VW */
{
   ...
   for(;;)
   {
        /* check license */
                  ...
                  {
						 /* ss005.301 : Changes */
                     SPrint("License expired or invalid licenses found.\n");
                          exit(-1);
                  }
#ifdef WIN32
        Sleep(*ttl);
...
#endif
{
   ...
   if ((pthread_create(&tId, &attr, lActvTmr, (void*)&res)) != 0)   {
   ...
   HANDLE tHd;
   res = reso;
   tHd = CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)lActvTmr, (VOID*)&res, 0, (LPDWORD) &tId);
#else
#ifdef SS_VW
   res = reso;
   tId = taskSpawn("VWSS_LCTSK", 160, VX_STDIO, 5000, (FUNCPTR)lActvTmr,
...
/* ss002.301 : Modications */
   ...
   /*ss012.301 : Fix log related issue to suite MT and NS implementations*/
#ifdef SS_LICENSE_CHECK
        /* Check if valid license is available or not */
        if (_get_keys(0))
        {
            /*ss012.301 : Fix log related issue to suite MT and NS implementations*/
            sprintf(prntBufLoc,"No valid licenses or license expired.\n");
...
/********************************************************************30**
         ...
         End of file:     sm_gen.c@@/main/tenb_5.0_SIB_BINWIN/1 - Tue Nov 17 15:17:52 2015
...
/main/1+     ss016.LPR sasahni   1. Added Initialization of callback for SIGSEGV signal
*********************************************************************91*/
