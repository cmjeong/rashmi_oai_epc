...
/********************************************************************20**
     ...
     Sid:      ss_rtr.c@@/main/2 - Mon Apr  5 18:01:19 2010
...
#endif
{
   ...
   for (i = 0;  i < count;  i++)
   {
      ...
      if (routes[i] == RTENC)
      {
         SSLOGERROR(ERRCLS_INT_PAR, ESS471, ERRZERO, "Invalid route ID");
         RETVALUE(RFAILED);
      }
/* ss029.103: modification: following check removed for multiple proc support */
      ...
      {
         SSLOGERROR(ERRCLS_INT_PAR, ESS472, ERRZERO,
   ...
   if (rtrTsk == NULLP)
   {
      SSLOGERROR(ERRCLS_INT_PAR, ESS473, ERRZERO, "Null pointer");
      RETVALUE(RFAILED);
   }
#endif
...
#endif
{
   ...
   for (i = 0;  i < count;  i++)
   {
      ...
      if (routes[i] == RTENC)
      {
         SSLOGERROR(ERRCLS_INT_PAR, ESS475, ERRZERO, "Invalid route ID");
         RETVALUE(RFAILED);
      }
      ...
      if (osCp.rtrTskTbl[routes[i]] == NULLP)
      {
         SSLOGERROR(ERRCLS_INT_PAR, ESS476, ERRZERO,
...
/********************************************************************30**
         ...
         End of file:     ss_rtr.c@@/main/2 - Mon Apr  5 18:01:19 2010
...
/main/2      ---      rp   1. SSI enhancements for Multi-core architecture
                              support
*********************************************************************91*/
