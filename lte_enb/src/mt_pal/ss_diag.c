...
/********************************************************************20**
     ...
     Sid:      ss_diag.c@@/main/T2K3K_Merge_BR/1 - Wed Oct  9 11:32:55 2013
...
#endif
{
   ...
   if (trgBufPtr->isValid == FALSE)
   {
      ...
      {
         strcpy(trgBufPtr->varStr, string);
         trgBufPtr->isValid = TRUE;
      }

   }
   /* Insert information into fixed buffer so, writer thread can pick it up */
   ssDiagFix(tknId, 0, entId, instId, logLvl, logType, splArg, varLen, (SsDiagVarBuf *)trgBufPtr, 0, 0, NULL);
