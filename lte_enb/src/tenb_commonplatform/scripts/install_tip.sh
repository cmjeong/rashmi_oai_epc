...
extractNumberOfCell() {
firstLine="$1"
numCells=$(echo $firstLine | awk '{print $3}')
}
line=$(head -n 1 ../bin/wr_cfg.txt); 
extractNumberOfCell "$line"

#update config file
line1=$(head -n 1 ../config/configFile)
if [[ "$line1" =~ "OAM_NUM_CELL_ENTRIES" ]]; then
ex -c ':1d' -c ':wq' ../config/configFile
#tail -n +2 ../config/configFile
cp configFile refConfigFile
file="refConfigFile"
cat - $file <<<"OAM_NUM_CELL_ENTRIES "$numCells > configFile
rm -rf refConfigFile
fi

#update mib-factory.nv
file="mib-factory.nv"
MO="[FAP_LTE/0]"
PARAM="LTE_X_TIP_NUM_OF_CELLS/0 = "$numCells
echo "$MO 
$PARAM" >> mib-factory.nv

dos2unix mib-factory.nv
dos2unix configFile
cd ../scripts
...
cp -f ./tipAddObjFile       $CONFIG_PATH/tmp/
