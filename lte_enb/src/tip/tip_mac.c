...
#endif
{
   ...
   cellCfg->puschSubBand.subbandStart  = wrSmDfltPuschSubBndStrt;
   cellCfg->puschSubBand.numSubbands  = wrSmDfltPuschNoSubbnds;//rcvCellCfg->puschConfigInfo.numberOfSubBands;
   cellCfg->puschSubBand.size  = wrSmDfltPuschSubBandSz;
#if 0
      ...
      case MAC_UECONFIG_REQ:
         {
            ...
            /* TODO - need to update logic for CA + Multicell */
            cfgReqInfo->u.cfgInfo.u.ueCfg.rguDlSapId = 0; 
            cfgReqInfo->u.cfgInfo.u.ueCfg.rguUlSapId = 0; 
...
)
{
  ...
  switch(hdr->msgId)
  {
     case MAC_CELLCONFIG_REQ:
     {
        ...
        cfgReqInfo->u.cfgInfo.u.cellCfg.cellId = hdr->cellId;
        /*Hack for 12 cell support -- Need to add different logic 
         * for CA + Multicell */
        rgSchCb[0].genCfg.startCellId = hdr->cellId;
        cfgReqInfo->u.cfgInfo.u.cellCfg.macInst = 0;
     ...
     case MAC_UERECONFIG_REQ:
     {
        ...
        TIP_EVENT_LOG(hdr->cellId, 0, TIP_MOD_RRC, TIP_MOD_MAC,
              TIP_MSG_MAC_UERECONFIG_REQ, hdr->transId);
     ...
     case MAC_UEDELETE_REQ:
     {
        ...
        TIP_EVENT_LOG(hdr->cellId, 0, TIP_MOD_RRC, TIP_MOD_MAC,
              TIP_MSG_MAC_UEDELETE_REQ, hdr->transId);
        cfg->key = tipPrepareLL2Key(hdr);
        cfg->upperLyrTrans = hdr->transId;
