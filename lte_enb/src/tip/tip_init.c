...
)
{
   switch(event)
   {
      case TIP_MAC_INIT_IND_TMR:
      {
        ...
        TIP_EVENT_LOG(0, 0, TIP_MOD_MAC, TIP_MOD_RRC,
              TIP_MSG_MAC_INIT_IND, 0);
      ...
      case TIP_RLC_INIT_IND_TMR:
      {
        ...
        TIP_EVENT_LOG(0, 0, TIP_MOD_RLC, TIP_MOD_RRC,
              TIP_MSG_RLC_INIT_IND, 0);
      ...
      case TIP_PDCP_INIT_IND_TMR:
      {
        ...
        TIP_EVENT_LOG(0, 0, TIP_MOD_PDCP, TIP_MOD_RRC,
              TIP_MSG_PDCP_INIT_IND, 0);
...
)
{
   switch(pkt->msgId)
   {
      case MAC_INIT_REQ:
         {
            TIP_EVENT_LOG(pkt->cellId, 0, TIP_MOD_RRC, TIP_MOD_MAC,
                  TIP_MSG_MAC_INIT_REQ, pkt->transId);
            tipStopTmr(NULLP, TIP_MAC_INIT_IND_TMR);
            pkt->msgId = MAC_INIT_RSP;
            tipCpyVmInfofrmInitReq(VMTYPE_LL2, pkt);
            TIP_EVENT_LOG(pkt->cellId, 0, TIP_MOD_MAC, TIP_MOD_RRC,
                  TIP_MSG_MAC_INIT_RSP, pkt->transId);
         }
         break;
      case RLC_INIT_REQ:
         {
            TIP_EVENT_LOG(pkt->cellId, 0, TIP_MOD_RRC, TIP_MOD_RLC,
                  TIP_MSG_RLC_INIT_REQ, pkt->transId);
            tipStopTmr(NULLP, TIP_RLC_INIT_IND_TMR);
            pkt->msgId = RLC_INIT_RSP;
            TIP_EVENT_LOG(pkt->cellId, 0, TIP_MOD_RLC, TIP_MOD_RRC,
                  TIP_MSG_RLC_INIT_RSP, pkt->transId);
         } 
         break;
      case PDCP_INIT_REQ:
         { 
            TIP_EVENT_LOG(pkt->cellId, 0, TIP_MOD_RRC, TIP_MOD_PDCP,
                  TIP_MSG_PDCP_INIT_REQ, pkt->transId);
            tipStopTmr(NULLP, TIP_PDCP_INIT_IND_TMR);
            pkt->msgId = PDCP_INIT_RSP;
            tipCpyVmInfofrmInitReq(VMTYPE_UL2, pkt);
            TIP_EVENT_LOG(pkt->cellId, 0, TIP_MOD_PDCP, TIP_MOD_RRC,
                  TIP_MSG_PDCP_INIT_RSP, pkt->transId);
         } 
         break;
      default:
         {
   ...
   tipBldSendInitResp(pkt->dstModId, pkt->msgId, pkt->transId, pkt->cellId);
   RETVALUE(ROK);
}
...
)
{
   ...
   switch(tipMsg->msgId)
   {
      ...
      case MAC_INIT_IND:
         {
            ...
            else
            {
               ...
               printf("Already ongoing MAC_INIT procedure. Dropping "
                     "MAC_INIT_IND [cellIdx: %d | msgId: %d | state: %d]\n",
                     cellIdx, tipMsg->msgId,vmAssocInfo[cellIdx].macAssocState);
            }
         }
         break;
      case RLC_INIT_IND:
         {
            ...
            else
            {
               ...
               printf("Already ongoing RLC_INIT procedure. Dropping "
                     "RLC_INIT_IND [cellIdx: %d | msgId: %d | state: %d]\n",
                     cellIdx, tipMsg->msgId,vmAssocInfo[cellIdx].rlcAssocState);
            }
         }
         break;
      case PDCP_INIT_IND:
         {
            ...
            else
            {
               RLOG_ARG2(L_DEBUG, DBG_CELLID, cellIdx,
                     ...
                     vmAssocInfo[cellIdx].rlcAssocState);
               printf("Already ongoing PDCP_INIT procedure. Dropping "
                     "PDCP_INIT_IND [cellIdx: %d | msgId: %d | state: %d]\n",
                     cellIdx, tipMsg->msgId,vmAssocInfo[cellIdx].rlcAssocState);
            }
         }
         break;
      ...
      case MAC_INIT_RSP:
         {
            ...
            printf("INIT Response received from MAC for cellIdx: %d\n", cellIdx);
         }
         break;
      case RLC_INIT_RSP:
         {
            TIP_EVENT_LOG(cellIdx, 0, TIP_MOD_RLC, TIP_MOD_RRC,
                  TIP_MSG_RLC_INIT_RSP, initRspTransId);
      ...
      case PDCP_INIT_RSP:
         {
            TIP_EVENT_LOG(cellIdx, 0, TIP_MOD_PDCP, TIP_MOD_RRC,
                  TIP_MSG_PDCP_INIT_RSP, initRspTransId);
