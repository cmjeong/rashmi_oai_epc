...
*/
#ifdef ANSI
PUBLIC S16 tipSendRrcUeIdChgCfm
(
Pst               *pst,
SuId              suId,
U32               transId,
...
#else
PUBLIC S16 tipSendRrcUeIdChgCfm(pst, spId, transId, ueInfo, status)
Pst               *pst;
SuId              suId;
U32               transId;
CkwUeInfo         *ueInfo;
...
#endif
{
   ...
   TRC3(tipSendRrcUeIdChgCfm)
    
   hdr = (TipPacket*)sendBuf;
   cellId = ueInfo->cellId;
   bufLen = TIP_HDR_LEN + sizeof(RlcUeIdentityChangeRsp);

   chgRsp = (RlcUeIdentityChangeRsp*)hdr->message;
   chgRsp->ueInfo.ueIndex = 0; //TODO::To fetch Ue IDX from Enb APP Ue-CB and fill.
   chgRsp->ueInfo.cRnti   = ueInfo->ueId;

   chgRsp->status = status.status;
   
   tipFillHdr(pst, cellId, RLC_UEIDENTITYCHANGE_RSP, transId, msgLen, hdr);
   
   if(SPutSBuf(pst->region, pst->pool, (Data *)ueInfo, sizeof(CkwUeInfo)) != ROK)
   {
       RETVALUE(RFAILED);
   }

   RETVALUE(tipSendToL3(msgLen, (U8 *)&sendBuf)); 
}
