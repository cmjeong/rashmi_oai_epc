...
void *tipSendTtiIndEgToRecvData (void *in)
{
  ...
  static U32 numTxSentCnt = 1;
  static U32 numRxSentCnt = 1;
  Buffer *mBuf = NULLP;
  pst.dstProcId = SFndProcId();
  pst.srcProcId = SFndProcId();
  printf("\n\n********* Waiting for start post trigger (DAM cell add) **********\n\n");
  while(!strtPst)
  {
     usleep(10000);
  }
  printf("\n\n********* TIP EGTP TICK PST STARTED **********\n\n");
  while(1)
  {
     usleep(500);
     if(numTxSentCnt == 1000000)
     {
       numTxSentCnt = 30;
       numRxSentCnt = 30;
       ysEgTxPrcCnt = 0;
     ...
     pst.dstProcId = SFndProcId();
     pst.srcProcId = SFndProcId();
     if((numTxSentCnt - ysEgTxPrcCnt) < 100)
     {
     ...
     if((numRxSentCnt - ysEgRxPrcCnt) < 100)
     {
        mBuf = NULLP;
        SGetMsg(0, 0, (Buffer **) &mBuf);
        if(mBuf == NULLP)
     ...
     if((numTxSentCnt % 3000) == 0)
     {
       printf("Number of Rx/Tx Evnt sent [%d:%d] Reecived at egtp [%d:%d]\n",
                           numTxSentCnt,numRxSentCnt, ysEgTxPrcCnt, ysEgRxPrcCnt);
     }
  }
   pthread_exit(NULLP);
}
