...
#ifdef TIP_LOWER_L2
EXTERN S16 tipSendRlcCmnCfgRsp ARGS((Pst *pst, SuId suId,CkwCfgCfmInfo *cfmInfo));
#endif
...
#endif
{
   TRC3(KwUiKwuDatInd)
    ...
    if(pst->dstEnt == ENTNH)
    {
       ret = tipSendCcchMsgInd(pst, suId, datInd, mBuf);
    }
    else
    {
       ret = tipSendPdcpMsgInd(pst, suId, datInd, mBuf);
    }
    RETVALUE(ret);
#else
   /* jump to specific primitive depending on configured selector */
...
#ifdef TIP_LOWER_L2
   tipSendRlcDatCfm(pst, suId, datCfm);
#else
   /* jump to specific primitive depending on configured selector */
...
#ifdef TIP_LOWER_L2
   tipSendRlcStaInd(pst, suId, staInd);
#else
   /* jump to specific primitive depending on configured selector */
...
#endif
{
   ...
   /* jump to specific primitive depending on configured selector */
#ifdef TIP_LOWER_L2
   tipSendRlcCmnCfgRsp(pst, suId, cfmInfo); 
#else
...
#if 1
   //RETVALUE(tipSendRrcUeIdChgCfm(pst, suId, transId, ueInfo, status));
#else
   /* jump to specific primitive depending on configured selector */
