...
#ifdef HI_LKSCTP
/* hi023.201: added compile time flag */
   ...
   if (hiCb.init.cfgDone)
   {
      ...
      hiCb.cfg.maxPathReTx  = hiGen->maxPathReTx;
      hiCb.cfg.maxAssocReTx = hiGen->maxAssocReTx;
      hiCb.cfg.maxInitReTx  = hiGen->maxInitReTx;

#ifdef HI_LKSCTP_USER_CONFIG
      for (i = 0; i < (SpId)hiCb.cfg.numSaps; i++)
      {
         sap = hiCb.saps[i];
         if(sap != NULLP)
         {
            ...
            while ((iRet = cmHashListGetNext(&sap->conCbHlCp, (PTR)prevConCb, (PTR *)&conCb)) == ROK)
            {
               ...
               tPar.u.sctpParam.sockOpts[0].optVal.sctpPeerAddrParams.hbInterval    = 0;
               ...
               tPar.u.sctpParam.sockOpts[1].optVal.sctpPeerAssocParams.cookieLife       = 0;
               ...
               if (ret != ROK)
               {
                  HIDBGP(DBGMASK_LYR, (hiCb.init.prntBuf, "hiCfgGen : failed to set socket options\n"));
...
#ifdef HI_TLS
   /* OpenSSL contexts */
   sMemSize += hiCb.cfg.numContexts * sizeof (HiCtx);

   /* OpenSSL locks */
   hiTLSAddMem(&sMemSize);
#endif
...
#ifdef HI_TLS
   /* allocate space for OpenSSL contexts and zero the space */
   HI_ALLOC(hiCb.cfg.numContexts * sizeof (HiCtx), hiCb.tlsContexts);
   if (hiCb.tlsContexts == NULLP)
   {
      ret = LCM_REASON_MEM_NOAVAIL;
      goto hiCfgGenCleanup10;
   }

   cmMemset((U8 *)hiCb.tlsContexts, 0, hiCb.cfg.numContexts * sizeof (HiCtx));
   for (i = 0;  i < hiCb.cfg.numContexts;  i++)
      hiCb.tlsContexts[i].cfg.ctxId = HI_INVALID_OSSL_CTX;

   /* initialize the OpenSSL library if required */
   if (hiCb.cfg.initOpenSSL)
   {
      ret = hiInitOpenSSL();
      if (ret != LCM_REASON_NOT_APPL)
         goto hiCfgGenCleanup11;
   }

   /* set up thread support for TLS */
   ret = hiSetupTLSThreads();
   if (ret != LCM_REASON_NOT_APPL)
      goto hiCfgGenCleanup11;

   /* set up the PRNG */
   if (hiCb.cfg.prng.option == LHI_OSSL_PRNG_FILE)
      RAND_load_file(hiCb.cfg.prng.u.file, LHI_OSSL_PRNG_SEED_LEN);
   else if (hiCb.cfg.prng.option == LHI_OSSL_PRNG_SEED)
      RAND_seed(hiCb.cfg.prng.u.seed, LHI_OSSL_PRNG_SEED_LEN);
   else
   {
...
#ifdef HI_TLS
hiCfgGenCleanup11:
   HI_FREE(hiCb.cfg.numContexts * sizeof (HiCtx), hiCb.tlsContexts);
...
/* hi002.201 :  Updated for the support of configurable RTO parameters, HBeat value, 
      ...
      /* go through the SAP's connections hash list */
      sap->cfg.cookieLife  = cfg->cookieLife;
      sap->cfg.rtoInitial  = cfg->rtoInitial;
      sap->cfg.rtoMax      = cfg->rtoMax;
      sap->cfg.rtoMin      = cfg->rtoMin;
      sap->cfg.t1Init      = cfg->t1Init;
      sap->cfg.hBeatEnable = cfg->hBeatEnable;
      if (cfg->hBeatEnable == TRUE)
         sap->cfg.intervalTm = cfg->intervalTm;
      ...
      while ((iRet = cmHashListGetNext(&sap->conCbHlCp, (PTR)prevConCb, (PTR *)&conCb)) == ROK)
      {
         ...
         if ((conCb->type == HI_SCTP_ASSOC) && (conCb->state == HI_ST_CONNECTED))
         {
            ...
            tPar.u.sctpParam.sockOpts[1].optVal.sctpPeerAssocParams.assocMaxReTx     = 0;
            tPar.u.sctpParam.sockOpts[1].optVal.sctpPeerAssocParams.numberOfPeerDest = 0;
            tPar.u.sctpParam.sockOpts[1].optVal.sctpPeerAssocParams.peerRwnd         = 0;
            tPar.u.sctpParam.sockOpts[1].optVal.sctpPeerAssocParams.localRwnd        = 0;
            ...
            if (ret != ROK)
            {
               HIDBGP(DBGMASK_LYR, (hiCb.init.prntBuf, "hiCfgSap : failed to set socket options\n"));
               /* hi023.201: changed RETVOID to RETVALUE as this function
                             returns S16 type */
               RETVALUE(RFAILED);
            }
...
*       Fun:    hiCfgCtx
*
*       Desc:   TLS/OpenSSL context configuration. Stores the specified
*               context configuration.
*
*       Ret:    LCM_REASON_NOT_APPL             - ok
*               LCM_REASON_GENCFG_NOT_DONE      - failure
*               LCM_REASON_INVALID_PAR_VAL      - failure
*               LHI_REASON_MISC_FAILURE         - failure
*
*       Notes:  None
*
*       File:   hi_bdy2.c
...
#endif
{
   ...
   {
      HIDBGP(DBGMASK_LYR, (hiCb.init.prntBuf,
         "hiCfgCtx(): general configuration not done\n"));
      RETVALUE(LCM_REASON_GENCFG_NOT_DONE);
   }
#endif
...
#if (ERRCLASS & ERRCLS_INT_PAR)
   if (cfg->ctxId < 0  ||  cfg->ctxId >= hiCb.cfg.numContexts)
   {
      HILOGERROR_INT_PAR(EHI069, (ErrVal)cfg->ctxId, 0,
         "hiCfgCtx(): ctxId out of range");
      RETVALUE(LCM_REASON_INVALID_PAR_VAL);
   }

   if (hiCb.tlsContexts[cfg->ctxId].cfg.ctxId != HI_INVALID_OSSL_CTX)
   {
      HILOGERROR_INT_PAR(EHI070, (ErrVal)cfg->ctxId, 0,
         "hiCfgCtx(): context already configured");
      RETVALUE(LCM_REASON_RECONFIG_FAIL);
   }

   if (cfg->cacheSessions  &&  cfg->sessIdCtxLen > LHI_OSSL_SESSIDCTXLEN)
   {
      HILOGERROR_INT_PAR(EHI071, (ErrVal)cfg->sessIdCtxLen, 0,
         "hiCfgCtx(): invalid session id context length");
      RETVALUE(LCM_REASON_INVALID_PAR_VAL);
   }
#endif
   ...
   /* copy the configuration */
   cmMemcpy((U8 *)&hiCb.tlsContexts[cfg->ctxId].cfg,
            (U8 *)cfg, sizeof (HiCtxCfg));


   /* create the context */
   /* hi008.105 : To return proper error code in case of 
                  context creation failure */
   if ((ret = hiCreateTLSCtx(&hiCb.tlsContexts[cfg->ctxId])) != ROK)
   {
      HILOGERROR_INT_PAR(EHI072, (ErrVal)cfg->ctxId, 0,
         "hiCfgCtx(): context creation failed");
      RETVALUE(ret);
   }
...
#endif
{
   ...
   for (i = 0;  i < hiCb.cfg.numContexts;  i++)
   {
      if (hiCb.tlsContexts[i].cfg.ctxId != HI_INVALID_OSSL_CTX)
         hiDestroyTLSCtx(&hiCb.tlsContexts[i]);
   }
   hiCloseTLSThreads();
   HI_FREE(hiCb.cfg.numContexts * sizeof (HiCtx), hiCb.tlsContexts);
#endif
...
#ifdef HI_LKSCTP
      genSts->numRxSctpMsg      += fdGrp->genRxSts.numRxSctpMsg;
#endif
   }
...
#ifdef HI_LKSCTP
      genSts->numTxSctpMsg      += sap->txSts.numTxSctpMsg;
#endif
   }
...
#ifdef HI_LKSCTP
      sapSts->numRxSctpMsg      += fdGrp->rxSts[sap->spId].numRxSctpMsg;
#endif
   }
...
#ifdef HI_LKSCTP
   sapSts->numTxSctpMsg         = sap->txSts.numTxSctpMsg;
#endif
   ...
   *type                = CM_INET_RAW;
   ...
   switch (srvc)
   {
      ...
      case HI_SRVC_LKSCTP:
         ...
         con->suAssocId = suConId;
         con->protocol  = CM_PROTOCOL_LKSCTP;
         con->flag = HI_FL_SCTP;
         *type = CM_INET_SEQPACKET;
         break;
#endif
    ...
    */
#ifdef HI_LKSCTP
   if ((*type == CM_INET_STREAM) || (*type == CM_INET_SEQPACKET))
#else
...
#ifdef HI_LKSCTP
   /* initialise the assoc map hash list, if its SCT sap */
   if (sap->type == HI_SCT_SAP)
   {   
      ret = cmHashListInit(&conCb->assocMapHlCp, hiCb.cfg.numAssocBins,
            HI_GET_OFFSET(HiAssocMapCb, assocMapHlEnt), FALSE,
            CM_HASH_KEYTYPE_U32MOD,
            hiCb.init.region, hiCb.init.pool);
      if (ret != ROK)
      {
         HILOGERROR_ADD_RES(EHI083, 0, 0,
               "hiCompleteConCb(): failed to Intialize the assoc hash list");
         /* hash failure, send an alarm */
         alInfo.spId = sap->spId;
         alInfo.type = LHI_ALARMINFO_TYPE_NTPRSNT;
         hiSendAlarm(LCM_CATEGORY_INTERNAL, LCM_EVENT_INV_EVT,
...
#ifdef HI_LKSCTP
      if (sap->type == HI_SCT_SAP)
         cmHashListDeinit(&conCb->assocMapHlCp);
#endif
...
/*hi014.105  1. Modified statistics req to include messages Tx/Rx in bytes per con per thread */
...
#ifdef HI_TLS
   /* shut down the TLS connection, if this is TLS */
   if ((conCb->srvcType & 0x0F) == HI_SRVC_TLS)
      hiDisconnectTLS(conCb);
#endif
       ...
       &&  conCb->sap->state == HI_ST_BND)
   {
      ...
      if (sap->type == HI_SCT_SAP)
      {
         ...
         HiUiSctTermInd(&sap->uiPst, sap->suId, conCb->suConId, 
                        SCT_ASSOCID_SU, tMsg->sctDiscInd.status, 
                        tMsg->sctDiscInd.cause, &tMsg->sctDiscInd.rtrvInfo);
      }
      else
#endif
            ...
            &&  conCb->sap->state == HI_ST_BND)
   {
      ...
      if (sap->type == HI_SCT_SAP)
      {
         if (conCb->type == HI_SCTP_ENDP)
            HiUiSctEndpCloseCfm(&sap->uiPst, sap->suId, conCb->suConId, SCT_OK, SCT_CAUSE_NOT_APPL);
         else
            HiUiSctTermCfm(&sap->uiPst, sap->suId, conCb->suConId, SCT_OK, SCT_CAUSE_NOT_APPL);
      }
      else
#endif
...
#ifdef HI_LKSCTP
   /* If this was association abort */
   if (sap->type == HI_SCT_SAP && 
      ((tMsg->type == HI_THR_DELCON_DISCIND && tMsg->sctDiscInd.status == SCT_STATUS_ABRT) ||
       (tMsg->type == HI_THR_DELCON_DISCCFM && tMsg->disc.action == HI_ABORT))) 
     
   {
      ...
      tPar.u.sctpParam.sockOpts[0].level  = CM_SOCKOPT_LEVEL_SOCKET;
      tPar.u.sctpParam.sockOpts[0].option = CM_SOCKOPT_OPT_LINGER;
      tPar.u.sctpParam.sockOpts[0].optVal.sockLinger.enable     = TRUE;
      tPar.u.sctpParam.sockOpts[0].optVal.sockLinger.lingerTime = 0;
...
#ifdef HI_LKSCTP
   if (tMsg->type == HI_THR_DELCON_DISCCFM 
       &&  tMsg->disc.action != HI_CLOSE && tMsg->disc.action != HI_ABORT)
#else
   if (tMsg->type == HI_THR_DELCON_DISCCFM
...
#ifdef HI_LKSCTP
   else if ((tMsg->type == HI_THR_ADDCON_CONCFM) && (conCb->protocol != CM_PROTOCOL_LKSCTP))
#else
   ...
   if (ret == ROK)
   {
      ...
      if (ret == ROK)
      {
         ...
         if (ret == ROK)
         {
            ...
            {
#ifdef HI_LKSCTP
               ret = SUnpkU16(&tMsg.sctDiscInd.cause, mBuf);
               if (ret == ROK)
               {
                  ret = SUnpkU16(&tMsg.sctDiscInd.status, mBuf);
               }
...
#endif
{
   ...
   if (tMsg->type == HI_THR_DELCON_DISCIND)
   {
      ...
      if (ret == ROK)
      {
         ret = SPkU16(tMsg->sctDiscInd.status, mBuf);
         if (ret == ROK)
         {
            ret = SPkU16(tMsg->sctDiscInd.cause, mBuf);
         }
      }
#endif
   }
...
#ifdef HI_LKSCTP
   getVerCfmInfo->numUif = 2;
   getVerCfmInfo->uifList[1].intfId  = SCTIF;
   getVerCfmInfo->uifList[1].intfVer = SCTIFVER;
#endif /* HI_LKSCTP */
...
#endif
{
   ...
   switch (setVerInfo->intf.intfId)
   {
      case HITIF: 
         ...
         for (i = 0; i < hiCb.cfg.numSaps; i++)
         {
            ...
            /* hi028.201: Included the SCT interface also in case of 
               KSCTP */
#ifdef HI_LKSCTP
            if ((sap->state == HI_ST_BND) && (sap->type == HI_HIT_SAP))
#else
      ...
      case SCTIF:
         {
            /* repeat for all SAPs on the SCT interface */
            for (i = 0; i < hiCb.cfg.numSaps; i++)
            {
               /* If it is an unbound SAP then the remote entity,
                * instance and proc ID would not be available and
               ...
               /* update interface version in pst only if SAP is bound */
               if ((sap->state == HI_ST_BND) && (sap->type == HI_SCT_SAP))
               {
                  switch (setVerInfo->grpType)
                  {
                     case SHT_GRPTYPE_ALL:
...
#endif
{
   ...
   if (newSize > sap->cfg.txqCongStrtLim)
   {
...
#ifdef HI_LKSCTP
            if (conCb->sap->type == HI_SCT_SAP)
            {
               HI_SCTP_FLW_CNTRL(conCb, SCT_FLC_DROP, 
                                 LHI_EVENT_TXQ_CONG_DATA_DROP, &alInfo);
            }
            else
            {
               HI_FLW_CNTRL(conCb, HI_FLC_DROP, 
                            LHI_EVENT_TXQ_CONG_DATA_DROP, &alInfo);
            }
#else
      ...
      else 
      {
         ...
         if (!(conCb->flc & HI_SENT_FLC_STRT))
         {
            ...
            if (conCb->sap->type == HI_SCT_SAP)
            {
               HI_SCTP_FLW_CNTRL(conCb, SCT_FLC_START, LHI_EVENT_TXQ_CONG_ON, 
                                 &alInfo);
            }
            else
            {
   ...
   else if (newSize < sap->cfg.txqCongStopLim)
   {
      ...
      if (conCb->flc & HI_SENT_FLC_STRT  ||  conCb->flc & HI_SENT_FLC_DROP)
      {
         ...
         if (conCb->sap->type == HI_SCT_SAP)
         {
            HI_SCTP_FLW_CNTRL(conCb, SCT_FLC_STOP, LHI_EVENT_TXQ_CONG_OFF, 
                              &alInfo);
         }
         else
         {
