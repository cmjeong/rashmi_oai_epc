...
#endif
{
  ...
  SZ_DIAG_LVL0(0x03160008, SZ_DIAG_SPCONID, datEvnt->spConnId,
   ...
   if(peer->nodeType == LSZ_NODE_ENB)
   {
      /* sz008.301:Removed SGetIE for MMEs1apID, as we are extracting above*/
      if(((SztMME_UE_S1AP_ID *)mmeS1apId)->val != conCb->lclConRefNo)
      {
         /* Error Indication to the user */
         SZ_SEND_LCL_ERR_IND(PRSNT_NODEF, datEvnt->spConnId,
                          uSapCb, SZT_CAUSE_INV_CONID, ((SztMME_UE_S1AP_ID *)mmeS1apId)->val);
         SZ_FREE_SDU(datEvnt->pdu);
         /* sz008.301: corrected ret value */
         RETVALUE(RFAILED);
      }
...
#endif
{
   ...
   {
#if (ERRCLASS & ERRCLS_INT_PAR)
      SZLOGERROR(ERRCLS_INT_PAR, ESZ180, (ErrVal) 0,
       "SzUiSztPeerAssocRsp: failed to find the control block for dyamic peer\n");
#endif
   ...
   {
#if (ERRCLASS & ERRCLS_INT_PAR)
      SZLOGERROR(ERRCLS_INT_PAR, ESZ181, (ErrVal) 0,
       "SzUiSztPeerAssocRsp: user responded not to configure the dynamic peer\n");
#endif
#ifdef SCT3
   ...
   {
      /* sz005.301: Provided the flexibility to not to configure the 
   ...
   SZ_SEND_PEER_ALARM(peerCb->peerId, LSZ_EVENT_UI_PEERASSOC_RSP, LSZ_CAUSE_DYNAM_PEERCFG_OK);

   /* Send the association response */
#ifdef SCT3
   SzLiSctAssocRsp(&peerCb->sctSapCb->pst, peerCb->sctSapCb->spId, 
                   peerCb->sctSapCb->endpCb.locSpEndpId, &assocParams, 0, SCT_OK, NULLP);
#else
   SzLiSctAssocRsp(&peerCb->sctSapCb->pst, peerCb->sctSapCb->spId, 
...
#endif
{
   ...
   {
#if (ERRCLASS & ERRCLS_INT_PAR)
      SZLOGERROR(ERRCLS_INT_PAR, ESZ188, (ErrVal) sapCb->suId, 
...
#endif
{
   ...
   {
#if (ERRCLASS & ERRCLS_INT_PAR)
      SZLOGERROR(ERRCLS_INT_PAR, ESZ189, (ErrVal) peerAssocRsp->peerId,
   ...
   {
#if (ERRCLASS & ERRCLS_INT_PAR)
      SZLOGERROR(ERRCLS_INT_PAR, ESZ190, (ErrVal) peerAssocRsp->peerId,
       "szValidatePeerAssocRsp: common procedure stream id recieved is invalid\n");
#endif
