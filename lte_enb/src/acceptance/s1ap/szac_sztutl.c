...
/********************************************************************20**
     ...
     Sid:      szac_sztutl.c@@/main/4 - Mon Jan 10 22:16:04 2011
...
#endif
{
   ...
   TRC2(szAcUtlSztFillUDatReqPdu);

#ifdef ALIGN_64BIT
   CMXTA_DBG_PARAM((_cmxtap, "szAcUtlSztFillUDatReqPdu(), tcId (%d)\n", tcCb->tcId));
#else
   CMXTA_DBG_PARAM((_cmxtap, "szAcUtlSztFillUDatReqPdu(), tcId (%ld)\n", tcCb->tcId));
#endif
   ...
   switch(eventType)
   {
      ...
      case SZT_MSG_ID_SETUP_RSP:
         {
            if(szAcUtlSztFillS1SetupRsp(tcCb, spCb, &(uDatEvnt->pdu)) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      ...
      case SZT_MSG_ID_PAGING:
         {
            if(szAcUtlSztFillPaging(tcCb, spCb, &(uDatEvnt->pdu)) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      ...
      case SZT_MSG_ID_OVLD_START:
         {
            if(szAcUtlSztFillOverloadStart(tcCb, spCb, &(uDatEvnt->pdu)) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      case SZT_MSG_ID_OVLD_STOP:
         {
            if(szAcUtlSztFillOverloadStop(tcCb, spCb, &(uDatEvnt->pdu)) != ROK)
               RETVALUE(RFAILED);
         }
         break;
...
} /* szAcUtlSztFillUDatReqPdu */

/*
*
*       Fun:   szAcUtlSztFillConReqPdu
*
*       Desc:  Call handler to fill common field
*
*       Ret:   ROK
...
#endif
{
   ...
   TRC2(szAcUtlSztFillConReqPdu);

#ifdef ALIGN_64BIT
   CMXTA_DBG_PARAM((_cmxtap, "szAcUtlSztFillConReqPdu(), tcId (%d)\n", tcCb->tcId));
#else
   CMXTA_DBG_PARAM((_cmxtap, "szAcUtlSztFillConReqPdu(), tcId (%ld)\n", tcCb->tcId));
#endif
   ...
   switch(eventType)
   {
      case SZT_MSG_ID_INIT_UE_MSG:
         {
            if(szAcUtlSztFillInitUeMsg(tcCb, spCb, &(conEvnt->pdu)) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      case SZT_MSG_ID_DL_NAS_TPT:
         {
            if(szAcUtlSztFillDwnlnkNasTpt(tcCb, spCb, &(conEvnt->pdu)) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      ...
      case SZT_MSG_ID_HO_REQ:
         {
            if(szAcUtlSztFillHovrRqst(tcCb, spCb, &(conEvnt->pdu)) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      case SZT_MSG_ID_PATH_SWTCH_REQ:
         {
            if(szAcUtlSztFillPathSwRqst(tcCb, spCb, &(conEvnt->pdu)) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      default:
...
} /* szAcUtlSztFillConReqPdu */

/*
*
*       Fun:   szAcUtlSztFillConRspPdu
*
*       Desc:  Call handler to fill common field
*
*       Ret:   ROK
...
#endif
{
   ...
   TRC2(szAcUtlSztFillConRspPdu);

#ifdef ALIGN_64BIT
   CMXTA_DBG_PARAM((_cmxtap, "szAcUtlSztFillConRspPdu(), tcId (%d)\n", tcCb->tcId));
#else
   CMXTA_DBG_PARAM((_cmxtap, "szAcUtlSztFillConRspPdu(), tcId (%ld)\n", tcCb->tcId));
#endif
   ...
   switch(eventType)
   {
      case SZT_MSG_ID_CXT_SETUP_REQ:
         {
            if(szAcUtlSztFillInitCxtSetupReq(tcCb, spCb, &conEvnt->pdu) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      case SZT_MSG_ID_DL_NAS_TPT:
      ...
      case SZT_MSG_ID_UL_NAS_TPT:
         {
            if(szAcUtlSztFillUplnkNasTpt(tcCb, spCb, &(conEvnt->pdu)) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      case SZT_MSG_ID_NO_DATA:
...
} /* szAcUtlSztFillConRspPdu */

/*
*
*       Fun:   szAcUtlSztFillRelReqPdu
*
*       Desc:  Call handler to fill common field
*
*       Ret:   ROK
...
#endif
{
   ...
   TRC2(szAcUtlSztFillRelReqPdu);

#ifdef ALIGN_64BIT
   CMXTA_DBG_PARAM((_cmxtap, "szAcUtlSztFillRelReqPdu(), tcId (%d)\n", tcCb->tcId));
#else
   CMXTA_DBG_PARAM((_cmxtap, "szAcUtlSztFillRelReqPdu(), tcId (%ld)\n", tcCb->tcId));
#endif
...
} /* szAcUtlSztFillRelReqPdu */

/*
*
*       Fun:   szAcUtlSztFillRelRspPdu
*
*       Desc:  Call handler to fill common field
*
*       Ret:   ROK
...
#ifdef ALIGN_64BIT
   CMXTA_DBG_PARAM((_cmxtap, "szAcUtlSztFillRelRspPdu(), tcId (%d)\n", tcCb->tcId));
#else
   CMXTA_DBG_PARAM((_cmxtap, "szAcUtlSztFillRelRspPdu(), tcId (%ld)\n", tcCb->tcId));
#endif
...
} /* szAcUtlSztFillRelRspPdu */

/*
*
*       Fun:   szAcUtlSztFillDatReqPdu
*
*       Desc:  Call handler to fill common field
*
*       Ret:   ROK
...
#endif
{
   ...
   TRC2(szAcUtlSztFillDatReqPdu);

#ifdef ALIGN_64BIT
   CMXTA_DBG_PARAM((_cmxtap, "szAcUtlSztFillDatReqPdu(), tcId (%d)\n", tcCb->tcId));
#else
   CMXTA_DBG_PARAM((_cmxtap, "szAcUtlSztFillDatReqPdu(), tcId (%ld)\n", tcCb->tcId));
#endif
   ...
   switch(eventType)
   {
      case SZT_MSG_ID_CXT_SETUP_REQ:
         {
            if(szAcUtlSztFillInitCxtSetupReq(tcCb, spCb, &datEvnt->pdu) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      ...
      case SZT_MSG_ID_UL_NAS_TPT:
         {
            if(szAcUtlSztFillUplnkNasTpt(tcCb, spCb, &(datEvnt->pdu)) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      case SZT_MSG_ID_DL_NAS_TPT:
         {
            if(szAcUtlSztFillDwnlnkNasTpt(tcCb, spCb, &(datEvnt->pdu)) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      ...
      case SZT_MSG_ID_E_RAB_BRR_SETUP_REQ:
         {
            if(szAcUtlSztFillE_RABSetupRqst(tcCb, spCb, &(datEvnt->pdu)) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      ...
      case SZT_MSG_ID_UE_CXT_REL_REQ:
         {
            if(szAcUtlSztFillUeCxtRlsReq(tcCb, spCb, &(datEvnt->pdu)) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      case SZT_MSG_ID_ERR_IND:
      ...
      case SZT_MSG_ID_TRACE_START:
         {
           if(szAcUtlSztFillTraceStart(tcCb, spCb, &(datEvnt->pdu)) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      ...
      case SZT_MSG_ID_CELL_TRAFFIC_TRACE:
         {
           if(szAcUtlSztFillCellTrafficTrace(tcCb, spCb, &(datEvnt->pdu)) != ROK)
               RETVALUE(RFAILED);
         }
         break;
      ...
      default:
         CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillDatReqPdu: Invalid Msg, msg = %d line =  %d\n",
                        eventType,__LINE__));
         RETVALUE(RFAILED);
   }
...
#ifndef SZ_ENC_DEC
#ifdef ANSI
PUBLIC S16 szAcUtlSztFillS1SetupRsp
(
CmXtaTCCb    *tcCb,
CmXtaSpCb    *spCb,
S1apPdu      **pdu
...
#else
PUBLIC S16 szAcUtlSztFillS1SetupRsp(tcCb, spCb, pdu)
CmXtaTCCb    *tcCb;
CmXtaSpCb    *spCb;
S1apPdu      **pdu;
#endif
...
#else
PUBLIC S16 szAcUtlSztFillS1SetupRsp(pdu)
S1apPdu      **pdu;
#endif
#endif   /* SZ_ENC_DEC */
{
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillS1SetupRsp: cmAllocEvnt failed, line%d\n",__LINE__));
#endif
      RETVALUE(RFAILED);
   }
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillS1SetupRsp: cmGetMem failed, line %d\n",__LINE__));
#endif
      RETVALUE(RFAILED);
   }
   ...
   *pdu = szS1SetupRspPdu;
   ...
   RETVALUE(ROK);
} /* End of szAcUtlSztFillS1SetupRsp */
...
#ifndef SZ_ENC_DEC
#ifdef ANSI
PUBLIC S16 szAcUtlSztFillInitUeMsg
(
CmXtaTCCb    *tcCb,
CmXtaSpCb    *spCb,
S1apPdu      **pdu
...
#else
PUBLIC S16 szAcUtlSztFillInitUeMsg(tcCb, spCb, pdu)
CmXtaTCCb    *tcCb;
CmXtaSpCb    *spCb;
S1apPdu      **pdu;
#endif
...
#else
PUBLIC S16 szAcUtlSztFillInitUeMsg(pdu)
S1apPdu      **pdu;
#endif
#endif   /* SZ_ENC_DEC */
{
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillInitUeMsg: cmAllocEvnt failed, line %d\n",__LINE__));
#endif
      RETVALUE(RFAILED);
   }
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillInitUeMsg: cmGetMem failed, line %d\n",__LINE__));
#endif
      RETVALUE(RFAILED);
   }
...
} /* End of szAcUtlSztFillInitUeMsg */

/*
*
*       Fun:   szAcUtlSztFillInitCxtSetupReq
*
...
#ifndef SZ_ENC_DEC
#ifdef ANSI
PUBLIC S16 szAcUtlSztFillInitCxtSetupReq
(
CmXtaTCCb    *tcCb,
CmXtaSpCb    *spCb,
S1apPdu      **pdu
...
#else
PUBLIC S16 szAcUtlSztFillInitCxtSetupReq(tcCb, spCb, pdu)
CmXtaTCCb    *tcCb;
CmXtaSpCb    *spCb;
S1apPdu      **pdu;
#endif
...
#else
PUBLIC S16 szAcUtlSztFillInitCxtSetupReq(pdu)
S1apPdu      **pdu;
#endif
#endif /* SZ_ENC_DEC */
{
   ...
   TRC2(szAcUtlSztFillInitCxtSetupReq);
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillInitCxtSetupReq: cmAllocEvnt failed, line %d\n",__LINE__));
#endif /* SZ_ENC_DEC */
      RETVALUE(RFAILED);
   }
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillInitCxtSetupReq: cmGetMem failed, line %d\n",__LINE__));
#endif /* SZ_ENC_DEC */
      RETVALUE(RFAILED);
   }
   ...
   for(idx = 0; idx < numComp; idx++)
   {
           ...
           (Ptr *)&(ie1->value.u.sztE_RABToBeSetupItemCtxtSUReq.iE_Extns.member)) !=ROK)
      {
       ...
       cmMemcpy((char *)correlatn_id,"2541",sizeof(correlatn_id));
   ...
   if(!buggy)
   {
      ...
      cmMemset(mmeGrpId,0,sizeof(mmeGrpId));
      cmMemcpy((char *)mmeGrpId,"11",sizeof(mmeGrpId));
      len = 2;
#ifndef SZ_ENC_DEC
   cmXtaXmlGetVal(tcCb, spCb->childStep, CMXTA_ATTRIB_TXT,"mmeGrpVal",&(mmeGrpId));
   cmXtaXmlGetVal(tcCb, spCb->childStep, CMXTA_ATTRIB_U16,"mmeGrpLen",&(len));
      ...
      cmMemset(mmeCode,0,sizeof(mmeCode));
      cmMemcpy((char *)mmeCode,"1",sizeof(mmeCode));
      len = 1;
#ifndef SZ_ENC_DEC
   cmXtaXmlGetVal(tcCb, spCb->childStep, CMXTA_ATTRIB_TXT,"mmeCodeVal",&(mmeCode));
   cmXtaXmlGetVal(tcCb, spCb->childStep, CMXTA_ATTRIB_U16,"mmeCodeLen",&(len));
...
#ifndef SZ_ENC_DEC
#ifdef ANSI
PUBLIC S16 szAcUtlSztFillUeCxtRlsReq
(
CmXtaTCCb  *tcCb,
CmXtaSpCb  *spCb,
S1apPdu    **pdu
...
#else
PUBLIC S16 szAcUtlSztFillUeCxtRlsReq(tcCb, spCb, pdu)
CmXtaTCCb  *tcCb;
CmXtaSpCb  *spCb;
S1apPdu    **pdu;
#endif
...
#else
PUBLIC S16 szAcUtlSztFillUeCxtRlsReq(pdu)
S1apPdu   **pdu;
#endif
#endif   /* SZ_ENC_DEC */

{
   ...
   TRC2(szAcUtlSztFillUeCxtRlsReq);
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillUeCxtRlsReq: cmAllocEvnt failed,line %d\n",__LINE__));
#endif
      RETVALUE(RFAILED);
   }
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillUeCxtRlsReq: cmGetMem failed, line %d\n",__LINE__));
#endif
      RETVALUE(RFAILED);
   }
...
} /* End of szAcUtlSztFillUeCxtRlsReq */

/*
*
*       Fun:   szAcUtlSztFillUplnkNasTpt
*
...
#ifndef SZ_ENC_DEC
#ifdef ANSI
PUBLIC S16 szAcUtlSztFillUplnkNasTpt
(
CmXtaTCCb    *tcCb,
CmXtaSpCb    *spCb,
S1apPdu      **pdu
...
#else
PUBLIC S16 szAcUtlSztFillUplnkNasTpt(tcCb, spCb, pdu)
CmXtaTCCb    *tcCb;
CmXtaSpCb    *spCb;
S1apPdu      **pdu;
#endif
...
#else
PUBLIC S16 szAcUtlSztFillUplnkNasTpt(pdu)
S1apPdu      **pdu;
#endif
#endif   /*SZ_ENC_DEC */
{
   ...
   TRC2(szAcUtlSztFillUplnkNasTpt);
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillUplnkNasTpt: cmAllocEvnt failed, line %d\n",__LINE__));
#endif
      RETVALUE(RFAILED);
   }
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillUplnkNasTpt: cmGetMem failed, line %d\n",__LINE__));
#endif
      RETVALUE(RFAILED);
   }
   ...
   cmMemset(tptAddr, 0, sizeof(tptAddr));
   cmMemcpy((char *)tptAddr,"9999",sizeof(tptAddr));
   len = 4;
...
} /* End of szAcUtlSztFillUplnkNasTpt */

/*
*
*       Fun:   szAcUtlSztFillDwnlnkNasTpt
*
...
#ifndef SZ_ENC_DEC
#ifdef ANSI
PUBLIC S16 szAcUtlSztFillDwnlnkNasTpt
(
CmXtaTCCb    *tcCb,
CmXtaSpCb    *spCb,
S1apPdu      **pdu
...
#else
PUBLIC S16 szAcUtlSztFillDwnlnkNasTpt(tcCb, spCb, pdu)
CmXtaTCCb    *tcCb;
CmXtaSpCb    *spCb;
S1apPdu      **pdu;
#endif
...
#else
PUBLIC S16 szAcUtlSztFillDwnlnkNasTpt(pdu)
S1apPdu      **pdu;
#endif
#endif   /* SZ_ENC_DEC */
{
...
#endif
   TRC2(szAcUtlSztFillDwnlnkNasTpt);
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillDwnlnkNasTpt: cmAllocEvnt failed, line %d\n",__LINE__));
#endif
      RETVALUE(RFAILED);
   }
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillDwnlnkNasTpt: cmGetMem failed, line %d\n",__LINE__));
#endif
      RETVALUE(RFAILED);
   }
...
} /* End of szAcUtlSztFillDwnlnkNasTpt */
...
#ifndef SZ_ENC_DEC
#ifdef ANSI
PUBLIC S16 szAcUtlSztFillPaging
(
CmXtaTCCb  *tcCb,
CmXtaSpCb  *spCb,
S1apPdu    **pdu
...
#else
PUBLIC S16 szAcUtlSztFillPaging(tcCb, spCb, pdu)
CmXtaTCCb  *tcCb;
CmXtaSpCb  *spCb;
S1apPdu    **pdu;
#endif
...
#else
PUBLIC S16 szAcUtlSztFillPaging(pdu)
S1apPdu    **pdu;
#endif
#endif /* SZ_ENC_DEC */
{
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillPaging: cmAllocEvnt failed,line %d\n",__LINE__));
#endif /* SZ_ENC_DEC */
      RETVALUE(RFAILED);
   }
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillPaging: cmGetMem failed, line %d\n",__LINE__));
#endif /* SZ_ENC_DEC */
      RETVALUE(RFAILED);
   }
...
/*
*
*       Fun:   szAcUtlSztFillE_RABSetupRqst 
*
...
#ifndef SZ_ENC_DEC
#ifdef ANSI
PUBLIC S16 szAcUtlSztFillE_RABSetupRqst
(
CmXtaTCCb  *tcCb,
CmXtaSpCb  *spCb,
S1apPdu    **pdu
...
#else
PUBLIC S16 szAcUtlSztFillE_RABSetupRqst(tcCb, spCb, pdu)
CmXtaTCCb  *tcCb;
CmXtaSpCb  *spCb;
S1apPdu    **pdu;
#endif
...
#else
PUBLIC S16 szAcUtlSztFillE_RABSetupRqst(pdu)
S1apPdu    **pdu;
#endif
#endif /* SZ_ENC_DEC */
{
...

   {
      ...
      {
#ifndef SZ_ENC_DEC
         CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillE_RABSetupRqst: cmGetMem failed, line %d\n",__LINE__));
#endif /* SZ_ENC_DEC */
         RETVALUE(RFAILED);
      }
...
#ifndef SZ_ENC_DEC
#ifdef ANSI
PUBLIC S16 szAcUtlSztFillHovrRqst
(
CmXtaTCCb  *tcCb,
CmXtaSpCb  *spCb,
S1apPdu    **pdu
...
#else
PUBLIC S16 szAcUtlSztFillHovrRqst(tcCb, spCb, pdu)
CmXtaTCCb  *tcCb;
CmXtaSpCb  *spCb;
S1apPdu    **pdu;
#endif
...
#else
PUBLIC S16 szAcUtlSztFillHovrRqst(pdu)
S1apPdu    **pdu;
#endif
#endif /* SZ_ENC_DEC */
{
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillHovrRqst: cmAllocEvnt failed, line %d\n",__LINE__));
#endif /* SZ_ENC_DEC */
      RETVALUE(RFAILED);
   }
...

   {
      ...
      {
#ifndef SZ_ENC_DEC
         CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillHovrRqst:: cmGetMem failed, line %d\n",__LINE__));
#endif /* SZ_ENC_DEC */
         RETVALUE(RFAILED);
      }
...
#ifndef SZ_ENC_DEC
#ifdef ANSI
PUBLIC S16 szAcUtlSztFillPathSwRqst
(
CmXtaTCCb  *tcCb,
CmXtaSpCb  *spCb,
S1apPdu    **pdu
...
#else
PUBLIC S16 szAcUtlSztFillPathSwRqst(tcCb, spCb, pdu)
CmXtaTCCb  *tcCb;
CmXtaSpCb  *spCb;
S1apPdu    **pdu;
#endif
...
#else
PUBLIC S16 szAcUtlSztFillPathSwRqst( pdu)
S1apPdu    **pdu;
#endif
#endif /* SZ_ENC_DEC */
{
   ...
   {
#ifndef SZ_ENC_DEC
      CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillPathSwRqst: cmAllocEvnt failed, line %d\n",__LINE__));
#endif /* SZ_ENC_DEC */
      RETVALUE(RFAILED);
   }
...

   {
      ...
      {
#ifndef SZ_ENC_DEC
         CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillPathSwRqst:: cmGetMem failed, line %d\n",__LINE__));
#endif /* SZ_ENC_DEC */
         RETVALUE(RFAILED);
      }
...
#ifndef SZ_ENC_DEC
#ifdef ANSI
PUBLIC S16 szAcUtlSztFillTraceStart
(
CmXtaTCCb  *tcCb,
CmXtaSpCb  *spCb,
S1apPdu    **pdu
...
#else
PUBLIC S16 szAcUtlSztFillTraceStart(tcCb, spCb, pdu)
CmXtaTCCb  *tcCb;
CmXtaSpCb  *spCb;
S1apPdu    **pdu;
#endif
...
#else
PUBLIC S16 szAcUtlSztFillTraceStart(pdu)
S1apPdu     **pdu;
#endif
#endif   /* SZ_ENC_DEC */
{
...

   {
      ...
      {
#ifndef SZ_ENC_DEC
         CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillTraceStartPdu: cmGetMem failed, line %d\n",__LINE__));
#endif
         RETVALUE(RFAILED);
      }
...
/*
*
*       Fun:   szAcUtlSztFillOverloadStart
*
...
#ifndef SZ_ENC_DEC
#ifdef ANSI
PUBLIC S16 szAcUtlSztFillOverloadStart
(
CmXtaTCCb  *tcCb,
CmXtaSpCb  *spCb,
S1apPdu    **pdu
...
#else
PUBLIC S16 szAcUtlSztFillOverloadStart(tcCb, spCb, pdu)
CmXtaTCCb  *tcCb;
CmXtaSpCb  *spCb;
S1apPdu    **pdu;
#endif
...
#else
PUBLIC S16 szAcUtlSztFillOverloadStart(pdu)
S1apPdu     **pdu;
#endif
#endif   /* SZ_ENC_DEC */
{
   ...
   TRC2(szAcUtlSztFillOverloadStart);
...

   {
      ...
      {
#ifndef SZ_ENC_DEC
         CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillOverloadStart: cmGetMem failed, line %d\n",__LINE__));
#endif
         RETVALUE(RFAILED);
      }
...
/*
*
*       Fun:   szAcUtlSztFillOverloadStop
*
...
#ifndef SZ_ENC_DEC
#ifdef ANSI
PUBLIC S16 szAcUtlSztFillOverloadStop
(
CmXtaTCCb  *tcCb,
CmXtaSpCb  *spCb,
S1apPdu    **pdu
...
#else
PUBLIC S16 szAcUtlSztFillOverloadStop(tcCb, spCb, pdu)
CmXtaTCCb  *tcCb;
CmXtaSpCb  *spCb;
S1apPdu    **pdu;
#endif
...
#else
PUBLIC S16 szAcUtlSztFillOverloadStop(pdu)
S1apPdu     **pdu;
#endif
#endif   /* SZ_ENC_DEC */
{
...
#endif
   TRC2(szAcUtlSztFillOverloadStop);
...

   {
	 ...
	 if (cmGetMem(eNBConfigTfrPdu, (1 * sizeof(SztProtExtnField_SONConfigTfr_ExtIEs)), 
	   ...
	   (Ptr *)&(ie4->extensionValue.u.sztX2TNLConfigInfo.eNBX2TportLyrAddres.member)) !=ROK)
      {
       ...
       cmMemset(tptAddr, 0, sizeof(tptAddr));
       cmMemcpy((char *)tptAddr,"1111",sizeof(tptAddr));
       len = 4;
	...
	if (cmGetMem(eNBConfigTfrPdu, (1 * sizeof(SztENBX2ExtTLA)), 
			   (Ptr *)&(ie5->extensionValue.u.sztENBX2ExtTLAs.member)) !=ROK)
      {
       ...
       cmMemset(tptAddr, 0, sizeof(tptAddr));
       cmMemcpy((char *)tptAddr,"1122",sizeof(tptAddr));
			   ...
			   (Ptr *)&(ie6->gTPTLAa.member)) !=ROK)
      {
	   ...
	   cmMemset(tptAddr, 0, sizeof(tptAddr));
       cmMemcpy((char *)tptAddr,"1231",sizeof(tptAddr));
       len = 4;
...
#ifndef SZ_ENC_DEC
#ifdef ANSI
PUBLIC S16 szAcUtlSztFillCellTrafficTrace
(
CmXtaTCCb  *tcCb,
CmXtaSpCb  *spCb,
S1apPdu    **pdu
...
#else
PUBLIC S16 szAcUtlSztFillCellTrafficTrace(tcCb, spCb, pdu)
CmXtaTCCb  *tcCb;
CmXtaSpCb  *spCb;
S1apPdu    **pdu;
#endif
...
#else
PUBLIC S16 szAcUtlSztFillCellTrafficTrace(pdu)
S1apPdu     **pdu;
#endif
#endif   /* SZ_ENC_DEC */
{
   ...
   TRC2(szAcUtlSztFillCellTrafficTrace);
   ...
   cmXtaXmlGetVal(tcCb, spCb->childStep, CMXTA_ATTRIB_U8,"critBug",&(critBug));
      ...
      {
#ifndef SZ_ENC_DEC
         CMXTA_DBG_ERR((_cmxtap,"szAcUtlSztFillCellTrafficTrace: cmGetMem failed, line %d\n",__LINE__));
#endif
         RETVALUE(RFAILED);
      }
...
#endif
{
   ...
   switch(eventType)
   {
      ...
      case SZT_MSG_ID_SETUP_RSP:
           szAcUtlSztFillS1SetupRsp(tcCb, spCb, &(sztEncDecEvnt->u.pdu));
           break;
      ...
      case SZT_MSG_ID_INIT_UE_MSG:
           szAcUtlSztFillInitUeMsg(tcCb, spCb, &(sztEncDecEvnt->u.pdu));
           break;

      case SZT_MSG_ID_CXT_SETUP_REQ:
           szAcUtlSztFillInitCxtSetupReq(tcCb, spCb, &(sztEncDecEvnt->u.pdu));
           break;
      ...
      case SZT_MSG_ID_UL_NAS_TPT:
           szAcUtlSztFillUplnkNasTpt(tcCb, spCb, &(sztEncDecEvnt->u.pdu));
           break;

      case SZT_MSG_ID_DL_NAS_TPT:
           szAcUtlSztFillDwnlnkNasTpt(tcCb, spCb, &(sztEncDecEvnt->u.pdu));
           break;
      ...
      case SZT_MSG_ID_UE_CXT_REL_REQ:
           szAcUtlSztFillUeCxtRlsReq(tcCb, spCb, &(sztEncDecEvnt->u.pdu));
           break;
      ...
      case SZT_MSG_ID_PAGING:
           szAcUtlSztFillPaging(tcCb, spCb, &(sztEncDecEvnt->u.pdu));
           break;
      ...
      case SZT_MSG_ID_E_RAB_BRR_SETUP_REQ:
           szAcUtlSztFillE_RABSetupRqst(tcCb, spCb, &(sztEncDecEvnt->u.pdu));
           break;
      ...
      case SZT_MSG_ID_HO_REQ:
           szAcUtlSztFillHovrRqst(tcCb, spCb, &(sztEncDecEvnt->u.pdu));
           break;

      case SZT_MSG_ID_PATH_SWTCH_REQ:
           szAcUtlSztFillPathSwRqst(tcCb, spCb, &(sztEncDecEvnt->u.pdu));
           break;
      ...
      case SZT_MSG_ID_TRACE_START:
           szAcUtlSztFillTraceStart(tcCb, spCb, &(sztEncDecEvnt->u.pdu));
           break;
      ...
      case SZT_MSG_ID_OVLD_START:
           szAcUtlSztFillOverloadStart(tcCb, spCb, &(sztEncDecEvnt->u.pdu));
           break;

      case SZT_MSG_ID_OVLD_STOP:
           szAcUtlSztFillOverloadStop(tcCb, spCb, &(sztEncDecEvnt->u.pdu));
           break;
...
#ifdef S1AP_REL11IE
   SztProtExtnField_LastVisitedEUTRANCellInform_ExtIEs *ie2;
   U32 timeUEStayedInCellEnhancedGranularity = 0;
#endif
...
#else
            szFillTknU16(&(ie1->val.e_UTRAN_Cell.iE_Extns.noComp), 1);

	    if (cmGetMem(iePdu, (1 * sizeof(SztProtExtnField_LastVisitedEUTRANCellInform_ExtIEs)), 
			   (Ptr *)&(ie1->val.e_UTRAN_Cell.iE_Extns.member)) !=ROK)
           {
#ifndef SZ_ENC_DEC
               CMXTA_DBG_ERR((_cmxtap,"iePdu: cmGetMem failed, line %d\n", __LINE__));
#endif
               RETVALUE(RFAILED);
           }

	    ie2 = &(ie1->val.e_UTRAN_Cell.iE_Extns.member[0]);
		   
            szFillTknU8(&(ie2->pres), PRSNT_NODEF);
            szFillTknU32(&(ie2->id), Sztid_Time_UE_StayedInCell_EnhancedGranularity);
            szFillTknU32(&(ie2->criticality), SztCriticalityignoreEnum);
            timeUEStayedInCellEnhancedGranularity = 1;
#ifndef SZ_ENC_DEC			
	cmXtaXmlGetVal(tcCb, spCb->childStep, CMXTA_ATTRIB_U32,"timeUEStayedInCellEnhancedGranularity", &(timeUEStayedInCellEnhancedGranularity));
#endif
           szFillTknU32(&(ie2->extensionValue.u.sztTime_UE_StayedInCell_EnhancedGranularity), timeUEStayedInCellEnhancedGranularity);
#endif
         }
         break;
...
/********************************************************************30**
         ...
         End of file:     szac_sztutl.c@@/main/4 - Mon Jan 10 22:16:04 2011
