...
#endif
{
   ...
   if( szCb.genSta.nmbPeer >= szCb.genCfg.nmbPeer )
   {
      ...
      SZLOGERROR(ERRCLS_ADD_RES, ESZ060, (ErrVal) 0,
   ...
   if ((ret = szNdbAllocDynamPeerCb(suId, &peerCb)) != ROK)
   {
      ...
      SZLOGERROR(ERRCLS_INT_PAR, ESZ061, (ErrVal) 0,
       "szLiDynamPeerConfig: Failed to allocate control block for dynamic peer\n");
#endif /* if ERRCLASS */
   ...
   cmMemcpy((U8 *)peerCb->assocParams,(U8 *)assocParams, sizeof (SctAssocIndParams));
   ...
   {
#if (ERRCLASS & ERRCLS_INT_PAR)
      SZLOGERROR(ERRCLS_INT_PAR, ESZ062, (ErrVal) 0,
            "szLiDynamPeerConfig: Failed to insert in the dynamic peerAssoc hash list\n");
#endif /* if ERRCLASS */
