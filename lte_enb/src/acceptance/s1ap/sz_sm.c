...
#endif
{
   ...
   {
      szGetIE(&(pdu->pdu), Sztid_eNB_UE_S1AP_ID, &eNbId);
      if((eNbId != NULLP) && ((SztENB_UE_S1AP_ID *)eNbId)->val != conCb->rmtConRefNo)
      {
         /* Error Indication to the user */
         SZ_SEND_LCL_ERR_IND(PRSNT_NODEF, conCb->cd.spConnId, 
               peerCb->sztSapCb, SZT_CAUSE_INV_CONID, ((SztENB_UE_S1AP_ID *)eNbId)->val);
         SZ_FREE_SDU(pdu);
         RETVALUE(ret);
      }
   }
...
#endif
{
...
#if (ERRCLASS & ERRCLS_INT_PAR)
      SZLOGERROR(ERRCLS_INT_PAR, ESZ145, (ErrVal) conCb->lclConRefNo,
   ...
   if(ret == RFAILED)
   {
      SZLOGERROR(ERRCLS_DEBUG, ESZ146, (ErrVal) conCb->lclConRefNo,
...
#endif
{
   ...
   if(peerCb->nodeType == LSZ_NODE_ENB)
   {
      ...
      if (eNbS1apId == NULLP)
      {
         ...
         if (ueS1apId != NULLP)
         {
            switch (ueS1apId->choice.val)
            {
               case UE_S1AP_IDS_UE_S1AP_ID_PAIR:
               {
                  eNbS1apId = (TknU8*)&ueS1apId->val.uE_S1AP_ID_pair.eNB_UE_S1AP_ID;
                  szErrCause = SztCauseRadioNwunknown_pair_ue_s1ap_idEnum;
                  break;
               }
            }
      ...
      if ((eNbS1apId == NULLP) || (((SztENB_UE_S1AP_ID *)eNbS1apId)->val != conCb->rmtConRefNo))
      {
         szUtilsGetMsgIdx(&evnt, pdu);
...
#endif
{
   ...
   if(peerCb->nodeType == LSZ_NODE_ENB)
   {
      ...
      {
#if (ERRCLASS & ERRCLS_INT_PAR)
            SZLOGERROR(ERRCLS_INT_PAR, ESZ152, (ErrVal)ret,
                  "szSmIdleStPeerErr: Lower Interface send failed");
#endif
      }
   }
...
#endif
{
   ...
   if(ret == RFAILED)
   {
      SZLOGERROR(ERRCLS_DEBUG, ESZ156, (ErrVal) conCb->lclConRefNo,
