...
/********************************************************************20**
     ...
     Sid:      czac_sctutl_r11.c@@/main/TeNB_Main_BR/tenb_main_ccb/1 - Wed Jul 22 20:08:32 2015
...
#include "czac_sct.x"
#include "czac_czt.x"
EXTERN CmPAsnElmDef *czMsgDb_r11[];
 ...
 */
#ifdef ANSI
PUBLIC S16  czAcUtlBuildX2APPdu_r11
(
...
#else /* ANSI */
PUBLIC S16  czAcUtlBuildX2APPdu_r11 (tcCb, spCb,mBuf)
CmXtaTCCb    *tcCb;    /* tcCb data structure  */
CmXtaSpCb    *spCb;    /* spCb data structure  */
Buffer       **mBuf
#endif /* ANSI */
{
   ...
   switch(eventType)
   {
      case CZT_MSG_X2_SETUP:
           czAcUtlCztFillX2SetupReq_R11(tcCb, spCb, (&cztEvent));
           break;
      ...
      case CZT_MSG_HO_REQ:
           czAcUtlCztFillHovrRqst_R11(tcCb, spCb, (&cztEvent));
           break;
      ...
      case CZT_MSG_RSRC_ST_REQ:
            czAcUtlCztFillResStatusRqst_R11(tcCb, spCb, &cztEvent);
            break;

      case CZT_MSG_RSRC_ST_UPD:
            czAcUtlCztFillResStatusUpd_R11(tcCb, spCb, &cztEvent);
            break;

      case CZT_MSG_RSRC_ST_RSP:
            czAcUtlCztResStatusResp_R11(tcCb, spCb, &cztEvent);
            break;

      case CZT_MSG_RSRC_ST_FAIL:
            czAcUtlCztFillResStatusFail_R11(tcCb, spCb, &cztEvent);
            break;

      case CZT_MSG_LOAD_IND:
            czAcUtlCztFillLoadInd_R11(tcCb, spCb, &cztEvent);
            break;
      ...
      case CZT_MSG_MOB_CHANGE_REQ:
       ...
       case CZT_MSG_MOB_CHANGE_FAIL:
        ...
        case CZT_MSG_RLF_INDICATION:
         {
            czAcUtlCztFillRlfInd_R11(tcCb, spCb, &cztEvent);
            break;
         }
      ...
      ret = cmPAsnEncMsg((TknU8 *)(&(cztEvent->pdu)), CM_PASN_USE_MBUF, 
            ...
            (CmPAsnElmDef **)czMsgDb_r11, &asnErr, 
            &len, dbgFlag, FALSE );
      if (ret == RFAILED)
      {
         /* It's an ASN.1 Error */
         ...
         if (cztEvent != NULLP)
         {
            CMXTA_DBG_INFO((_cmxtap, "czAcUtlBuildX2APPdu_r11(): Freeing the cztEvent\n"));
...
/**********************************************************************
         ...
         End of file:     czac_sctutl_r11.c@@/main/TeNB_Main_BR/tenb_main_ccb/1 - Wed Jul 22 20:08:32 2015
