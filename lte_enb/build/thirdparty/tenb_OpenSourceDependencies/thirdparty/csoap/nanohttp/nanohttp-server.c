...
httpd_init(int argc, char *argv[])
{
  ...
  if(_httpd_tos != 0)
  {
    return hsocket_bind_with_tos(&_httpd_socket, _httpd_address, _httpd_port, _httpd_tos);
  }
  else
  {
    return hsocket_bind(&_httpd_socket, _httpd_address, _httpd_port);
  }
}
