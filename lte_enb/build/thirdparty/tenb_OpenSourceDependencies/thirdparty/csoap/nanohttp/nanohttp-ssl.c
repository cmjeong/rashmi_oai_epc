...
_hssl_parse_arguments(int argc, char **argv)
{
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    else if(!strcmp(argv[i - 1], NHTTP_ARG_KEY))
    {
      key = argv[i];
    }
...
_hssl_server_context_init(void)
{
  ...
  if (!(SSL_CTX_use_PrivateKey_file(context, key, SSL_FILETYPE_PEM)))
  {
    log_error2("Cannot read key file: \"%s\"", key);
    SSL_CTX_free(context);
...
/*  log_verbose4("sock->sock=%d, sock->ssl=%p, len=%" PRIi32 "", sock->sock, sock->ssl, len); */
