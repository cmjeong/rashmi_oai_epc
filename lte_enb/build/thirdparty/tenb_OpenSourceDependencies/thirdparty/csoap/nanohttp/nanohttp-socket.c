...
hsocket_open(hsocket_t * dsock, const char *hostname, int port, int ssl)
{
  ...
  if (ssl)
  {
    ...
    if ((status = hssl_client6_ssl(dsock)) != H_OK)
    {
...
/*--------------------------------------------------
FUNCTION: hsocket6_open
----------------------------------------------------*/
herror_t hsocket6_open(hsocket6_t *dsock, const char *hostname, int port, int ssl)
{
  struct sockaddr_in6 address;
  struct hostent *host;
  char ip[INET6_ADDRSTRLEN];

  if ((dsock->sock = socket(AF_INET6, SOCK_STREAM, 0)) <= 0)
    return herror_new("hsocket_open", HSOCKET_ERROR_CREATE,
                      "Socket error (%s)", strerror(errno));
  ...
  /* set server addresss */
  address.sin6_family = host->h_addrtype;
...
----------------------------------------------------*/
herror_t
hsocket_bind_with_tos(hsocket_t * dsock, uint32_t address, int port, int tos)
{
  hsocket_t sock;
  struct sockaddr_in addr;
  int opt = 1;
  ...
  int retVal = setsockopt(sock.sock, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));

  // NOTE: the following hugly code ensures tos value to be of correct type
  // when removing this code remember to remove TOSValues.h too
  if ( tos == 0 )
  {
      tos = QOS_VALUE; //QOS_CLASS_E;
  }
  if(setsockopt(sock.sock, SOL_IP, IP_TOS, &tos, ((socklen_t)sizeof(tos)) ) != 0)
  {
    log_error2("Error setting setsockopt for TOS (%s)", strerror(errno));
    return herror_new("hsocket_bind", HSOCKET_ERROR_CREATE,
...
_hsocket_sys_accept(hsocket_t * sock, hsocket_t * dest)
{
  ...
  if(setsockopt(dest->sock, SOL_IP, IP_TOS, &tos, ((socklen_t)sizeof(tos)) ) != 0)
  {
      log_warn2("nanohttp-socket: setsockopt on accept failed (%s)", strerror(errno));
  }
