...
/**
  ...
  @see hsocket_listen
 */
  herror_t hsocket_bind(hsocket_t * sock, uint32_t address, int port);


/**
  Binds a socket to a given port number. After bind you
  can call hsocket_listen() to listen to the port.
  As per hsocket_bind but with additional QoS ToS parameter.
  ...
  @see hsocket_listen
 */
  herror_t hsocket_bind_with_tos(hsocket_t * sock, uint32_t address, int port, int tos);
