...
#endif /* ANSI */
{
   ...
   VB_MME_DBG_INFO((VB_MME_PRNTBUF, "suId(%d), sztLclErrInd->suConnId(%d), sztLclErrInd->spConnId(%d), \
            sztLclErrInd->transId(%d), sztLclErrInd->causeType (%d), sztLclErrInd->causeValue (%d)", suId, sztLclErrInd->suConnId.val, \
             sztLclErrInd->spConnId.val, sztLclErrInd->transId.val, sztLclErrInd->causeType, sztLclErrInd->causeValue));
   ...
   switch(uDatEvnt->pdu->pdu.choice.val)
   {
      case S1AP_PDU_INITIATINGMSG:
      {
         switch(uDatEvnt->pdu->pdu.val.initiatingMsg.procedureCode.val)
         {
            ...
            default:
            {
               VB_MME_DBG_ERROR((VB_MME_PRNTBUF, "Unhandled Procedure Code: %u",
                               uDatEvnt->pdu->pdu.val.initiatingMsg.procedureCode.val));
   ...
   if (nasPduPres == TRUE)
   {
      ...
      if (vbEvnt->secHT != CM_NAS_SEC_HDR_TYPE_PLAIN_NAS_MSG)
      {
         ...
         if(ROK != ret)
         {
            ...
            vbEvnt->pdu = NULLP;
				/*Federated memleak fixes ccpu00150405*/
            CM_FREE_NASEVNT(&vbEvnt);
            EDM_FREE(nasPdu->val, nasPdu->len);
            RETVALUE(ret);
         }
   ...
   switch(msgType)
   {
      ...
      case Sztid_E_RABRls:
      {
         ...
         VB_MME_DBG_INFO((VB_MME_PRNTBUF,"Sending E-RAB Release Cmd With "\
               "DeAct Ded Bearer Req for BearerId: %d.\n",esmCb->bId));
