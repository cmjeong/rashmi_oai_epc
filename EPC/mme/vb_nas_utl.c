...
#endif
{
   ...
   if(ROK != ret)
   {
      ...
      {
         ret  = vbMmeSndAttachReject(ueCb, CM_EMM_CONGESTION, 0);
      }
      RETVALUE(ret);
   }
...
)
{
   ...
   if (VB_MME_CHK_MASK(pUeCb->bitmask, VB_EMM_HO_WITH_MME_CHANGE))
   {
      if (VB_MME_HO_TGT_MME == pUeCb->pHoData->hoMmeType)
      {
         ...
         {
            VB_MME_DBG_ERROR((VB_MME_PRNTBUF, "Invalid Handover Procedure"));
         }
      ...
      else
      {
         ...
         {
             if ((NULLP != pUeCb->pUeS3TunCb) && \
               (pUeCb->pUeS3TunCb->remoteTeId != 0))
            {
               VB_MME_DBG_INFO((VB_MME_PRNTBUF, "Sending Relocation Cancel "\
                     "Request to target SGSN"));
   ...
   {
      VB_MME_DBG_INFO((VB_MME_PRNTBUF,"Aborting handover in case of no mme change\n"));
      VB_MME_END_EMM_PROC(pUeCb);
      if(pUeCb->pHoData->pNewUeCb != NULLP)
      {
         pUeCb->ueTunnelCb->ueCb = pUeCb;
         vbMmeUtlDelHOUeCb(pUeCb->pHoData->pNewUeCb);
      }
   }
