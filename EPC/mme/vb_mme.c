...
#endif
{
      ...
      (VB_MME_ESM_PROC == esmCb->ueCb->proc) || ( VB_MME_PDN_CONN_PROC == esmCb->ueCb->proc )))
   {
      ...
      if(VB_MME_ATTACH_PROC == esmCb->ueCb->proc)
      {
         ...
         {
#ifdef CM_NAS_SEC
            if((evnt->m.esmEvnt != NULLP) && (evnt->m.esmEvnt->u.infoRsp.apn.pres == TRUE))
            {
                esmCb->apn.pres = TRUE;
                cmMemset(esmCb->apn.apn,0, CM_ESM_MAX_LEN_ACCESS_PTNAME);
                if(evnt->m.esmEvnt->u.infoRsp.apn.len <= CM_ESM_MAX_LEN_ACCESS_PTNAME)
                {
                   cmMemcpy((U8* )&esmCb->apn.apn, (U8* )&evnt->m.esmEvnt->u.infoRsp.apn.apn, evnt->m.esmEvnt->u.infoRsp.apn.len );
                   esmCb->apn.len = evnt->m.esmEvnt->u.infoRsp.apn.len;
                   esmCb->apn.apn[esmCb->apn.len] = '\0';
                }
                else
                {
                   cmMemcpy((U8* )&esmCb->apn.apn, (U8* )&evnt->m.esmEvnt->u.infoRsp.apn.apn, CM_ESM_MAX_LEN_ACCESS_PTNAME );
                   esmCb->apn.len = CM_ESM_MAX_LEN_ACCESS_PTNAME;
                }
                for(i = 0; i < VB_HSS_MAX_PDN_SUB_CTXT; i++)
                {
                   if(esmCb->ueCb->ueCtxt.ueHssCtxt.subCntxt[i].subCntxtinUse == FALSE)
                   {
                      cmMemcpy((U8* )&esmCb->ueCb->ueCtxt.ueHssCtxt.subCntxt[i].apn, (U8* )&esmCb->apn.apn, esmCb->apn.len);
                      esmCb->ueCb->ueCtxt.ueHssCtxt.subCntxt[i].subCntxtinUse = TRUE;
                      esmCb->ueCb->ueCtxt.ueHssCtxt.subCntxt[i].apn[esmCb->apn.len] = '\0';
                      break;
                   }
                }
            }
...
#endif
{
   ...
   VB_MME_DBG_INFO((VB_MME_PRNTBUF, "ueCb->proc: %d", ueCb->proc));
   switch(ueCb->proc)
   {
       case VB_MME_DETACH_NW_PROC:
       {
...
)
{
   ...
   if (ROK != vbMmeUtlFndEnbCb(&enbCb, peerId))
   {
      VB_MME_DBG_ERROR((VB_MME_PRNTBUF, "Unexpected eNodeB Cb PeerId(%u)",\
            peerId));
      RETVALUE(ret);
   }
