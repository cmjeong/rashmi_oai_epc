...
#endif
{
   ...
   t3.val = VB_MME_EGT_T3_RESP_TMR;
#ifdef ALIGN_64BIT
   VB_MME_DBG_INFO((VB_MME_PRNTBUF, "DEBUGS: Sending CBRSP transId(%d)", esmCb->tranId.val));
#else
   VB_MME_DBG_INFO((VB_MME_PRNTBUF, "DEBUGS: Sending CBRSP transId(%ld)", esmCb->tranId.val));
#endif
...
#ifdef ALIGN_64BIT
   VB_MME_DBG_INFO((VB_MME_PRNTBUF, "DEBUGS: CBReq TransIdVal(%d)",transId.val));
#else
   VB_MME_DBG_INFO((VB_MME_PRNTBUF, "DEBUGS: CBReq TransIdVal(%ld)",transId.val));
#endif
   /*DEDICATED BEARER*/
   if(egMsg != NULLP)
   {
      tunEnt->seqNum = egMsg->msgHdr.seqNumber;
      tunEnt->ueCb->proc = VB_MME_NULL_PROC;
...
#endif
{
   ...
   for (indx = lastPdnId; indx < CM_ESM_MAX_BEARER_ID; indx++)
   { 
      if (NULLP != ueCb->esmList[indx])
      {
         ret = vbMmeSndMBReq(ueCb, ueCb->esmList[indx], FALSE);
         if (ret == ROK)
         {
            RETVALUE(ROK);
         }
...
)
{
    ...
    VB_MME_DBG_INFO((VB_MME_PRNTBUF, "Getting ESM CB TransId [%u]",\
                pTransId->val));
    ret = vbMmeUtlGetEsmCbOnS11TranId(pUeCb, &pEsmCb, pTransId);
    if (ROK == ret)
    {
        ...
        else
        {
            ...
            {
              ret = vbMmeUtlModPdnConn(pUeCb, pEsmCb->bId-CM_ESM_BEARER_ID_INDX+1);
              if(ROK == ret)
              {
              ...
              /* If all the MB Rsp for this UE has come, then do HO clean up*/
    ...
    {
        VB_MME_DBG_ERROR((VB_MME_PRNTBUF, "Getting ESM CB TransId [%u]",\
                    pTransId->val));
    }
