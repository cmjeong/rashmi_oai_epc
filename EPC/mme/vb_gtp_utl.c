 ...
 *    otherwise FAILURE
 *****************************************************************************/
PUBLIC S16 vbMmeEgtUtlAddEUStaTfrTransCont
(
EgMsg             *pEgMsg,
VbMmeEUStaTfrTransCont *pEuTransCont
)
{
   S16               ret = ROK;
   EgIe              egIe;
   ...
   VB_TRC2(vbMmeEgtUtlAddEUStaTfrTransCont);

   cmMemset((U8 *)&egIe, 0, sizeof(EgIe));
   VB_MME_SET_IE_LVL_INFO(ieLvlInfo, 0, EG_F_CONTAINER_IETYPE, 0);
   egIe.t.fContainer.containerType = 3; /* EUTRAN Transparent container */
   egIe.t.fContainer.fContainer.length = pEuTransCont->itm[0].len;
   cmMemcpy(egIe.t.fContainer.fContainer.val,\
         pEuTransCont->itm[0].pVal, pEuTransCont->itm[0].len);
   egUtlEgtBuildEgIeHeader(&egIe, EG_F_CONTAINER_IETYPE, 0, EG_F_CONT);

   ret = EgUtilGmAppendIe(pEgMsg, &ieLvlInfo, &egIe);

   RETVALUE(ret);
} /* vbMmeEgtUtlAddEUStaTfrTransCont */
