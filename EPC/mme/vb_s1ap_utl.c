...
#endif
{
    ...
    if(rspMember != NULLP)
    {
        ...
        */
       if (((ueCb->proc == VB_MME_PDN_CONN_PROC) || (ueCb->proc == VB_MME_ESM_PROC)) && 
           ((esmCb->proc == VB_MME_ACT_DEF_BEAR_PROC) || (esmCb->proc == VB_MME_ACT_DED_BEAR_PROC)))
       {
          if ( ueCb->rxd_dfltorded_bearer_acc)
          {
             if(esmCb->proc == VB_MME_ACT_DEF_BEAR_PROC)
             {
             ...
             ueCb->rxd_dfltorded_bearer_acc = FALSE;
          }
          else
          {
...
#endif
{
   ...
   {
      VB_MME_DBG_ERROR((VB_MME_PRNTBUF,"No esmCb found for Bearer Id(%d)", bearerId));
      vbMmeSendDeleteBearerCmd(esmCb); 
...
)
{
  ...
  if (VB_MME_CHK_MASK(pUeCb->bitmask, VB_EMM_HO_WITH_MME_CHANGE))
   ...
   else
   {
      ...
      pOldUeCb->pHoData->numRabs = pUeCb->pHoData->numRabs;
      for(i = 0; i< pUeCb->pHoData->numRabs ; i++)
      {
         pOldUeCb->pHoData->rabInfo[i].dlTransportLyrAddr = pUeCb->pHoData->\
                                                            rabInfo[i].dlTransportLyrAddr;
         pOldUeCb->pHoData->rabInfo[i].dlGtpTeid = pUeCb->pHoData->rabInfo[i].dlGtpTeid;
         pOldUeCb->pHoData->rabInfo[i].ulTransportLyrAddr = pUeCb->pHoData->rabInfo[i].\
                                                            ulTransportLyrAddr;
         pOldUeCb->pHoData->rabInfo[i].ulGtpTeid = pUeCb->pHoData->rabInfo[i].ulGtpTeid;
         pOldUeCb->pHoData->rabInfo[i].bId = pUeCb->pHoData->rabInfo[i].bId;
      }
...
)
{
   ...
   {
      pUeCb->pHoData->numRabs = rabIe->noComp.val;
      for(numRab=0; numRab<rabIe->noComp.val; numRab++)
      { 
         for(count=0; count<pUeCb->creBid.nmbBearerId; count++)
         { 
            /* Handling only for default bearer */
            pEsmCb = pUeCb->esmList[count];
      /* Bearer ID */
            if (pEsmCb->bId == rabIe->member[numRab].value.u.sztE_RABAdmtdItem.\
         e_RAB_ID.val)
      {
               pUeCb->pHoData->rabInfo[numRab].bId = pEsmCb->bId;
         /* eNB GTP DL IP Address and Teid */
         VB_MME_DEC_S1AP_IPADDR(pEsmCb->s1EnbAddr,\
                     rabIe->member[numRab].value.u.sztE_RABAdmtdItem.transportLyrAddr);
         VB_MME_DEC_S1AP_TEID(pEsmCb->s1EnbTeId,\
                     rabIe->member[numRab].value.u.sztE_RABAdmtdItem.gTP_TEID);

         /* DL Trasport Layaer address and TEID */
               if (0 != rabIe->member[numRab].value.u.sztE_RABAdmtdItem.dL_gTP_TEID.len)
         {
                  VB_MME_DEC_S1AP_IPADDR(pUeCb->pHoData->rabInfo[numRab].dlTransportLyrAddr,
                        rabIe->member[numRab].value.u.sztE_RABAdmtdItem.\
                  dL_transportLyrAddr);
                  VB_MME_DEC_U32(pUeCb->pHoData->rabInfo[numRab].dlGtpTeid,\
                        rabIe->member[numRab].value.u.sztE_RABAdmtdItem.dL_gTP_TEID.val);
         }

         /* UL Trasport Layaer address and TEID */
               if (0 != rabIe->member[numRab].value.u.sztE_RABAdmtdItem.uL_GTP_TEID.len)
         {
                  VB_MME_DEC_S1AP_IPADDR(pUeCb->pHoData->rabInfo[numRab].ulTransportLyrAddr,
                        rabIe->member[numRab].value.u.sztE_RABAdmtdItem.uL_TportLyrAddr);
                  VB_MME_DEC_U32(pUeCb->pHoData->rabInfo[numRab].ulGtpTeid,\
                        rabIe->member[numRab].value.u.sztE_RABAdmtdItem.uL_GTP_TEID.val);
               }
         }
      }
      }
   ...
   vbMmeUtlGetS1apIE(&pPdu->pdu, Sztid_Tget_ToSrc_TprntCont,\
         (TknU8**)&pSztTransCont);
   if (NULLP == pSztTransCont)
   {
      VB_MME_DBG_ERROR((VB_MME_PRNTBUF, "Retrieving Target to Source Container"));
      RETVALUE(RFAILED);
   }
...
)
{
   ...
   if (ROK == ret)
   {
      ...
      else
      {
          ...
          *                 * to SGW */
         ret = vbMmeUtlModPdnConn(pUeCb,0);
         if (ROK != ret)
         {
...
)
{
  ...
  U8                            nullKenb[CM_EMM_MAX_KENB] = {'\0'};
   ...
   vbUtlFillTknU16(&(ie->value.u.sztE_RABToBeSetupLstHOReq.noComp), pUeCb->creBid.nmbBearerId);
      if ((cmGetMem(pPdu,\
               ((pUeCb->creBid.nmbBearerId) * (sizeof(SztProtIE_SingleCont_E_RABToBeSetupItemHOReqIEs))),\
   ...
   for(count=0; count<pUeCb->creBid.nmbBearerId; count++)
   {
      esmCb = pUeCb->esmList[count];
      for(i = 0; i < VB_HSS_MAX_PDN_SUB_CTXT; i++)
      {
         if(pUeCb->ueCtxt.ueHssCtxt.subCntxt[i].subCntxtinUse == TRUE)
         {
            if(pUeCb->ueCtxt.ueHssCtxt.subCntxt[i].qos.qci == esmCb->epsQos.qci)
            {
               pdnCtxt = &pUeCb->ueCtxt.ueHssCtxt.subCntxt[i]; 
            }
         }
      }
      if(pdnCtxt == NULLP)
      {
         cmFreeMem((Ptr)pPdu);
         VB_MME_LOGERROR(ERRCLS_DEBUG, 1, ERRZERO, "pdnCtxt is NULL\n");
         RETVALUE(RFAILED);
      }
      ie1 = &(ie->value.u.sztE_RABToBeSetupLstHOReq.member[count]);
      ...
      vbUtlFillTknU32(&(ie1->value.u.sztE_RABToBeSetupItemHOReq.
               e_RABlevelQosParams.qCI), esmCb->epsQos.qci); 
      ...
      vbUtlFillTknU32(&(ie1->value.u.sztE_RABToBeSetupItemHOReq.
            ...
            pdnCtxt->qos.arp.priLevel);
      if( pUeCb->pHoData->hoType == VB_EMM_HO_UTRAN_TO_LTE )
      {
         isPrempCap = pdnCtxt->qos.arp.preCapbFlg;
         isPrempVul = pdnCtxt->qos.arp.preVlnbFlg;
      }
      else
      {
    ...
    */
   ie->value.u.sztSecurCntxt.nextHopParameter.pres = NOTPRSNT;
   vbUtlFillTknU32(&(ie->value.u.sztSecurCntxt.nextHopChainingCount),0);
...
/* there are 6 items being filled */
   ...
   vbUtlFillTknU16(&(pPdu->pdu.val.successfulOutcome.value.u.sztHovrCmmd.\
         protocolIEs.noComp), noCmp);
   if ((cmGetMem(&pPdu->memCp, (noCmp * sizeof(SztProtIE_Field_HovrCmmdIEs)), 
   ...
   vbUtlFillTknU32(&(ie->criticality), SztCriticalityignoreEnum);
   vbUtlFillTknU16(&(ie->value.u.sztE_RABSubjtoDataFwdingLst.noComp), pUeCb->pHoData->numRabs);

   if ((cmGetMem(pPdu, (pUeCb->pHoData->numRabs) * (sizeof(SztProtIE_SingleCont_E_RABDataFwdingItemIEs)), 
   ...
   for(i=0; i< pUeCb->pHoData->numRabs; i++)
   {
      for(count=0; count<pUeCb->creBid.nmbBearerId; count++)
      {
         if (NULLP != pUeCb->esmList[count])
         {
            if(pUeCb->esmList[count]->bId == pUeCb->pHoData->rabInfo[i].bId)
            {
               pEsmCb = pUeCb->esmList[count];
            }
         }
      ...
      if(pEsmCb == NULLP) 
      {
         VB_MME_DBG_ERROR(( VB_MME_PRNTBUF, "pEsmCb is NULL\n"));
         RETVALUE(RFAILED);
      }

   /* RAB ID */
      pRabIe = &(ie->value.u.sztE_RABSubjtoDataFwdingLst.member[i]);
   ...
   /* DL Tansport Layer Address */
      VB_MME_ENC_U32(pEsmCb->rncIpAddr.u.ipv4NetAddr, tptAddr);
...
)
{
   ...
   if (ROK == ret)
   {
      ...
      else
      {
         ...
         pNewUeCb->pHoData->staTfrTCont.noComp = pUeCb->pHoData->staTfrTCont.noComp;
         for(i =0; i< pUeCb->pHoData->staTfrTCont.noComp; i++)
         {
            if (pUeCb->pHoData->staTfrTCont.itm[i].len > 0)
               {
               VB_MME_ALLOC(&pNewUeCb->pHoData->staTfrTCont.itm[i].pVal,\
                     pUeCb->pHoData->staTfrTCont.itm[i].len);
               if (NULLP != pNewUeCb->pHoData->staTfrTCont.itm[i].pVal)
               {
                  pNewUeCb->pHoData->staTfrTCont.itm[i].len = \
                                                              pUeCb->pHoData->staTfrTCont.itm[i].len;
                  cmMemcpy(pNewUeCb->pHoData->staTfrTCont.itm[i].pVal,
                        pUeCb->pHoData->staTfrTCont.itm[i].pVal,\
                        pUeCb->pHoData->staTfrTCont.itm[i].len);
...
)
{
   ...
   if(pSztTransCont != NULLP)
   {
      pUeCb->pHoData->staTfrTCont.noComp = pSztTransCont->bearers_SubjToStatusTfrLst.noComp.val;
      for(i =0 ; i< pSztTransCont->bearers_SubjToStatusTfrLst.noComp.val; i++)
      {
         VB_MME_ALLOC(&pUeCb->pHoData->staTfrTCont.itm[i].pVal,\
         sizeof(SztBrs_SubjToStatusTfr_Item));
   /*KWORKS Fix*/
         if ((NULLP != pUeCb->pHoData->staTfrTCont.itm[i].pVal) && (pSztTransCont != NULLP))
   {
            pUeCb->pHoData->staTfrTCont.itm[i].len = sizeof(SztBrs_SubjToStatusTfr_Item);
            cmMemcpy(pUeCb->pHoData->staTfrTCont.itm[i].pVal,
                  (U8 *)&(pSztTransCont->bearers_SubjToStatusTfrLst.member[i].\
   ...
   {
      ret = RFAILED;
      VB_MME_DBG_ERROR((VB_MME_PRNTBUF, "pSztTransCont is NULL\n"));
   }
   RETVALUE(ret); 
...
)
{
   ...
   vbUtlFillTknU32(&(pPdu->pdu.val.initiatingMsg.criticality),\
         SztCriticalityrejectEnum);
   vbUtlFillTknU8(&(pPdu->pdu.val.initiatingMsg.value.u.\
         sztMMEStatusTfr.pres),PRSNT_NODEF);

   numComp = 3;
   vbUtlFillTknU16(&(pPdu->pdu.val.initiatingMsg.value.u.sztMMEStatusTfr.\
         protocolIEs.noComp), numComp);
   if ((cmGetMem(&pPdu->memCp,\
      (numComp * sizeof(SztProtIE_Field_MMEStatusTfrIEs)),
      (Ptr *)&pPdu->pdu.val.initiatingMsg.value.u.
      sztMMEStatusTfr.protocolIEs.member)) != ROK)
   {
      VB_MME_LOGERROR(ERRCLS_DEBUG, 1, ERRZERO, "Memory Allocation"\
   ...
   trCont->bearers_SubjToStatusTfrLst.noComp.val = pUeCb->pHoData->staTfrTCont.noComp;

   if ((cmGetMem(pPdu,\
               (sizeof(SztProtIE_SingleCont_Brs_SubjToStatusTfr_ItemIEs)) * (pUeCb->pHoData->staTfrTCont.noComp),\
   ...
   for(i = 0 ; i< pUeCb->pHoData->staTfrTCont.noComp; i++)
   {
      brsTfrItem = (SztBrs_SubjToStatusTfr_Item *)pUeCb->pHoData->staTfrTCont.itm[i].pVal;

      trItem = &trCont->bearers_SubjToStatusTfrLst.member[i];
...
)
{
   ...
   if (VB_MME_CHK_MASK(pUeCb->bitmask, VB_EMM_HO_WITH_MME_CHANGE))
   {
      ...
      {
         VB_MME_DBG_ERROR((VB_MME_PRNTBUF, "Invalid Handover Type"));
      }
