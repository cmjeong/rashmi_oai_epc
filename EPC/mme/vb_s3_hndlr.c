   ...
   if (ROK == ret)
   {
      ...
      if (ROK == ret)
      {
         ...
         VB_MME_DBG_INFO((VB_MME_PRNTBUF, "Adding S3 Tunnel to "\
                        "Tunnel Hash Table, Tunnel ID [%u]", pS3Tun->localTeIdKey));
...
)
{
   ...
   VB_MME_DBG_INFO((VB_MME_PRNTBUF, "Getting S3 Tunnel, Tunnel ID: [%u]",\
         locTeid.val));
   ...
   {
      VB_MME_DBG_ERROR((VB_MME_PRNTBUF, "Getting S3 Tunnel, Tunnel ID: [%u]",\
            locTeid.val));
   }
...
)
{
   ...
   VB_TRC2(vbMmeSndS3RCReq);

   VB_MME_DBG_INFO((VB_MME_PRNTBUF, "Building Relocation Cancel "\
         "Request "));
   ret = vbMmeUtlBldS3RCReq(pUeCb, &pEgMsg);
   if (ROK == ret)
   {
      teidTkn.pres = PRSNT_NODEF;
      teidTkn.val  = pUeCb->pUeS3TunCb->localTeIdKey;
 ...
 * 
 * @return
 *    SUCCESS if Relocation Cancel Response is sent to SGSN 
 *    successfully otherwise FAILURE
 *
 *****************************************************************************/
...
)
{
   ...
   VB_TRC2(vbMmeSndS3RCRsp);

   VB_MME_DBG_INFO((VB_MME_PRNTBUF, "Building Relocation Cancel "\
         "Response"));
   ret = vbMmeUtlBldS3RCRsp(pUeCb, cause, &pEgMsg);
   if (ROK == ret)
   {
      teidTkn.pres = PRSNT_NODEF;
      teidTkn.val  = pUeCb->pUeS3TunCb->localTeIdKey;
...
)
{
   ...
   if (ROK == ret)
   {
      ...
      VB_MME_DBG_INFO((VB_MME_PRNTBUF, "Sending Relocation Cancel "\
               "Response with Success Cause to Source SGSN"));
      ret = vbMmeSndS3RCRsp(pUeCb, EGT_GTPC_CAUSE_ACC_REQ);
      if (ROK != ret)
      {
      ...
      VB_MME_DBG_INFO((VB_MME_PRNTBUF, "Initiating Intial Context Setup "\
            "Release Procedure"));
      ret = vbMmeInitCtxtRelProc(pUeCb, CAUSE_RADIONW, 
	SztCauseRadioNwho_failure_in_target_EPC_eNB_or_target_systemEnum, 0);
      if (ROK != ret)
      {
