...
)
{
   ...
   if (ROK == ret)
   {
      ...
      if (ROK == ret)
      {
         ...
         cmMemset((U8 *)&egIe, 0, sizeof(EgIe));
         egIe.ieInst = 0;
         egIe.t.fTeid.teid = pUeCb->pHoData->rabInfo[0].dlGtpTeid;
         egIe.t.fTeid.isIPv4Pres = TRUE;
         egIe.t.fTeid.intfType = EGT_INT_X2_DL_ENB_GTP_U;
         vbUtlIpv4InetToArr((egIe.t.fTeid.ip4Addr),\
               &(pUeCb->pHoData->rabInfo[0].dlTransportLyrAddr.u.ipv4NetAddr));
      ...
      if (ROK == ret)
      {
         ...
         cmMemset((U8 *)&egIe, 0, sizeof(EgIe));
         egIe.ieInst = 0;
         egIe.t.fTeid.teid = pUeCb->pHoData->rabInfo[0].ulGtpTeid;
         egIe.t.fTeid.isIPv4Pres = TRUE;
         egIe.t.fTeid.intfType = EGT_INT_X2_UL_ENB_GTP_U;
         vbUtlIpv4InetToArr((egIe.t.fTeid.ip4Addr),\
               &(pUeCb->pHoData->rabInfo[0].ulTransportLyrAddr.u.ipv4NetAddr));
...
)
{
   ...
   if (ROK == ret)
   {
      ...
      if (ROK == ret && pEgIe != NULLP)
      {
         pUeCb->pHoData->rabInfo[0].dlGtpTeid = pEgIe->t.fTeid.teid;
         pUeCb->pHoData->rabInfo[0].dlTransportLyrAddr.type = CM_INET_IPV4ADDR_TYPE;
         vbMmeEgtUtlIpv4ArrToInet(pEgIe->t.fTeid.ip4Addr,
               &(pUeCb->pHoData->rabInfo[0].dlTransportLyrAddr.u.ipv4NetAddr));
      }
      else
   ...
   if (ROK == ret)
   {
      ...
      if (ROK == ret && pEgIe != NULLP)
      {
         pUeCb->pHoData->rabInfo[0].ulGtpTeid = pEgIe->t.fTeid.teid;
         pUeCb->pHoData->rabInfo[0].ulTransportLyrAddr.type = CM_INET_IPV4ADDR_TYPE;
         vbMmeEgtUtlIpv4ArrToInet(pEgIe->t.fTeid.ip4Addr,
               &(pUeCb->pHoData->rabInfo[0].ulTransportLyrAddr.u.ipv4NetAddr));
      }
      else
...
)
{
   ...
   if (ret == ROK && pEgIe != NULLP)
   {
      pUeCb->pHoData->staTfrTCont.itm[0].len = pEgIe->t.fContainer.fContainer.length;
      VB_MME_ALLOC(&pUeCb->pHoData->staTfrTCont.itm[0].pVal, pUeCb->pHoData->\
               staTfrTCont.itm[0].len);
      if (NULLP != pUeCb->pHoData->staTfrTCont.itm[0].pVal)
      {
         cmMemcpy(pUeCb->pHoData->staTfrTCont.itm[0].pVal,\
               pEgIe->t.fContainer.fContainer.val,
               pUeCb->pHoData->staTfrTCont.itm[0].len);
      }
      else
      {
...
)
{
   ...
   VB_MME_DBG_INFO((VB_MME_PRNTBUF, "Adding E-UTRAN Transparent "\
         "Container"));
   ret = vbMmeEgtUtlAddEUStaTfrTransCont(*ppEgMsg, &pUeCb->pHoData->staTfrTCont);
   if (ROK != ret)
   {
      VB_MME_DBG_ERROR((VB_MME_PRNTBUF, "Adding E-UTRAN Transparent "\
            "Container"));
