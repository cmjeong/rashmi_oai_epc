...
)
{
   ...
   if (ROK == ret)
   {
      ...
      if (ROK == ret)
      {
         ...
         VB_MME_DBG_INFO((VB_MME_PRNTBUF, "Adding S10 Tunnel to "\
               "Tunnel Hash Table, Tunnel ID [%u]", pS10Tun->localTeIdKey));
   ...
   {
      VB_MME_DBG_ERROR((VB_MME_PRNTBUF, "Getting S10 Tunnel, Teid: [%u]",\
            locTeid.val));
   }
...
)
{
   ...
   VB_MME_DBG_INFO((VB_MME_PRNTBUF, "Getting S10 Tunnel, Tunnel ID: [%u]",\
         locTeid.val));
   ...
   {
      VB_MME_DBG_ERROR((VB_MME_PRNTBUF, "Getting S10 Tunnel, Tunnel ID: [%u]",\
            locTeid.val));
   }
