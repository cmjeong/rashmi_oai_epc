      ...
      case S1AP_PDU_INITIATINGMSG:
      {
         ...
         if((sztS_TMSI != NULLP) && (sztS_TMSI->pres.pres == PRSNT_NODEF))
         {
            ...
            if((ROK == ret) && (NULLP != (*ueCb)) && 
                  ...
                  (evnt->m.emmEvnt->msgId != CM_EMM_MSG_SERVICE_REQ))
            {
...
/*
 *
 *       Fun:   vbMmeUtlDelHoTrns
 *
 *       Desc:  Deletes all esmcbs from hash list
 *
 *       Ret:   
 *
 *       Notes: None
 ...
 */
#ifdef ANSI
PUBLIC S16 vbMmeUtlDelHoTrns
(
VbMmeUeCb    *ueCb
)
#else
PUBLIC S16 vbMmeUtlDelHoTrns(ueCb)
VbMmeUeCb    *ueCb;
#endif
{
   U8              indx      = 0;
   VB_TRC2(vbMmeUtlDelHoTrns)
   /* Delete All Transaction */
      for (indx = 0; indx < CM_ESM_MAX_BEARER_ID; indx++)
      {
         if (ueCb->esmTrnList.esmInfo[indx].esmCb)
         {
            ...
            vbMmeStopTmr((PTR)ueCb->esmTrnList.esmInfo[indx].esmCb, VB_TMR_ESM_3485);
            vbMmeStopTmr((PTR)ueCb->esmTrnList.esmInfo[indx].esmCb, VB_TMR_ESM_3486);
            vbMmeStopTmr((PTR)ueCb->esmTrnList.esmInfo[indx].esmCb, VB_TMR_ESM_3489);
            vbMmeStopTmr((PTR)ueCb->esmTrnList.esmInfo[indx].esmCb, VB_TMR_ESM_3495);
            VB_MME_FREE(ueCb->esmTrnList.esmInfo[indx].esmCb,\
                  sizeof(VbMmeEsmCb));
            ueCb->esmTrnList.esmInfo[indx].esmCb =  NULL;
            ueCb->esmTrnList.esmInfo[indx].trnId =  0;
   ...
   for (indx = 0; indx < CM_ESM_MAX_BEARER_ID; indx++)
   {
      ...
      {
         VB_MME_FREE(ueCb->esmList[indx], sizeof(VbMmeEsmCb));
         ueCb->esmList[indx] = NULLP;
      }
   }
   /* Remove All Queued Procedure */
...
{  
   ...
   *s1PagingReqPduPtr = NULLP;
   ...
   /*vb005.101 : UeId Index calculation ccpu0011748*/
   /*KWORKS Fix*/
   if(ueCb->ueCtxt.ueImsiLen <= VB_HSS_IMSI_LEN)
   {
      vbMmeUtlChar2BCD(ueCb->ueCtxt.ueImsi, ueCb->ueCtxt.ueImsiLen, &imsiInt);
   }
   else
   {
      VB_MME_DBG_ERROR((VB_MME_PRNTBUF, "ImsiLen is out of bounds."));
      RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   *s1SetupRspPduPtr = NULLP;
   ...
   /*KWORKS Fix*/
   if(((vbMmeCb.mmeCfg.mmeName.length) > 0) && (vbMmeCb.mmeCfg.mmeName.length < LVB_MME_MAX_NAME))
   {
   ...
   {
      VB_MME_DBG_ERROR((VB_MME_PRNTBUF, "vbUtlBldS1SetupRsp: mmeName Length is out of bounds."));
      cmFreeMem((Ptr)s1SetupRspPdu);
      RETVALUE(RFAILED);
   }
...
#ifdef VB_MME_NAS_SEC
    if(type == CM_EMM_MID_TYPE_IMEI)
    {
       (*evnt)->secHT = CM_NAS_SEC_HDR_TYPE_INT_PRTD_ENC /*CM_NAS_SEC_HDR_TYPE_INT_PRTD*/;
    }
#endif
    /* Fill Header information */
    /* Security header type is "Plain NAS message, not security protected" */
...
#endif
{
   ...
   if(ueEmmCb->ueCb == NULLP)
   {
      VB_MME_DBG_ERROR ((VB_MME_PRNTBUF, 
            "UeCb pointer is NULLP"));
      RETVALUE(RFAILED);
   }
...
#ifdef VB_MME_NAS_SEC
   if((ueEmmCb->ueCb != NULLP) && (ueEmmCb->ueCb->creBid.nmbBearerId))
   {
      (*evnt)->secHT = CM_NAS_SEC_HDR_TYPE_INT_PRTD_ENC;
   }
   else
...
#endif /* ANSI */
{
   ...
   if (esmCb->apn.pres)
   {
      ...
      cmMemcpy((U8 *)(msg->u.actReq.apn.apn + 1), (U8 *)esmCb->apn.apn,
            esmCb->apn.len);
#else
      cmMemcpy((U8 *)(msg->u.actReq.apn.apn), (U8 *)esmCb->apn.apn,
            esmCb->apn.len);
#endif
      finalApnLen += esmCb->apn.len; 
      for(pdnCnt = 0; pdnCnt < VB_HSS_MAX_PDN_SUB_CTXT; pdnCnt++)
      {
          if((ret = (cmMemcmp((U8 *)&esmCb->apn.apn,
                     (U8 *)&esmCb->ueCb->ueCtxt.ueHssCtxt.subCntxt[pdnCnt].apn,
                     esmCb->apn.len))) == 0)
          {
               pdnCtxt= &esmCb->ueCb->ueCtxt.ueHssCtxt.subCntxt[pdnCnt];
               /* Mark this APN as allocated APN */
               pdnCtxt->subCntxtinUse = TRUE;
               pdnCtxt->ipAddr = pdnAddr;
               break;
          }
      }
   }
   ...
   msg->u.actReq.pAddr.pres  = TRUE;
      if(pdnAddr.type == CM_TPTADDR_IPV4)
   {
...
#ifdef IPV6_SUPPORTED
      else if(pdnAddr.type == CM_TPTADDR_IPV6)
   {
      msg->u.actReq.pAddr.pdnType  = CM_ESM_PDN_IPV6;
         msg->u.actReq.pAddr.len  = 9;
      cmMemcpy((U8 *)(msg->u.actReq.pAddr.addrInfo),
               (U8 *)(&(pdnAddr.u.ipv6NetAddr[8])), 8);
   }
   else
   {
...
#endif
{
       ...
       (Sztid_uplinkNASTport == msgType))
   {
      ...
      if (NULLP != eCgi)
      {
         ...
         {
            VB_MME_DBG_ERROR((VB_MME_PRNTBUF, "Unable to Retrieve the ECGI."));
            RETVALUE(RFAILED);
         }
         RETVALUE(ROK);
      }
...
#endif
{
   ...
   VB_TRC2(vbMmeUtlGetEsmCbOnS11TranId)
...
)
{
   ...
   pTgtUeCb->creBid = pSrcUeCb->creBid;
   pTgtUeCb->transCntr = pSrcUeCb->transCntr;
   for(count=0; count<pSrcUeCb->creBid.nmbBearerId; count++)
   {
      if (NULLP != pSrcUeCb->esmList[count])
   {
      VB_MME_ALLOC(&pNewEsmCb, sizeof(VbMmeEsmCb));
      if (NULLP != pNewEsmCb)
      {
            cmMemcpy((U8 *)pNewEsmCb, (U8 *)pSrcUeCb->esmList[count],\
               sizeof(VbMmeEsmCb));
         pNewEsmCb->ueCb = pTgtUeCb;
            pTgtUeCb->esmList[count] = pNewEsmCb;
	...
	if (pNewEsmCb != NULLP)
	{
         if(pNewEsmCb->bearerType != CM_ESM_EPS_DED_BEARER)
         {
   ...
   {
               cmMemcpy((U8 *)pPdnCb, (U8 *)pSrcUeCb->pdnList[count], sizeof(VbMmePdnCb));
   ...
   if (pSrcUeCb->pHoData->srcToTgtTCont.len != 0)
   {
      ...
      /*KWORKS Fix*/
      if (NULLP == pTgtUeCb->pHoData->srcToTgtTCont.pVal)
      {
...
)
{
   ...
   for(i = 0; i< pHoData->staTfrTCont.noComp; i++)
   {
      if (NULLP != pHoData->staTfrTCont.itm[i].pVal)
      {
         VB_MME_FREE(pHoData->staTfrTCont.itm[i].pVal, pHoData->staTfrTCont.itm[i].len);
         pHoData->staTfrTCont.itm[i].pVal = NULLP;
         pHoData->staTfrTCont.itm[i].len = 0;
      }
   }

   if(NULLP != pHoData->hoTgt.tgtRncId.laiId.pLMNidentity.val)
   {
      VB_MME_FREE(pHoData->hoTgt.tgtRncId.laiId.pLMNidentity.val, 
                  pHoData->hoTgt.tgtRncId.laiId.pLMNidentity.len);
      pHoData->hoTgt.tgtRncId.laiId.pLMNidentity.val = NULLP;
      pHoData->hoTgt.tgtRncId.laiId.pLMNidentity.len = 0;
   }
   VB_MME_FREE(pHoData, sizeof(VbMmeHoData));
   pUeCb->pHoData = NULLP;
   VB_MME_DBG_INFO((VB_MME_PRNTBUF, " InterRAT HO Data is deleted "));
...
)
{
   ...
   for(indx = 0; indx < VB_MME_MAX_PDN_CONN; indx++)
   {
      if(pUeCb->pdnList[indx])
      {
         VB_MME_FREE(pUeCb->pdnList[indx], sizeof(VbMmePdnCb));
         pUeCb->pdnList[indx] = NULLP;
      }
   }

   vbMmeUtlDelHoTrns(pUeCb);
