...
#endif
{
   ...
   else if(CM_EMM_MSG_ID_RSP == emmMsgType)
   {
      ueCb->proc = VB_MME_ATTACH_PROC;
   }
   else if(CM_EMM_MSG_TAU_CMP == emmMsgType)
   {
      ueCb->proc = VB_MME_TAU_PROC;
   }
   ...
   VB_MME_DBG_INFO((VB_MME_PRNTBUF, "Changing ueCb->proc to (%d) \n", ueCb->proc));
...
#endif
{
   ...
   switch(esmMsgType)
   {
      case CM_ESM_MSG_ACTV_DED_BEAR_ACC:
      {
         ...
         {
             VB_MME_DBG_INFO((VB_MME_PRNTBUF, "Received ACTV_DED_BEAR_ACC message and"
                     "waiting for E-RAB setup rsp from the eNB\n"));
             ueCb->rxd_dfltorded_bearer_acc = TRUE;
         }
         break;
      }
