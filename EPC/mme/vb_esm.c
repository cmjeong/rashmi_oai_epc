...
#endif
{
   ...
   if(msgIdx <= CM_ESM_EVNT_ESM_STATUS)
   {
      if( esmCb->ueCb )
      {
         VB_MME_DBG_INFO((VB_MME_PRNTBUF,"UE CB PROC [%d]\n", esmCb->ueCb->proc));
      }
      ret = vbEsmIncStTbl[msgIdx][esmCb->state](esmCb, evnt);
   }
   else
...
#endif
{
   ...
   /* MemLeak fix */
   VB_MME_FREE(esmCb->retrBuf.val, CM_MAX_EMM_ESM_PDU);
...
#endif
{
   ...
   if(evnt->m.esmEvnt->u.conReq.apn.pres)
   {
      ...
      if(evnt->m.esmEvnt->u.conReq.apn.len <= CM_ESM_MAX_LEN_ACCESS_PTNAME)
      {
         cmMemcpy((U8* )esmCb->apn.apn, (U8* )evnt->m.esmEvnt->u.conReq.apn.apn, 
               evnt->m.esmEvnt->u.conReq.apn.len);
         esmCb->apn.len = evnt->m.esmEvnt->u.conReq.apn.len;
         esmCb->apn.apn[esmCb->apn.len] = '\0';
      }
      else
      {
         cmMemcpy((U8* )esmCb->apn.apn, (U8* )evnt->m.esmEvnt->u.conReq.apn.apn, CM_ESM_MAX_LEN_ACCESS_PTNAME);
      ...
      for(i = 0; i < VB_HSS_MAX_PDN_SUB_CTXT; i++)
      {
         if(esmCb->ueCb->ueCtxt.ueHssCtxt.subCntxt[i].subCntxtinUse == FALSE)
         {
            cmMemcpy((U8* )esmCb->ueCb->ueCtxt.ueHssCtxt.subCntxt[i].apn, 
                  (U8* )esmCb->apn.apn, esmCb->apn.len);
            esmCb->ueCb->ueCtxt.ueHssCtxt.subCntxt[i].subCntxtinUse = TRUE;
            esmCb->ueCb->ueCtxt.ueHssCtxt.subCntxt[i].apn[esmCb->apn.len] = '\0';
            VB_MME_DBG_INFO((VB_MME_PRNTBUF, " APN_NAME : %s", 
                     esmCb->ueCb->ueCtxt.ueHssCtxt.subCntxt[i].apn));
            break;
         }
      }
