...
#endif
{
   ...
   (*ueCb)->imsi = *imsi;
#else
   (*ueCb)->imsi.len = imsi->len;
   cmMemcpy((U8 *)&(*ueCb)->imsi.buf,imsi->buf,imsi->len);
   ...
   ret = cmHashListInsert(&avPgwCb.ueCbHlst, (PTR)(*ueCb), (U8 *)(*ueCb)->imsi.buf,
                        (U16)(*ueCb)->imsi.len);
   if(ROK!= ret)
   { 
...
#endif
{
   ...
   else
   {
      ...
      ueCb->numBearers -= 1;
      if(0 == ueCb->numBearers)
      {
         printf("DEBUGS: Clearing the UeCb and number of Bearers currently active:%d.\n",ueCb->numBearers);
         ret = avPgwDelUeCb(&ueCb);
         if(ROK != ret)
         {
            AV_PGW_SND_DBG_ERROR((AV_PGW_SND_PRNTBUF, "Deletion of UeCb"));
            AV_PGW_SND_RETVALUE(ret);
         }
      }
   }
...
PUBLIC S16 avPgwUpdtTftParams(AvPgwUeTunnelCb *ueTunnCb, AvPgwTft *tft)
{
    ...
    for (pfCnt = 0; pfCnt < tft->noOfPfs; pfCnt++) 
    {
        ...
        if (tft->pfList[pfCnt].localPort.pres) 
        {
           ...
           ueTunnCb->tft.pktFilters[pfCnt]->presence_mask |= GTP_SNGL_LOC_PORT_TY_PKT_FLTR_MASK;
        }
        if (tft->pfList[pfCnt].remotePort.pres) 
        {
           ueTunnCb->tft.pktFilters[pfCnt]->component.remPort = tft->pfList[pfCnt].remotePort.port;
           ueTunnCb->tft.pktFilters[pfCnt]->presence_mask |= GTP_SNGL_REM_PORT_TY_PKT_FLTR_MASK;
           printf("DEBUGS: Updated the ueTunnCb with the remote port as %d.\n",ueTunnCb->tft.pktFilters[pfCnt]->component.remPort);
        }
