   ...
   switch(state)
   {
      ...
      case AV_PGW_EGTPC_TUNNEL_PROC:
      {
         ...
         else
         {
            ...
            if(egMsg != NULLP)
            {
               ret = avPgwEgtCTunnelProc(tunnelCb, egMsg->msgHdr.msgType, egMsg, &msgQueued);
            }
         }
...
#endif
{
   ...
   else
   {
      ...
      imsi.len = egIe->t.valStr8.length;
#else
      AV_PDNGW_SENDTSK_ALLOC(imsi, sizeof(AvImsi));
      if(imsi == NULLP)
      {
         AV_PGW_SND_DBG_ERROR((AV_PGW_SND_PRNTBUF, "IMSI Allocation has failed\n"));
         RETVALUE(ROK);
      }
      imsi->len = egIe->t.valStr8.length;
/*      AV_PDNGW_SENDTSK_ALLOC(imsi->buf, imsi->len);*/
      cmMemcpy(imsi->buf, (U8 *)&(egIe->t.valStr8.val), AV_PGW_IMSI_LEN);
#endif
   }
   ...
   {
      AV_PDNGW_SENDTSK_FREE(imsi, sizeof(AvImsi));
...
PUBLIC S16 avPgwTrigDBReq(U32 ipAddr)
{
   S16 retVal = ROK;
   AvPgwPdnCb        *pdnCb = NULLP;
   EgMsg             *dbReq = NULLP;
   ...
   AV_PGW_SND_TRC2(avPgwTrigDBReq)
   ...
   for(loopVar = 0; loopVar < AV_PGW_MAX_BEARERS; loopVar++)
   {
      if(pdnCb->ueCb->bearerTable[loopVar] != NULLP)
      {
         if(pdnCb->ueCb->bearerTable[loopVar]->s5s8UTun != NULLP)
         {
            if(pdnCb->ueCb->bearerTable[loopVar]->s5s8UTun->tunnelType == AV_PGW_DATA_TUNNEL_DED) 
            {
               s5s8UDedTun = pdnCb->ueCb->bearerTable[loopVar]->s5s8UTun;
               AV_PGW_SND_DBG_INFO((AV_PGW_SND_PRNTBUF,
                        "PDN-GW: Found Dedicated Tunnel with Bearer Id %u \n",s5s8UDedTun->bearerId));
               break;
            }
         }
      }
   }
   if(loopVar == AV_PGW_MAX_BEARERS)
   {
      AV_PGW_RCV_DBG_ERROR((AV_PGW_RCV_PRNTBUF,"NO DEDICATED TUNNEL FOUND"));
      RETVALUE(RFAILED);
   }
   avPgwAddPendingTunnelCb(pdnCb, &s5s8UDedTun, &avPgwCb.pgwCfg.pgwAddrCfg.selfAddr,AV_PGW_DATA_TUNNEL_DED);

   tunnelCb = pdnCb->s5s8CTun;
   localTeId.pres = TRUE;
   localTeId.val = tunnelCb->localTeIdKey;
   retVal = avPgwBldDBReq(pdnCb, &dbReq);
   if (ROK == retVal)
   {
      create.pres = PRSNT_NODEF;
      create.val = FALSE;
      ...
      retVal = EuLiEgtSigReq(&avPgwCb.egtCSapCb->pst, avPgwCb.egtCSapCb->spId,
            &(tunnelCb->lclAddr), &(tunnelCb->remAddr), localTeId,
            pdnCb->transId, create, t3, dbReq);
      if(retVal != ROK)
      {
         AV_PGW_RCV_DBG_ERROR((AV_PGW_RCV_PRNTBUF,
                  "Sending of Delete Bearer Request"));
         RETVALUE(retVal);
      }
