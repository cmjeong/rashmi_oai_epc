...
PRIVATE Buffer *ysPalPkTfuDlCqiInd ARGS(( Pst * pst, SuId suId, TfuDlCqiIndInfo * cqiInd));
...
#endif
{
   ...
   ttiInd[0]->cells.cellId = ysCb.cellCb.cellId;

   ttiInd[0]->timingInfo = *timingInfo;
...
#ifdef SS_CAVIUM   
               cmPkTfuDlCqiInd(&ysCb.cellCb.peerPst, 
                     ysCb.schTfuSap.suId, cqiInd);
#else
               mBuf = ysPalPkTfuDlCqiInd(&ysCb.cellCb.peerPst, 
                                 ysCb.schTfuSap.suId, cqiInd);
               if(mBuf != NULLP)
               {
                  ysPalSend2Peer(mBuf);
