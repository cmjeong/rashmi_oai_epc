...
/**********************************************************************
     ...
     Sid:      cm_esm_edm.c@@/main/6 - Thu Jul  5 10:56:01 2012
...
#endif
{
   ...
   if(protCfgOpt->pres)
   {
      ...
      *len += 24;
      ...
      /* protocol Id list */
      /* cm_esm_edm_c_001.main_5  Added support to send DNS IP */
#if 0 
      for(i=0; readLen > 0; i++)
      {
         /*cm00119160:type cast to U8*/
         buf[(*indx)++] = (U8)protCfgOpt->p[i].pid & 0xff00;
         buf[(*indx)++] = protCfgOpt->p[i].pid & 0x00ff;
         buf[(*indx)++] = protCfgOpt->p[i].len;
         *len += 24;
         readLen -= 3;

         cmMemcpy((U8* )&buf[*indx], (U8* )protCfgOpt->p[i].val, protCfgOpt->p[i].len);
...
#endif
      /* container Id list */
      /* Not encoding container Id list as we dont know the length */
      /* cm_esm_edm_c_001.main_5 Added support to send DNS IP */
      /* for(i=0; i < CM_ESM_MAX_CONT_ID; i++) */
      for(i=0; readLen > 0; i++)
      {
         buf[(*indx)++] = protCfgOpt->c[i].cid & 0xff00;
         buf[(*indx)++] = protCfgOpt->c[i].cid & 0x00ff;
         buf[(*indx)++] = protCfgOpt->c[i].len;
         *len += 24;
         readLen -= 3; /* added */
         cmMemcpy((U8* )&buf[*indx], (U8* )protCfgOpt->c[i].val, protCfgOpt->c[i].len);
         (*indx) += protCfgOpt->c[i].len;
         *len += (protCfgOpt->c[i].len * 8);
         readLen -= protCfgOpt->c[i].len;  
      }
   }
...
#endif
{
   ...
   for(i=0; len > 0 ;i++)
   {
      ...
      protCfgOpt->p[i].pid   |= ((0x00ff  & buf[(*indx)++]) << 8);
...
/**********************************************************************
         ...
         End of file:     cm_esm_edm.c@@/main/6 - Thu Jul  5 10:56:01 2012
