...
/**********************************************************************
     ...
     Sid:      cm_emm_edm.c@@/main/6 - Thu Jul  5 10:42:54 2012
...
/* cm_emm_edm_c_001.main_5 Added function for TAU */
PRIVATE S16 cmEmmEncEpsUpdType  ARGS((U8* buf, U32* indx, CmEmmMsg *msg, U16 *len));
PRIVATE S16 cmEmmEncNonCrntNasKeySetID  ARGS((U8* buf, U32* indx, CmEmmMsg *msg, U16 *len));
PRIVATE S16 cmEmmEncGPRSCkSeqNo  ARGS((U8* buf, U32* indx, CmEmmMsg *msg, U16 *len));
PRIVATE S16 cmEmmEncAdditionalGUTI ARGS((U8* buf, U32* indx, CmEmmMsg *msg, U16 *len));
PRIVATE S16 cmEmmEncUENwCapab  ARGS((U8* buf, U32* indx, CmEmmMsg *msg, U16 *len));
...
/* cm_emm_edm_c_001.main_5 Added function for TAU */
PRIVATE S16 cmEmmDecEpsUpdType  ARGS((U8* buf, U32* indx, CmEmmMsg *msg, U16 *len));
PRIVATE S16 cmEmmDecNonCrntNasKeySetID  ARGS((U8* buf, U32* indx, CmEmmMsg *msg, U16 *len));
PRIVATE S16 cmEmmDecNonGPRSCkSeqNo ARGS((U8* buf, U32* indx, CmEmmMsg *msg, U16 *len));
PRIVATE S16 cmEmmDecAdditionalGUTI ARGS((U8* buf, U32* indx, CmEmmMsg *msg, U16 *len));
PRIVATE S16 cmEmmDecUENwCapab  ARGS((U8* buf, U32* indx, CmEmmMsg *msg, U16 *len));
/*cm00119161:Guarded under flag CM_MME*/
#ifdef CM_MME
/* cm_emm_edm_c_001.main_3: Added function for msg service req */
...
#endif
{
   ...
   EDM_DBG_INFO((EDM_PRNTBUF, "cmEmmDecMsg: "
         "Decoding EMM message"));
...
/* cm_emm_edm_c_001.main_1: Modified flag name CM_NAS_TEST */
#ifdef CM_NAS_TEST
   ...
   for (i = 0; ((indx < len) && (lastIE == FALSE)); i++)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            {
               EDM_DBG_ERROR((EDM_PRNTBUF, "Error: Missing Parameter not present\n"));
               CM_FREE_NASEVNT(evnt);
               RETVALUE(ret);
            }
   ...
   if (indx < len)
   {
      EDM_DBG_ERROR((EDM_PRNTBUF, "Error: There are more elements left than expected\n"));
      CM_FREE_NASEVNT(evnt);
      RETVALUE(RFAILED);
   }
...
#endif
{
   ...
   if (buff == NULLP)
   {
      EDM_DBG_ERROR((EDM_PRNTBUF, "cmEmmEncMsg: Not enough memory to encode (%d)\n",
                   __LINE__));
      RETVALUE(RFAILED);
   }
   ...
   for (lstIndx = 0; (ieLen > 0) && (lstIndx < CM_EMM_MAX_TAI_LIST); lstIndx++)
   {
      ...
      switch(lst->partLst[lstIndx].type)
      {
         ...
         default:
         {
            ...
            EDM_DBG_ERROR((EDM_PRNTBUF,"Partial TA identity list type (%d) unsupported \n",
                     lst->partLst[lstIndx].type));
            RETVALUE(RFAILED);
         }
      } /* switch */
...
/**********************************************************************
         ...
         End of file:     cm_emm_edm.c@@/main/6 - Thu Jul  5 10:42:54 2012
...
/main/6      ---      cm_emm_edm_c_001.main_5 akaranth 1.Added function for TAU. 
                                           psb      2. TIC_ID:ccpu00117545: Fixed warning for gcc 
